...
function getTemplate()
{
    /*$template = cache()->remember('view.template', 7 * 24 * 60 * 60, function () {
        return \App\Models\LMS\ViewTemplate::where('status', true)->first();
    });*/
...
{
    return App\Models\LMS\Setting::getSeoMetas($page);
}
...
function getSocials()
{
    return App\Models\LMS\Setting::getSocials();
}
...
{
    return App\Models\LMS\Setting::getFooterColumns();
}
...
{
    return App\Models\LMS\Setting::getGeneralSettings($key);
}
...
{
    return App\Models\LMS\Setting::getFeaturesSettings($key);
}
...
{
    return App\Models\LMS\Setting::getCookieSettings($key);
}
...
{
    return App\Models\LMS\Setting::getFinancialSettings($key);
}

function getFinancialCurrencySettings($key = null)
{
    return App\Models\LMS\Setting::getFinancialCurrencySettings($key);
}
...
{
    return App\Models\LMS\Setting::getHomeHeroSettings($section);
}
...
{
    return App\Models\LMS\Setting::getHomeVideoOrImageBoxSettings();
}
...
{
    return App\Models\LMS\Setting::getPageBackgroundSettings($page);
}
...
{
    return App\Models\LMS\Setting::getCustomCssAndJs($key);
}
...
{
    return App\Models\LMS\Setting::getOfflineBankSettings($key);
}
...
{
    $settings = App\Models\LMS\Setting::getReferralSettings();
...
{
    return App\Models\LMS\Setting::getReportReasons();
}
...
{
    return App\Models\LMS\Setting::getNotificationTemplates($template);
}
...
{
    return App\Models\LMS\Setting::getContactPageSettings($key);
}
...
{
    return App\Models\LMS\Setting::get404ErrorPageSettings($key);
}
...
{
    return App\Models\LMS\Setting::getHomeSectionsSettings($key);
}
...
{
    $links = App\Models\LMS\Setting::getNavbarLinksSettings();
...
{
    return App\Models\LMS\Setting::getPanelSidebarSettings();
}
...
{
    return App\Models\LMS\Setting::getFindInstructorsSettings();
}
...
{
    return App\Models\LMS\Setting::getRewardProgramSettings();
}
...
{
    return App\Models\LMS\Setting::getRewardsSettings();
}
...
{
    return App\Models\LMS\Setting::getStoreSettings($key);
}

function getBecomeInstructorSectionSettings()
{
    return App\Models\LMS\Setting::getBecomeInstructorSectionSettings();
}

function getForumSectionSettings()
{
    return App\Models\LMS\Setting::getForumSectionSettings();
}

function getRegistrationPackagesGeneralSettings($key = null)
{
    return App\Models\LMS\Setting::getRegistrationPackagesGeneralSettings($key);
}

function getRegistrationPackagesInstructorsSettings($key = null)
{
    return App\Models\LMS\Setting::getRegistrationPackagesInstructorsSettings($key);
}

function getRegistrationPackagesOrganizationsSettings($key = null)
{
    return App\Models\LMS\Setting::getRegistrationPackagesOrganizationsSettings($key);
}

function getMobileAppSettings($key = null)
{
    return App\Models\LMS\Setting::getMobileAppSettings($key);
}

function getMaintenanceSettings($key = null)
{
    return App\Models\LMS\Setting::getMaintenanceSettings($key);
}

function getGeneralOptionsSettings($key = null)
{
    return App\Models\LMS\Setting::getGeneralOptionsSettings($key);
}

function getGiftsGeneralSettings($key = null)
{
    return App\Models\LMS\Setting::getGiftsGeneralSettings($key);
}

function getRemindersSettings($key = null)
{
    return App\Models\LMS\Setting::getRemindersSettings($key);
}

function getGeneralSecuritySettings($key = null)
{
    return App\Models\LMS\Setting::getGeneralSecuritySettings($key);
}
...
function getAdvertisingModalSettings()
{
    ...
    $settings = App\Models\LMS\Setting::getAdvertisingModalSettings();
...
{
    return \App\Models\LMS\Setting::getOthersPersonalizationSettings($key);
}

function getInstallmentsSettings($key = null)
{
    return \App\Models\LMS\Setting::getInstallmentsSettings($key);
}

function getInstallmentsTermsSettings($key = null)
{
    return \App\Models\LMS\Setting::getInstallmentsTermsSettings($key);
}

function getRegistrationBonusSettings($key = null)
{
    return \App\Models\LMS\Setting::getRegistrationBonusSettings($key);
}

function getRegistrationBonusTermsSettings($key = null)
{
    return \App\Models\LMS\Setting::getRegistrationBonusTermsSettings($key);
}

function getStatisticsSettings($key = null)
{
    return \App\Models\LMS\Setting::getStatisticsSettings($key);
}
...
{
    $settings = App\Models\LMS\Setting::getThemeColorsSettings();
    ...
    if (!empty($settings) and count($settings)) {
        ...
        if ($admin) {
            foreach (\App\Models\LMS\Setting::$rootAdminColors as $color) {
                if (!empty($settings['admin_' . $color])) {
                    $result .= "--$color:" . $settings['admin_' . $color] . ';' . PHP_EOL;
                }
            }
        } else {
            foreach (\App\Models\LMS\Setting::$rootColors as $color) {
...
{
    $settings = App\Models\LMS\Setting::getThemeFontsSettings();
        ...
        foreach ($settings as $type => $setting) {
            ...
            if (!empty($settings[$type]['regular'])) {
                $result .= "@font-face {
                      ...
                      font-weight: 400;
                      font-display: swap;
                      src: url(".get_path_lms()."{$settings[$type]['regular']}) format('woff2');
                    }";
            }
            ...
            if (!empty($settings[$type]['bold'])) {
                $result .= "@font-face {
                      ...
                      font-weight: bold;
                      font-display: swap;
                      src: url(".get_path_lms()."{$settings[$type]['bold']}) format('woff2');
                    }";
            }
            ...
            if (!empty($settings[$type]['medium'])) {
                $result .= "@font-face {
                      ...
                      font-weight: 500;
                      font-display: swap;
                      src: url(".get_path_lms()."{$settings[$type]['medium']}) format('woff2');
                    }";
            }
...
{
    $key = 'site_language';
    $name = \App\Models\LMS\Setting::$generalName;
    ...
    $setting = cache()->remember('settings.getDefaultLocale', 24 * 60 * 60, function () use ($name) {
        $setting = \Illuminate\Support\Facades\DB::table('lms_settings')
            ->where('page', $name)
            ->where('name', $name)
            ->join('lms_setting_translations', 'lms_settings.id', '=', 'lms_setting_translations.setting_id')
            ->select('lms_settings.*', 'lms_setting_translations.value')
            ->first();
...
function sendNotification($template, $options, $user_id = null, $group_id = null, $sender = 'system', $type = 'single')
{
    ...
    $notificationTemplate = \App\Models\LMS\NotificationTemplate::where('id', $templateId)->first();
    ...
    if (!empty($notificationTemplate)) {
        ...
        $check = \App\Models\LMS\Notification::where('user_id', $user_id)
        ...
        if (empty($check) or !in_array($template, $ignoreDuplicateTemplates)) {
            \App\Models\LMS\Notification::create([
                'user_id' => $user_id,
                'group_id' => $group_id,
                'title' => $title,
                'message' => $message,
            ...
            if (env('APP_ENV') == 'production') {
                $user = \App\Models\LMS\User::where('id', $user_id)->first();
...
function checkCourseForSale($course, $user)
{
    if (!$course->canSale()) {
        $toastData = [
            ...
            'msg' => trans('lms/cart.course_not_capacity'),
            'status' => 'error'
        ];
    ...
    if ($course->checkUserHasBought($user)) {
        $toastData = [
            ...
            'msg' => trans('lms/site.you_bought_webinar'),
            'status' => 'error'
        ];
        return back()->with(['toast' => $toastData]);
    }
    ...
    if ($course->creator_id == $user->id or $course->teacher_id == $user->id) {
        $toastData = [
            'title' => trans('lms/public.request_failed'),
            'msg' => trans('lms/cart.cant_purchase_your_course'),
            'status' => 'error'
        ];
        return back()->with(['toast' => $toastData]);
    }
    ...
    if ($isRequiredPrerequisite) {
        $toastData = [
            'title' => trans('lms/public.request_failed'),
            'msg' => trans('lms/cart.this_course_has_required_prerequisite'),
            'status' => 'error'
        ];
        return back()->with(['toast' => $toastData]);
    }
...
function checkProductForSale($product, $user)
{
    if ($product->getAvailability() < 1) {
        $toastData = [
            'title' => trans('lms/public.request_failed'),
            'msg' => trans('lms/update.product_not_availability'),
            'status' => 'error'
        ];
        return back()->with(['toast' => $toastData]);
    }
    ...
    if ($product->creator_id == $user->id) {
        $toastData = [
            'title' => trans('lms/public.request_failed'),
            'msg' => trans('lms/update.cant_purchase_your_product'),
            'status' => 'error'
        ];
        return back()->with(['toast' => $toastData]);
    }
...
function getTranslateAttributeValue($model, $key, $loca = null)
{
    ...
    try {
        ...
        if ($model->getTable() === 'lms_settings' and in_array($model->name, \App\Models\LMS\Setting::getSettingsWithDefaultLocal())) {
            $locale = \App\Models\LMS\Setting::$defaultSettingsLocale;
        }
    ...
    if ($priceDisplay != 'only_price') {
        ...
        if ($tax > 0) {
            ...
            if ($priceDisplay == 'total_price') {
                ...
                $price = $price . ($showTaxInPrice ? ('+' . $taxPrice . ' ' . trans('lms/cart.tax')) : '');
            }
        }
    } elseif ($format) {
        $price = handlePriceFormat($price, $decimal, $decimalSeparator, $thousandsSeparator);
...
function checkShowCookieSecurityDialog()
{
    ...
    if (getFeaturesSettings('cookie_settings_status')) {
        ...
        if (auth()->guard('lms_user')->check()) {
            $checkDB = \App\Models\LMS\UserCookieSecurity::where('user_id', auth()->guard('lms_user')->id())
                ->first();
...
{
    return \App\Models\LMS\NavbarButton::where('role_id', $roleId)
