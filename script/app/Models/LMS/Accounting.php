...
class Accounting extends Model
{
    ...
    protected static function boot()
    {
        ...
        Accounting::creating(function($model) {
            $model->domain_id = domain_info('domain_id');
        });
    }

    protected $table = "lms_accounting";
    ...
    {
        return $this->belongsTo('App\Models\LMS\OrderItem', 'order_item_id', 'id');
    }

    public function installmentOrderPayment()
    {
        return $this->belongsTo('App\Models\LMS\InstallmentOrderPayment', 'installment_payment_id', 'id');
    }
    ...
    public static function createAccountingBuyer($orderItem, $type = null)
    {
        if ($type !== 'credit') {
            Accounting::create([
                ...
                'description' => trans('lms/public.paid_for_sale'),
                'created_at' => time()
            ]);
        }

        $deductionDescription = trans('lms/public.paid_form_online_payment');
        if (!empty($orderItem->reserveMeeting)) {
            ...
            $deductionDescription = trans('lms/meeting.paid_for_x_hour', ['hours' => convertMinutesToHourAndMinute($minute)]);
        } elseif ($type == 'credit') {
            $deductionDescription = trans('lms/public.paid_form_credit');
        }
        ...
        if (!empty($orderItem->webinar_id)) {
            ...
            $notifyOptions['[c.title]'] = trans('lms/meeting.reservation_appointment');
        } elseif (!empty($orderItem->product_id)) {
            $notifyOptions['[c.title]'] = $orderItem->product->title;
        } elseif (!empty($orderItem->installment_payment_id)) {
            $notifyOptions['[c.title]'] = ($orderItem->installmentPayment->type == 'upfront') ? trans('lms/update.installment_upfront') : trans('lms/update.installment');
        } else if (!empty($orderItem->subscribe_id)) {
            $notifyOptions['[c.title]'] = $orderItem->subscribe->title . ' ' . trans('lms/financial.subscribe');
        } else if (!empty($orderItem->promotion_id)) {
            $notifyOptions['[c.title]'] = $orderItem->promotion->title . ' ' . trans('lms/panel.promotion');
        } else if (!empty($orderItem->registration_package_id)) {
            $notifyOptions['[c.title]'] = $orderItem->registrationPackage->title . ' ' . trans('lms/update.registration_package');
        }

        if (!empty($orderItem->gift_id) and !empty($orderItem->gift)) {
            $notifyOptions['[c.title]'] .= ' (' . trans('lms/update.a_gift_for_name_on_date_without_bold', ['name' => $orderItem->gift->name, 'date' => dateTimeFormat($orderItem->gift->date, 'j M Y H:i')]) . ')';
        }
    ...
    public static function createAccountingTax($orderItem)
    {
        Accounting::create([
            ...
            'type_account' => Accounting::$asset,
            'type' => Accounting::$addiction,
            'description' => trans('lms/public.tax_get_form_buyer'),
            'created_at' => time()
        ]);
    ...
    public static function createAccountingSeller($orderItem)
    {
        ...
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'description' => trans('lms/public.income_sale'),
            'created_at' => time()
        ]);
    ...
    public static function createAccountingSystemForSubscribe($orderItem)
    {
        ...
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'description' => trans('lms/public.income_for_subscribe'),
            'created_at' => time()
        ]);
    ...
    public static function createAccountingCommission($orderItem)
    {
        ...
        Accounting::create([
            ...
            'type_account' => Accounting::$income,
            'type' => Accounting::$addiction,
            'description' => trans('lms/public.get_commission_from_seller'),
            'created_at' => time()
        ]);
        ...
        if (!empty($affiliateUser) and $affiliateCommissionPrice > 0) {
            Accounting::create([
                ...
                'type_account' => Accounting::$income,
                'type' => Accounting::$addiction,
                'description' => trans('lms/public.get_commission_from_referral'),
                'created_at' => time()
            ]);
        }
    ...
    public static function createAffiliateUserAmountAccounting($userId, $referredUserId, $amount)
    {
        if ($amount) {
            Accounting::create([
                ...
                'type_account' => Accounting::$income,
                'type' => Accounting::$addiction,
                'description' => trans('lms/public.get_amount_from_referral'),
                'created_at' => time()
            ]);
    ...
    public static function refundAccountingBuyer($sale)
    {
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'type_account' => Accounting::$asset,
            'description' => trans('lms/public.refund_money_to_buyer'),
            'created_at' => time()
        ]);
    ...
    public static function refundAccountingTax($sale)
    {
        if (!empty($sale->tax) and $sale->tax > 0) {
            Accounting::create([
                ...
                'type_account' => Accounting::$asset,
                'type' => Accounting::$deduction,
                'description' => trans('lms/public.refund_tax'),
                'created_at' => time()
            ]);
        }
    ...
    public static function refundAccountingCommission($sale)
    {
        if (!empty($sale->commission) and $sale->commission > 0) {
            Accounting::create([
                ...
                'type_account' => Accounting::$income,
                'type' => Accounting::$deduction,
                'description' => trans('lms/public.refund_commission'),
                'created_at' => time()
            ]);
        }
    ...
    public static function refundAccountingSeller($sale)
    {
        ...
        Accounting::create([
            ...
            'type_account' => Accounting::$income,
            'type' => Accounting::$deduction,
            'description' => trans('lms/public.refund_income'),
            'created_at' => time()
        ]);
    ...
    public static function charge($order)
    {
        Accounting::create([
            ...
            'description' => trans('lms/public.charge_account'),
            'created_at' => time()
        ]);
    ...
    public static function createAccountingSystemForPromotion($orderItem)
    {
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'description' => trans('lms/public.income_for_promotion'),
            'created_at' => time()
        ]);
    }
    ...
    public static function createAccountingSystemForRegistrationPackage($orderItem)
    {
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'description' => trans('lms/update.paid_for_registration_package'),
            'created_at' => time()
        ]);
    }
    ...
    public static function createAccountingForSaleWithSubscribe($item, $subscribe, $itemName)
    {
        ...
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'type_account' => Accounting::$income,
            'description' => trans('lms/public.paid_form_subscribe'),
            'created_at' => time()
        ]);
    ...
    public static function createAccountingSystemForInstallmentPayment($orderItem)
    {
        Accounting::create([
            ...
            'type' => Accounting::$addiction,
            'description' => ($orderItem->installmentPayment->type == 'upfront') ? trans('lms/update.installment_upfront') : trans('lms/update.installment'),
            'created_at' => time()
        ]);
    }
    ...
    public static function createRegistrationBonusUserAmountAccounting($userId, $amount, $typeAccount)
    {
        ...
        if (!empty($amount) and empty($check)) { //
            Accounting::updateOrCreate([
                ...
                'amount' => $amount,
                'description' => trans('lms/update.get_amount_from_registration_bonus'),
                'created_at' => time()
            ]);
