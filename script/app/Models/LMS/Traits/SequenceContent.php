...
namespace App\Models\LMS\Traits;
...
trait SequenceContent
{
    ...
    public function checkSequenceContent($user = null, $test = false)
    {
        ...
        $user = $user ?: auth()->guard('lms_user')->user();
    ...
    {
        if ($this->table == 'lms_webinar_chapters') {
            ...
            if ($this->table == 'lms_sessions') {
                $type = WebinarChapterItem::$chapterSession;
            } else if ($this->table == 'lms_text_lessons') {
                $type = WebinarChapterItem::$chapterTextLesson;
            } else if ($this->table == 'lms_quizzes') {
                $type = WebinarChapterItem::$chapterQuiz;
            } else if ($this->table == 'lms_webinar_assignments') {
    ...
    private function checkAllPassedItems($chapterItems)
    {
        ...
        foreach ($chapterItems as $chapterItem) {
            ...
            } else if ($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterFile and !empty($chapterItem->file)) {
                $item = $chapterItem->file;
            } else if ($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterTextLesson and !empty($chapterItem->textLesson)) {
                $item = $chapterItem->textLesson;
            } else if ($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterAssignment) {
                $item = $chapterItem->assignment;
            } else if ($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterQuiz) {
                $item = $chapterItem->quiz;
            }

            // Only check previous content that has the check_previous_parts enabled => Vahid Daghighy
            if (!empty($item)) {
                if ($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterSession or
                    $chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterFile or
                    $chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterTextLesson
                ) {
    ...
    /*private function getPreviousItems()
    {
        ...
        if (!empty($webinar)) {
            ...
            $query = $this->newQuery();
            $query->join('lms_webinar_chapter_items', 'lms_webinar_chapter_items.item_id', "{$this->table}.id");
            $query->select("{$this->table}.*", DB::raw('lms_webinar_chapter_items.order as itemOrder'));
            ...
            $query->where("lms_webinar_chapter_items.chapter_id", $this->chapter_id);
            $query->whereIn("lms_webinar_chapter_items.user_id", $creatorIds);
            $query->where('status', 'active');
            $query->where("{$this->table}.id", '!=', $this->id);

            if (!empty($currentItemOrder)) {
                $query->where("lms_webinar_chapter_items.order", '<', $currentItemOrder->order);
            }
