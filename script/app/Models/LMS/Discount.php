...
class Discount extends Model
{
    ...
    protected static function boot()
    {
        ...
        Discount::creating(function($model) {
            $model->domain_id = domain_info('domain_id');
        });
    }

    public $timestamps = false;
    protected $table = 'lms_discounts';
    ...
    {
        return $this->hasOne('App\Models\LMS\DiscountUser', 'discount_id', 'id');
    }

    public function discountCourses()
    {
        return $this->hasMany('App\Models\LMS\DiscountCourse', 'discount_id', 'id');
    }

    public function discountBundles()
    {
        return $this->hasMany('App\Models\LMS\DiscountBundle', 'discount_id', 'id');
    }

    public function discountCategories()
    {
        return $this->hasMany('App\Models\LMS\DiscountCategory', 'discount_id', 'id');
    }

    public function discountGroups()
    {
        return $this->hasMany('App\Models\LMS\DiscountGroup', 'discount_id', 'id');
    }
    ...
    public function checkValidDiscount()
    {
        if ($this->expired_at < time()) {
            return trans('lms/update.discount_code_has_expired'); // expired
        }
        ...
        if ($this->source == self::$discountSourceCourse or $this->source == self::$discountSourceCategory) {
            ...
            if (empty($webinarCount) or count($webinarCount) < 1) {
                return trans('lms/update.discount_code_is_for_courses_error');
            }
            ...
            if (empty($meetingCount) or count($meetingCount) < 1) {
                return trans('lms/update.discount_code_is_for_meetings_error');
            }
        }
        ...
        if ($this->source == self::$discountSourceCourse) {
            ...
            if (!$hasSpecialWebinars) {
                return trans('lms/update.your_coupon_is_valid_for_another_course');
            }
        }
        ...
        if ($this->source == self::$discountSourceBundle) {
            ...
            if (!$hasSpecialBundles) {
                return trans('lms/update.your_coupon_is_valid_for_another_bundle');
            }
        }
        ...
        if ($this->source == self::$discountSourceProduct) {
            ...
            if (!$hasSpecialProducts) {
                return trans('lms/update.your_coupon_is_valid_for_another_products_type');
            }
        }
        ...
        if ($this->source == self::$discountSourceCategory) {
            ...
            if (!$hasSpecialCategories) {
                return trans('lms/update.your_coupon_is_valid_for_another_category');
            }
        }
        ...
        if ($this->type == 'special_users') {
            ...
            if (empty($userDiscount)) {
                return trans('lms/cart.coupon_invalid'); // not for this user
            }
        }
        ...
        if (!empty($this->minimum_order)) { // check user orders minimum amounts
            ...
            if ($this->minimum_order > $totalCartsPrice) {
                return trans('lms/update.discount_code_minimum_order_error', ['min_order' => $this->minimum_order]); // the minimum order is less than the discount amount
            }
        }
