...
use App\Models\LMS\Api\Traits\WebinarChartTrait;
use App\Models\LMS\CourseForum;
use App\Models\LMS\Webinar as Model;
...
use App\Models\LMS\CourseLearning;
...
class Webinar extends Model
{
    ...
    public function getQuizzesAverageGradeAttribute()
    {
        $quizzes = Quiz::where('webinar_id', $this->id)
            ->join('lms_quizzes_results', 'lms_quizzes_results.quiz_id', 'quizzes.id')
            ->select(DB::raw('avg(lms_quizzes_results.user_grade) as result_grade'))
            ->whereIn('lms_quizzes_results.status', ['passed', 'failed'])
            ->groupBy('lms_quizzes_results.quiz_id')
            ->get();
    ...
    public function getAssignmentsAverageGradeAttribute()
    {
        $assignments = WebinarAssignment::where('webinar_id', $this->id)
            ->join('lms_webinar_assignment_history', 'lms_webinar_assignment_history.assignment_id', 'webinar_assignments.id')
            ->select(DB::raw('avg(lms_webinar_assignment_history.grade) as result_grade'))
            ->whereIn('lms_webinar_assignment_history.status', ['passed', 'not_passed'])
            ->groupBy('lms_webinar_assignment_history.assignment_id')
            ->get();
    ...
    public function getForumsMessagesCountAttribute()
    {
        $forums = CourseForum::where('webinar_id', $this->id)
            ->join('lms_course_forum_answers', 'lms_course_forum_answers.forum_id', 'course_forums.id')
            ->select(DB::raw('count(lms_course_forum_answers.id) as count'))
            ->groupBy('lms_course_forum_answers.forum_id')
            ->get();
    ...
    public function getForumsStudentsCountAttribute($webinarId)
    {
        $forums = CourseForum::where('webinar_id', $webinarId)
            ->join('lms_course_forum_answers', 'lms_course_forum_answers.forum_id', 'course_forums.id')
            ->select(DB::raw('count(distinct lms_course_forum_answers.user_id) as count'))
            ->groupBy('lms_course_forum_answers.forum_id')
            ->get();
        ...
        if (!empty($sort)) {
            ...
            if ($sort == 'bestsellers') {
                $query->leftJoin('lms_sales', function ($join) {
                    $join->on('lms_webinars.id', '=', 'lms_sales.webinar_id')
                        ->whereNull('refund_at');
                })
                    ->whereNotNull('lms_sales.webinar_id')
                    ->select('lms_webinars.*', 'lms_sales.webinar_id', DB::raw('count(lms_sales.webinar_id) as salesCounts'))
            ...
            if ($sort == 'best_rates') {
                $query->leftJoin('lms_webinar_reviews', function ($join) {
                    ...
                    ->whereNotNull('rates')
                    ->select('lms_webinars.*', DB::raw('avg(rates) as rates'))
                    ->groupBy('lms_webinars.id')
                    ->orderBy('rates', 'desc');
            }
            ...
            $query->orderBy('lms_webinars.created_at', 'desc')
    ...
    {
        return $this->hasMany('App\Models\LMS\Api\Ticket', 'webinar_id', 'id');
    }

    public function chapters()
    {
        return $this->hasMany('App\Models\LMS\Api\WebinarChapter', 'webinar_id', 'id');
    }

    public function sessions()
    {
        return $this->hasMany('App\Models\LMS\Api\Session', 'webinar_id', 'id');
    }

    public function files()
    {
        return $this->hasMany('App\Models\LMS\Api\File', 'webinar_id', 'id');
    }

    public function textLessons()
    {
        return $this->hasMany('App\Models\LMS\Api\TextLesson', 'webinar_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\LMS\Tag', 'webinar_id', 'id');
    }

    public function purchases()
    {
        return $this->hasMany('App\Models\LMS\Purchase', 'webinar_id', 'id');
    }

    public function comments()
    {
        return $this->hasMany('App\Models\LMS\Api\Comment', 'webinar_id', 'id');
    }

    public function reviews()
    {
        return $this->hasMany('App\Models\LMS\Api\WebinarReview', 'webinar_id', 'id');
    }

    public function prerequisites()
    {
        return $this->hasMany('App\Models\LMS\Api\Prerequisite', 'webinar_id', 'id');
    }

    public function faqs()
    {
        return $this->hasMany('App\Models\LMS\Api\Faq', 'webinar_id', 'id');
    }

    public function quizzes()
    {
        return $this->hasMany('App\Models\LMS\Api\Quiz', 'webinar_id', 'id');
    }
}
