...
use App\Models\LMS\Product as Model;
...
class Product extends Model
{
    public function getLabelAttribute()
    {
        if ($this->ordering and !empty($this->inventory) and $this->getAvailability() < 1) {
            return trans('lms/update.out_of_stock');
        } elseif (!$this->ordering and $this->getActiveDiscount()) {
            return trans('lms/update.ordering_off');
        } elseif ($this->getActiveDiscount()) {
            return trans('lms/public.offer', ['off' => $this->getActiveDiscount()->percent]);
        } else {
            switch ($this->status) {
                case \App\Models\LMS\Product::$active:
                    return trans('lms/public.active');
                case Product::$inactive:
                    return trans('lms/public.rejected');
                case Product::$draft:
                    return trans('lms/public.draft');
                case Product::$pending:
                    return trans('lms/public.waiting');
                default:
                    return null;
            }
        }
    ...
    public function checkProductForSale($user)
    {
        if ($this->getAvailability() < 1) {
            ...
            return apiResponse2(0, 'not_availability', trans('lms/update.product_not_availability'));
        }

        if ($this->creator_id == $user->id) {

            return apiResponse2(0, 'same_user', trans('lms/update.cant_purchase_your_product'));
        }
    ...
    public function scopeHandleFilters($query)
    {
        ...
        if (!empty($sort)) {
            ...
            if ($sort == 'bestsellers') {
                $query->leftJoin('lms_product_orders', function ($join) {
                    $join->on('products.id', '=', 'lms_product_orders.product_id')
                        ->whereNotNull('lms_product_orders.sale_id')
                        ->whereNotIn('lms_product_orders.status', [ProductOrder::$canceled, ProductOrder::$pending]);
                })
                    ->select('products.*', DB::raw('sum(lms_product_orders.quantity) as salesCounts'))
                    ->groupBy('lms_product_orders.product_id')
                    ->orderBy('salesCounts', 'desc');
            }

            if ($sort == 'best_rates') {
                $query->leftJoin('lms_product_reviews', function ($join) {
                    $join->on('products.id', '=', 'lms_product_reviews.product_id');
                    $join->where('lms_product_reviews.status', 'active');
                })
                    ->whereNotNull('rates')
                    ->select('products.*', DB::raw('avg(rates) as rates'))
                    ->groupBy('lms_product_reviews.product_id')
                    ->orderBy('rates', 'desc');
            }
        }
    ...
    {
        return $this->hasMany('App\Models\LMS\Api\Comment', 'product_id', 'id');
    }
