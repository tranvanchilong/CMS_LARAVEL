...
use App\Models\LMS\WebinarAssignmentHistory as Model;
...
class WebinarAssignmentHistory extends Model
{
    ...
    {
        if ($this->messages) {
            return $this->messages->where('sender_id', apiauth()->guard('lms_user')->id)->first()->created_at ?? null;
        }
        return null;
    }
    ...
    {
        if ($this->messages) {
            return $this->messages->where('sender_id', apiauth()->guard('lms_user')->id)->last()->created_at ?? null;
        }
        return null;
    }
    ...
    {
        return $this->belongsTo('App\Models\LMS\Api\WebinarAssignment', 'assignment_id', 'id');
    }
}
