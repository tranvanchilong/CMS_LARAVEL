...
class Session extends Model implements TranslatableContract
{
    ...
    protected static function boot()
    {
        ...
        Session::creating(function($model) {
    ...
    {
        return $this->hasOne('App\Models\LMS\User', 'user_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\LMS\SessionRemind', 'session_id', 'id');
    }

    public function learningStatus()
    {
        return $this->hasOne('App\Models\LMS\CourseLearning', 'session_id', 'id');
    }
    ...
    {
        return $this->hasOne('App\Models\LMS\AgoraHistory', 'session_id', 'id');
    }
    ...
    public function getJoinLink($zoom_start_link = false)
    {
        ...
        if ($this->session_api == 'big_blue_button') {
            $link = 'panel/sessions/' . $this->id . '/joinToBigBlueButton';
        }

        // if ($zoom_start_link and auth()->guard('lms_user')->check() and auth()->guard('lms_user')->id() == $this->creator_id and $this->session_api == 'zoom') {
        //     $link = $this->zoom_start_link;
        // }

        if ($this->session_api == 'agora') {
            $link = 'panel/sessions/' . $this->id . '/joinToAgora';
        }

        if ($this->session_api == 'jitsi') {
            $link = 'panel/sessions/' . $this->id . '/joinToJitsi';
        }
