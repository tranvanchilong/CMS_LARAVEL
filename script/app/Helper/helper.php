...
function get_path_lms()
{
    $user_id = domain_info('user_id');
    $path = '/uploads/'.$user_id.'/lms';
    return $path;
}

function get_public_path_lms($filePath)
{
    $path = realpath(public_path().'/../../').get_path_lms().$filePath;
    return $path;
}
...
function languages()
{
    $url=Request::getHost();
    $url=str_replace('www.','',$url);
    $domain=\App\Domain::where('domain',$url)->where('status',1)->first();
    $custom_domain=\App\Models\Requestdomain::where('domain',$url)->where('status',1)->first();
    $languages=Useroption::where('key','languages')->where('user_id',$domain->user_id ?? $custom_domain->user_id)->first();
    if($languages && $languages->value){
        return json_decode($languages->value);
    }
    return [];
}

function languages_auth()
{
    $languages=Useroption::where('key','languages')->where('user_id',auth()->id())->first();
...
{
    $language_name=Option::where('key','languages')->first();
    $language_name=json_decode($language_name->value);
    return $language_name->$code ?? '';
}

function find_style($style)
{
    $path_section_image = 'script/resources/views/sections/'.$style.'/image.jpg';
    return $path_section_image;
}
...
function imageUploadSizes($type)
{
    if($type == 'thumbnail'){
        $sizes = ["key" => "thumbnail","width" => "640"];
    }elseif($type == 'top_bar'){
        $sizes = ["key" => "thumbnail","height" => "50","width" => "1320"];
    }elseif($type == 'slider'){
        $sizes = ["key" => "thumbnail","width" => "1400"];
    }elseif($type == 'section'){
        $sizes = ["key" => "thumbnail","width" => "960"];
    }
...
function ImageThumnail($url){
    if (str_contains($url, 'thumbnail.webp')) {
        $resizes = rtrim($url,"thumbnail.webp");
        $resizes_image = explode('.',$url); 
        $ext = $resizes_image[count($resizes_image) - 1];
        $imageNames = $resizes.'.'.$ext;
    }else{
        $imageNames = $url;
    }
    return $imageNames ?? '';
...
function ThemeMenuFp($menus,$path){
    $menus=$menus;
    return view($path.'.parent',compact('menus'));
}
function ThemeFooterMenu($position,$path,$class = ''){
    $menu_position = $position;
    $menus=Helper::menu_query_with_name($menu_position);
    return view($path.'.parent',compact('menus', 'class'));
}
...
function permalink_type($type)
{
    $url = Request::getHost();
    $url = str_replace('www.','',$url);

    if (!Cache::has($url.'_permalinks')) {
        $domain =\App\Domain::where('domain',$url)->where('status',1)->first();
        $custom_domain = \App\Models\Requestdomain::where('domain',$url)->where('status',1)->first();

        if($domain){
            Cache::forever($url.'_permalinks', $domain->permalinks);
        }
        if($custom_domain){
            Cache::forever($url.'_permalinks', $custom_domain->parentdomain->permalinks);
        }
    }

    $permalinks = Cache::get($url.'_permalinks');

    if(!$permalinks || !isset($permalinks[$type]) || !$permalinks[$type]){
        return str_replace('_','-',$type);
    }

    return $permalinks[$type];
}
function resizeImage($image, $c_type, $level, $fileName, $path) {
    ...
    // cek & ricek
    if(in_array($c_type, array('jpeg','jpg','JPG','JPEG','gif','GIF','png','PNG','webp'))) { // jpeg, png, gif only
       
         $result['data'] = Helper::createImg($image, $im_name, $im_type, $im_size, $c_type, $level, $fileName, $path);
...
function imageHeight($images,$imageSizes)
{
    $getImageHeigth = (($images->height() / $images->width()) * $imageSizes['width']);
    return $getImageHeigth;
}

function allowedExts()
{
    $allowedExts = array('jpeg','jpg','JPG','JPEG','gif','GIF','png','PNG','svg');
    return $allowedExts;
}

function calculateCommissionAffilationClubPoint($order)
{
    if(feature_is_activated('affiliate_status', auth()->user()->id)){
...
function feature_is_activated($identifier, $user_id)
{
    $affiliate_status = Useroption::where('user_id',$user_id)->where('key',$identifier)->where('value',1)->first();                               
    $affiliate_status = !empty($affiliate_status) ? $affiliate_status->value : null;
...
{
    $find_sections = \File::directories('script/resources/views/sections');
    $category_sections=[];
    if($find_sections){
        foreach($find_sections as $find_section){
            if(is_dir($find_section)){
                if(file_exists($find_section.'/config.json') && file_exists($find_section.'/index.blade.php') && file_exists($find_section.'/image.jpg')){
                    $find_section = json_decode(file_get_contents($find_section.'/config.json'));
                    if(!empty($find_section->key) && !empty($find_section->title) && !empty($find_section->category) && !empty($find_section->data_type)){
                        $section = [
                            "key" => $find_section->key,
                            "title" => $find_section->title,
                            "data_type" => $find_section->data_type
                        ];
                        if(!isset($category_sections[$find_section->category])) {
                            $category_sections[$find_section->category] = [
                                'title' => $find_section->category,
                                'style' => []
                            ];
                        }
                        $category_sections[$find_section->category]['style'][$find_section->key] = $section;
                    }
                }
            }
        }
    ...
    return array_values($category_sections);
}

function check_file_in_sections($section)
{
    $file_path = 'script/resources/views/sections/'.$section.'/index.blade.php';
