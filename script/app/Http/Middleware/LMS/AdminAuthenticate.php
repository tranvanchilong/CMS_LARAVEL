...
use App\Http\Controllers\LMS\Admin\SidebarController;
...
class AdminAuthenticate
{
    ...
    public function handle($request, Closure $next)
    {
        if (auth()->guard('lms_user')->check() and auth()->guard('lms_user')->user()->isAdmin()) {
            ...
            if (auth()->guard('lms_user')->user()->hasPermission('admin_notifications_list')) {
                // $adminUser = User::find(1);
                $adminUser = auth()->guard('lms_user')->user();
