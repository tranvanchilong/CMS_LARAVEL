...
class MyPurchaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = $query = ProductOrder::query()
            ->where(function ($query) use ($user, $giftsIds) {
                $query->where('lms_product_orders.buyer_id', $user->id);
                $query->orWhereIn('lms_product_orders.gift_id', $giftsIds);
            })
        ...
        $data = [
            'pageTitle' => trans('lms/update.product_purchases_lists_page_title'),
        ...
        return view('lms.web.default.panel.store.my-purchases', $data);
    }
    ...
    public function invoice($saleId, $orderId)
    {
        ...
        $productOrder = $productOrder = ProductOrder::query()
            ->where(function ($query) use ($user, $giftsIds) {
