...
class ReserveMeetingController extends Controller
{
    public function reservation(Request $request)
    {
        ...
        $openReserveCount = deepClone($reserveMeetingsQuery)->where('status', \App\Models\LMS\ReserveMeeting::$open)->count();
        ...
        $data = [
            'pageTitle' => trans('lms/meeting.meeting_list_page_title'),
        ...
        return view('lms.'. getTemplate() . '.panel.meeting.reservation', $data);
    }

    public function requests(Request $request)
    {
        $meetingIds = Meeting::where('creator_id', auth()->guard('lms_user')->user()->id)->pluck('id');
        ...
        $pendingReserveCount = deepClone($reserveMeetingsQuery)->where('status', \App\Models\LMS\ReserveMeeting::$pending)->count();
        $totalReserveCount = deepClone($reserveMeetingsQuery)->count();
        ...
        $data = [
            'pageTitle' => trans('lms/meeting.meeting_requests_page_title'),
            'reserveMeetings' => $reserveMeetings,
        ...
        return view('lms.'. getTemplate() . '.panel.meeting.requests', $data);
    }
        ...
        if (!empty($ReserveMeeting)) {
            ...
            if (!empty($session)) {
                SessionTranslation::updateOrCreate([
                    ...
                    'title' => trans('lms/update.new_in-app_call_session'),
                    'description' => trans('lms/update.new_in-app_call_session'),
                ]);
