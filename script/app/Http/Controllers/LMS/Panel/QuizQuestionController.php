...
use App\Models\LMS\QuizzesQuestionsAnswerTranslation;
use App\Models\LMS\QuizzesQuestionTranslation;
...
class QuizQuestionController extends Controller
{
    public function store(Request $request)
    {
        ...
        if (!empty($data['image']) and !empty($data['video'])) {
            return response()->json([
                'code' => 422,
                'errors' => [
                    'image' => [trans('lms/update.quiz_question_image_validation_by_video')],
                    'video' => [trans('lms/update.quiz_question_image_validation_by_video')],
                ]
            ], 422);
        }
        ...
        if ($data['type'] == QuizzesQuestion::$multiple and !empty($data['answers'])) {
            ...
            if (!$hasCorrect) {
                return response([
                    'code' => 422,
                    'errors' => [
                        'current_answer' => [trans('lms/quiz.current_answer_required')]
                    ],
                ], 422);
            }
        }
    ...
    public function destroy(Request $request, $id)
    {
        QuizzesQuestion::where('id', $id)
            ->where('creator_id', auth()->guard('lms_user')->user()->id)
            ->delete();
