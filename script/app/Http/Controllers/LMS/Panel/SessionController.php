...
use App\Models\LMS\SessionTranslation;
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($webinar) and $webinar->canAccess($user)) {
            ...
            if ($sessionDate->getTimestamp() < $webinar->start_date) {
                $error = [
                    'date' => [trans('lms/webinars.session_date_must_larger_webinar_start_date', ['start_date' => dateTimeFormat($webinar->start_date, 'j M Y')])]
                ];
    ...
    private function handleZoomApi($session, $user)
    {
        try {
            ...
            return response()->json([
                ...
                'zoom_error_msg' => trans('lms/update.zoom_error_msg')
            ], 422);
        }
    }
        ...
        if (!empty($session) and !empty($user)) {
            ...
            if (!empty($agoraHistory) and !empty($agoraHistory->end_at)) {
                $toastData = [
                    ...
                    'msg' => trans('lms/update.this_live_has_been_ended'),
                    'status' => 'error'
                ];
                return redirect('/lms/panel')->with(['toast' => $toastData]);
            }
            ...
            $canAccessError = trans('lms/update.you_cannot_enter_this_session');
            ...
            if ($user->id == $session->creator_id) {
                ...
                if (!empty($session->reserve_meeting_id)) {
                    $ReserveMeeting = ReserveMeeting::where('id', $session->reserve_meeting_id)
                        ...
                        ->where('status', \App\Models\LMS\ReserveMeeting::$open)
                        ->first();
                ...
                if ($canAccess) {
                    ...
                    if (!$canAccess) {
                        $canAccessError = trans('lms/update.join_to_the_session_has_been_disabled_by_the_instructor');
                    }
                }
            }
            ...
            if ($canAccess) {
                ...
                $data = [
                    'pageTitle' => trans('lms/update.live_session'),
                    'session' => $session,
                    'isHost' => $isHost,
                    'appId' => $appId,
                ...
                return view('lms.web.default.course.agora.index', $data);
            } else {
                $toastData = [
                    'title' => trans('lms/public.request_failed'),
                    'msg' => $canAccessError,
    ...
    public function toggleUsersJoinToAgora($id)
    {
        ...
        if (!empty($session)) {
            ...
            if ($session->creator_id == $user->id or (!empty($webinar) and $webinar->canAccess($user))) {
                ...
                return response()->json([
                    'code' => 200,
                    'heading' => trans('lms/public.request_success'),
                    'text' => $userJoin ? trans('lms/update.joining_users_to_the_session_is_enabled') : trans('lms/update.joining_users_to_the_meeting_was_stopped'),
    ...
    public function joinToJitsi($id)
    {
        ...
        if (!empty($session) and !empty($user)) {
            ...
            if (!empty($webinar)) {
                ...
                if ($canAccess) {
                    $data = [
                        'pageTitle' => trans('lms/update.jitsi_live_class'),
                        'session' => $session
                    ];

                    return view('lms.web.default.course.jitsi.join_live', $data);
                }
            }
        }
