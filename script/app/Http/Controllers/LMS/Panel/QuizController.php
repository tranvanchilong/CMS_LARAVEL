...
use App\Models\LMS\QuizzesQuestionsAnswer;
...
class QuizController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($quizzes as $quiz) {
            ...
            $countSuccess = $quiz->quizResults
                ->where('status', \App\Models\LMS\QuizzesResult::$passed)
                ->pluck('user_id')
                ->count();
        ...
        $data = [
            'pageTitle' => trans('lms/quiz.quizzes_list_page_title'),
            'quizzes' => $quizzes,
        ...
        return view('lms.'. getTemplate() . '.panel.quizzes.lists', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/quiz.new_quiz_page_title'),
            'webinars' => $webinars,
        ...
        return view('lms.'. getTemplate() . '.panel.quizzes.create', $data);
    }
    ...
    public function start(Request $request, $id)
    {
        ...
        if ($quiz) {
            ...
            if (!empty($quiz->webinar_id)) {
                ...
                if (!empty($quiz->expiry_days)) {
                    ...
                    if (!$hasAccess) {
                        $toastData = [
                            'title' => trans('lms/public.request_failed'),
                            'msg' => trans('lms/update.your_access_to_this_quiz_has_been_expired'),
                            'status' => 'error'
                        ];
                        return back()->with(['toast' => $toastData]);
                    }
            ...
            if (!isset($quiz->attempt) or ($userQuizDone->count() < $quiz->attempt and !$status_pass)) {
                ...
                $data = [
                    'pageTitle' => trans('lms/quiz.quiz_start'),
                    'quiz' => $quiz,
                ...
                return view('lms.'. getTemplate() . '.panel.quizzes.start', $data);
            } else {
                $toastData = [
                    'title' => trans('lms/public.request_failed'),
                    'msg' => trans('lms/quiz.cant_start_quiz'),
                    'status' => 'error'
                ];
                return back()->with(['toast' => $toastData]);
            }
        ...
        if ($quiz) {
            ...
            if (!empty($quizResultId)) {
                ...
                if (!empty($quizResult)) {
                    ...
                    return redirect('/lms/panel/quizzes/'.$quizResult->id.'/status')->with(['quizResultId' => $quizResult]);
                }
            }
        }
        abort(404);
    ...
    public function status($quizResultId)
    {
        ...
        if ($quizResult) {
            ...
            $data = [
                'pageTitle' => trans('lms/quiz.quiz_status'),
            ...
            return view('lms.'. getTemplate() . '.panel.quizzes.status', $data);
        }
        abort(404);
    }
    ...
    {
        $query = QuizzesResult::where('user_id', auth()->guard('lms_user')->user()->id);

        $quizResultsCount = deepClone($query)->count();
        $passedCount = deepClone($query)->where('status', \App\Models\LMS\QuizzesResult::$passed)->count();
        $failedCount = deepClone($query)->where('status', \App\Models\LMS\QuizzesResult::$failed)->count();
        $waitingCount = deepClone($query)->where('status', \App\Models\LMS\QuizzesResult::$waiting)->count();
        ...
        $data = [
            'pageTitle' => trans('lms/quiz.my_results'),
        ...
        return view('lms.'. getTemplate() . '.panel.quizzes.my_results', $data);
    }
    ...
    public function opens(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/quiz.open_quizzes'),
            'quizzes' => $quizzes
        ];

        return view('lms.'. getTemplate() . '.panel.quizzes.opens', $data);
    }
    ...
    public function results(Request $request)
    {
        ...
        if (!$user->isUser()) {
            ...
            $data = [
                'pageTitle' => trans('lms/quiz.results'),
            ...
            return view('lms.'. getTemplate() . '.panel.quizzes.results', $data);
        }
    ...
    public function showResult($quizResultId)
    {
        ...
        if (!empty($quizResult)) {
            ...
            $data = [
                'pageTitle' => trans('lms/quiz.result'),
                'quiz' => $quizResult->quiz,
                'quizResult' => $quizResult,
            ...
            return view('lms.'. getTemplate() . '.panel.quizzes.quiz_result', $data);
        }
