...
use App\Models\LMS\UserZoomApi;
...
class UserController extends Controller
{
    public function setting($step = 1)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/panel.settings'),
            'user' => $user,
        ...
        return view('lms.'. getTemplate() . '.panel.setting.index', $data);
    }
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($data['organization_id']) and !empty($data['user_id'])) {
            $organization = auth()->guard('lms_user')->user();
        ...
        if (!empty($user)) {
            ...
            $url = '/lms/panel/setting';
            if (!empty($organization)) {
                $userType = $user->isTeacher() ? 'instructors' : 'students';
                $url = "/lms/panel/manage/{$userType}/{$user->id}/edit";
            }
            ...
            $toastData = [
                ...
                'msg' => trans('lms/panel.user_setting_success'),
                'status' => 'success'
            ];
    ...
    public function createImage($user, $img)
    {
        ...
        return Storage::disk('public')->url(get_path_lms().$folderPath . $file);
    }
    ...
    public function manageUsers(Request $request, $user_type)
    {
        ...
        if ($organization->isOrganization() and in_array($user_type, $valid_type)) {
            ...
            $data = [
                'pageTitle' => trans('lms/public.' . $user_type),
                'user_type' => $user_type,
            ...
            return view('lms.'. getTemplate() . '.panel.manage.' . $user_type, $data);
        }
    ...
    public function createUser($user_type)
    {
        ...
        if ($organization->isOrganization() and in_array($user_type, $valid_type)) {
            ...
            $data = [
                'pageTitle' => trans('lms/public.new') . ' ' . trans('lms/quiz.' . $user_type),
                'new_user' => true,
                'user_type' => $user_type,
                'user' => $organization,
                'categories' => $categories,
    ...
    public function storeUser(Request $request, $user_type)
    {
        ...
        if ($organization->isOrganization() and in_array($user_type, $valid_type)) {
            ...
            return redirect('/lms/panel/manage/' . $user_type . '/' . $user->id . '/edit');
        }
    ...
    public function deleteAccount(Request $request)
    {
        ...
        if (!empty($user)) {
            ...
            return response()->json([
                'code' => 200,
                'title' => trans('lms/public.request_success'),
                'text' => trans('lms/update.delete_account_request_stored_msg'),
                'dont_reload' => true
            ]);
        }
