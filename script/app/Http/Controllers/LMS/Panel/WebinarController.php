...
class WebinarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['pageTitle'] = trans('lms/webinars.webinars_list_page_title');

        return view('lms.'. getTemplate() . '.panel.webinar.index', $data);
    }
    ...
    public function invitations(Request $request)
    {
        ...
        $data = $this->makeMyClassAndInvitationsData($query, $user, $request);
        $data['pageTitle'] = trans('lms/panel.invited_classes');
    ...
    public function organizationClasses(Request $request)
    {
        ...
        if (!empty($user->organ_id)) {
            ...
            $data = [
                'pageTitle' => trans('lms/panel.organization_classes'),
                'webinars' => $webinars,
            ];

            return view('lms.'. getTemplate() . '.panel.webinar.organization_classes', $data);
        }
    ...
    public function create(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/webinars.new_page_title'),
            'teachers' => $teachers,
        ...
        return view('lms.'. getTemplate() . '.panel.webinar.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $url = '/lms/panel/webinars';
        if ($data['get_next'] == 1) {
            $url = '/lms/panel/webinars/' . $webinar->id . '/step/2';
        }
    ...
    public function edit(Request $request, $id, $step = 1)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/webinars.new_page_title_step', ['step' => $step]),
            'currentStep' => $step,
            'isOrganization' => $isOrganization,
            'userLanguages' => getUserLanguagesLists(),
        ...
        $data['pageTitle'] = trans('lms/public.edit') . ' ' . $webinar->title;
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($getNextStep) {
            ...
            $url = '/lms/panel/webinars/' . $webinar->id . '/step/' . (($nextStep <= $stepCount) ? $nextStep : $stepCount);
        }

        if ($webinarRulesRequired) {
            $url = '/lms/panel/webinars/' . $webinar->id . '/step/8';
    ...
    public function duplicate($id)
    {
        ...
        if (!empty($webinar)) {
            ...
            $title = $webinar->title . ' ' . trans('lms/public.copy');
            ...
            return redirect('/lms/panel/webinars/' . $newWebinar->id . '/edit');
        }
    ...
    public function invoice($webinarId, $saleId)
    {
        ...
        if (!empty($sale)) {
            ...
            if (!empty($webinar)) {
                ...
                return view('lms.'. getTemplate() . '.panel.webinar.invoice', $data);
            }
        }
    ...
    public function purchases(Request $request)
    {
        ...
        $query = Sale::query()
            ->where(function ($query) use ($user, $giftsIds) {
                $query->where('lms_sales.buyer_id', $user->id);
            ...
            ->where(function ($query) {
                $query->where(function ($query) {
                    $query->whereNotNull('lms_sales.webinar_id')
                        ->where('lms_sales.type', 'webinar')
                        ->whereHas('webinar', function ($query) {
                            $query->where('status', 'active');
                        });
                });
                $query->orWhere(function ($query) {
                    $query->whereNotNull('lms_sales.bundle_id')
                        ->where('lms_sales.type', 'bundle')
        ...
        $webinarsHours = deepClone($query)->join('lms_webinars', 'lms_webinars.id', 'lms_sales.webinar_id')
            ->select(DB::raw('sum(lms_webinars.duration) as duration'))
            ->sum('duration');
        $bundlesHours = deepClone($query)->join('lms_bundle_webinars', 'lms_bundle_webinars.bundle_id', 'lms_sales.bundle_id')
        ...
        $upComing = deepClone($query)->join('lms_webinars', 'lms_webinars.id', 'lms_sales.webinar_id')
            ->where('lms_webinars.start_date', '>', $time)
            ->count();

        $data = [
            'pageTitle' => trans('lms/webinars.webinars_purchases_page_title'),
            'sales' => $sales,
        ...
        return view('lms.'. getTemplate() . '.panel.webinar.purchases', $data);
    }
    ...
    public function orderItems(Request $request)
    {
        ...
        if (!empty($itemIds) and is_array($itemIds) and count($itemIds)) {
            switch ($tableName) {
                ...
                case 'lms_webinar_extra_descriptions_learning_materials':
                    foreach ($itemIds as $order => $id) {
                ...
                case 'lms_webinar_extra_descriptions_company_logos':
                    foreach ($itemIds as $order => $id) {
                        WebinarExtraDescription::where('id', $id)
                            ->where('creator_id', $user->id)
                ...
                case 'lms_webinar_extra_descriptions_requirements':
                    foreach ($itemIds as $order => $id) {
                        WebinarExtraDescription::where('id', $id)
                            ->where('creator_id', $user->id)
