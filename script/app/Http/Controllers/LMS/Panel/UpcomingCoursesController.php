...
use App\Models\LMS\UpcomingCourseTranslation;
use App\Models\LMS\UpcomingCourse;
use App\Models\LMS\UpcomingCourseFilterOption;
use App\Models\LMS\UpcomingCourseFollower;
...
class UpcomingCoursesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/update.my_upcoming_courses'),
        ...
        return view('lms.'. getTemplate() . '.panel.upcoming_courses.lists', $data);
    }
    ...
    public function create()
    {
        ...
        $data = [
            'pageTitle' => trans('lms/update.new_upcoming_course'),
            'currentStep' => 1,
        ...
        return view('lms.'. getTemplate() . '.panel.upcoming_courses.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($upcomingCourse)) {
            ...
            $url = '/lms/panel/upcoming_courses';
            if ($data['get_next'] == 1) {
                $url = '/lms/panel/upcoming_courses/' . $upcomingCourse->id . '/step/2';
            }
    ...
    public function edit(Request $request, $id, $step = 1)
    {
        ...
        $data['pageTitle'] = trans('lms/public.edit') . ' | ' . $upcomingCourse->title;
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($getNextStep) {
            ...
            $url = '/lms/panel/upcoming_courses/' . $upcomingCourse->id . '/step/' . (($nextStep <= 4) ? $nextStep : 4);
        }

        if ($upcomingCourseRulesRequired) {
            $url = '/lms/panel/upcoming_courses/' . $upcomingCourse->id . '/step/4';
    ...
    public function followers($id)
    {
        ...
        if (!empty($upcomingCourse)) {
            ...
            $data = [
                'pageTitle' => trans('lms/update.course_followers'),
                'upcomingCourse' => $upcomingCourse,
                'followers' => $followers,
            ];

            return view('lms.web.default.panel.upcoming_courses.followers', $data);
        }
    ...
    public function storeAssignCourse(Request $request, $id)
    {
        ...
        if (empty($webinar)) {
            return response([
                'code' => 422,
                'errors' => [
                    'course' => [trans('lms/validation.required', ['attribute' => 'course'])]
                ],
            ], 422);
        }
        ...
        if (!empty($upcomingCourse)) {
            ...
            return response()->json([
                'code' => 200,
                'msg' => trans('lms/update.assign_published_course_successful')
            ]);
        }
    ...
    public function followings()
    {
        ...
        $data = [
            'pageTitle' => trans('lms/update.following_courses'),
            'upcomingCourses' => $upcomingCourses
        ];

        return view('lms.web.default.panel.upcoming_courses.followings', $data);
    }
}
