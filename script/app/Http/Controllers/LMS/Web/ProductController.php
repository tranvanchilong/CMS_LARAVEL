...
class ProductController extends Controller
{
    ...
    public function handleFilters(Request $request, $query, $isRewardProducts = false)
    {
                ...
                $query->where(function ($query) {
                    $query->where('unlimited_inventory', true)
                        ->orWhereHas('productOrders', function ($query) {
                            $query->havingRaw('lms_products.inventory > sum(quantity)')
        ...
        if (!empty($filterOption) and is_array($filterOption)) {
            ...
            $query->whereIn('lms_products.id', $productIdsFilterOptions);
        }
        ...
        if (!empty($sort)) {
            ...
            if ($sort == 'bestsellers') {
                $query->leftJoin('lms_product_orders', function ($join) {
                    $join->on('lms_products.id', '=', 'lms_product_orders.product_id')
                        ->whereNotNull('lms_product_orders.sale_id')
                        ->whereNotIn('lms_product_orders.status', [ProductOrder::$canceled, ProductOrder::$pending]);
                })
                    ->select('lms_products.*', DB::raw('sum(lms_product_orders.quantity) as salesCounts'))
            ...
            if ($sort == 'best_rates') {
                $query->leftJoin('lms_product_reviews', function ($join) {
                    $join->on('lms_products.id', '=', 'lms_product_reviews.product_id');
                    $join->where('lms_product_reviews.status', 'active');
                })
                    ->whereNotNull('rates')
                    ->select('lms_products.*', DB::raw('avg(rates) as rates'))
        ...
        if (auth()->guard('lms_user')->check()) {
            ...
            if (!empty($product) and $product_id == $product->id) {
                if (empty($product->point)) {
                    $toastData = [
                        'title' => '',
                        'msg' => trans('lms/update.can_not_buy_this_product_with_point'),
                        'status' => 'error'
                    ];
                ...
                if ($availablePoints < $product->point) {
                    $toastData = [
                        'title' => '',
                        'msg' => trans('lms/update.you_have_no_enough_points_for_this_product'),
                        'status' => 'error'
                    ];
                    return back()->with(['toast' => $toastData]);
                }
                ...
                $toastData = [
                    'title' => '',
                    'msg' => trans('lms/update.success_pay_product_with_point_msg'),
                    'status' => 'success'
                ];
                return back()->with(['toast' => $toastData]);
            }
