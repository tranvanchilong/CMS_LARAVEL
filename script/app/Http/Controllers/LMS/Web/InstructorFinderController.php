...
class InstructorFinderController extends Controller
{
    ...
    {
        $query = User::where('lms_users.status', 'active')
        ...
        foreach ($mapUsers as $mapUser) {
            ...
            $mapUser->profileUrl = url(get_path_lms().$mapUser->getProfileUrl());
        ...
        return view('lms.web.default.instructorFinder.index', $data);
    }
    ...
    private function handleFilters($query, Request $request)
    {
        ...
        $query->whereIn('lms_users.role_name', $role);
        ...
        if (!empty($levelOfTraining) and in_array($levelOfTraining, UserLevelOfTraining::$levelOfTraining)) {
            ...
            $query->whereRaw('lms_users.level_of_training & ? > 0', [$levelBit]);
        }
        ...
        if (!empty($meetingSupport) and $meetingSupport != 'all') {
            $query->where('lms_users.meeting_type', $meetingSupport);
        }
        ...
        if (!empty($sort)) {
            ...
            // order by meetings
            $query->leftJoin('lms_meetings', 'lms_meetings.creator_id', '=', 'lms_users.id')
                ->select('lms_users.*', DB::raw('count(lms_meetings.id) as meetingCounts'))
                ->groupBy('lms_users.id')
                ->orderBy('meetingCounts', 'desc')
                ->orderBy('lms_users.id', 'desc');
        }
    ...
    private function handleDaysAndTimeFilter($query, Request $request)
    {
        ...
        if (!empty($minTime) and !empty($maxTime)) {
            ...
            if (!empty($days) and is_array($days)) {
                $meetingsTimes = MeetingTime::whereIn('lms_meeting_times.day_label', $days)
                    ->get();
    ...
    public function wizard(Request $request)
    {
        ...
        if ($step > 4) {
            ...
            $url = '/lms/instructor-finder?' . http_build_query($params);
        ...
        return view('lms.web.default.instructorFinder.wizard', $data);
    }
}
