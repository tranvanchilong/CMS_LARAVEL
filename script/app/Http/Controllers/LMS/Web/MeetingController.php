...
class MeetingController extends Controller
{
    public function reserve(Request $request)
    {
        ...
        if (!empty($user)) {
                ...
                if (!empty($meetingTime)) {
                    ...
                    if ($meeting->creator_id == $user->id) {
                        $toastData = [
                            ...
                            'msg' => trans('lms/update.cant_reserve_your_appointment'),
                            'status' => 'error'
                        ];
                    ...
                    if (!empty($meeting) and !$meeting->disabled) {
                        if (!empty($meeting->amount) and $meeting->amount > 0) {
                            ...
                            if (!empty($reserveMeeting) and $reserveMeeting->locked_at) {
                                $toastData = [
                                    'title' => trans('lms/public.request_failed'),
                                    'msg' => trans('lms/meeting.locked_time'),
                                    'status' => 'error'
                                ];
                                return response()->json($toastData);
                            }
                            ...
                            if (!empty($reserveMeeting) and $reserveMeeting->reserved_at) {
                                $toastData = [
                                    'title' => trans('lms/public.request_failed'),
                                    'msg' => trans('lms/meeting.reserved_time'),
                                    'status' => 'error'
                                ];
                                return response()->json($toastData);
                            }
                            ...
                            $toastData = [
                                ...
                                'msg' => trans('lms/update.meeting_added_to_cart'),
                                'redirect' => '/lms/cart'
                            ];
                            return response()->json($toastData);
                        } else {
                        ...
                        $toastData = [
                            'title' => trans('lms/public.request_failed'),
                            'msg' => trans('lms/meeting.meeting_disabled'),
                            'status' => 'error'
                        ];
                        return response()->json($toastData);
                    }
            ...
            $toastData = [
                'title' => trans('lms/public.request_failed'),
                'msg' => trans('lms/meeting.select_time_to_reserve'),
                'status' => 'error'
            ];
            return response()->json($toastData);
        }
        ...
        $toastData = [
            'title' => trans('lms/public.request_failed'),
            'msg' => trans('lms/public.not_login_toast_msg_lang'),
            'status' => 'error'
        ];
        return response()->json($toastData);
    }
    ...
    {
        // $date = $day . ' ' . $clock;
        $utcDate = convertTimeToUTCzone($day, $instructorTimezone);
        ...
        if ($selectedMeetingType == 'in_person' and in_array($meetingTime->meeting_type, ['in_person', 'all'])) {
            if ($meeting->in_person) {
                ...
                $toastData = [
                    'status' => 'error',
                    'title' => trans('lms/public.request_failed'),
                    'msg' => trans('lms/update.in_person_meetings_unavailable'),
                ];
                ...
                if ($selectedMeetingType == $type and in_array($meetingTime->meeting_type, ['all', $type])) {
                    ...
                    if ($studentCount < $meeting->$meetingMinVar) {
                        ...
                        $toastData = [
                            'status' => 'error',
                            'title' => trans('lms/public.request_failed'),
                            'msg' => trans('lms/update.group_meeting_max_student_count_hint', ['max' => $meeting->$meetingMaxVar]),
                        ];
                        $hourlyAmount = response()->json($toastData);
                        $status = false;
    ...
    private function handleFreeMeetingReservation($user, $meeting, $meetingTime, $day, $selectedMeetingType, $studentCount)
    {
        ...
        $toastData = [
            'title' => '',
            'msg' => trans('lms/cart.success_pay_msg_for_free_meeting'),
            'status' => 'success'
        ];
        return response()->json($toastData);
    }
