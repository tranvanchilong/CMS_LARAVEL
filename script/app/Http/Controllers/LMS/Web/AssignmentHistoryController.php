                ...
                if (!empty($assignmentHistory) and $historyId == $assignmentHistory->id) {
                    ...
                    if ($user->id != $assignment->creator_id) {
                        ...
                        if (!$deadline or (!empty($assignment->attempts) and $submissionTimes >= $assignment->attempts)) {
                            $toastData = [
                                'title' => !$deadline ? trans('lms/update.assignment_deadline_error_title') : trans('lms/update.assignment_submission_error_title'),
                                'msg' => !$deadline ? trans('lms/update.assignment_deadline_error_desc') : trans('lms/update.assignment_submission_error_desc'),
                            ];
        ...
        if (!empty($assignment)) {
            ...
            if (!empty($webinar) and $webinar->checkUserHasBought($user)) {
                ...
                if (!empty($message) and !empty($message->file_path)) {
                    $filePath = get_public_path_lms($message->file_path);
