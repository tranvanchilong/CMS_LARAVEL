...
use App\Models\LMS\ForumTopicAttachment;
use App\Models\LMS\ForumTopicBookmark;
use App\Models\LMS\ForumTopicLike;
use App\Models\LMS\ForumTopicPost;
...
class ForumController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('lms.web.default.forum.index', $data);
    }
    ...
    public function search(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/update.search_results_for', ['temp' => $search]),
            'pageDescription' => '',
            'pageRobot' => '',
            'topics' => $topics,
        ...
        return view('lms.web.default.forum.topics_search', $data);
    }
    ...
    public function topics(Request $request, $slug)
    {
        ...
        if (!empty($forum)) {
            $query = ForumTopic::where('lms_forum_topics.forum_id', $forum->id);
            ...
            return view('lms.web.default.forum.topics', $data);
        }
    ...
    private function handleTopics(Request $request, $query, &$resultCount)
    {
        ...
        if (!empty($search)) {
            ...
            $query->where(function ($query) use ($topicsIds, $search) {
                $query->whereIn('lms_forum_topics.id', $topicsIds)
                    ->orWhere('lms_forum_topics.title', 'like', "%$search%")
                    ->orWhere('lms_forum_topics.description', 'like', "%$search%");
            });
        }

        $query->orderBy('lms_forum_topics.pin', 'desc');

        if (!empty($sort) and $sort != 'newest') {
            if ($sort == 'popular_topics') {
                $query->join('lms_forum_topic_posts', 'lms_forum_topic_posts.topic_id', 'lms_forum_topics.id')
                    ->select('lms_forum_topics.*', DB::raw("count(lms_forum_topic_posts.topic_id) as topic_posts_count"))
                    ->orderBy('topic_posts_count', 'desc');
            } elseif ($sort == 'not_answered') {
                $query->whereDoesntHave('posts');
                $query->orderBy('lms_forum_topics.created_at', 'desc');
            }
        } else {
            $query->orderBy('lms_forum_topics.created_at', 'desc');
        }
    ...
    {
        return User::leftJoin('lms_forum_topics', 'lms_forum_topics.creator_id', 'lms_users.id')
            ->leftJoin('lms_forum_topic_posts', 'lms_forum_topic_posts.user_id', 'lms_users.id')
            ->select('lms_users.id', 'lms_users.full_name', 'lms_users.avatar', DB::raw("count(lms_forum_topics.creator_id) as topics, count(lms_forum_topic_posts.user_id) as posts"), DB::raw("(count(lms_forum_topics.creator_id) + count(lms_forum_topic_posts.user_id)) as all_posts"))
            ->whereHas('forumTopics')
            ->groupBy('lms_forum_topics.creator_id')
            ->groupBy('lms_forum_topic_posts.user_id')
    ...
    private function getPopularTopics()
    {
        return ForumTopic::query()
            ->join('lms_forum_topic_posts', 'lms_forum_topic_posts.topic_id', 'lms_forum_topics.id')
            ->select('lms_forum_topics.*', DB::raw("count(lms_forum_topic_posts.topic_id) as posts_count"))
            ->whereHas('creator')
            ->with([
                'creator' => function ($query) {
                    $query->select('id', 'full_name', 'avatar');
    ...
    public function createTopic(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/update.create_new_topic'),
            'pageDescription' => '',
            'pageRobot' => '',
            'forums' => $forums
        ];

        return view('lms.web.default.forum.create_topic', $data);
    }
    ...
    public function storeTopic(Request $request)
    {
        ...
        if (!empty($forum) and $forum->checkUserCanCreateTopic($user)) {
            ...
            $toastData = [
                ...
                'msg' => trans('lms/update.new_topic_successfully_created'),
                'status' => 'success'
            ];

            $url = '/lms/forums/' . $topic->forum->slug . '/topics';
    ...
    public function topicDownloadAttachment($forumSlug, $topicSlug, $attachmentId)
    {
        ...
        if (!empty($forum)) {
            ...
            if (!empty($topic)) {
                ...
                if (!empty($attachment)) {
                    $filePath = get_public_path_lms($attachment->path);
    ...
    public function posts($forumSlug, $topicSlug)
    {
        ...
        if (!empty($forum) and $forum->checkUserCanCreateTopic()) {
            ...
            if (!empty($topic)) {
                ...
                return view('lms.web.default.forum.posts', $data);
            }
        }
    ...
    public function storeTopicReport(Request $request, $forumSlug, $topicSlug)
    {
        ...
        if (!empty($user)) {
            ...
            $notifyOptions = [
                '[u.name]' => $user->full_name,
                '[content_type]' => trans('lms/public.' . $data['item_type'])
            ];
    ...
    public function postDownloadAttachment($forumSlug, $topicSlug, $postId)
    {
        ...
        if (!empty($user) and !empty($forum) and $forum->checkUserCanCreateTopic($user)) {
            ...
            if (!empty($topic)) {
                ...
                if (!empty($post)) {
                    $filePath = get_public_path_lms($post->attach);
