...
class UserController extends Controller
{
    ...
    public function profile($id)
    {
        ...
        if (!empty($meeting) and !empty($meeting->meetingTimes)) {
            ...
            $authUser = auth()->guard('lms_user')->user();
            // Installments
            /*
            if (getInstallmentsSettings('status') and (empty($authUser) or $authUser->enable_installments)) {
                ...
                $installments = $installmentPlans->getPlans('lms_meetings', null, null, null, $user->id);
            }*/
            ...
            if (getFeaturesSettings('cashback_active') and (empty($authUser) or !$authUser->disable_cashback)) {
                ...
                $cashbackRules = $cashbackRulesMixin->getRules('lms_meetings', null, null, null, $user->id);
            }
        }
        ...
        if (auth()->guard('lms_user')->check()) {
            $authUserIsFollower = $followers->where('follower', auth()->guard('lms_user')->id())
        ...
        $data = [
            'pageTitle' => $user->full_name . ' ' . trans('lms/public.profile'),
            'user' => $user,
        ...
        return view('lms.web.default.user.profile', $data);
    }
    ...
    public function instructors(Request $request)
    {
        ...
        $pageTitle = !empty($seoSettings['title']) ? $seoSettings['title'] : trans('lms/home.instructors');
        $pageDescription = !empty($seoSettings['description']) ? $seoSettings['description'] : trans('lms/home.instructors');
        $pageRobot = getPageRobot('instructors');

        $data = $this->handleInstructorsOrOrganizationsPage($request, Role::$teacher);

        $data['title'] = trans('lms/home.instructors');
        $data['page'] = 'instructors';
        $data['pageTitle'] = $pageTitle;
        $data['pageDescription'] = $pageDescription;
        $data['pageRobot'] = $pageRobot;

        return view('lms.web.default.pages.instructors', $data);
    }

    public function organizations(Request $request)
    {
        $seoSettings = getSeoMetas('organizations');
        $pageTitle = !empty($seoSettings['title']) ? $seoSettings['title'] : trans('lms/home.organizations');
        $pageDescription = !empty($seoSettings['description']) ? $seoSettings['description'] : trans('lms/home.organizations');
        $pageRobot = getPageRobot('organizations');

        $data = $this->handleInstructorsOrOrganizationsPage($request, Role::$organization);

        $data['title'] = trans('lms/home.organizations');
    ...
    public function handleInstructorsOrOrganizationsPage(Request $request, $role)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/home.instructors'),
            'instructors' => $instructors,
    ...
    public function makeNewsletter(Request $request)
    {
        ...
        if (!empty($check)) {
            if (!empty($check->user_id) and !empty($user_id) and $check->user_id != $user_id) {
                $toastData = [
                    ...
                    'msg' => trans('lms/update.this_email_used_by_another_user'),
                    'status' => 'error'
                ];
        ...
        $toastData = [
            ...
            'msg' => trans('lms/site.create_newsletter_success'),
            'status' => 'success'
        ];
        return back()->with(['toast' => $toastData]);
    }
        ...
        if (!empty($id)) {
            ...
            if (!empty($user) and !empty($user->email)) {
                ...
                try {
                    ...
                    return response()->json([
                        'code' => 500,
                        'message' => trans('lms/site.server_error_try_again')
                    ]);
                }
            }
            ...
            return response()->json([
                'code' => 403,
                'message' => trans('lms/site.user_disabled_public_message')
            ]);
        }
    }
}
