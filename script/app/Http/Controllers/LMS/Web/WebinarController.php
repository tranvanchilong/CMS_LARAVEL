...
class WebinarController extends Controller
{
    ...
    public function course($slug, $justReturnData = false)
    {
        ...
        $course = Webinar::where('slug', $slug)
            ->with([
                ...
                'files' => function ($query) use ($user) {
                    $query->join('lms_webinar_chapters', 'lms_webinar_chapters.id', '=', 'lms_files.chapter_id')
                        ->select('lms_files.*', DB::raw('lms_webinar_chapters.order as chapterOrder'))
                        ->where('lms_files.status', WebinarChapter::$chapterActive)
                        ->orderBy('chapterOrder', 'asc')
                        ->orderBy('lms_files.order', 'asc')
                        ->with([
        ...
        return view('lms.web.default.course.index', $data);
    }
        ...
        if (!empty($webinar) and $this->checkCanAccessToPrivateCourse($webinar)) {
            ...
            if (!empty($file) and $file->downloadable) {
                ...
                if ($canAccess) {
                    ...
                    $toastData = [
                        'title' => trans('lms/public.not_access_toast_lang'),
                        'msg' => trans('lms/public.not_access_toast_msg_lang'),
                        'status' => 'error'
                    ];
        ...
        if (!empty($webinar) and $this->checkCanAccessToPrivateCourse($webinar)) {
            ...
            if (!empty($file)) {
                ...
                if ($canAccess) {
                    ...
                    if (\Illuminate\Support\Facades\File::exists(get_public_path_lms($filePath))) {
                        $data = [
                            'pageTitle' => $file->title,
                            'path' => url(get_path_lms().$filePath)
                        ];
                        return view('lms.web.default.course.learningPage.interactive_file', $data);
                    }
            ...
            if (!empty($file)) {
                ...
                if ($canAccess) {
                    ...
                    if (in_array($file->storage, $notVideoSource)) {
                        ...
                        return response()->file(get_public_path_lms($file->file));
                    }
                }
            }
        }
    ...
    public function getLesson(Request $request, $slug, $lesson_id)
    {
        ...
        if (!empty($course) and $this->checkCanAccessToPrivateCourse($course)) {
            ...
            if (!empty($textLesson)) {
                ...
                return view('lms.'. getTemplate() . '.course.text_lesson', $data);
            }
        }
            ...
            if (!empty($course)) {
                ...
                if (!empty($course->price) and $course->price > 0) {
                    $toastData = [
                        ...
                        'msg' => trans('lms/cart.course_not_free'),
                        'status' => 'error'
                    ];
                    return back()->with(['toast' => $toastData]);
                }
    ...
    public function reportWebinar(Request $request, $id)
    {
        if (auth()->guard('lms_user')->check()) {
            ...
            if (!empty($webinar)) {
                ...
                $notifyOptions = [
                    '[u.name]' => $user->full_name,
                    '[content_type]' => trans('lms/product.course')
                ];
        ...
        if (auth()->guard('lms_user')->check()) {
            ...
            if (!empty($course)) {
                if (empty($course->points)) {
                    $toastData = [
                        'title' => '',
                        'msg' => trans('lms/update.can_not_buy_this_course_with_point'),
                        'status' => 'error'
                    ];
                    return back()->with(['toast' => $toastData]);
                }
                ...
                if ($availablePoints < $course->points) {
                    $toastData = [
                        'title' => '',
                        'msg' => trans('lms/update.you_have_no_enough_points_for_this_course'),
                        'status' => 'error'
                    ];
                    return back()->with(['toast' => $toastData]);
                }
                ...
                $toastData = [
                    'title' => '',
                    'msg' => trans('lms/update.success_pay_course_with_point_msg'),
                    'status' => 'success'
                ];
                return back()->with(['toast' => $toastData]);
            }
