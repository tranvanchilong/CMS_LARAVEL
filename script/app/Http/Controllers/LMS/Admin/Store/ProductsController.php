...
use App\Models\LMS\ProductMedia;
use App\Models\LMS\ProductOrder;
use App\Models\LMS\ProductSelectedFilterOption;
...
use App\Models\LMS\ProductTranslation;
...
class ProductsController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_products');
        ...
        $data = [
            'pageTitle' => trans('lms/update.products'),
            'products' => $products,
        ...
        return view('lms.admin.store.products.lists', $data);
    }

    public function inHouseProducts(Request $request)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_in_house_products');
        ...
        $data = [
            'pageTitle' => trans('lms/update.in-house-products'),
            'products' => $products,
            'categories' => $categories,
    ...
    private function handleFilters($query, $request)
    {
        ...
        if (!empty($status)) {
            $query->where('lms_products.status', $status);
        }
        ...
        if (!empty($sort)) {
            switch ($sort) {
                case 'has_discount':
                    ...
                    break;
                case 'sales_asc':
                    $query->join('lms_product_orders', 'lms_products.id', '=', 'lms_product_orders.product_id')
                        ->leftJoin('lms_sales', function ($join) {
                            $join->on('lms_product_orders.id', '=', 'lms_sales.product_order_id')
                                ->whereNull('lms_sales.refund_at');
                        })
                        ->select('lms_products.*', 'lms_sales.product_order_id', 'lms_sales.refund_at', DB::raw('count(lms_sales.product_order_id) as sales_count'))
                        ->whereNotNull('lms_sales.product_order_id')
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_products.id')
                ...
                case 'income_asc':
                    $query->join('lms_product_orders', 'lms_products.id', '=', 'lms_product_orders.product_id')
                        ...
                        ->select('lms_products.*', 'lms_sales.product_order_id', 'lms_sales.total_amount', 'lms_sales.refund_at', DB::raw('(sum(lms_sales.total_amount) - (sum(lms_sales.tax) + sum(lms_sales.commission))) as amounts'))
                        ->whereNotNull('lms_sales.product_order_id')
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_products.id')
                        ->orderBy('amounts', 'asc');
                ...
                case 'inventory_asc':
                    $query->join('lms_product_orders', 'lms_products.id', '=', 'lms_product_orders.product_id')
                        ...
                        ->select('lms_products.*', 'lms_sales.product_order_id', 'lms_sales.refund_at', DB::raw('(lms_products.inventory - sum(lms_product_orders.quantity)) as remaining_inventory'))
                        ->whereNotNull('lms_products.inventory')
                        ->whereNotNull('lms_product_orders.sale_id')
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_products.id')
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_new_product');

        $data = [
            'pageTitle' => trans('lms/update.create_new_product'),
        ];

        return view('lms.admin.store.products.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            'creator_id' => 'required|exists:lms_users,id',
            'type' => 'required|in:' . implode(',', Product::$productTypes),
            'title' => 'required|max:255',
            'slug' => 'max:255|unique:lms_products,slug',
        ...
        if ($product) {
            ...
            $url = '/lms/admin/store/products/' . $product->id . '/edit?locale=' . $data['locale'];
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_edit_product');
        ...
        $data = [
            'pageTitle' => trans('lms/update.edit_product'),
            'product' => $product,
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_delete_product');
        ...
        $rules = [
            ...
            'type' => 'required|in:' . implode(',', Product::$productTypes),
            'title' => 'required|max:255',
            'slug' => 'max:255|unique:lms_products,slug,' . $product->id,
            'seo_description' => 'required|max:255',
            'summary' => 'required',
            'description' => 'required',
            'point' => 'nullable|integer',
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_settings');
        ...
        $data = [
            'pageTitle' => trans('lms/update.store_settings'),
            'itemValue' => !empty($setting) ? $setting->value : null,
        ];

        return view('lms.admin.store.settings', $data);
    }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_export_products');
