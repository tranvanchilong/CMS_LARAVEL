...
class OrderController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_products_orders');
    ...
    private function handleTopStats($query, $status = null): array
    {
        ...
        $query->join('lms_sales', 'lms_sales.id', 'lms_product_orders.sale_id')
            ->select('lms_product_orders.*', 'lms_sales.total_amount');
        $orders = $query->get();
    ...
    private function returnDataToView(Request $request, $query, $inHouseOrders = false)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/update.orders_lists'),
            'orders' => $orders,
        ...
        return view('lms.admin.store.orders.lists', $data);
    }

    public function inHouseOrders(Request $request)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_in_house_orders');

        removeContentLocale();

        $adminRoleIds = Role::where('is_admin', true)->pluck('id')->toArray();

        $query = ProductOrder::where('lms_product_orders.status', '!=', ProductOrder::$pending)
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_products_orders_refund');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_products_orders_invoice');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_products_orders_export');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_store_products_orders_tracking_code');
