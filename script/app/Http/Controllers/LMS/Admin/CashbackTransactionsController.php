...
class CashbackTransactionsController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_cashback_transactions');
        ...
        $data = [
            'pageTitle' => trans('lms/update.cashback_transactions'),
        ...
        return view('lms.admin.cashback.transactions', $data);
    }

    private function getQuery()
    {
        return Accounting::query()
            ->leftJoin('lms_order_items', 'lms_accounting.order_item_id', 'lms_order_items.id')
            ->select('lms_accounting.*', DB::raw('lms_order_items.total_amount as purchase_amount'))
    ...
    public function refund($id)
    {
        ...
        $toastData = [
            ...
            'msg' => trans('lms/update.cashback_transaction_refunded_successful'),
            'status' => 'success'
        ];
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_cashback_history');

        $query = Accounting::query()
            ->leftJoin('lms_order_items', 'lms_accounting.order_item_id', 'lms_order_items.id')
            ->select('lms_accounting.*', DB::raw('sum(lms_accounting.amount) as total_cashback'),
                DB::raw('max(lms_accounting.created_at) as last_cashback'), DB::raw('sum(lms_order_items.total_amount) as purchase_amount'))
            ->where('is_cashback', true)
            ->where('system', false)
            ->where('type', Accounting::$addiction)
            ->groupBy('lms_accounting.user_id');
        ...
        $data = [
            'pageTitle' => trans('lms/update.cashback_transactions_history'),
            'transactions' => $transactions,
        ...
        return view('lms.admin.cashback.history', $data);
    }
