...
class SaleController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_sales_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/pages/financial.sales_page_title'),
            'sales' => $sales,
        ...
        return view('lms.admin.financial.sales.lists', $data);
    }
    ...
    private function makeTitle($sale)
    {
        if (!empty($sale->webinar_id) or !empty($sale->bundle_id)) {
            ...
            $sale->item_title = trans('lms/panel.meeting');
            $sale->item_id = $sale->meeting_id;
            $sale->item_seller = ($sale->meeting and $sale->meeting->creator) ? $sale->meeting->creator->full_name : trans('lms/update.deleted_item');
            $sale->seller_id = ($sale->meeting and $sale->meeting->creator) ? $sale->meeting->creator->id : '';
        } elseif (!empty($sale->subscribe_id)) {
            $sale->item_title = !empty($sale->subscribe) ? $sale->subscribe->title : trans('lms/update.deleted_subscribe');
            $sale->item_id = $sale->subscribe_id;
            $sale->item_seller = 'Admin';
            $sale->seller_id = '';
        } elseif (!empty($sale->promotion_id)) {
            $sale->item_title = !empty($sale->promotion) ? $sale->promotion->title : trans('lms/update.deleted_promotion');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_sales_refund');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_sales_invoice');
        ...
        if (!empty($sale)) {
            ...
            if (!empty($webinar)) {
                ...
                return view('lms.admin.financial.sales.invoice', $data);
            }
        }
