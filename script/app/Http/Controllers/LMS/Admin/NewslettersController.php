...
use App\Models\LMS\NewsletterHistory;
use Exception;
...
class NewslettersController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_newsletters_lists');

        $newsletters = Newsletter::orderBy('created_at', 'desc')
            ->paginate(10);

        $data = [
            'pageTitle' => trans('lms/admin/main.newsletters'),
            'newsletters' => $newsletters
        ];

        return view('lms.admin.newsletters.lists', $data);
    }

    public function send()
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_newsletters_send');

        $data = [
            'pageTitle' => trans('lms/update.send_newsletter')
        ];

        return view('lms.admin.newsletters.send', $data);
    }
    ...
    public function sendNewsletter(Request $request)
    {
        ...
        return redirect('/lms/admin/newsletters/history');
    }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_newsletters_history');

        $newsletters = NewsletterHistory::orderBy('created_at','desc')
            ->paginate(10);

        $data = [
            'pageTitle' => trans('lms/update.newsletters_history'),
            'newsletters' => $newsletters
        ];

        return view('lms.admin.newsletters.history', $data);
    }

    public function delete($id)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_newsletters_delete');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_newsletters_export_excel');
        ...
        return Excel::download($newslettersExport, trans('lms/admin/main.newsletters') . '.xlsx');
    }
}
