...
class NavbarLinksSettingsController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_additional_pages_navbar_links');
        ...
        return redirect('/lms/admin/additional_page/navbar_links');
    }
    ...
    public function edit(Request $request, $link_key)
    {
        ...
        if (!empty($settings)) {
            ...
            $data = [
                'pageTitle' => trans('lms/admin/pages/setting.settings_navbar_links'),
                'navbar_link' => $result,
                'navbarLinkKey' => $link_key,
                'selectedLocal' => $locale
            ];

            return view('lms.admin.additional_pages.navbar_links', $data);
        }
