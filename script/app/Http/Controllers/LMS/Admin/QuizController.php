...
use App\Models\LMS\QuizzesQuestion;
use App\Models\LMS\QuizzesResult;
use App\Models\LMS\QuizTranslation;
...
class QuizController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/pages/quiz.admin_quizzes_list'),
            'quizzes' => $quizzes,
        ...
        return view('lms.admin.quizzes.lists', $data);
    }
        ...
        if (!empty($sort)) {
            switch ($sort) {
                ...
                case 'students_count_asc':
                    $query->join('lms_quizzes_results', 'lms_quizzes_results.quiz_id', '=', 'lms_quizzes.id')
                        ->select('lms_quizzes.*', 'lms_quizzes_results.quiz_id', DB::raw('count(lms_quizzes_results.quiz_id) as result_count'))
                ...
                case 'passed_count_asc':
                    $query->join('lms_quizzes_results', 'lms_quizzes_results.quiz_id', '=', 'lms_quizzes.id')
                        ->select('lms_quizzes.*', 'lms_quizzes_results.quiz_id', DB::raw('count(lms_quizzes_results.quiz_id) as result_count'))
                        ->where('lms_quizzes_results.status', 'passed')
                        ->groupBy('lms_quizzes_results.quiz_id')
                        ->orderBy('result_count', 'asc');
                    break;
                ...
                case 'grade_avg_asc':
                    $query->join('lms_quizzes_results', 'lms_quizzes_results.quiz_id', '=', 'lms_quizzes.id')
                        ->select('lms_quizzes.*', 'lms_quizzes_results.quiz_id', 'lms_quizzes_results.user_grade', DB::raw('avg(lms_quizzes_results.user_grade) as grade_avg'))
                        ->groupBy('lms_quizzes_results.quiz_id')
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_create');

        $data = [
            'pageTitle' => trans('lms/quiz.new_quiz'),
        ];

        return view('lms.admin.quizzes.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'webinar_id' => 'required|exists:lms_webinars,id',
            'pass_mark' => 'required',
        ];
        ...
        if (!empty($webinar)) {
            ...
            $quiz = Quiz::create([
                ...
                'creator_id' => $webinar->creator_id,                'attempt' => $data['attempt'] ?? null,
            ...
            if ($request->ajax()) {
                ...
                if (empty($data['is_webinar_page'])) {
                    $redirectUrl = '/lms/admin/quizzes/' . $quiz->id . '/edit';
                }
            ...
            return back()->withErrors([
                'webinar_id' => trans('lms/validation.exists', ['attribute' => trans('lms/admin/main.course')])
            ]);
        }
    }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_edit');
        ...
        $data = [
            'pageTitle' => trans('lms/public.edit') . ' ' . $quiz->title,
            'webinars' => $webinars,
            'quiz' => $quiz,
        ...
        return view('lms.admin.quizzes.edit', $data);
    }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_delete');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_results');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/pages/quizResults.quiz_result_list_page_title'),
            'quizzesResults' => $quizzesResults,
            'quiz_id' => $id
        ];

        return view('lms.admin.quizzes.results', $data);
    }

    public function resultsExportExcel($id)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quiz_result_export_excel');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_results_delete');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_quizzes_lists_excel');
        ...
        return Excel::download(new QuizzesAdminExport($quizzes), trans('lms/quiz.quizzes') . '.xlsx');
    }
