...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    {
        $data = [
            'pageTitle' => trans('lms/auth.forget_password'),
        ];

        return view('lms.admin.auth.forgot_password', $data);
    }
    ...
    public function forgot(Request $request)
    {
        ...
        if (empty($user) or !in_array($user->role_id, $adminRoleIds)) {
            return back()->withErrors([
                'email' => [trans('lms/validation.exists', ['attribute' => trans('lms/auth.email')])]
            ]);
        }

        $token = \Illuminate\Support\Str::random(60);
        DB::table('lms_password_resets')->insert([
