...
trait DashboardTrait
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_daily_sales_statistics');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_income_statistics');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_new_sales');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_new_comments');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_new_tickets');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_new_reviews');
    ...
    public function getMonthAndYearSalesChart($type = 'month_of_year')
    {
        ...
        if ($type == 'day_of_month') {
            ...
            for ($month = 1; $month <= 12; $month++) {
                ...
                $labels[] = trans('lms/panel.month_' . $month);
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_recent_comments');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_recent_tickets');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_recent_webinars');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_general_dashboard_recent_courses');
    ...
    public function getTopSellingClasses()
    {
        return Webinar::where('status', Webinar::$active)
            ->join('lms_sales', 'lms_webinars.id', '=', 'lms_sales.webinar_id')
            ->select('lms_webinars.*', 'lms_sales.webinar_id',
                DB::raw('count(lms_sales.webinar_id) as sales_count'),
                DB::raw('sum(lms_sales.total_amount) as sales_amount')
            )->whereNull('lms_sales.refund_at')
            ->where('lms_sales.amount', '>', '0')
    ...
    public function getTopSellingAppointments()
    {
        return Meeting::where('disabled', false)
            ->join('lms_sales', 'lms_meetings.id', '=', 'lms_sales.meeting_id')
            ->select('lms_meetings.*', 'lms_sales.meeting_id',
                DB::raw('count(lms_sales.meeting_id) as sales_count'),
                DB::raw('sum(lms_sales.total_amount) as sales_amount')
            )->whereNull('lms_sales.refund_at')
            ->where('lms_sales.amount', '>', '0')
            ->groupBy('lms_sales.meeting_id')
            ->orderBy('sales_count', 'desc')
            ->limit(5)
            ->get();
    }
    ...
    public function getTopSellingTeachersAndOrganizations($role = 'teachers')
    {
        $users = User::where('status', 'active')
            ->join('lms_sales', 'lms_users.id', '=', 'lms_sales.seller_id')
            ->select('lms_users.*', 'lms_sales.seller_id',
                DB::raw('count(lms_sales.seller_id) as sales_count'),
                DB::raw('sum(lms_sales.total_amount) as sales_amount')
            )->whereNull('lms_sales.refund_at')
            ->where('lms_sales.amount', '>', '0')
            ->where('lms_users.role_name', (($role == 'teachers') ? Role::$teacher : Role::$organization))
    ...
    public function getMostActiveStudents()
    {
        return User::where('status', 'active')
            ->join('lms_sales', 'lms_users.id', '=', 'lms_sales.buyer_id')
            ->select('lms_users.*', 'lms_sales.buyer_id',
                DB::raw('count(lms_sales.webinar_id) as purchased_classes'),
                DB::raw('count(lms_sales.meeting_id) as reserved_appointments'),
                DB::raw('sum(lms_sales.total_amount) as total_cost')
            )->whereNull('lms_sales.refund_at')
            ->where('lms_sales.amount', '>', '0')
            ->where('lms_users.role_name', Role::$user)
