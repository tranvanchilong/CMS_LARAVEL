...
class EnrollmentController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_enrollment_history');
        ...
        $data = [
            'pageTitle' => trans('lms/public.history'),
            'sales' => $sales,
        ];
        ...
        return view('lms.admin.enrollment.history', $data);
    }
    ...
    private function makeTitle($sale)
    {
        ...
        $sale->item_title = $item ? $item->title : trans('lms/update.deleted_item');
        $sale->item_id = $item ? $item->id : '';
        $sale->item_seller = ($item and $item->creator) ? $item->creator->full_name : trans('lms/update.deleted_item');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_enrollment_add_student_to_items');

        $data = [
            'pageTitle' => trans('lms/update.add_student_to_a_class')
        ];

        return view('lms.admin.enrollment.add_student_to_a_class', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['webinar_id'])) {
            $rules['webinar_id'] = 'required|exists:lms_webinars,id';
        } elseif (!empty($data['bundle_id'])) {
            $rules['bundle_id'] = 'required|exists:lms_bundles,id';
        } elseif (!empty($data['product_id'])) {
            $rules['product_id'] = 'required|exists:lms_products,id';
        }
        ...
        if (!empty($user)) {
            ...
            if ($isOwner) {
                $errors = [
                    'user_id' => [trans('lms/cart.cant_purchase_your_course')],
                    'webinar_id' => [trans('lms/cart.cant_purchase_your_course')],
                    'bundle_id' => [trans('lms/cart.cant_purchase_your_course')],
                    'product_id' => [trans('lms/update.cant_purchase_your_product')],
                ];
            }

            if ((empty($errors) or !count($errors)) and $checkUserHasBought) {
                $errors = [
                    'user_id' => [trans('lms/site.you_bought_webinar')],
                    'webinar_id' => [trans('lms/site.you_bought_webinar')],
                    'bundle_id' => [trans('lms/update.you_bought_bundle')],
                    'product_id' => [trans('lms/update.you_bought_product')],
                ];
            }
            ...
            if (!empty($itemType) and !empty($itemId) and !empty($itemColumnName) and !empty($sellerId)) {
                ...
                if ($request->ajax()) {
                    ...
                    $toastData = [
                        ...
                        'msg' => trans('lms/webinars.success_store'),
                        'status' => 'success'
                    ];
                    return redirect('/lms/admin/enrollments/history')->with(['toast' => $toastData]);
                }
            }
        }

        $errors = [
            'user_id' => [trans('lms/update.something_went_wrong')],
            'webinar_id' => [trans('lms/update.something_went_wrong')],
            'bundle_id' => [trans('lms/update.something_went_wrong')],
            'product_id' => [trans('lms/update.something_went_wrong')],
        ];
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_enrollment_block_access');
        ...
        if (!empty($sale)) {
            ...
            $toastData = [
                'title' => trans('lms/public.request_success'),
                'msg' => trans('lms/update.delete-student-access_successfully'),
                'status' => 'success'
            ];
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_enrollment_enable_access');
        ...
        if (!empty($sale)) {
            ...
            $toastData = [
                'title' => trans('lms/public.request_success'),
                'msg' => trans('lms/update.enable-student-access_successfully'),
                'status' => 'success'
            ];
            return back()->with(['toast' => $toastData]);
        }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_sales_export');
