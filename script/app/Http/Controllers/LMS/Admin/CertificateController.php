...
use App\Models\LMS\CertificateTemplateTranslation;
...
class CertificateController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_certificate_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/main.certificate_list_page_title'),
        ...
        return view('lms.admin.certificates.lists', $data);
    }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_certificate_template_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/main.certificate_templates_list_page_title'),
            'templates' => $templates,
        ];

        return view('lms.admin.certificates.templates', $data);
    }

    public function CertificatesNewTemplate()
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_certificate_template_create');

        removeContentLocale();

        $data = [
            'pageTitle' => trans('lms/admin/main.certificate_new_template_page_title'),
        ];

        return view('lms.admin.certificates.new_templates', $data);
    }
    ...
    public function CertificatesTemplateStore(Request $request, $template_id = null)
    {
        ...
        return redirect('/lms/admin/certificates/templates');
    }
    ...
    public function CertificatesTemplatePreview(Request $request)
    {
        ...
        $data = [
            'pageTitle' => trans('lms/public.certificate'),
        ...
        $imgPath = get_public_path_lms($data['image']);
        $img = Image::make($imgPath);

        $img->text($body, $data['position_x'], $data['position_y'], function ($font) use ($data, $isRtl) {
            $font->file($isRtl ? get_public_path_lms('assets/default/fonts/vazir/Vazir-Medium.ttf') : get_public_path_lms('assets/default/fonts/Montserrat-Medium.ttf'));
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_certificate_template_edit');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/main.certificate_template_edit_page_title'),
            'template' => $template
        ];
        return view('lms.admin.certificates.new_templates', $data);
    }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_certificate_template_delete');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_certificate_export_excel');
