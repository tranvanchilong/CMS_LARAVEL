...
use App\Models\LMS\UserBadge;
use App\Models\LMS\UserBank;
use App\Models\LMS\UserManualPurchase;
use App\Models\LMS\UserMeta;
use App\Models\LMS\UserOccupation;
use App\Models\LMS\UserRegistrationPackage;
...
class UserController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_staffs_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/main.staff_list_title'),
            'users' => $users,
            'staffsRoles' => $staffsRoles,
        ];

        return view('lms.admin.users.staffs', $data);
    }

    public function organizations(Request $request, $is_export_excel = false)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_organizations_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/main.organizations'),
            'users' => $users,
        ...
        return view('lms.admin.users.organizations', $data);
    }

    public function students(Request $request, $is_export_excel = false)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_users_list');
        ...
        $data = [
            'pageTitle' => trans('lms/public.students'),
            'users' => $users,
        ...
        return view('lms.admin.users.students', $data);
    }

    public function instructors(Request $request, $is_export_excel = false)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_instructors_list');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/main.instructors'),
            'users' => $users,
        ...
        return view('lms.admin.users.instructors', $data);
    }
        ...
        if (!empty($sort)) {
            switch ($sort) {
                case 'sales_classes_asc':
                    $query->join('lms_sales', 'lms_users.id', '=', 'lms_sales.seller_id')
                        ->select('lms_users.*', 'lms_sales.seller_id', 'lms_sales.webinar_id', 'lms_sales.refund_at', DB::raw('count(lms_sales.seller_id) as sales_count'))
                ...
                case 'purchased_classes_asc':
                    $query->join('lms_sales', 'lms_users.id', '=', 'lms_sales.buyer_id')
                        ->select('lms_users.*', 'lms_sales.buyer_id', 'lms_sales.refund_at', DB::raw('count(lms_sales.buyer_id) as purchased_count'))
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_sales.buyer_id')
                ...
                case 'purchased_classes_amount_asc':
                    $query->join('lms_sales', 'lms_users.id', '=', 'lms_sales.buyer_id')
                        ->select('lms_users.*', 'lms_sales.buyer_id', 'lms_sales.amount', 'lms_sales.refund_at', DB::raw('sum(lms_sales.amount) as purchased_amount'))
                        ->groupBy('lms_sales.buyer_id')
                        ->whereNull('lms_sales.refund_at')
                ...
                case 'sales_appointments_asc':
                    $query->join('lms_sales', 'lms_users.id', '=', 'lms_sales.seller_id')
                        ->select('lms_users.*', 'lms_sales.seller_id', 'lms_sales.meeting_id', 'lms_sales.refund_at', DB::raw('count(lms_sales.seller_id) as sales_count'))
                        ->whereNotNull('lms_sales.meeting_id')
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_sales.seller_id')
                        ->orderBy('sales_count', 'asc');
                    break;
                ...
                case 'purchased_appointments_asc':
                    $query->join('lms_sales', 'lms_users.id', '=', 'lms_sales.buyer_id')
                        ->select('lms_users.*', 'lms_sales.buyer_id', 'lms_sales.meeting_id', 'lms_sales.refund_at', DB::raw('count(lms_sales.buyer_id) as purchased_count'))
                        ->whereNotNull('lms_sales.meeting_id')
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_sales.buyer_id')
                        ->orderBy('purchased_count', 'asc');
                ...
                case 'purchased_appointments_amount_asc':
                    $query->join('lms_sales', 'lms_users.id', '=', 'lms_sales.buyer_id')
                        ->select('lms_users.*', 'lms_sales.buyer_id', 'lms_sales.amount', 'lms_sales.meeting_id', 'lms_sales.refund_at', DB::raw('sum(lms_sales.amount) as purchased_amount'))
                        ->whereNotNull('lms_sales.meeting_id')
                        ->whereNull('lms_sales.refund_at')
                        ->groupBy('lms_sales.buyer_id')
                        ->orderBy('purchased_amount', 'asc');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_users_create');

        $roles = Role::orderBy('created_at', 'desc')->get();
        $userGroups = Group::orderBy('created_at', 'desc')->where('status', 'active')->get();

        $data = [
            'pageTitle' => trans('lms/admin/main.user_new_page_title'),
            'roles' => $roles,
            'userGroups' => $userGroups,
        ];


        return view('lms.admin.users.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            $username => ($username == 'mobile') ? 'required|numeric|unique:lms_users' : 'required|string|email|max:255|unique:lms_users',
            'full_name' => 'required|min:3|max:128',
            'role_id' => 'required|exists:lms_roles,id',
        ...
        if (!empty($data['role_id'])) {
            ...
            if (!empty($role)) {
                ...
                return redirect('/lms/admin/users/' . $user->id . '/edit');
            }
        }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_users_edit');
        ...
        $data = [
            'pageTitle' => trans('lms/admin/pages/users.edit_page_title'),
            'user' => $user,
        ...
        if (auth()->guard('lms_user')->user()->can('admin_forum_topics_lists')) {
        ...
        return view('lms.admin.users.edit', $data);
    }
    ...
    private function getPurchasedClassesData($user)
    {
        $manualAddedClasses = Sale::whereNull('refund_at')
            ...
            ->where('lms_sales.manual_added', true)
            ->where('lms_sales.access_to_purchased_item', true)
            ->whereHas('webinar')
            ->with([
                'webinar'
            ])
        ...
        $manualDisabledClasses = Sale::whereNull('refund_at')
            ->where('buyer_id', $user->id)
            ->whereNotNull('webinar_id')
            ->where('lms_sales.access_to_purchased_item', false)
            ->whereHas('webinar')
            ->with([
                'webinar'
            ])
        ...
        $purchasedClasses = Sale::whereNull('refund_at')
            ...
            ->whereNotNull('webinar_id')
            ->where('lms_sales.access_to_purchased_item', true)
            ->where('lms_sales.manual_added', false)
            ->whereHas('webinar')
            ->with([
                'webinar'
            ])
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->validate($request, [
            'full_name' => 'required|min:3|max:128',
            'role_id' => 'required|exists:lms_roles,id',
            'email' => (!empty($user->email)) ? 'required|email|unique:lms_users,email,' . $user->id . ',id,deleted_at,NULL' : 'nullable|email|unique:lms_users',
            'mobile' => (!empty($user->mobile)) ? 'required|numeric|unique:lms_users,mobile,' . $user->id . ',id,deleted_at,NULL' : 'nullable|numeric|unique:lms_users',
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_users_delete');
    ...
    public function acceptRequestToInstructor($id)
    {
        ...
        if (!empty($becomeInstructors)) {
            ...
            return redirect('/lms/admin/users/' . $user->id . '/edit')->with(['msg' => trans('lms/admin/pages/users.user_role_updated')]);
        }
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_users_impersonate');

        $user = User::findOrFail($user_id);

        if ($user->isAdmin()) {
            return redirect('/lms/admin');
        }

        session()->put(['impersonated' => $user->id]);

        return redirect('/lms/panel');
    }

    public function exportExcelOrganizations(Request $request)
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_users_export_excel');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_update_user_registration_package');
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_update_user_meeting_settings');
    ...
    public function disableCashbackToggle($id)
    {
        ...
        $toastData = [
            ...
            'msg' => trans('lms/update.cashback_was_disabled_for_the_user'),
            'status' => 'success'
        ];
    ...
    public function disableRegitrationBonusStatus($id)
    {
        ...
        $toastData = [
            'title' => trans('lms/public.request_success'),
            'msg' => trans('lms/update.registration_bonus_was_disabled_for_the_user'),
            'status' => 'success'
        ];
    ...
    public function disableInstallmentApproval($id)
    {
        ...
        $toastData = [
            'title' => trans('lms/public.request_success'),
            'msg' => trans('lms/update.installment_was_disabled_for_the_user'),
            'status' => 'success'
        ];
