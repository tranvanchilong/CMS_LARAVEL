...
class WebinarStatisticController extends Controller
{
    ...
    {
        $this->authorizeForUser(auth()->guard('lms_user')->user(),'admin_webinar_statistics');
        ...
        if (!empty($webinar)) {
            ...
            $data = [
                'pageTitle' => trans('lms/update.course_statistics'),
                'webinar' => $webinar,
            ...
            return view('lms.admin.webinars.course_statistics.index', $data);
        }
    ...
    private function getAssignmentsAverageGrade($webinarId)
    {
        $assignments = WebinarAssignment::where('webinar_id', $webinarId)
            ->join('lms_webinar_assignment_history', 'lms_webinar_assignment_history.assignment_id', 'lms_webinar_assignments.id')
    ...
    private function getCourseForumsMessagesCount($webinarId)
    {
        $forums = CourseForum::where('webinar_id', $webinarId)
            ->join('lms_course_forum_answers', 'lms_course_forum_answers.forum_id', 'lms_course_forums.id')
    ...
    {
        $labels = [
            trans('lms/public.students'),
            trans('lms/public.instructors'),
            trans('lms/home.organizations'),
        ];
    ...
    {
        $labels = [
            trans('lms/quiz.passed'),
            trans('lms/public.pending'),
            trans('lms/quiz.failed'),
        ];
    ...
    {
        $labels = [
            trans('lms/update.completed'),
            trans('lms/webinars.in_progress'),
            trans('lms/update.not_started'),
        ];
