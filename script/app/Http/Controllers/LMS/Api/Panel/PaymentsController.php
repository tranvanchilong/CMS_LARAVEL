...
class PaymentsController extends Controller
{
    ...
    public function paymentByCredit(Request $request)
    {
        ...
        if ($user->getAccountingCharge() < $order->amount) {
            ...
            return apiResponse2(0, 'not_enough_credit', trans('lms/api.payment.not_enough_credit'));
    ...
    public function paymentRequest(Request $request)
    {
        ...
        if (!$paymentChannel) {
            return apiResponse2(0, 'disabled_gateway', trans('lms/api.payment.disabled_gateway'));
        }
        ...
        try {
            ...
            if (!$paymentChannel) {
                return apiResponse2(0, 'gateway_error', trans('lms/api.payment.gateway_error'));
            }
    ...
    public function paymentVerify(Request $request, $gateway)
    {
        ...
        try {
            ...
            if (!empty($order)) {
                ...
                return redirect('/lms/payments/status');
            } else {
                $toastData = [
                    'title' => trans('lms/cart.fail_purchase'),
                    'msg' => trans('lms/cart.gateway_error'),
                    'status' => 'error'
                ];
    ...
    public function payStatus(Request $request)
    {
        ...
        $order = Order::where('id', $orderId)
            ->where('user_id', auth()->guard('lms_user')->id())
            ->first();

        if (!empty($order)) {
            $data = [
                'pageTitle' => trans('lms/public.cart_page_title'),
                'order' => $order,
            ];

            return view('lms.web.default.cart.status_pay', $data);
        }
    ...
    public function webChargeGenerator(Request $request)
    {
        return apiResponse2(1, 'generated', trans('lms/api.link.generated'),
            [
                'link' => URL::signedRoute('my_api.web.charge', [apiauth()->guard('lms_user')->id])
            ]
        );
