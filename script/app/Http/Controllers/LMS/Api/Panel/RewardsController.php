...
use App\Models\LMS\Api\RewardAccounting;
...
class RewardsController extends Controller
{
    ...
    public function exchange(Request $request)
    {
        ...
        if ($availablePoints > 0 and $earnByExchange > 0) {
            ...
            Accounting::create([
                ...
                'description' => trans('lms/update.exchange_reward_points_to_wallet'),
                'created_at' => time(),
            ]);
        }
    ...
    public function buyWithPoint(Request $request, $id)
    {
        ...
        if (empty($course->points)) {
            return apiResponse2(0, 'no_points', trans('lms/update.can_not_buy_this_course_with_point'));
        }

        $availablePoints = $user->getRewardPoints();

        if ($availablePoints < $course->points) {
            return   apiResponse2(0, 'no_enough_points', trans('lms/update.you_have_no_enough_points_for_this_course'));
     ...
     return   apiResponse2(1, 'paid',trans('lms/update.success_pay_course_with_point_msg'));
