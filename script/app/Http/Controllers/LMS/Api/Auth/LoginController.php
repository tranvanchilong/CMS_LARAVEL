    ...
    */
    ...
    protected function attemptLogin(Request $request)
    {
        ...
        if (!$token = auth('api')->attempt($credentials)) {
            return apiResponse2(0, 'incorrect', trans('lms/auth.incorrect'));
        }
    ...
    public function afterLogged(Request $request, $token, $verify = false)
    {
        ...
        if ($user->ban) {
            ...
            if (!empty($endBan) and $endBan > $time) {
                auth('api')->logout();
                return apiResponse2(0, 'banned_account', trans('lms/auth.banned_account'));
        ...
        if ($user->status != User::$active and !$verify) {
            ...
            if ($checkConfirmed['status'] == 'send') {
                ...
                return apiResponse2(0, 'not_verified', trans('lms/api.auth.not_verified'));
        ...
        if ($user->status != User::$active) {
            \auth('api')->logout();
            return apiResponse2(0, 'inactive_account', trans('lms/auth.inactive_account'));
        }
        ...
        return apiResponse2(1, 'login', trans('lms/auth.login'), $data);
    ...
    public function logout()
    {
        auth('api')->logout();
        if (!apiAuth()) {
            return apiResponse2(1, 'logout', trans('lms/auth.logout'));
        }
        return apiResponse2(0, 'failed', trans('lms/auth.logout.failed'));
    }
