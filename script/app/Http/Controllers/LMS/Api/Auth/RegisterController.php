...
use App\Http\Controllers\LMS\Api\Auth\VerificationController;
    ...
    */
    ...
    private function stepOne(Request $request)
    {
        ...
        if ($registerMethod !== $username && $username) {
            return apiResponse2(0, 'invalid_register_method', trans('lms/api.auth.invalid_register_method'));
        }
        ...
        if ($userCase) {
            ...
            if ($checkConfirmed['status'] == 'verified') {
                if ($userCase->full_name) {
                    return apiResponse2(0, 'already_registered', trans('lms/api.auth.already_registered'));
                } else {
                    $userCase->update(['password' => Hash::make($data['password'])]);
                    return apiResponse2(0, 'go_step_3', trans('lms/api.auth.go_step_3'), [
                        'user_id' => $userCase->id
                    ]);
                }
            } else {
                $userCase->update(['password' => Hash::make($data['password'])]);
                return apiResponse2(0, 'go_step_2', trans('lms/api.auth.go_step_2'), [
                    'user_id' => $userCase->id
                ]);
            }
    ...
    private function stepThree(Request $request)
    {
        ...
        return apiResponse2(1, 'login', trans('lms/api.auth.login'), $data);
