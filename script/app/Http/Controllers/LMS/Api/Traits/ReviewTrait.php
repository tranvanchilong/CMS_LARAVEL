...
namespace App\Http\Controllers\LMS\Api\Traits;
...
trait ReviewTrait
{
    public function store(Request $request)
    {
        ...
        if (!$itemObj->checkUserHasBought($user)) {
            return apiResponse2(0, 'not_purchased',
                trans('lms/cart.you_not_purchased_this_course')
            );
        }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $review->delete();
        return apiResponse2(1, 'deleted', trans('lms/webinars.your_review_deleted')
        );
