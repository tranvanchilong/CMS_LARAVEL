...
class CartItemInfo
{
    ...
    private function getReserveMeetingInfo($cart, $creator)
    {
        ...
        $info['itemUrl'] = null;
        $info['title'] = trans('lms/meeting.reservation_appointment') . ' ' . ((!empty($cart->reserveMeeting->student_count) and $cart->reserveMeeting->student_count > 1) ? '(' . trans('lms/update.reservation_appointment_student_count', ['count' => $cart->reserveMeeting->student_count]) . ')' : '');
    ...
    private function getSubscribeInfo($cart, $subscribe)
    {
        ...
        $info['extraHint'] = trans('lms/public.subscribe');
        $info['rate'] = null;
        $info['quantity'] = null;
    ...
    private function getRegistrationPackageInfo($cart, $registrationPackage)
    {
        ...
        $info['profileUrl'] = null;
        $info['teacherName'] = null;
        $info['extraHint'] = trans('lms/update.registration_package');
        $info['rate'] = null;
        $info['quantity'] = null;
    ...
    private function getInstallmentOrderInfo($cart, $installmentPayment)
    {
        ...
        if (!empty($installmentOrder)) {
            ...
            $info['extraPriceHint'] = ($installmentPayment->type == 'upfront') ? trans('lms/update.installment_upfront') : trans('lms/update.installment');
        }
