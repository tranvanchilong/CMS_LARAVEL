<?php

namespace App\Exports;

use Illuminate\Support\Collection;
use Maatwebsite\Excel\Concerns\FromCollection;
use Maatwebsite\Excel\Concerns\WithHeadings;
use Maatwebsite\Excel\Concerns\WithMapping;

class InstallmentPurchasesExport implements FromCollection, WithHeadings, WithMapping
{
    protected $orders;

    public function __construct($orders)
    {
        $this->orders = $orders;
    }

    /**
     * @return Collection
     */
    public function collection()
    {
        return $this->orders;
    }

    /**
     * @inheritDoc
     */
    public function headings(): array
    {
        return [
            trans('lms/admin/main.user'),
            trans('lms/admin/main.mobile'),
            trans('lms/admin/main.email'),
            trans('lms/admin/main.title'),
            trans('lms/update.target'),
            trans('lms/update.product'),
            trans('lms/update.target_type'),
            trans('lms/panel.purchase_date'),
            trans('lms/financial.total_amount'),
            trans('lms/update.upfront'),
            trans('lms/update.installments_count'),
            trans('lms/update.installments_amount'),
            trans('lms/update.overdue'),
            trans('lms/update.overdue_amount'),
            trans('lms/update.first_unpaid_installment_date'),
            trans('lms/update.days_left'),
            trans('lms/admin/main.status'),
        ];
    }

    /**
     * @inheritDoc
     */
    public function map($order): array
    {
        $product = '';
        $productType = '';

        if (!empty($order->webinar_id)) {
            $product = $order->webinar->title;
            $productType = trans('lms/update.target_types_courses');
        } elseif (!empty($order->bundle_id)) {
            $product = $order->bundle->title;
            $productType = trans('lms/update.target_types_bundles');
        } elseif (!empty($order->product_id)) {
            $product = $order->product->title;
            $productType = trans('lms/update.target_types_store_products');
        } elseif (!empty($order->subscribe_id)) {
            $product = trans('lms/admin/main.purchased_subscribe');
            $productType = trans('lms/update.target_types_subscription_packages');
        } elseif (!empty($order->registration_package_id)) {
            $product = trans('lms/update.purchased_registration_package');
            $productType = trans('lms/update.target_types_registration_packages');
        }

        $upfront = '--';

        if (!empty($order->installment->upfront)) {
            $upfront = ($order->installment->upfront_type == 'percent') ? $order->installment->upfront . '%' : handlePrice($order->installment->upfront);
        }

        $stepsFixedAmount = $order->installment->steps->where('amount_type', 'fixed_amount')->sum('amount');
        $stepsPercents = $order->installment->steps->where('amount_type', 'percent')->sum('amount');
        $installmentsAmount = ($stepsFixedAmount ? handlePrice($stepsFixedAmount) : '') . ($stepsPercents ? (($stepsFixedAmount ? ' + ' : '') . $stepsPercents . '%') : '');

        $status = "";
        if ($order->status == "pending_verification") {
            $status = trans('lms/update.pending_verification');
        } elseif ($order->status == "open") {
            $status = trans('lms/admin/main.open');
        } elseif ($order->status == "rejected") {
            $status = trans('lms/public.rejected');
        } elseif ($order->status == "canceled") {
            $status = trans('lms/public.canceled');
        } elseif ($order->status == "refunded") {
            $status = trans('lms/update.refunded');
        }

        return [
            $order->user->id . ' - ' . $order->user->full_name,
            $order->user->mobile,
            $order->user->email,
            $order->installment->title,
            trans('lms/update.target_types_'.$order->installment->target_type),
            $product,
            $productType,
            dateTimeFormat($order->created_at, 'j M Y'),
            handlePrice($order->getCompletePrice()),
            $upfront,
            $order->installment->steps_count,
            $installmentsAmount,
            $order->overdue_count,
            handlePrice($order->overdue_amount),
            $order->upcoming_date,
            $order->days_left,
            $status
        ];
    }
}
