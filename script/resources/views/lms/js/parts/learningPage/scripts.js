(function ($) {
    $('body').on('click', '.panel-file-manager', function (e) {
        e.preventDefault();
        $(this).filemanager('file', {prefix: '/lms/laravel-filemanager'});
    });
    ...
    function handleDownloadCertificateHtml(result) {
        ...
        if (result && result !== '') {
            otherHtml = `
                <a href="/lms/panel/quizzes/results/${result}/showCertificate" target="_blank" class="btn btn-primary btn-sm mt-15">${downloadLang}</a>
            `;
        } else {
            otherHtml = `
    ...
    function handleQuizHtml(quiz) {
        ...
        if (quiz.can_try) {
            otherHtml = `
                <a href="/lms/panel/quizzes/${quiz.id}/start" target="_blank" class="btn btn-primary btn-sm mt-15">${quizPageLang}</a>
            `;
        } else {
            otherHtml = `
    ...
    function handleLiveSessionFinishedHtml(session) {
        ...
        const otherHtml = `
                <a href="/lms${courseUrl}" class="btn btn-white btn-sm mt-15">${coursePageLang}</a>
        `;
    ...
    function handleLiveSessionNotStartedHtml(session) {
        ...
        const otherHtml = `
            <div class="d-flex align-items-center mt-15">
                ...
                <a href="/lms${courseUrl}" class="btn btn-white btn-sm ml-10">${coursePageLang}</a>
            </div>
        `;
    ...
    function handleLiveSessionHtml(session) {
        ...
        const otherHtml = `
            <div class="d-flex align-items-center mt-15">
                <a href="/lms${session.join_url}" target="_blank" class="btn btn-primary btn-sm ">${joinTheClassLang}</a>
                <a href="/lms${courseUrl}" class="btn btn-white btn-sm ml-10">${coursePageLang}</a>
            </div>
        `;
        ...
        if ((file.online_viewer && file.online_viewer !== '0') || (file.downloadable && file.downloadable !== '0')) {
            ...
            if ((file.online_viewer && file.online_viewer !== '0')) {
                ...
                if ((file.downloadable && file.downloadable !== '0')) {
                        html += `<div class="d-flex align-items-center justify-content-between rounded-sm mt-15 p-15 border-dashed-gray300">
                                ...
                                <a href="/lms${courseUrl}/file/${file.id}/download" class="btn btn-primary btn-sm" target="_blank">${downloadLang}</a>
                            </div>`;
                }
                ...
                const otherHtml = `<a href="/lms${courseUrl}/file/${file.id}/download" class="btn btn-primary btn-sm mt-15" target="_blank">${downloadLang}</a>`;
            ...
            switch (file.storage) {
                ...
                case 'upload_archive':
                    ...
                    const hint = file.title;
                    const img = 'download.svg';
                    const otherHtml = `<a href="/lms${courseUrl}/file/${file.id}/showHtml" target="_blank" class="btn btn-primary btn-sm mt-15">${showLang}</a>`;
    ...
    function handleTextLessonHtml(textLesson) {
        let html = `<div class="text-lesson-content p-15 p-lg-30">
                    ...
                    <div class="pb-5 mt-15 main-image rounded-lg w-100">
                        <img src="{{get_path_lms()}}${textLesson.image}" class="img-cover bg-gray200" alt="${textLesson.title}"/>
                    </div>
        ...
        if (textLesson.attachments && Object.keys(textLesson.attachments).length) {
            ...
            Object.keys(textLesson.attachments).forEach(key => {
                ...
                html += `<div class="col-12 col-lg-3 mt-10 mt-lg-0">
                            <a href="/lms${courseUrl}/file/${attachment.file.id}/download" class="d-flex align-items-center p-10 border border-gray200 rounded-sm">
        ...
        return `<div class="d-flex align-items-center justify-content-center w-100 h-100">
                    <div class="learning-content-box d-flex align-items-center justify-content-center flex-column p-15 p-lg-30 rounded-lg">
                        <div class="learning-content-box-icon">
                            <img src="/assets/lms/assets/default/img/learning/${img}" alt="downloadable icon">
                        </div>
    ...
    function contentEmptyStateHtml() {
        const html = `<div class="learning-page-forum-empty d-flex align-items-center justify-content-center flex-column">
            <div class="learning-page-forum-empty-icon d-flex align-items-center justify-content-center">
                <img src="/assets/lms/assets/default/img/learning/content-empty.svg" class="img-fluid" alt="">
            </div>
