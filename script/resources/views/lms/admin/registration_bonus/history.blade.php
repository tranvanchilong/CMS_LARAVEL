        ...
        <div class="section-body">
            ...
            <div class="row">
                    ...
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.achieved_users')}}</h4>
                            </div>
                            <div class="card-body">
                                {{ $achievedUsers }}
                            </div>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.unlocked_bonus_users')}}</h4>
                            </div>
                            <div class="card-body">
                                {{ $unlockedBonusUsers }}
                            </div>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.total_bonus')}}</h4>
                            </div>
                            <div class="card-body">
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.unlocked_bonus')}}</h4>
                            </div>
                            <div class="card-body">
                ...
                <div class="card-body">
                    <form method="get" class="mb-0">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="role_id" class="form-control select2" data-allow-clear="true"
                                            data-placeholder="{{ trans('lms/update.select_user_role') }}">
                                        <option value="">{{ trans('lms/admin/main.all') }}</option>
                            ...
                            {{--<div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label">{{trans('lms/update.bonus_wallet')}}</label>
                                    <select name="bonus_wallet" class="form-control populate">
                                        <option value="">{{trans('lms/admin/main.all')}}</option>

                                        <option value="income_wallet" {{ (request()->get('bonus_wallet') == 'income_wallet') ? 'selected' : '' }}>{{ trans('lms/update.income_wallet') }}</option>
                                        <option value="balance_wallet" {{ (request()->get('bonus_wallet') == 'balance_wallet') ? 'selected' : '' }}>{{ trans('lms/update.balance_wallet') }}</option>
                                    </select>
                                </div>
                            </div>--}}
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label">{{trans('lms/update.bonus_status')}}</label>
                                    <select name="bonus_status" class="form-control populate">
                                        <option value="">{{trans('lms/admin/main.all')}}</option>

                                        <option value="locked" {{ (request()->get('bonus_status') == 'locked') ? 'selected' : '' }}>{{ trans('lms/update.locked') }}</option>
                                        <option value="unlocked" {{ (request()->get('bonus_status') == 'unlocked') ? 'selected' : '' }}>{{ trans('lms/update.unlocked') }}</option>
                                    </select>
                                </div>
                            </div>
            ...
            <div class="row">
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            @if($authUser->can('admin_registration_bonus_export_excel'))
                                <div class="text-right">
                                    <a href="/lms{{ getAdminPanelUrl('/registration_bonus/export?'. http_build_query(request()->all())) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped font-14 ">
                                    <tr>
                                        <th class="text-left">{{ trans('lms/update.user_id') }}</th>
                                        <th class="text-left">{{ trans('lms/admin/main.name') }}</th>
                                        <th class="text-left">{{trans('lms/admin/main.role')}}</th>

                                        <th>{{trans('lms/update.bonus')}}</th>
                                        ...
                                        @if (!empty($registrationBonusSettings['unlock_registration_bonus_with_referral']) and !empty($registrationBonusSettings['number_of_referred_users']))
                                            <th>{{trans('lms/update.referred_users')}}</th>

                                            @if (!empty($registrationBonusSettings['enable_referred_users_purchase']))
                                                <th>{{trans('lms/update.referred_purchases')}}</th>
                                            @endif
                                        @endif

                                        {{--<th>{{trans('lms/update.bonus_wallet')}}</th>--}}
                                        <th>{{trans('lms/update.registration_date')}}</th>
                                        <th>{{trans('lms/update.bonus_status')}}</th>
                                    ...
                                    @foreach($users as $user)
                                        <tr class="text-center">
                                            ...
                                            {{--<td class="font-12">{{ !empty($user->bonus_wallet) ? trans('lms/update.'.$user->bonus_wallet) : '-' }}</td>--}}
                                            ...
                                            <td class="text-center mb-2" width="120">
                                                ...
                                                @if($authUser->can('admin_users_edit'))
                                                    @include('lms.admin.includes.delete_button',[
                                                            'url' => '/lms'.getAdminPanelUrl("/users/{$user->id}/disable_registration_bonus"),
                                                            'tooltip' => trans('lms/update.disable_registration_bonus'),
