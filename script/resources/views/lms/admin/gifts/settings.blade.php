                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-12 col-md-6">
                                    <form action="/lms{{ getAdminPanelUrl('/gifts/settings') }}" method="post">
                                        {{ csrf_field() }}
                                        <input type="hidden" name="page" value="general">
                                        <input type="hidden" name="name" value="{{ \App\Models\LMS\Setting::$giftsGeneralSettingsName }}">
                                        ...
                                        <div class="form-group custom-switches-stacked">
                                            <label class="custom-switch pl-0 d-flex align-items-center">
                                                ...
                                                <label class="custom-switch-description mb-0 cursor-pointer" for="giftsStatusSwitch">{{ trans('lms/admin/main.active') }}</label>
                                            </label>
                                            <div class="text-muted text-small">{{ trans('lms/update.gifts_setting_active_hint') }}</div>
                                        </div>
                                            ...
                                            @foreach($otherSwitches as $otherSwitch)
                                                <div class="form-group custom-switches-stacked">
                                                    <label class="custom-switch pl-0 d-flex align-items-center">
                                                        ...
                                                        <span class="custom-switch-indicator"></span>
                                                        <label class="custom-switch-description mb-0 cursor-pointer" for="{{ $otherSwitch }}Switch">{{ trans("lms/update.{$otherSwitch}") }}</label>
                                                    </label>
                                                    <div class="text-muted text-small">{{ trans("lms/update.{$otherSwitch}_hint") }}</div>
                                                </div>
                                            @endforeach
...
@push('scripts_bottom')
    <script src="/assets/lms/assets/default/js/admin/settings/gifts_settings.min.js"></script>
@endpush
