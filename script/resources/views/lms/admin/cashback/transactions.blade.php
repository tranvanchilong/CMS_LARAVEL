                                ...
                                <div class="form-group">
                                    ...
                                    <select name="target_type" class="form-control populate">
                                        ...
                                        @foreach(\App\Models\LMS\CashbackRule::$targetTypes as $type)
                                            <option value="{{ $type }}" @if(request()->get('target_type') == $type) selected @endif>{{ trans('lms/update.target_types_'.$type) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="card-header">
                            @if($authUser->can('admin_cashback_transactions'))
                                <div class="text-right">
                                    <a href="/lms{{ getAdminPanelUrl('/cashback/transactions/excel?'. http_build_query(request()->all())) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                                </div>
                            @endif
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14 ">
                                    <tr>
                                        ...
                                        <th class="">{{trans('lms/admin/main.description')}}</th>
                                        <th>{{trans('lms/admin/main.amount')}}</th>
                                        <th>{{trans('lms/update.cashback_amount')}}</th>
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td class="text-left">
                                                <div class="">
                                                    @if(!empty($transaction->webinar_id))
                                                        <a href="/lms{{ !empty($transaction->webinar) ? $transaction->webinar->getUrl() : '' }}"
                                                    ...
                                                    @elseif(!empty($transaction->bundle_id))
                                                        <a href="/lms{{ !empty($transaction->bundle) ? $transaction->bundle->getUrl() : '' }}"
                                                    ...
                                                    @elseif(!empty($transaction->product_id))
                                                        <a href="/lms{{ !empty($transaction->product) ? $transaction->product->getUrl() : '' }}"
                                                    ...
                                                    @elseif(!empty($transaction->meeting_time_id))
                                                        <div class="font-14">#{{ $transaction->meeting_time_id }} {{ trans('lms/admin/main.meeting') }}</div>
                                                        <span class="d-block font-12">{{ trans('lms/update.target_types_meetings') }}</span>
                                                    ...
                                                    @elseif(!empty($transaction->promotion_id))
                                                        <span class="font-14">{{ trans('lms/admin/main.purchased_promotion') }}</span>
                                            ...
                                            <td class="">
                                                @if($transaction->system)
                                                    <span class="font-12 text-danger">{{ trans('lms/admin/main.refund') }}</span>
                                                @else
                                                    <span class="font-12 text-success">{{ trans('lms/update.successful') }}</span>
                                                @endif
                                            </td>
                                                ...
                                                @if($authUser->can('admin_cashback_transactions'))
                                                    @if(!$transaction->system)
                                                        @include('lms.admin.includes.delete_button',[
                                                                'url' => '/lms'.getAdminPanelUrl('/cashback/transactions/'. $transaction->id .'/refund'),
