    ...
    <div class="col-12 col-md-5">
        ...
        <div class="form-group mt-15 ">
            ...
            <select name="target_type" class="js-target-types-input custom-select @error('target_types')  is-invalid @enderror">
                @foreach(\App\Models\LMS\CashbackRule::$targetTypes as $type)
                    <option value="{{ $type }}" @if(!empty($rule) and $rule->target_type == $type) selected @endif>{{ trans('lms/update.target_types_'.$type) }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group mt-15 js-select-target-field {{ empty($rule) ? 'd-none' : '' }}">
            ...
            @php
                $targets = [
                    'courses' => \App\Models\LMS\CashbackRule::$courseTargets,
                    'store_products' => \App\Models\LMS\CashbackRule::$productTargets,
                    'bundles' => \App\Models\LMS\CashbackRule::$bundleTargets,
                    'meetings' => \App\Models\LMS\CashbackRule::$meetingTargets,
                    'subscription_packages' => \App\Models\LMS\CashbackRule::$subscriptionTargets,
                    'registration_packages' => \App\Models\LMS\CashbackRule::$registrationPackagesTargets,
                ];
            @endphp
            ...
            <select name="target" class="js-target-input custom-select  @error('target')  is-invalid @enderror">
                ...
                @foreach($targets as $targetKey => $targetItems)
                    @foreach($targetItems as $target)
                        <option value="{{ $target }}" class="js-target-option js-target-option-{{ $targetKey }} {{ (!empty($rule) and $rule->target_type == $targetKey) ? '' : 'd-none' }}" @if(!empty($rule) and $rule->target == $target) selected @endif>{{ trans('lms/update.'.$target) }}</option>
                    @endforeach
                @endforeach
            </select>
