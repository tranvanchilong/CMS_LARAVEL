    ...
    <div class="col-12 col-md-4">
        <div class="form-group ">
            ...
            <select name="amount_type" class="js-amount-type-select form-control">
                <option value="fixed_amount" {{ (!empty($rule) and $rule->amount_type == 'fixed_amount') ? 'selected' : '' }}>{{ trans('lms/update.fixed_amount') }}</option>
                <option value="percent" {{ (!empty($rule) and $rule->amount_type == 'percent') ? 'selected' : '' }}>{{ trans('lms/update.percent') }}</option>
            </select>
            @error('amount_type')
            <div class="invalid-feedback">
                {{ $message }}
    ...
    <div class="col-12 col-md-6">
        ...
        <div class="js-apply-cashback-per-item form-group {{ (empty($rule) or $rule->amount_type == 'fixed_amount') ? '' : 'd-none' }}">
            <div class="d-flex align-items-center">
                <label class="" for="perItemSwitch">{{ trans('lms/update.apply_cashback_per_item') }}</label>
            ...
            <p class="font-12 text-muted mt-1">{{ trans('lms/update.apply_cashback_per_item_hint') }}</p>
        </div>


        <div class="js-max-amount-field form-group {{ (!empty($rule) and $rule->amount_type == 'percent') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('lms/admin/main.max_amount') }}</label>
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.cashback_max_amount_hint') }}</div>
            @error('max_amount')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group">
            <label class="input-label">{{ trans('lms/admin/main.min_amount') }}</label>
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.cashback_min_amount_hint') }}</div>
            @error('min_amount')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
