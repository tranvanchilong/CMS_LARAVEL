...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('lms/update.quizzes_related') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{trans('lms/admin/main.dashboard')}}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('lms/update.quizzes_related') }}</div>
            </div>
        </div>
        ...
        <div class="section-body">
            <section class="card">
                <div class="card-body">
                    <form action="/lms{{ getAdminPanelUrl() }}/certificates" method="get" class="row mb-0">
                        ...
                        <div class="col-12 col-md-3 d-flex align-items-center justify-content-end">
                            <button type="submit" class="btn btn-primary w-100">{{ trans('lms/public.show_results') }}</button>
                        </div>
                    </form>
                </div>
            </section>
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            @if($authUser->can('admin_certificate_export_excel'))
                                <div class="text-right">
                                    <a href="/lms{{ getAdminPanelUrl() }}/certificates/excel?{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                                </div>
                            @endif
                        </div>
                                    ...
                                    @foreach($certificates as $certificate)
                                        <tr>
                                            ...
                                            <td class="text-left">
                                                ...
                                                <small class="d-block text-left">{{ !empty($certificate->quiz->webinar) ? $certificate->quiz->webinar->title : trans('lms/update.delete_item') }})</small>
                                            </td>
                                            ...
                                            <td>
                                                <a href="/lms{{ getAdminPanelUrl() }}/certificates/{{ $certificate->id }}/download" target="_blank" class="btn-transparent text-primary" data-toggle="tooltip" title="{{ trans('lms/quiz.download_certificate') }}">
