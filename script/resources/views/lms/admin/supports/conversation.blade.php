    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">{{ trans('lms/admin/main.conversation') }}</div>
            </div>
        </div>
                        ...
                        <div class="card-body chat-content">
                            ...
                            @foreach($support->conversations as $conversations)
                                <div class="chat-item chat-{{ !empty($conversations->sender_id) ? 'right' : 'left' }}">
                                    <img src="{{get_path_lms()}}{{ !empty($conversations->sender_id) ? $conversations->sender->getAvatar() : $conversations->supporter->getAvatar() }}">
                                    ...
                                    <div class="chat-details">
                                        ...
                                        <div class="chat-time">
                                            ...
                                            @if(!empty($conversations->attach))
                                                <a href="/lms{{ url($conversations->attach) }}" target="_blank" class="text-success"><i class="fa fa-paperclip"></i> {{ trans('lms/admin/main.open_attach') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="row">
                <div class="col-12 ">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="/lms{{ getAdminPanelUrl() }}/supports/{{ $support->id }}/conversation" method="post">
                                {{ csrf_field() }}

                                <div class="form-group mt-15">
                                    <label class="input-label">{{ trans('lms/site.message') }}</label>
                                ...
                                <div class="row">
                                    <div class="col-12 col-md-8">
                                        <div class="form-group">
                                            <label class="input-label">{{ trans('lms/admin/main.attach') }}</label>
                                    ...
                                    <div class="col-12 col-md-4 text-right mt-4">
                                        <button type="submit" class="btn btn-primary">{{ trans('lms/site.send_message') }}</button>

                                        @if($support->status != 'close')
                                            <a href="/lms{{ getAdminPanelUrl() }}/supports/{{ $support->id }}/close" class="btn btn-danger ml-1">{{ trans('lms/admin/main.close_conversation') }}</a>
                                        @endif
                                    </div>
                                </div>
                            </form>
