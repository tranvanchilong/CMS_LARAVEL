        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{trans('lms/admin/main.classes_with_support')}}</h4>
                        </div>
                        <div class="card-body">
                            {{ $classesWithSupport }}
                        </div>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    <tr>
                                        <th>{{trans('lms/admin/main.subject')}}</th>
                                    ...
                                    @foreach($supports as $support)
                                        <tr>
                                            ...
                                            <td class="text-left">
                                                <a href="/lms{{ $support->webinar->getUrl() }}" target="_blank">{{ $support->webinar->title }}</a>
                                            </td>

                                            <td class="text-left">
                                                <a href="/lms{{ $support->webinar->teacher->getProfileUrl() }}" target="_blank">{{ $support->webinar->teacher->full_name }}</a>
                                            </td>

                                            <td class="text-left">
                                                <a href="/lms{{ $support->user->getProfileUrl() }}" target="_blank">{{ $support->user->full_name }}</a>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                @if($support->status == 'close')
                                                    <span class="text-warning">{{ trans('lms/admin/main.close') }}</span>
                                                @else
                                            ...
                                            <td class="text-center" width="50">
                                                @if($authUser->can('admin_supports_reply'))
                                                    <a href="/lms{{ getAdminPanelUrl() }}/supports/{{ $support->id }}/conversation" class="btn-transparent text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.conversation') }}">
                                                        <i class="fa fa-reply" aria-hidden="true"></i>
                                                    </a>
                                                @endif

                                                @if($authUser->can('admin_supports_delete'))
                                                    @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/supports/'.$support->id.'/delete' , 'btnClass' => ''])
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
