                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-md-8 col-lg-6">
                            <form action="/lms{{ getAdminPanelUrl() }}/financial/special_offers/{{ !empty($specialOffer) ? $specialOffer->id.'/update' : 'store' }}"
                                  method="Post">
                                {{ csrf_field() }}

                                <div class="form-group">
                                    <label>{{ trans('lms/admin/main.name') }}</label>
                                    <input type="text" name="name"
                                           class="form-control  @error('name') is-invalid @enderror"
                                           value="{{ !empty($specialOffer) ? $specialOffer->name : old('name') }}"
                                           placeholder="{{ trans('lms/admin/main.name_placeholder') }}"/>
                                    @error('name')
                                ...
                                <div class="form-group">
                                    <label>{{ trans('lms/admin/main.type') }}</label>

                                    <select name="type" class="js-offer-type form-control @error('type')  is-invalid @enderror">

                                        @foreach($types as $type => $typeItem)
                                            <option value="{{ $type }}" {{ (!empty($specialOffer) and !empty($specialOffer->{$typeItem})) ? 'selected' : '' }}>{{ trans('lms/update.'.$type) }}</option>
                                        @endforeach
                                    </select>
                                    @error('type')
                                    <div class="invalid-feedback">
                                ...
                                <div class="js-course-field form-group {{ (empty($specialOffer) or !empty($specialOffer->webinar_id)) ? '' : 'd-none' }}">
                                    <label>{{ trans('lms/admin/main.class') }}</label>

                                    <select name="webinar_id" class="form-control search-webinar-select2 @error('webinar_id')  is-invalid @enderror" data-placeholder="{{ trans('lms/update.search_and_select_class') }}">
                                ...
                                <div class="js-bundle-field form-group {{ (!empty($specialOffer) and !empty($specialOffer->bundle_id)) ? '' : 'd-none' }}">
                                    <label>{{ trans('lms/update.bundle') }}</label>

                                    <select name="bundle_id" class="form-control search-bundle-select2 @error('bundle_id')  is-invalid @enderror" data-placeholder="{{ trans('lms/update.search_and_select_bundle') }}">
                                ...
                                <div class="js-subscribe-field form-group {{ (!empty($specialOffer) and !empty($specialOffer->subscribe_id)) ? '' : 'd-none' }}">
                                    <label>{{ trans('lms/public.subscribe') }}</label>

                                    <select name="subscribe_id" class="form-control @error('subscribe_id')  is-invalid @enderror">
                                        <option value="">{{ trans('lms/update.select_subscribe') }}</option>
                                ...
                                <div class="js-registration_package-field form-group {{ (!empty($specialOffer) and !empty($specialOffer->registration_package_id)) ? '' : 'd-none' }}">
                                    <label>{{ trans('lms/update.registration_package') }}</label>

                                    <select name="registration_package_id" class="form-control @error('registration_package_id')  is-invalid @enderror">
                                        <option value="">{{ trans('lms/update.select_registration_package') }}</option>
                                ...
                                <div class="form-group">
                                    <label class="input-label">{{ trans('lms/admin/main.from_date') }}</label>
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                ...
                                <div class="form-group">
                                    <label class="input-label">{{ trans('lms/admin/main.to_date') }}</label>
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text" id="dateRangeLabel">
                                                <i class="fa fa-calendar"></i>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control custom-select @error('status')  is-invalid @enderror">
                                        <option value="active" {{ !empty($specialOffer) and $specialOffer->status == \App\Models\LMS\SpecialOffer::$active ? 'selected' : '' }}>{{ trans('lms/panel.active') }}</option>
                                        <option value="inactive" {{ !empty($specialOffer) and $specialOffer->status == \App\Models\LMS\SpecialOffer::$inactive ? 'selected' : '' }}>{{ trans('lms/panel.inactive') }}</option>
                                    </select>
                                    @error('status')
                                    <div class="invalid-feedback">
                                        {{ $message }}
...
@push('scripts_bottom')
    <script src="/assets/lms/assets/default/js/admin/special_offers.min.js"></script>
@endpush
