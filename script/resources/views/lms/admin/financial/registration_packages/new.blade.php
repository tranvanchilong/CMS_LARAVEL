...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('lms/admin/main.new_package') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{trans('lms/admin/main.dashboard')}}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('lms/update.registration_packages') }}</div>
            </div>
        </div>
        ...
        <div class="section-body card">
            ...
            <div class="d-flex align-items-center justify-content-between">
                <div class="">
                    <h2 class="section-title ml-4">{{ !empty($package) ? trans('lms/admin/main.edit') : trans('lms/admin/main.create') }}</h2>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12 col-md-8 col-lg-6">
                    <div class="card-body">
                        <form action="/lms{{ getAdminPanelUrl() }}/financial/registration-packages/{{ !empty($package) ? $package->id.'/update' : 'store' }}" method="Post">
                            {{ csrf_field() }}
                            ...
                            <div class="form-group">
                                <label>{{ trans('lms/admin/main.short_description') }}</label>
                                <input type="text" name="description"
                                       class="form-control @error('description') is-invalid @enderror"
                                       value="{{ !empty($package) ? $package->description : old('description') }}"
                                       placeholder="{{ trans('lms/admin/main.short_description_placeholder') }}"
                                />
                                @error('description')
                                <div class="invalid-feedback">
                                    {{ $message }}
                            ...
                            <div class="form-group">
                                <label>{{ trans('lms/admin/main.days') }}</label>
                            ...
                            <div class="form-group">
                                <label>{{ trans('lms/admin/main.price') }} ({{ $currency }})</label>
                            ...
                            <div class="form-group">
                                <label>{{ trans('lms/admin/main.role') }}</label>
                                <select name="role" class="form-control @error('role') is-invalid @enderror">
                                    <option value="">{{ trans('lms/admin/main.select') }}</option>
                                    <option value="instructors" {{ ((!empty($package) and $package->role == 'instructors') or old('role') == 'instructors') ? 'selected' : '' }}>{{ trans('lms/admin/main.instructor') }}</option>
                                    <option value="organizations" {{ ((!empty($package) and $package->role == 'organizations') or old('role') == 'organizations') ? 'selected' : '' }}>{{ trans('lms/admin/main.organization') }}</option>
                                </select>
                                @error('role')
                                <div class="invalid-feedback">
                                    {{ $message }}
                            ...
                            <div class="form-group js-organization-inputs {{ ((!empty($package) and $package->role == 'organizations') or old('role') == 'organizations') ? '' : 'd-none' }}">
                                <label>{{ trans('lms/update.instructors_count') }}</label>
                                ...
                                @enderror
                                <div class="text-muted text-small mt-1">{{ trans('lms/update.saas_package_condition_hint') }}</div>
                            </div>

                            <div class="form-group js-organization-inputs {{ ((!empty($package) and $package->role == 'organizations') or old('role') == 'organizations') ? '' : 'd-none' }}">
                                <label>{{ trans('lms/update.students_count') }}</label>
                            ...
                            <div class="form-group js-organization-inputs js-instructor-inputs {{ ((!empty($package) and in_array($package->role, ['instructors', 'organizations'])) or in_array(old('role'), ['instructors', 'organizations'])) ? '' : 'd-none' }}">
                                <label>{{ trans('lms/update.courses_capacity') }}</label>
                            ...
                            <div class="form-group js-organization-inputs js-instructor-inputs {{ ((!empty($package) and in_array($package->role, ['instructors', 'organizations'])) or in_array(old('role'), ['instructors', 'organizations'])) ? '' : 'd-none' }}">
                                <label>{{ trans('lms/update.courses_count') }}</label>
                            ...
                            <div class="form-group js-organization-inputs js-instructor-inputs {{ ((!empty($package) and in_array($package->role, ['instructors', 'organizations'])) or in_array(old('role'), ['instructors', 'organizations'])) ? '' : 'd-none' }}">
                                <label>{{ trans('lms/update.meeting_count') }}</label>
                            ...
                            <div class="form-group js-organization-inputs js-instructor-inputs {{ ((!empty($package) and in_array($package->role, ['instructors', 'organizations'])) or in_array(old('role'), ['instructors', 'organizations'])) ? '' : 'd-none' }}">
                                <label>{{ trans('lms/update.product_count') }}</label>
                            ...
                            <div class="form-group custom-switches-stacked">
                                <label class="custom-switch pl-0">
                                    ...
                                    <label class="custom-switch-description mb-0 cursor-pointer" for="statusSwitch">{{ trans('lms/admin/pages/financial.status') }}</label>
                                </label>
                            </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/admin/new_registration_packages.min.js"></script>
@endpush
