    ...
    <div class="row">
        <div class="col-12 col-md-6">
            <form action="/lms{{ getAdminPanelUrl() }}/settings/main" method="post">
                ...
                <input type="hidden" name="name" value="{{ \App\Models\LMS\Setting::$registrationPackagesOrganizationsName }}">
                ...
                <div class="form-group custom-switches-stacked">
                    <label class="custom-switch pl-0 d-flex align-items-center">
                        ...
                        <label class="custom-switch-description mb-0 cursor-pointer" for="organizationsStatusSwitch">{{ trans('lms/update.registration_packages_organizations_status') }}</label>
                    </label>
                    <div class="text-muted text-small mt-1">{{ trans('lms/update.registration_packages_organizations_status_hint') }}</div>
                </div>
                <h2 class="section-title">{{ trans('lms/update.organization_default_values') }}</h2>
                @foreach(['instructors_count','students_count','courses_capacity','courses_count','meeting_count','product_count'] as $str)
                    <div class="form-group">
                        <label>{{ trans('lms/update.'.$str) }}</label>
