    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">{{ trans('lms/admin/main.offline_payments') }}</div>
            </div>
        </div>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <select name="status" data-plugin-selectTwo class="form-control populate">
                                            ...
                                            <option value="approved" @if(request()->get('status') == 'approved') selected @endif>{{trans('lms/admin/main.approved')}}</option>
                                            <option value="reject" @if(request()->get('status') == 'reject') selected @endif>{{trans('lms/admin/main.rejected')}}</option>
                                        </select>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="@if($pageType == 'requests') col-md-3 @else col-md-2 @endif">
                                <div class="form-group">
                                    ...
                                    <select name="sort" data-plugin-selectTwo class="form-control populate">
                                        ...
                                        <option value="pay_date_asc" @if(request()->get('sort') == 'pay_date_asc') selected @endif>{{trans('lms/admin/main.Transaction_time_ascending')}}</option>
                                        <option value="pay_date_desc" @if(request()->get('sort') == 'pay_date_desc') selected @endif>{{trans('lms/admin/main.Transaction_time_descending')}}</option>
                                    </select>
                                </div>
                            </div>
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            @if($authUser->can('admin_offline_payments_export_excel'))
                                <a href="/lms{{ getAdminPanelUrl() }}/financial/offline_payments/excel?{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                            @endif
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    <tr>
                                        ...
                                        <th>{{trans('lms/admin/main.bank')}}</th>
                                        <th>{{trans('lms/admin/main.referral_code')}}</th>
                                        <th>{{trans('lms/admin/main.phone')}}</th>
                                        <th>{{trans('lms/update.attachment')}}</th>
                                        <th width=180px>{{trans('lms/admin/main.transaction_time')}}</th>
                                        ...
                                        @foreach($offlinePayments as $offlinePayment)
                                            <tr>
                                               ...
                                               @else
                                                ...
                                                @if($pageType == 'requests')
                                                    <td>
                                                        @if($offlinePayment->status == \App\Models\LMS\OfflinePayment::$waiting)
                                                            @if($authUser->can('admin_offline_payments_approved'))
                                                                @include('lms.admin.includes.delete_button',[
                                                                        'url' => '/lms'.getAdminPanelUrl().'/financial/offline_payments/'. $offlinePayment->id .'/approved',
                                                                        'tooltip' => trans('lms/financial.approve'),
                                                                        'btnIcon' => 'fa-check'
                                                                    ])
                                                            @endif

                                                            @if($authUser->can('admin_offline_payments_reject'))
                                                                @include('lms.admin.includes.delete_button',[
                                                                        'url' => '/lms'.getAdminPanelUrl().'/financial/offline_payments/'. $offlinePayment->id .'/reject',
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('lms/admin/main.offline_payment_hint_title_1')}}</div>
                        <div class=" text-small font-600-bold">{{trans('lms/admin/main.offline_payment_hint_description_1')}}</div>
                    </div>
                </div>

                <div class="col-md-6">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('lms/admin/main.offline_payment_hint_title_2')}}</div>
                        <div class=" text-small font-600-bold">{{trans('lms/admin/main.offline_payment_hint_description_2')}}</div>
                    </div>
                </div>
            </div>
        </div>
