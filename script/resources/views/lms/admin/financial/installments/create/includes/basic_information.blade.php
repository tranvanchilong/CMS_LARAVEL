<div class="row">
    <div class="col-12 col-md-5">
        ...
        <div class="form-group mt-15">
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_title_hint') }}</div>
            @error('title')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group mt-15">
            <label class="input-label">{{ trans('lms/update.main_title') }}</label>
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_main_title_hint') }}</div>
            @error('main_title')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group mt-15">
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_description_hint') }}</div>
            @error('description')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group mt-15">
            <label class="input-label">{{ trans('lms/update.banner') }}</label>
            <div class="input-group">
                ...
                @enderror
                <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_banner_hint') }}</div>
            </div>
        </div>
        ...
        <div id="installmentOptionsCard" class="mt-3">
            <div class="d-flex align-items-center justify-content-between">
                <h5 class="fs-15">{{ trans('lms/update.options') }}</h5>

                <button type="button" class="js-add-btn btn btn-success">
                    <i class="fa fa-plus"></i>
                    {{ trans('lms/update.add_option') }}
                </button>
            </div>

            @if(!empty($installment) and !empty($installment->options))
                @php
                    $installmentOptions = explode(\App\Models\LMS\Installment::$optionsExplodeKey, $installment->options);
                @endphp
        ...
        <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_options_hint') }}</div>
        <div class="form-group mt-15">
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_capacity_hint') }}</div>
            @error('capacity')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group ">
            ...
            <select name="group_ids[]" class="custom-select select2" multiple data-placeholder="{{ trans('lms/admin/main.select_users_group') }}">

                @foreach($userGroups as $userGroup)
                    <option value="{{ $userGroup->id }}" {{ in_array($userGroup->id, $selectedGroupIds) ? 'selected' : '' }}>{{ $userGroup->name }}</option>
                @endforeach
            </select>
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_user_group_hint') }}</div>
            @error('group_ids')
            <div class="invalid-feedback d-block">
                {{ $message }}
            </div>
            @enderror
            <p class="text-muted font-12 mt-1">{{ trans('lms/') }}</p>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_start_date_hint') }}</div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.installment_end_date_hint') }}</div>
        </div>
