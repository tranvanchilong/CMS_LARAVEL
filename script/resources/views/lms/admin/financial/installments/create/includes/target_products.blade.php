<div class="row">
    <div class="col-12 col-md-5">
        ...
        <div class="form-group mt-15 ">
            <label class="input-label d-block">{{ trans('lms/update.target_types') }}</label>

            <select name="target_type" class="js-target-types-input custom-select @error('target_types')  is-invalid @enderror">
                @foreach(\App\Models\LMS\Installment::$targetTypes as $type)
                    <option value="{{ $type }}" @if(!empty($installment) and $installment->target_type == $type) selected @endif>{{ trans('lms/update.target_types_'.$type) }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group mt-15 js-select-target-field {{ empty($installment) ? 'd-none' : '' }}">
            <label class="input-label d-block">{{ trans('lms/update.select_target') }}</label>

            @php
                $targets = [
                    'courses' => \App\Models\LMS\Installment::$courseTargets,
                    'store_products' => \App\Models\LMS\Installment::$productTargets,
                    'bundles' => \App\Models\LMS\Installment::$bundleTargets,
                    'meetings' => \App\Models\LMS\Installment::$meetingTargets,
                    'subscription_packages' => \App\Models\LMS\Installment::$subscriptionTargets,
                    'registration_packages' => \App\Models\LMS\Installment::$registrationPackagesTargets,
                ];
            @endphp

            <select name="target" class="js-target-input custom-select  @error('target')  is-invalid @enderror">
                <option value="">{{ trans('lms/update.select_target') }}</option>

                @foreach($targets as $targetKey => $targetItems)
                    @foreach($targetItems as $target)
                        <option value="{{ $target }}" class="js-target-option js-target-option-{{ $targetKey }} {{ (!empty($installment) and $installment->target_type == $targetKey) ? '' : 'd-none' }}" @if(!empty($installment) and $installment->target == $target) selected @endif>{{ trans('lms/update.'.$target) }}</option>
                    @endforeach
                @endforeach
            </select>
        ...
        <div class="form-group js-specific-categories-field {{ (!empty($installment) and $installment->target == "specific_categories") ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('lms/update.specific_categories') }}</label>

            <select name="category_ids[]" id="categories" class="custom-select select2" multiple data-placeholder="{{ trans('lms/public.choose_category') }}">
        ...
        <div class="form-group js-specific-sellers-field {{ (!empty($installment) and $installment->target == "specific_sellers") ? '' : 'd-none' }}">
            <label class="input-label">{{trans('lms/update.specific_sellers')}}</label>
        ...
        <div class="form-group js-specific-courses-field {{ (!empty($installment) and $installment->target == "specific_courses") ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('lms/update.specific_courses') }}</label>
        ...
        <div class="form-group js-specific-products-field {{ (!empty($installment) and $installment->target == "specific_products") ? '' : 'd-none' }}">
            <label class="input-label">{{trans('lms/update.specific_products')}}</label>
        ...
        <div class="form-group js-specific-bundles-field {{ (!empty($installment) and $installment->target == "specific_bundles") ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('lms/update.specific_bundles') }}</label>
        ...
        <div class="form-group js-subscription-specific-packages-field {{ (!empty($installment) and $installment->target_type == "subscription_packages" and $installment->target == "specific_packages") ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('lms/update.specific_packages') }}</label>
            <select name="subscribe_ids[]" multiple="multiple" class="form-control select2" data-placeholder="{{ trans('lms/update.select_packages') }}">
        ...
        <div class="form-group js-registration-specific-packages-field {{ (!empty($installment) and $installment->target_type == "registration_packages" and $installment->target == "specific_packages") ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('lms/update.specific_packages') }}</label>
            <select name="registration_package_ids[]" multiple="multiple" class="form-control select2" data-placeholder="{{ trans('lms/update.select_packages') }}">
