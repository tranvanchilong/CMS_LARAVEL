...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('lms/admin/main.type_'.$classesType.'s') }} {{trans('lms/admin/main.list')}}</h1>
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">{{ trans('lms/admin/main.type_'.$classesType.'s') }}</div>
            </div>
        </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="sort" data-plugin-selectTwo class="form-control populate">
                                        ...
                                        <option value="public_courses" @if(request()->get('sort') == 'public_courses') selected @endif>{{trans('lms/update.public_courses')}}</option>
                                        <option value="courses_private" @if(request()->get('sort') == 'courses_private') selected @endif>{{trans('lms/update.courses_private')}}</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="card-header">
                            @if($authUser->can('admin_webinars_export_excel'))
                                <div class="text-right">
                                    <a href="/lms{{ getAdminPanelUrl() }}/webinars/excel?{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                                </div>
                            @endif
                        </div>
                            ...
                            <div>
                                <table class="table table-striped font-14 ">
                                    <tr>
                                        ...
                                        <th>{{trans('lms/admin/main.students_count')}}</th>
                                    ...
                                    @foreach($webinars as $webinar)
                                        <tr class="text-center">
                                            ...
                                            <td class="font-12">
                                                <a href="/lms{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/students" target="_blank" class="">{{ $webinar->sales->count() }}</a>
                                            </td>
                                                ...
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu text-left webinars-lists-dropdown">
                                                        ...
                                                        @if($authUser->can('admin_webinars_edit'))
                                                            @if($webinar->status == \App\Models\LMS\Webinar::$pending)
                                                                @include('lms.admin.includes.delete_button',[
                                                                    'url' => '/lms'.getAdminPanelUrl().'/webinars/'.$webinar->id.'/approve',
                                                                ...
                                                                @include('lms.admin.includes.delete_button',[
                                                                    'url' => '/lms'.getAdminPanelUrl().'/webinars/'.$webinar->id.'/reject',
                                                            ...
                                                            @elseif($webinar->status == \App\Models\LMS\Webinar::$active)
                                                                @include('lms.admin.includes.delete_button',[
                                                                    'url' => '/lms'.getAdminPanelUrl().'/webinars/'.$webinar->id.'/unpublish',
                                                                    'btnClass' => 'd-flex align-items-center text-danger text-decoration-none btn-transparent btn-sm mt-1',
                                                        ...
                                                        @if($authUser->can('admin_webinar_notification_to_students'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/sendNotification" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 ">
                                                        ...
                                                        @if($authUser->can('admin_webinar_students_lists'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/students" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('lms/admin/main.students') }}">
                                                        ...
                                                        @if($authUser->can('admin_webinar_statistics'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/statistics" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('lms/admin/main.students') }}">
                                                        ...
                                                        @if($authUser->can('admin_support_send'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/supports/create?user_id={{ $webinar->teacher->id }}" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1" title="{{ trans('lms/admin/main.send_message_to_teacher') }}">
                                                        ...
                                                        @if($authUser->can('admin_webinars_edit'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/edit" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('lms/admin/main.edit') }}">
