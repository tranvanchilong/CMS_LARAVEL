...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-lg-3 col-md-6 col-sm-6 col-12">
            <div class="card card-statistic-1">
                ...
                <div class="card-wrap">
                    <div class="card-header">
                        <h4>{{ trans('lms/update.average_learning') }}</h4>
                    </div>
                    <div class="card-body">
                        {{ $averageLearning }}
                    </div>
    ...
    <div class="card">
        <div class="card-header">
            @if($authUser->can('admin_webinar_notification_to_students'))
                <a href="/lms{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/sendNotification" class="btn btn-primary mr-2">{{ trans('lms/notification.send_notification') }}</a>
            @endif

            @if($authUser->can('admin_enrollment_add_student_to_items'))
                <button type="button" id="addStudentToCourse" class="btn btn-primary mr-2">{{ trans('lms/update.add_student_to_course') }}</button>
            @endif
            <div class="h-10"></div>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif(!$student->access_to_purchased_item)
                                    <div class="mt-0 mb-1 font-weight-bold text-danger">{{ trans('lms/update.access_blocked') }}</div>
                                @else
                            ...
                            <td class="text-center mb-2" width="120">
                                @if(!empty($student->id))
                                    ...
                                    @if($authUser->can('admin_webinar_students_delete'))
                                        @if(!$student->access_to_purchased_item)
                                            @include('lms.admin.includes.delete_button',[
                                                'url' => '/lms'.getAdminPanelUrl().'/enrollments/'. $student->sale_id .'/enable-access',
                                                'tooltip' => trans('lms/update.enable-student-access'),
                                                'btnIcon' => 'fa-check'
                                            ])
                                        @else
                                            @include('lms.admin.includes.delete_button',[
                                                        'url' => '/lms'.getAdminPanelUrl().'/enrollments/'. $student->sale_id .'/block-access',
    ...
    <div id="addStudentToCourseModal" class="d-none">
        <h3 class="section-title after-line">{{ trans('lms/update.add_student_to_course') }}</h3>
        <div class="mt-25">
