...
<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="session_{{ !empty($session) ? $session->id :'record' }}">
        <div class="d-flex align-items-center" href="#collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-controls="collapseSession{{ !empty($session) ? $session->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="font-weight-bold text-dark-blue d-block cursor-pointer">{{ !empty($session) ? $session->title : trans('lms/public.add_new_sessions') }}</div>
        </div>

        <div class="d-flex align-items-center">

            @if(!empty($session) and $session->status != \App\Models\LMS\WebinarChapter::$chapterActive)
                <span class="disabled-content-badge mr-10">{{ trans('lms/public.disabled') }}</span>
            @endif

            @if(!empty($session))
                <button type="button" data-item-id="{{ $session->id }}" data-item-type="{{ \App\Models\LMS\WebinarChapterItem::$chapterSession }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-transparent text-gray mr-10">
            ...
            @if(!empty($session))
                <a href="/lms{{ getAdminPanelUrl() }}/sessions/{{ $session->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
    ...
    <div id="collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-labelledby="session_{{ !empty($session) ? $session->id :'record' }}" class=" collapse @if(empty($session)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form session-form" data-action="/lms{{ getAdminPanelUrl() }}/sessions/{{ !empty($session) ? $session->id . '/update' : 'store' }}">
                <input type="hidden" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][webinar_id]" value="{{ !empty($webinar) ? $webinar->id :'' }}">

                <div class="form-group">
                    <label class="input-label">{{ trans('lms/webinars.select_session_api') }}</label>
                    ...
                    <div class="js-session-api">
                        @foreach(getFeaturesSettings("available_session_apis") as $sessionApi)
                            <div class="custom-control custom-radio custom-control-inline">
                                ...
                                <label class="custom-control-label" for="{{ $sessionApi }}_api_{{ !empty($session) ? $session->id : '' }}">{{ trans('lms/update.session_api_'.$sessionApi) }}</label>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="js-zoom-not-complete-alert mt-10 text-danger d-none">
                        {{ trans('lms/webinars.your_zoom_settings_are_not_complete') }}
                        <a href="/lms/panel/setting/step/8" class="text-primary" target="_blank">{{ trans('lms/public.go_to_settings') }}</a>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group js-moderator-secret {{ (empty($session) or $session->session_api != 'big_blue_button') ? 'd-none' :'' }}">
                            <label class="input-label">{{ trans('lms/public.moderator_password') }}</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][title]" class="js-ajax-title form-control" value="{{ !empty($session) ? $session->title : '' }}" placeholder="{{ trans('lms/forms.maximum_255_characters') }}"/>
                            <div class="invalid-feedback"></div>
                        </div>

                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/public.date') }}</label>
                        ...
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/public.duration') }} <span class="braces">({{ trans('lms/public.minutes') }})</span></label>
                        ...
                        @if(!empty(getFeaturesSettings('extra_time_to_join_status')) and getFeaturesSettings('extra_time_to_join_status'))
                            <div class="form-group">
                                <label class="input-label">{{ trans('lms/update.extra_time_to_join') }} <span class="braces">({{ trans('lms/public.minutes') }})</span></label>
                        ...
                        <div class="form-group mt-20">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer input-label" for="sessionStatusSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('lms/public.active') }}</label>
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][status]" class="custom-control-input" id="sessionStatusSwitch{{ !empty($session) ? $session->id : '_record' }}" {{ (empty($session) or $session->status == \App\Models\LMS\Session::$Active) ? 'checked' : ''  }}>
                        ...
                        <div class="js-agora-chat-and-rec  {{ (empty($session) or $session->session_api !== 'agora') ? 'd-none' : '' }}">
                            @if(getFeaturesSettings('agora_chat'))
                                <div class="form-group mt-20">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="sessionAgoraChatSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('lms/update.chat') }}</label>
                                        <div class="custom-control custom-switch">
                            ...
                            {{--
                                                        <div class="form-group mt-20">
                                                            <div class="d-flex align-items-center justify-content-between">
                                                                <label class="cursor-pointer input-label" for="sessionAgoraRecordSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('lms/update.record') }}</label>
                                                                <div class="custom-control custom-switch">
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            <div class="form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="SequenceContentSessionSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('lms/update.sequence_content') }}</label>
                                    <div class="custom-control custom-switch">
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($session) and ($session->check_previous_parts or !empty($session->access_after_day))) ? '' : 'd-none' }}">
                                <div class="form-group">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="checkPreviousPartsSessionSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('lms/update.check_previous_parts') }}</label>
                                        <div class="custom-control custom-switch">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][access_after_day]" value="{{ (!empty($session)) ? $session->access_after_day : '' }}" class="js-ajax-access_after_day form-control" placeholder="{{ trans('lms/update.access_after_day_placeholder') }}"/>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                        @endif
                ...
                <div class="mt-30 d-flex align-items-center">
                    <button type="button" class="js-save-session btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>

                    @if(!empty($session))
                        @if(!$session->isFinished())
                            <a href="/lms{{ $session->getJoinLink(true) }}" target="_blank" class="ml-10 btn btn-sm btn-secondary">{{ trans('lms/footer.join') }}</a>
                        @else
                            <button type="button" class="js-session-has-ended ml-10 btn btn-sm btn-secondary disabled">{{ trans('lms/footer.join') }}</button>
                        @endif
                    @endif
