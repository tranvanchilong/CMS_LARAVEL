<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="file_{{ !empty($assignment) ? $assignment->id :'record' }}">
        <div class="d-flex align-items-center" href="#collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" aria-controls="collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="font-weight-bold text-dark-blue d-block cursor-pointer">{{ !empty($assignment) ? $assignment->title . ($assignment->accessibility == 'free' ? " (". trans('lms/public.free') .")" : '') : trans('lms/update.add_new_assignments') }}</div>
        </div>

        <div class="d-flex align-items-center">

            @if(!empty($assignment) and $assignment->status != \App\Models\LMS\WebinarChapter::$chapterActive)
                <span class="disabled-content-badge mr-10">{{ trans('lms/public.disabled') }}</span>
            @endif

            @if(!empty($assignment))
                <button type="button" data-item-id="{{ $assignment->id }}" data-item-type="{{ \App\Models\LMS\WebinarChapterItem::$chapterAssignment }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-transparent text-gray mr-10">
            ...
            @if(!empty($assignment))
                <a href="/lms{{ getAdminPanelUrl() }}/assignments/{{ $assignment->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
    ...
    <div id="collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" aria-labelledby="file_{{ !empty($assignment) ? $assignment->id :'record' }}" class=" collapse @if(empty($assignment)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form assignment-form" data-action="/lms{{ getAdminPanelUrl() }}/assignments/{{ !empty($assignment) ? $assignment->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/quiz.grade') }}</label>
                        ...
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/update.pass_grade') }}</label>
                        ...
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/update.attempts') }}</label>
                        ...
                        <div class="js-assignment-attachments-items form-group mt-15">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="input-label mb-0">{{ trans('lms/public.attachments') }}</label>
                            ...
                            <div class="assignment-attachments-main-row js-ajax-attachments position-relative">
                                <div class="mt-10 p-10 border rounded">
                                    <div class="mb-10">
                                        <label class="input-label">{{ trans('lms/public.title') }}</label>
                                        <input type="text" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][attachments][assignmentTemp][title]" class="form-control" placeholder="{{ trans('lms/forms.maximum_255_characters') }}"/>
                                    </div>
                                    ...
                                    <div class="input-group product-images-input-group">
                                        ...
                                        <input type="text" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][attachments][assignmentTemp][attach]" id="attachments_assignmentTemp" value="" class="form-control" placeholder="{{ trans('lms/update.assignment_attachments_placeholder') }}"/>
                                    </div>
                                </div>
                                ...
                                @foreach($assignment->attachments as $attachment)
                                    <div class="js-ajax-attachments position-relative">
                                        <div class="mt-10 p-10 border rounded">
                                            <div class="mb-10">
                                                <label class="input-label">{{ trans('lms/public.title') }}</label>
                                                <input type="text" name="ajax[{{ $assignment->id }}][attachments][{{ $attachment->id }}][title]" value="{{ $attachment->title }}" class="form-control" placeholder="{{ trans('lms/forms.maximum_255_characters') }}"/>
                                            </div>
                                            ...
                                            <div class="input-group product-images-input-group">
                                                ...
                                                <input type="text" name="ajax[{{ $assignment->id }}][attachments][{{ $attachment->id }}][attach]" id="attachments_{{ $attachment->id }}" value="{{ $attachment->attach }}" class="form-control" placeholder="{{ trans('lms/update.assignment_attachments_placeholder') }}"/>
                                            </div>
                                        </div>
                        ...
                        <div class="form-group mt-20">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer input-label" for="assignmentStatusSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}">{{ trans('lms/public.active') }}</label>
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][status]" class="custom-control-input" id="assignmentStatusSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}" {{ (empty($assignment) or $assignment->status == \App\Models\LMS\File::$Active) ? 'checked' : ''  }}>
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            <div class="form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="SequenceContentAssignmentSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}">{{ trans('lms/update.sequence_content') }}</label>
                                    <div class="custom-control custom-switch">
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($assignment) and ($assignment->check_previous_parts or !empty($assignment->access_after_day))) ? '' : 'd-none' }}">
                                <div class="form-group">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="checkPreviousPartsAssignmentSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}">{{ trans('lms/update.check_previous_parts') }}</label>
                                        <div class="custom-control custom-switch">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][access_after_day]" value="{{ (!empty($assignment)) ? $assignment->access_after_day : '' }}" class="js-ajax-access_after_day form-control" placeholder="{{ trans('lms/update.access_after_day_placeholder') }}"/>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                        @endif
                ...
                <div class="mt-30 d-flex align-items-center">
                    <button type="button" class="js-save-assignment btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>
