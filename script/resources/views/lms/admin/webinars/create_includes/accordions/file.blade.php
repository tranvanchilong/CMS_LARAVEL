@if(!empty($file) and $file->storage == 'upload_archive')
    @include('lms.admin.webinars.create_includes.accordions.new_interactive_file',['file' => $file])
@else
    <li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
        <div class="d-flex align-items-center justify-content-between " role="tab" id="file_{{ !empty($file) ? $file->id :'record' }}">
            ...
            <span class="chapter-icon chapter-content-icon mr-10">
                ...
                <div class="font-weight-bold text-dark-blue d-block cursor-pointer">{{ !empty($file) ? $file->title . ($file->accessibility == 'free' ? " (". trans('lms/public.free') .")" : '') : trans('lms/public.add_new_files') }}</div>
            </div>

            <div class="d-flex align-items-center">

                @if(!empty($file) and $file->status != \App\Models\LMS\WebinarChapter::$chapterActive)
                    <span class="disabled-content-badge mr-10">{{ trans('lms/public.disabled') }}</span>
                @endif

                @if(!empty($file))
                    <button type="button" data-item-id="{{ $file->id }}" data-item-type="{{ \App\Models\LMS\WebinarChapterItem::$chapterFile }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-transparent text-gray mr-10">
                ...
                @if(!empty($file))
                    <a href="/lms{{ getAdminPanelUrl() }}/files/{{ $file->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
        ...
        <div id="collapseFile{{ !empty($file) ? $file->id :'record' }}" aria-labelledby="file_{{ !empty($file) ? $file->id :'record' }}" class=" collapse @if(empty($file)) show @endif" role="tabpanel">
            <div class="panel-collapse text-gray">
                <div class="js-content-form file-form" data-action="/lms{{ getAdminPanelUrl() }}/files/{{ !empty($file) ? $file->id . '/update' : 'store' }}">
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][title]" class="js-ajax-title form-control" value="{{ !empty($file) ? $file->title : '' }}" placeholder="{{ trans('lms/forms.maximum_255_characters') }}"/>
                                <div class="invalid-feedback"></div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                >
                                    ...
                                    @foreach(getFeaturesSettings('available_sources') ?? [] as $source)
                                        <option value="{{ $source }}" @if(!empty($file) and $file->storage == $source) selected @endif>{{ trans('lms/update.file_source_'.$source) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="d-flex align-items-center js-ajax-accessibility">
                                    <div class="custom-control custom-radio">
                                        ...
                                        <label class="custom-control-label font-14 cursor-pointer" for="accessibilityRadio1_{{ !empty($file) ? $file->id : 'record' }}">{{ trans('lms/public.free') }}</label>
                                    </div>
                                    ...
                                    <div class="custom-control custom-radio ml-15">
                                        ...
                                        <label class="custom-control-label font-14 cursor-pointer" for="accessibilityRadio2_{{ !empty($file) ? $file->id : 'record' }}">{{ trans('lms/public.paid') }}</label>
                                    </div>
                                </div>
                            ...
                            <div class="form-group js-file-path-input {{ (!empty($file) and $file->storage == 's3') ? 'd-none' : '' }}">
                                <div class="local-input input-group">
                                    ...
                                    <input type="text" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][file_path]" id="file_path{{ !empty($file) ? $file->id : 'record' }}" value="{{ (!empty($file)) ? $file->file : '' }}" class="js-ajax-file_path form-control" placeholder="{{ trans('lms/webinars.file_upload_placeholder') }}"/>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                            ...
                            <div class="form-group js-s3-file-path-input {{ (!empty($file) and $file->storage == 's3') ? '' : 'd-none' }}">
                                <div class="input-group">
                                    ...
                                    <div class="custom-file">
                                        ...
                                        <label class="custom-file-label cursor-pointer" for="s3File{{ !empty($file) ? $file->id : 'record' }}">{{ trans('lms/update.choose_file') }}</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row form-group js-file-type-volume d-none">
                                <div class="col-6">
                                    ...
                                    <select name="ajax[{{ !empty($file) ? $file->id : 'new' }}][file_type]" class="js-ajax-file_type form-control">
                                        ...
                                        @foreach(\App\Models\LMS\File::$fileTypes as $fileType)
                                            <option value="{{ $fileType }}" @if(!empty($file) and $file->file_type == $fileType) selected @endif>{{ trans('lms/update.file_type_'.$fileType) }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback"></div>
                                </div>
                                <div class="col-6 js-file-volume-field">
                                    ...
                                    <input type="text" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][volume]" value="{{ (!empty($file)) ? $file->volume : '' }}" class="js-ajax-volume form-control" placeholder="{{ trans('lms/webinars.online_file_volume') }}"/>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                            ...
                            <div class="js-online_viewer-input form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="online_viewerSwitch{{ !empty($file) ? $file->id : '_record' }}">{{ trans('lms/update.online_viewer') }}</label>
                            ...
                            <div class="js-downloadable-input form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="downloadableSwitch{{ !empty($file) ? $file->id : '_record' }}">{{ trans('lms/home.downloadable') }}</label>
                                    <div class="custom-control custom-switch">
                            ...
                            <div class="form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="fileStatusSwitch{{ !empty($file) ? $file->id : '_record' }}">{{ trans('lms/public.active') }}</label>
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][status]" class="custom-control-input" id="fileStatusSwitch{{ !empty($file) ? $file->id : '_record' }}" {{ (empty($file) or $file->status == \App\Models\LMS\File::$Active) ? 'checked' : ''  }}>
                            ...
                            @if(getFeaturesSettings('sequence_content_status'))
                                <div class="form-group mt-20">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="SequenceContentFileSwitch{{ !empty($file) ? $file->id : '_record' }}">{{ trans('lms/update.sequence_content') }}</label>
                                        <div class="custom-control custom-switch">
                                ...
                                <div class="js-sequence-content-inputs pl-5 {{ (!empty($file) and ($file->check_previous_parts or !empty($file->access_after_day))) ? '' : 'd-none' }}">
                                    <div class="form-group">
                                        <div class="d-flex align-items-center justify-content-between">
                                            <label class="cursor-pointer input-label" for="checkPreviousPartsFileSwitch{{ !empty($file) ? $file->id : '_record' }}">{{ trans('lms/update.check_previous_parts') }}</label>
                                            <div class="custom-control custom-switch">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][access_after_day]" value="{{ (!empty($file)) ? $file->access_after_day : '' }}" class="js-ajax-access_after_day form-control" placeholder="{{ trans('lms/update.access_after_day_placeholder') }}"/>
                                        <div class="invalid-feedback"></div>
                                    </div>
                                </div>
                            @endif
                    ...
                    <div class="mt-30 d-flex align-items-center">
                        <button type="button" class="js-save-file btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>
    ...
    @push('scripts_bottom')
        <script>
            var filePathPlaceHolderBySource = {
                upload: '{{ trans('lms/update.file_source_upload_placeholder') }}',
                youtube: '{{ trans('lms/update.file_source_youtube_placeholder') }}',
                vimeo: '{{ trans('lms/update.file_source_vimeo_placeholder') }}',
                external_link: '{{ trans('lms/update.file_source_external_link_placeholder') }}',
                google_drive: '{{ trans('lms/update.file_source_google_drive_placeholder') }}',
                dropbox: '{{ trans('lms/update.file_source_dropbox_placeholder') }}',
                iframe: '{{ trans('lms/update.file_source_iframe_placeholder') }}',
                s3: '{{ trans('lms/update.file_source_s3_placeholder') }}',
            }
        </script>
    @endpush
@endif
