...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{!empty($webinar) ?trans('lms//admin/main.edit'): trans('lms/admin/main.new') }} {{ trans('lms/admin/main.class') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{ trans('lms/admin/main.dashboard') }}</a>
                </div>
                <div class="breadcrumb-item active">
                    <a href="/lms{{ getAdminPanelUrl() }}/webinars">{{ trans('lms/admin/main.classes') }}</a>
                </div>
                <div class="breadcrumb-item">{{!empty($webinar) ?trans('lms//admin/main.edit'): trans('lms/admin/main.new') }}</div>
            </div>
        </div>
                ...
                <div class="col-12 ">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="post" action="/lms{{ getAdminPanelUrl() }}/webinars/{{ !empty($webinar) ? $webinar->id.'/update' : 'store' }}" id="webinarForm" class="webinar-form">
                                {{ csrf_field() }}
                                <section>
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="form-group mt-15">
                                                ...
                                                <textarea id="summernote" name="description" class="form-control @error('description')  is-invalid @enderror" placeholder="{{ trans('lms/forms.webinar_description_placeholder') }}">{!! !empty($webinar) ? $webinar->description : old('description')  !!}</textarea>
                                                @error('description')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-12 col-md-6">
                                            ...
                                            <div class="row mt-15">
                                                ...
                                                <div class="col-12 col-md-6">
                                                    <div class="form-group">
                                                        <label class="input-label">{{ trans('lms/public.duration') }} ({{ trans('lms/public.minutes') }})</label>
                                                        <div class="input-group">
                                                            <div class="input-group-prepend">
                                            ...
                                            <div class="form-group mt-30 d-flex align-items-center justify-content-between">
                                                <label class="" for="includeCertificateSwitch">{{ trans('lms/update.include_certificate') }}</label>
                                                <div class="custom-control custom-switch">
                                            ...
                                            <div class="form-group mt-30 d-flex align-items-center justify-content-between">
                                                <label class="" for="privateSwitch">{{ trans('lms/update.enable_waitlist') }}</label>
                                                <div class="custom-control custom-switch">
                                            ...
                                            <div class="form-group mt-15">
                                                ...
                                                <input type="text" name="price" value="{{ (!empty($webinar) and !empty($webinar->price)) ? convertPriceToUserCurrency($webinar->price) : old('price') }}" class="form-control @error('price')  is-invalid @enderror" placeholder="{{ trans('lms/public.0_for_free') }}"/>
                                                @error('price')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
                                            ...
                                            @if(!empty($webinar) and $webinar->creator->isOrganization())
                                                <div class="form-group mt-15">
                                                    ...
                                                    @enderror
                                                    <p class="font-12 text-gray mt-1">- {{ trans('lms/update.organization_price_hint') }}</p>
                                                </div>
                                            @endif

                                            <div id="partnerInstructorInput" class="form-group mt-15 {{ (!empty($webinar) && $webinar->partner_instructor) ? '' : 'd-none' }}">
                                                <label class="input-label d-block">{{ trans('lms/public.select_a_partner_teacher') }}</label>
                                                ...
                                                <select name="partners[]" multiple data-search-option="just_teacher_role" class="js-search-partner-user form-control {{ (!empty($webinar) && $webinar->partner_instructor) ? 'search-user-select22' : '' }}"
                                                        data-placeholder="{{ trans('lms/public.search_instructor') }}"
                                                >
                                                    @if(!empty($webinarPartnerTeacher))
                                                        ...
                                                        @endforeach
                                                    @else
                                                        <option selected disabled>{{ trans('lms/public.search_instructor') }}</option>
                                                    @endif
                                                </select>

                                                <div class="text-muted text-small mt-1">{{ trans('lms/admin/main.select_a_partner_hint') }}</div>
                                            </div>

                                            <div class="form-group mt-15">
                                                <label class="input-label d-block">{{ trans('lms/public.tags') }}</label>
                                                <input type="text" name="tags" data-max-tag="5" value="{{ !empty($webinar) ? implode(',',$webinarTags) : '' }}" class="form-control inputtags" placeholder="{{ trans('lms/public.type_tag_name_and_press_enter') }} ({{ trans('lms/admin/main.max') }} : 5)"/>
                                            </div>
                                                                ...
                                                                @if(!empty($prerequisite->prerequisiteWebinar->title))
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            ...
                                                                            @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/prerequisites/'. $prerequisite->id .'/delete', 'btnClass' => ' mt-1'])
                                                                        </td>
                                                                    </tr>
                                                                @endif
                                                            @endforeach
                                                            ...
                                                            @foreach($webinarQuizzes as $webinarQuiz)
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/webinar-quiz/'. $webinarQuiz->id .'/delete', 'btnClass' => ' mt-1'])
                                                                    </td>
                                                                    @endforeach
                                                                </tr>
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <button type="button" id="saveAndPublish" class="btn btn-success">{{ !empty($webinar) ? trans('lms/admin/main.save_and_publish') : trans('lms/admin/main.save_and_continue') }}</button>

                                        @if(!empty($webinar))
                                            <button type="button" id="saveReject" class="btn btn-warning">{{ ($webinar->status == "active") ? trans('lms/update.unpublish') : trans('lms/public.reject') }}</button>

                                            @include('lms.admin.includes.delete_button',[
                                                    'url' => '/lms'.getAdminPanelUrl().'/webinars/'. $webinar->id .'/delete',
...
@push('scripts_bottom')
    <script>
        ...
        var zoomJwtTokenInvalid = '{{ trans('lms/admin/main.teacher_zoom_jwt_token_invalid') }}';
        var editChapterLang = '{{ trans('lms/public.edit_chapter') }}';
        var requestFailedLang = '{{ trans('lms/public.request_failed') }}';
        var thisLiveHasEndedLang = '{{ trans('lms/update.this_live_has_been_ended') }}';
    ...
    <script src="/assets/lms/assets/default/js/admin/quiz.min.js"></script>
