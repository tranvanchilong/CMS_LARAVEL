...
<div class="tab-pane mt-3 fade" id="security" role="tabpanel" aria-labelledby="security-tab">
    ...
    <form action="/lms{{ getAdminPanelUrl() }}/settings/security" method="post">
        {{ csrf_field() }}
            ...
            <div class="col-12 col-md-6">
                ...
                <div class="form-group custom-switches-stacked">
                    <label class="custom-switch pl-0">
                        ...
                        <label class="custom-switch-description mb-0 cursor-pointer" for="loginDeviceLimit">{{ trans('lms/update.device_limit') }}</label>
                    </label>
                    <div class="text-muted text-small mt-1">{{ trans('lms/update.device_limit_hint') }}</div>
                </div>
                ...
                <div class="form-group js-device-limit-number {{ (!empty($itemValue) and !empty($itemValue['login_device_limit']) and $itemValue['login_device_limit']) ? '' : 'd-none' }}">
                    <label class="input-label">{{ trans('lms/update.number_of_allowed_devices') }}</label>
                    ...
                    <p class="font-12 text-gray mt-1 mb-0">{{ trans('lms/update.number_of_allowed_devices_hint') }}</p>
                </div>

                <h5 class="mt-5">{{ trans('lms/update.captcha_settings') }}</h5>
                @php
                ...
                @foreach($captchaSwitchs as $captchaSwitch)
                    <div class="form-group custom-switches-stacked">
                        <label class="custom-switch pl-0 mb-0">
                            ...
                            <span class="custom-switch-indicator"></span>
                            <label class="custom-switch-description mb-0 cursor-pointer" for="captchaSwitch{{ $captchaSwitch }}">{{ trans('lms/update.'.$captchaSwitch) }}</label>
                        </label>
                        <div class="text-muted text-small">{{ trans('lms/update.'.$captchaSwitch.'_hint') }}</div>
                    </div>
                @endforeach


                <h5 class="mt-5">{{ trans('lms/update.admin_panel_url') }}</h5>
                ...
                <div class="form-group mt-2">
                    ...
                    <p class="font-12 text-gray mt-1 mb-0">{{ trans('lms/update.admin_panel_url_hint') }}</p>
                </div>
