...
<div class="tab-pane mt-3 fade" id="general_options" role="tabpanel" aria-labelledby="general_options-tab">
    <div class="row">
        <div class="col-12 col-md-6">
            <form action="/lms{{ getAdminPanelUrl() }}/settings/general_options" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="page" value="general">
                <input type="hidden" name="general_options" value="general_options">

                <div class="mb-5">
                    <h5>{{ trans('lms/update.direct_publication_options') }}</h5>
                    ...
                    @foreach($directPublicationOptions as $directPublicationOption)
                        <div class="form-group mt-3 custom-switches-stacked">
                            <label class="custom-switch pl-0">
                                ...
                                <label class="custom-switch-description mb-0 cursor-pointer" for="{{ $directPublicationOption }}Switch">{{ trans("lms/update.{$directPublicationOption}") }}</label>
                            </label>
                            <p class="font-12 text-gray mb-0">{{ trans("lms/update.{$directPublicationOption}_hint") }}</p>
                        </div>
                    @endforeach
