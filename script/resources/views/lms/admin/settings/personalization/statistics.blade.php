...
@push('styles_top')
    ...
    <link rel="stylesheet" href="/assets/lms/assets/admin/vendor/bootstrap-colorpicker/bootstrap-colorpicker.min.css">
@endpush

<div class="mt-3" id="others_personalization">

    <form action="/lms{{ getAdminPanelUrl("/settings/personalization/statistics") }}" method="post">
        {{ csrf_field() }}
        ...
        <div class="form-group custom-switches-stacked">
            <label class="custom-switch pl-0">
                ...
                <label class="custom-switch-description mb-0 cursor-pointer" for="enableStatisticsSwitch">{{ trans('lms/update.enable_statistics') }}</label>
            </label>
        </div>
        ...
        <div class="form-group custom-switches-stacked">
            <label class="custom-switch pl-0">
                ...
                <span class="custom-switch-indicator"></span>
                <label class="custom-switch-description mb-0 cursor-pointer" for="display_default_statisticsSwitch">{{ trans('lms/update.display_default_statistics') }}</label>
            </label>
        </div>

        <div class="js-custom-statistics mb-4 {{ (!empty($itemValue) and !empty($itemValue['display_default_statistics'])) ? 'd-none' : '' }}">
            <button type="button" class="js-add-statistics btn btn-primary" data-path="/lms{{ getAdminPanelUrl("/settings/personalization/statistics/get-form") }}">{{ trans('lms/update.add_statistics') }}</button>

            <div class="row mt-3 bg-whitesmoke py-2 rounded-sm">
                <div class="col-1">{{ trans('lms/admin/main.icon') }}</div>
                <div class="col-3">{{ trans('lms/admin/main.title') }}</div>
                <div class="col-3">{{ trans('lms/admin/main.description') }}</div>
                <div class="col-1">{{ trans('lms/admin/main.count') }}</div>
                <div class="col-2">{{ trans('lms/admin/main.background') }}</div>
                <div class="col-2 text-right">{{ trans('lms/admin/main.actions') }}</div>
            </div>
            ...
            <ul class="draggable-lists list-group">
                @foreach($statistics as $statistic)
                    <li data-id="{{ $statistic->id }}" class="row align-items-center mt-2">
                        <div class="col-1">
                            <img src="{{get_path_lms()}}{{ $statistic->icon }}" alt="" class="img-fluid" width="40" height="40">
                        </div>
                        ...
                        <div class="col-2 text-right">
                            <div class="d-flex align-items-center justify-content-end">
                                ...
                                <button type="button" class="js-edit-statistic btn-transparent text-primary mr-2" data-path="/lms{{ getAdminPanelUrl("/settings/personalization/statistics/{$statistic->id}/editItem") }}"><i class="fa fa-edit"></i></button>

                                @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl("/settings/personalization/statistics/{$statistic->id}/deleteItem"),'btnClass' => 'text-danger mr-2 font-16'])
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/admin/vendor/bootstrap-colorpicker/bootstrap-colorpicker.min.js"></script>

    <script>
        var saveSuccessLang = '{{ trans('lms/webinars.success_store') }}';
    </script>

    <script src="/assets/lms/assets/default/js/admin/settings/statistics.min.js"></script>
@endpush
