...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{!empty($role) ?trans('lms//admin/main.edit'): trans('lms/admin/main.new') }} {{ trans('lms/admin/main.role') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{ trans('lms/admin/main.dashboard') }}</a>
                </div>
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}/roles">{{ trans('lms/admin/main.roles') }}</a>
                </div>
                <div class="breadcrumb-item">{{!empty($role) ?trans('lms//admin/main.edit'): trans('lms/admin/main.new') }}</div>
            </div>
        </div>
        @if ($errors->any())
        ...
        <div class="section-body">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="/lms{{ getAdminPanelUrl() }}/roles/{{ !empty($role) ? $role->id.'/update' : 'store' }}" method="Post">
                                ...
                                <div class="row">
                                    <div class="col-12 col-md-6 col-lg-6">
                                        ...
                                        <div class="form-group @error('caption') is-invalid @enderror">
                                            <label>{{ trans('lms/admin/main.caption') }}</label>
                                            <input type="text" name="caption" class="form-control" value="{{ !empty($role) ? $role->caption : old('caption') }}"
                                                   placeholder="{{ trans('lms/admin/main.create_field_name_placeholder') }}"/>
                                        ...
                                        @if(empty($role) or !$role->isDefaultRole())
                                            <div class="form-group mb-1">
                                                <label class="custom-switch pl-0">
                                                    ...
                                                    <span class="custom-switch-description">{{ trans('lms/admin/main.is_admin') }}</span>
                                                </label>
                                            </div>
                                            <div class="text-muted text-small mt-1">{{ trans('lms/admin/main.new_role_admin_access_hint') }}</div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="form-group {{ (!empty($role) && $role->is_admin) ? '' :'d-none'}}" id="sections">
                                    ...
                                    <h2 class="section-title">{{ trans('lms/admin/main.permission') }}</h2>
                                    <p class="section-lead">
                                        {{ trans('lms/admin/main.permission_description') }}
                                    </p>
...
@push('scripts_bottom')
    <script src="/assets/lms/assets/default/js/admin/roles.min.js"></script>
@endpush
