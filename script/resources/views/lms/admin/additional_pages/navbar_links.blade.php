...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('lms/admin/main.settings_navbar_links') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{ trans('lms/admin/main.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ trans('lms/admin/main.settings_navbar_links') }}</div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-12 col-md-8 col-lg-6">
                                    <form action="/lms{{ getAdminPanelUrl() }}/additional_page/navbar_links/store" method="post">
                                        {{ csrf_field() }}
                            ...
                            <div class="table-responsive mt-4">
                                <table class="table table-striped font-14">
                                    <tr>
                                        ...
                                        <th>{{ trans('lms/admin/main.order') }}</th>
                                    ...
                                    @if(!empty($items))
                                        @foreach($items as $key => $val)
                                            <tr>
                                                ...
                                                <td>{{ $val['order'] }}</td>
                                                <td>
                                                    <a href="/lms{{ getAdminPanelUrl() }}/additional_page/navbar_links/{{ $key }}/edit" class="btn-sm" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.edit') }}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>

                                                    @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/additional_page/navbar_links/'. $key .'/delete','btnClass' => 'btn-sm'])
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
