...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{!empty($filter) ?trans('lms//admin/main.edit'): trans('lms/admin/main.new') }} {{ trans('lms/admin/main.filters') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{ trans('lms/admin/main.dashboard') }}</a>
                </div>
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}/store/filters">{{ trans('lms/admin/main.filters') }}</a>
                </div>
                <div class="breadcrumb-item">{{!empty($filter) ?trans('lms//admin/main.edit'): trans('lms/admin/main.new') }}</div>
            </div>
        </div>
        <div class="section-body">
            <div class="row">
                <div class="col-12 col-md-6 col-lg-6">
                    <div class="card">
                        <div class="card-body">
                            <form action="/lms{{ getAdminPanelUrl() }}/store/filters/{{ !empty($filter) ? $filter->id.'/update' : 'store' }}"
                                  method="Post">
                                {{ csrf_field() }}
                                ...
                                <div id="filterOptions" class="ml-1">
                                    <div class="d-flex align-items-center justify-content-between mb-4">
                                        <strong class="d-block">{{ trans('lms/admin/main.add_options') }}</strong>

                                        <button type="button" class="btn btn-success add-btn "><i class="fa fa-plus"></i> {{ trans('lms/admin/main.add') }}</button>
                                    </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/admin/filters.min.js"></script>
@endpush
