        ...
        <div class="section-body">
            ...
            <div class="row">
                    ...
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.total_success_orders')}}</h4>
                            </div>
                            <div class="card-body">
                                {{ $successOrders['count'] }}
                            </div>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.total_canceled_orders')}}</h4>
                            </div>
                            <div class="card-body">
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.total_waiting_orders')}}</h4>
                            </div>
                            <div class="card-body">
                                {{ $waitingOrders['count'] }}
                            </div>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('lms/update.total_orders')}}</h4>
                            </div>
                            <div class="card-body">
                                {{ $totalOrders['count'] }}
                            </div>
                ...
                <div class="card-body">
                    <form method="get" class="mb-0">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="status" data-plugin-selectTwo class="form-control populate">
                                        ...
                                        @foreach(\App\Models\LMS\ProductOrder::$status as $str)
                                            @if($str != \App\Models\LMS\ProductOrder::$pending)
                                                <option value="{{ $str }}" @if(request()->get('status') == $str) selected @endif>{{ trans('lms/update.product_order_status_'.$str) }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="input-label">{{ trans('lms/admin/main.seller') }}</label>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="input-label">{{ trans('lms/update.customer') }}</label>
            ...
            <div class="row">
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            @if($authUser->can('admin_store_products_orders_export'))
                                <a href="/lms{{ getAdminPanelUrl() }}/store/orders/export?{{ !empty($inHouseOrders) ? 'in-house-orders=true&' : '' }}{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                            @endif
                        </div>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    <tr>
                                        <th>#</th>
                                        <th class="text-left">{{ trans('lms/update.customer') }}</th>
                                        <th class="text-left">{{ trans('lms/admin/main.seller') }}</th>
                                        <th>{{ trans('lms/admin/main.type') }}</th>
                                        <th>{{ trans('lms/update.quantity') }}</th>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-left">
                                                ...
                                                @elseif(!empty($order->gift) and !empty($order->gift))
                                                    ...
                                                    <span class="d-block mt-1 text-muted font-12">{!! trans('lms/update.a_gift_for_name_on_date',['name' => $order->gift->name, 'date' => (!empty($order->gift->date) ? dateTimeFormat($order->gift->date, 'j M Y H:i') : trans('lms/update.instantly'))]) !!}</span>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                @if($authUser->can('admin_store_products_orders_invoice'))
                                                    @if(!empty($order->product))
                                                        <a href="/lms{{ getAdminPanelUrl() }}/store/orders/{{ $order->id }}/invoice" target="_blank" title="{{ trans('lms/admin/main.invoice') }}"><i class="fa fa-print" aria-hidden="true"></i></a>
                                                    @endif
                                                @endif

                                                @if($authUser->can('admin_store_products_orders_refund'))
                                                    @include('lms.admin.includes.delete_button',[
                                                            'url' => '/lms'.getAdminPanelUrl().'/store/orders/'. $order->id .'/refund',
                                                ...
                                                @if($order->status == \App\Models\LMS\ProductOrder::$waitingDelivery)
                                                    @if($authUser->can('admin_store_products_orders_tracking_code'))
                                                        <button type="button"
                                                                data-sale-id="{{ $order->sale_id }}"
                                                                data-product-order-id="{{ $order->id }}"
                                                                data-toggle="tooltip" title="{{ trans('lms/update.enter_tracking_code') }}"
...
@push('scripts_bottom')
    <script>
        var enterTrackingCodeModalTitleLang = '{{ trans('lms/update.enter_tracking_code') }}';
        ...
        var trackingCodeSaveSuccessLang = '{{ trans('lms/update.tracking_code_success_save') }}';
    </script>

    <script src="/assets/lms/assets/default/vendors/sweetalert2/dist/sweetalert2.min.js"></script>
    <script src="/assets/lms/assets/default/js/admin/store/orders.min.js"></script>
@endpush
