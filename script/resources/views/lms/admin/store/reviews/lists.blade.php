        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{trans('lms/update.products_without_review')}}</h4>
                        </div>
                        <div class="card-body">
                ...
                <div class="card-body">
                    <table class="table table-striped font-14" id="datatable-details">
                        ...
                        @foreach($reviews as $review)
                            <tr>
                                <td class="text-left">
                                    <a href="/lms{{ $review->product->getUrl() }}" target="_blank">{{ $review->product->title }}</a>
                                </td>
                                ...
                                <td class="" width="50">
                                    @if($authUser->can('admin_store_products_reviews_status_toggle'))
                                        <a href="/lms{{ getAdminPanelUrl() }}/store/reviews/{{ $review->id }}/toggleStatus" class="btn-transparent text-primary mr-1" data-toggle="tooltip" data-placement="top" title="{{ ($review->status == 'active') ? 'Hidden' : 'Publish' }}">
                                            @if($review->status == 'active')
                                    ...
                                    @if($authUser->can('admin_store_products_reviews_detail_show'))
                                    ...
                                    @if($authUser->can('admin_reviews_reply'))
                                        <a href="/lms{{ getAdminPanelUrl("/store/reviews/{$review->id}/reply") }}" target="_blank" class="btn-transparent text-primary mr-1" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.reply') }}">
                                            <i class="fa fa-reply"></i>
                                        </a>
                                    @endif

                                    @if($authUser->can('admin_store_products_reviews_delete'))
                                        @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/store/reviews/'. $review->id.'/delete','btnClass' => ''])
                                    @endif
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex align-items-center justify-content-between border-bottom py-2">
                        <span class="font-weight-bold">{{ trans('lms/update.product') }}:</span>
                    ...
                    <div class="d-flex align-items-center justify-content-between border-bottom py-2">
                        <span class="font-weight-bold">{{ trans('lms/update.delivery') }}:</span>
                        <span class="js-delivery_quality"></span>
                    </div>

                    <div class="d-flex align-items-center justify-content-between border-bottom py-2">
                        <span class="font-weight-bold">{{ trans('lms/update.seller') }}:</span>
