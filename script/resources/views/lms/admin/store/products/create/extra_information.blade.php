<section>
    <h2 class="section-title after-line mt-2 mb-4">{{ trans('lms/public.extra_information') }}</h2>
    ...
    <div class="col-12 col-md-6 mt-15">
        ...
        <div class="form-group">
            ...
            <input type="number" name="price" value="{{ !empty($product) ? $product->price : old('price') }}" class="form-control @error('price')  is-invalid @enderror" placeholder="{{ trans('lms/public.0_for_free') }}"/>
            @error('price')
        ...
        @if($product->isPhysical())
            <div class="form-group">
                <label class="input-label">{{ trans('lms/update.delivery_fee') }}</label>
                <input type="number" name="delivery_fee" value="{{ !empty($product) ? $product->delivery_fee : old('delivery_fee') }}" class="form-control @error('delivery_fee')  is-invalid @enderror" placeholder="{{ trans('lms/update.0_for_free_delivery') }}"/>
                @error('delivery_fee')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="form-group">
                <label class="input-label">{{ trans('lms/update.delivery_estimated_time') }} ({{ trans('lms/public.day') }})</label>
        ...
        <div class="form-group js-inventory-inputs {{ (!empty($product) and $product->unlimited_inventory) ? 'd-none' : '' }}">
            <label class="input-label">{{ trans('lms/update.inventory') }}</label>
        ...
        <div class="form-group js-inventory-inputs {{ (!empty($product) and $product->unlimited_inventory) ? 'd-none' : '' }}">
            <label class="input-label">{{ trans('lms/update.inventory_warning') }}</label>
            ...
            <div class="text-muted text-small mt-1">{{ trans('lms/update.inventory_warning_hint') }}</div>
        ...
        <div class="form-group mb-1 d-flex align-items-center">
            <label class="cursor-pointer mb-0 input-label mr-2" for="unlimitedInventorySwitch">{{ trans('lms/update.unlimited_inventory') }}</label>
        ...
        <p class="text-gray font-12">{{ trans('lms/update.create_product_unlimited_inventory_hint') }}</p>
