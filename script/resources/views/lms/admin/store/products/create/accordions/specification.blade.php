<div class="accordion-row bg-white rounded-lg border border-gray300 mt-3 py-3 py-lg-4 px-2 px-lg-3">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="specification_{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}">
        <div class="d-flex align-items-center cursor-pointer" href="#collapseSpecification{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}" aria-controls="collapseSpecification{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}" data-parent="#specificationsAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <div class="font-weight-bold text-dark-blue d-block">{{ !empty($selectedSpecification) ? $selectedSpecification->specification->title : trans('lms/update.add_new_specification') }}</div>
        </div>

        <div class="d-flex align-items-center">
            @if(!empty($selectedSpecification) and $selectedSpecification->status != 'active')
                <span class="disabled-content-badge mr-3">{{ trans('lms/public.disabled') }}</span>
            @endif

            @if(!empty($selectedSpecification))
                @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/store/products/specifications/'. $selectedSpecification->id .'/delete','btnClass' => 'mr-3', 'btnText' => '<i class="fa fa-trash"></i>'])
            @endif
    ...
    <div id="collapseSpecification{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}" aria-labelledby="specification_{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}" class=" collapse @if(empty($selectedSpecification)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form specification-form" data-action="/lms{{ getAdminPanelUrl() }}/store/products/specifications/{{ !empty($selectedSpecification) ? $selectedSpecification->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group mt-15">
                            <label class="input-label d-block">{{ trans('lms/update.specification') }}</label>
                            ...
                            <select name="ajax[{{ !empty($selectedSpecification) ? $selectedSpecification->id : 'new' }}][specification_id]"
                                    ...
                                    data-placeholder="{{ trans('lms/update.search_and_select_specifications') }}"
                                    data-allow-clear="false"
                                {{ !empty($selectedSpecification) ? 'disabled' : '' }}
                            >

                                @if(!empty($productSpecifications))
                                    <option value="">{{ trans('lms/update.search_and_select_specifications') }}</option>
                            ...
                            <div class="form-group js-multi-values-input  {{ (!empty($selectedSpecification) and $selectedSpecification->type == 'multi_value') ? '' : 'd-none' }}">
                                <label class="input-label d-block">{{ trans('lms/update.parameters') }}</label>
                                ...
                                <select name="ajax[{{ !empty($selectedSpecification) ? $selectedSpecification->id : 'new' }}][multi_values][]"
                                        ...
                                        multiple
                                        data-placeholder="{{ trans('lms/update.select_specification_params') }}"
                                        data-allow-clear="false"
                                        data-search="false"
                                >
                        ...
                        <div class="form-group js-summery-input {{ (!empty($selectedSpecification) and $selectedSpecification->type == 'textarea') ? '' : 'd-none' }}">
                            <label class="input-label d-block">{{ trans('lms/update.description') }}</label>
                        ...
                        <div class="form-group mt-3 js-allow-selection-input {{ (!empty($selectedSpecification) and $selectedSpecification->type == 'multi_value') ? '' : 'd-none' }}">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer input-label" for="specificationAllowSelectionSwitch{{ !empty($selectedSpecification) ? $selectedSpecification->id : '_record' }}">{{ trans('lms/update.allow_user_selection') }}</label>
                            ...
                            <div class="text-muted text-small mt-1">{{ trans('lms/update.allow_user_selection_hint') }}</div>
                        </div>

                        <div class="form-group mt-3">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer input-label" for="specificationStatusSwitch{{ !empty($selectedSpecification) ? $selectedSpecification->id : '_record' }}">{{ trans('lms/public.active') }}</label>
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" name="ajax[{{ !empty($selectedSpecification) ? $selectedSpecification->id : 'new' }}][status]" class="custom-control-input" id="specificationStatusSwitch{{ !empty($selectedSpecification) ? $selectedSpecification->id : '_record' }}" {{ (empty($selectedSpecification) or $selectedSpecification->status == \App\Models\LMS\ProductSelectedSpecification::$Active) ? 'checked' : ''  }}>
                ...
                <div class="mt-30 d-flex align-items-center">
                    <button type="button" class="js-save-specification btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>

                    @if(empty($selectedSpecification))
                        <button type="button" class="btn btn-sm btn-danger ml-2 cancel-accordion">{{ trans('lms/public.close') }}</button>
                    @endif
                </div>
            </div>
        </div>
