<div class="accordion-row bg-white rounded-lg border border-gray300 mt-3 py-3 py-lg-4 px-2 px-lg-3">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="faq_{{ !empty($faq) ? $faq->id :'record' }}">
        <div class="d-flex align-items-center cursor-pointer" href="#collapseFaq{{ !empty($faq) ? $faq->id :'record' }}" aria-controls="collapseFaq{{ !empty($faq) ? $faq->id :'record' }}" data-parent="#faqsAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <div class="font-weight-bold text-dark-blue d-block">{{ !empty($faq) ? $faq->title : trans('lms/webinars.add_new_faqs') }}</div>
        </div>

        <div class="d-flex align-items-center">

            @if(!empty($faq))
                @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/store/products/faqs/'. $faq->id .'/delete','btnClass' => 'mr-3', 'btnText' => '<i class="fa fa-trash"></i>'])
            @endif
    ...
    <div id="collapseFaq{{ !empty($faq) ? $faq->id :'record' }}" aria-labelledby="faq_{{ !empty($faq) ? $faq->id :'record' }}" class=" collapse @if(empty($faq)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form faq-form" data-action="/lms{{ getAdminPanelUrl() }}/store/products/faqs/{{ !empty($faq) ? $faq->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($faq) ? $faq->id : 'new' }}][title]" class="js-ajax-title form-control" value="{{ !empty($faq) ? $faq->title : '' }}" placeholder="{{ trans('lms/forms.maximum_255_characters') }}"/>
