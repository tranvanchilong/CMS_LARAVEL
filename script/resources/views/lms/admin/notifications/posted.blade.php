                ...
                <div class="card-header">
                    @if($authUser->can('admin_notifications_send'))
                        <div class="text-right">
                            <a href="/lms{{ getAdminPanelUrl() }}/notifications/send" class="btn btn-primary">{{ trans('lms/notification.send_notification') }}</a>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped font-14" id="datatable-basic">
                            ...
                            <tr>
                                ...
                                <th class="text-center">{{ trans('lms/notification.receiver') }}</th>
                            ...
                            @foreach($notifications as $notification)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <button type="button" data-item-id="{{ $notification->id }}" class="js-show-description btn btn-outline-primary">{{ trans('lms/admin/main.show') }}</button>
                                        <input type="hidden" value="{{ nl2br($notification->message) }}">
                                    </td>
                                    <td class="text-center">{{ trans('lms/admin/main.notification_'.$notification->type) }}</td>
                                    <td class="text-center">
                                        @if(empty($notification->notificationStatus))
                                            <span class="text-danger">{{ trans('lms/admin/main.unread') }}</span>
                                        @else
                                            <span class="text-success">{{ trans('lms/admin/main.read') }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td width="100">
                                        ...
                                        @if($authUser->can('admin_notifications_delete'))
                                            @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/notifications/'. $notification->id.'/delete','btnClass' => ''])
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
...
@push('scripts_bottom')
    <script src="/assets/lms/assets/default/js/admin/notifications.min.js"></script>
@endpush
