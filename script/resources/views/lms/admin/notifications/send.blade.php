        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="post" action="/lms{{ getAdminPanelUrl() }}/notifications/{{ !empty($notification) ? $notification->id .'/update' : 'store' }}" class="form-horizontal form-bordered mt-4">
                        {{ csrf_field() }}
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" id="typeSelect" class="form-control @error('type') is-invalid @enderror">
                                        ...
                                        @foreach(\App\Models\LMS\Notification::$notificationsType as $type)
                                            <option value="{{ $type }}" @if(!empty($notification) and $notification->type == $type) selected @endif>{{ trans('lms/admin/main.notification_'.$type) }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">@error('type') {{ $message }} @enderror</div>
                                    <div class="text-muted text-small mt-1">{{ trans('lms/admin/main.new_notification_hint') }}</div>
                                </div>
                                ...
                                <div class="form-group @if(!$errors->has('webinar_id') and (empty($notification) or empty($notification->webinar))) d-none @endif" id="webinarSelect">
                                    <label class="input-label d-block">{{ trans('lms/admin/main.course') }}</label>
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                <button class="btn btn-primary" type="submit">{{ trans('lms/notification.send_notification') }}</button>
                            </div>
                        </div>
