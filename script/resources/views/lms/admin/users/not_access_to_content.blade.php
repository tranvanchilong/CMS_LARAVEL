...
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a href="/lms#">{{ $pageTitle }}</a></div>
            </div>
        </div>
    </section>
    ...
    <div class="card">
        <div class="card-header">
            @if($authUser->can('admin_users_not_access_content_toggle'))
                <button type="button" id="addNewUserToNotaccess" class="btn btn-primary">{{ trans('lms/admin/main.add_new') }}</button>
            @endif
        </div>
            ...
            <div class="table-responsive text-center">
                <table class="table table-striped font-14">
                    <tr>
                        ...
                        <th>{{ trans('lms/update.access_to_content') }}</th>
                    ...
                    @foreach($users as $user)
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="mt-0 mb-1 font-weight-bold text-danger">{{ trans('lms/admin/main.no') }}</div>
                            </td>
                            ...
                            <td class="text-center mb-2" width="120">
                                ...
                                @if($authUser->can('admin_users_not_access_content_toggle'))
                                    <a href="/lms{{ getAdminPanelUrl() }}/users/not-access-to-content/{{ $user->id }}/active" class="btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.active') }}">
                                        <i class="fa fa-arrow-up"></i>
                                    </a>
                                @endif

                                @if($authUser->can('admin_users_delete'))
                                    @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/users/'.$user->id.'/delete' , 'btnClass' => ''])
                                @endif
                            </td>
    ...
    <div id="addUserToNotAccessModal" class="d-none">
        <h3 class="section-title after-line">{{ trans('lms/update.add_to_not_access') }}</h3>
        <div class="mt-25">
            <form action="/lms{{ getAdminPanelUrl() }}/users/not-access-to-content/store" method="post">
                ...
                <div class="d-flex align-items-center justify-content-end mt-3">
                    <button type="button" class="js-save-add-user-to-not-access btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/admin/not_access_to_content.min.js"></script>
@endpush
