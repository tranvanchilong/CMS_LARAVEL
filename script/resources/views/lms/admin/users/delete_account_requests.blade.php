                    ...
                    @foreach($requests as $request)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <div class="mt-0 mb-1 font-weight-bold {{ ($request->user->status == 'active') ? 'text-success' : 'text-warning' }}">{{ trans('lms/admin/main.'.$request->user->status) }}</div>
                                @endif
                            </td>

                            <td class="text-center mb-2" width="120">
                                @if($authUser->can('admin_users_impersonate'))
                                    <a href="/lms{{ getAdminPanelUrl() }}/users/{{ $request->user->id }}/impersonate" target="_blank" class="btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.login') }}">
                                        <i class="fa fa-user-shield"></i>
                                    </a>
                                @endif

                                @if($authUser->can('admin_users_edit'))
                                    <a href="/lms{{ getAdminPanelUrl() }}/users/{{ $request->user->id }}/edit" class="btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.edit') }}">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                @endif

                                @if($authUser->can('admin_delete_account_requests_confirm'))
                                    @include('lms.admin.includes.delete_button',[
                                        'url' => '/lms'.getAdminPanelUrl().'/users/delete-account-requests/'.$request->id.'/confirm' ,
                                        'btnIcon' => 'fa-arrow-up',
                                        'tooltip' => trans('lms/update.confirm')
                                       ])
                                @endif
                            </td>
