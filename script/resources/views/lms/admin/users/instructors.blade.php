...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('lms/admin/main.instructors') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms#">{{ trans('lms/admin/main.instructors') }}</a></div>
                <div class="breadcrumb-item">{{ trans('lms/admin/main.users') }}</div>
            </div>
        </div>
    </section>
        ...
        <div class="row">
                ...
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{ trans('lms/admin/main.total_instructors') }}</h4>
                        </div>
                        <div class="card-body">
                            {{ $totalInstructors }}
                        </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{ trans('lms/admin/main.organizations_instructors') }}</h4>
                        </div>
                        <div class="card-body">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{ trans('lms/admin/main.inactive_instructors') }}</h4>
                        </div>
                        <div class="card-body">
                            {{ $inactiveInstructors }}
                        </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{ trans('lms/admin/main.ban_instructors') }}</h4>
                        </div>
                        <div class="card-body">
                            {{ $banInstructors }}
                        </div>
            ...
            <div class="card-body">
                <form method="get" class="mb-0">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select name="sort" data-plugin-selectTwo class="form-control populate">
                                    ...
                                    <option value="sales_classes_asc" @if(request()->get('sort') == 'sales_classes_asc') selected @endif>{{ trans('lms/admin/main.classes_sales_ascending') }}</option>
                                    <option value="sales_classes_desc" @if(request()->get('sort') == 'sales_classes_desc') selected @endif>{{ trans('lms/admin/main.classes_sales_descending') }}</option>
                                    <option value="purchased_classes_asc" @if(request()->get('sort') == 'purchased_asc') selected @endif>{{ trans('lms/admin/main.purchased_classes_ascending') }}</option>
                                    <option value="purchased_classes_desc" @if(request()->get('sort') == 'purchased_desc') selected @endif>{{ trans('lms/admin/main.purchased_classes_descending') }}</option>
                                    <option value="sales_appointments_asc" @if(request()->get('sort') == 'appointments_asc') selected @endif>{{ trans('lms/admin/main.sales_appointments_ascending') }}</option>
                                    <option value="sales_appointments_desc" @if(request()->get('sort') == 'appointments_desc') selected @endif> {{ trans('lms/admin/main.sales_appointments_descending') }}</option>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label class="input-label">{{ trans('lms/admin/main.user_group') }}</label>
    ...
    <div class="card">
        <div class="card-header">
            @if($authUser->can('admin_users_export_excel'))
                <a href="/lms{{ getAdminPanelUrl() }}/instructors/excel?{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
            @endif
            <div class="h-10"></div>
        </div>
            ...
            <div class="table-responsive text-center">
                <table class="table table-striped font-14">
                    <tr>
                        ...
                        <th>{{ trans('lms/admin/main.classes_sales') }}</th>
                        <th>{{ trans('lms/admin/main.appointments_sales') }}</th>
                        <th>{{ trans('lms/admin/main.purchased_classes') }}</th>
                        <th>{{ trans('lms/admin/main.purchased_appointments') }}</th>
                            ...
                            <td>
                                <div class="media-body">
                                    @if($user->ban and !empty($user->ban_end_at) and $user->ban_end_at > time())
                                        <div class="mt-0 mb-1 font-weight-bold text-danger">{{ trans('lms/admin/main.banned') }}</div>
                                        <div class="text-small font-600-bold">{{ trans('lms/admin/main.until') }} {{ dateTimeFormat($user->ban_end_at, 'j M Y') }}</div>
                                    @else
                                        <div class="mt-0 mb-1 font-weight-bold {{ ($user->status == 'active') ? 'text-success' : 'text-warning' }}">{{ trans('lms/admin/main.'.$user->status) }}</div>
                                        <div class="text-small font-600-bold {{ ($user->verified ? ' text-success ' : ' text-warning ') }}">({{ trans('lms/admin/main.'.($user->verified ? 'verified' : 'not_verified')) }})</div>
                                    @endif
                                </div>
                            </td>
    ...
    <section class="card">
        <div class="card-body">
            <div class="section-title ml-0 mt-0 mb-3"><h4>{{trans('lms/admin/main.hints')}}</h4></div>
            <div class="row">
                <div class="col-md-4">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{ trans('lms/admin/main.instructors_hint_title_1') }}</div>
                        <div class=" text-small font-600-bold mb-2">{{ trans('lms/admin/main.instructors_hint_description_1') }}</div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{ trans('lms/admin/main.instructors_hint_title_2') }}</div>
                        <div class=" text-small font-600-bold mb-2">{{ trans('lms/admin/main.instructors_hint_description_2') }}</div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{ trans('lms/admin/main.instructors_hint_title_3') }}</div>
                        <div class="text-small font-600-bold mb-2">{{ trans('lms/admin/main.instructors_hint_description_3') }}</div>
                    </div>
                </div>
