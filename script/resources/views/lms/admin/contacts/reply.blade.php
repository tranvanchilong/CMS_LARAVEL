                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header flex-column align-items-start">
                            <h4>{{ trans('lms/admin/main.user_name') }}: <span class="text-black-50">{{ $contact->name }}</span></h4>
                            <h4>{{ trans('lms/admin/main.email') }} : <span class="text-black-50">{{ $contact->email }}</span></h4>
                            <h4>{{ trans('lms/admin/main.phone') }} : <span class="text-black-50">{{ $contact->phone }}</span></h4>
                            <h4>{{ trans('lms/site.message') }} :</h4>
                            <p class="mt-2">{{ nl2br($contact->message) }}</p>
                        </div>

                        <div class="card-body ">
                            <form action="/lms{{ getAdminPanelUrl() }}/contacts/{{ $contact->id }}/reply" method="post">
                                {{ csrf_field() }}
