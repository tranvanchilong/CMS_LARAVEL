            ...
            <div class="row">
                <div class="col-12 col-md-12">
                    <div class="card">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="text-{{ (empty($reply->reply_id)) ? 'info' : 'warning' }}">
                                                            {{ (empty($reply->reply_id)) ? trans('lms/admin/main.main_comment') : trans('lms/admin/main.replied') }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @if($authUser->can('admin_comments_status'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/comments/{{ $page }}/{{ $reply->id }}/toggle" class="btn-transparent text-primary">
                                                        ...
                                                        @if($authUser->can('admin_comments_edit'))
                                                            <a href="/lms{{ getAdminPanelUrl() }}/comments/{{ $page }}/{{ $reply->id }}/edit" class="btn-transparent text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/admin/main.edit') }}">
                                                                <i class="fa fa-edit"></i>
                                                            </a>
                                                        @endif

                                                        @if($authUser->can('admin_comments_delete'))
                                                            @include('lms.admin.includes.delete_button',['url' => '/lms'.getAdminPanelUrl().'/comments/'. $page .'/'.$reply->id.'/delete','btnClass' => 'btn-sm mt-2'])
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                        ...
                        <div class="card-body ">
                            <form action="/lms{{ getAdminPanelUrl() }}/comments/{{ $page }}/{{ $comment->id }}/reply" method="post">
                                {{ csrf_field() }}
