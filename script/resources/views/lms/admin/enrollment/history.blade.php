...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('lms/update.enrollment_history') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/lms{{ getAdminPanelUrl() }}">{{trans('lms/admin/main.dashboard')}}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('lms/update.enrollment_history') }}</div>
            </div>
        </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="teacher_ids[]" multiple="multiple" data-search-option="just_teacher_role" class="form-control search-user-select2"
                                            data-placeholder="{{ trans('lms/update.search_instructor') }}">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="student_ids[]" multiple="multiple" data-search-option="just_student_role" class="form-control search-user-select2"
                                            data-placeholder="{{ trans('lms/webinars.select_student') }}">
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            @if($authUser->can('admin_enrollment_export'))
                                <a href="/lms{{ getAdminPanelUrl() }}/enrollments/export" class="btn btn-primary">{{ trans('lms/admin/main.export_xls') }}</a>
                            @endif
                        </div>
                                            ...
                                            <td>
                                                <span class="font-weight-bold">
                                                    @if($sale->manual_added)
                                                        <span class="text-warning">{{ trans('lms/public.manual') }}</span>
                                                    @else
                                                        {{ trans('lms/update.normal_purchased') }}
                                                    @endif
                                                </span>
                                            </td>
                                                ...
                                                @if($sale->access_to_purchased_item)
                                                    @if($authUser->can('admin_enrollment_block_access'))
                                                        @include('lms.admin.includes.delete_button',[
                                                                'url' => '/lms'.getAdminPanelUrl().'/enrollments/'. $sale->id .'/block-access',
                                                    ...
                                                    @endif
                                                @else
                                                    @if($authUser->can('admin_enrollment_enable_access'))
                                                        @include('lms.admin.includes.delete_button',[
                                                                'url' => '/lms'.getAdminPanelUrl().'/enrollments/'. $sale->id .'/enable-access',
