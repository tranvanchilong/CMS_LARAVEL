    ...
    <div class="d-flex flex-wrap">
        <div class="col-12 col-md-3">
            <div class="position-relative bg-info-light d-flex flex-column align-items-center justify-content-start rounded-lg w-100 h-100 p-3">
                ...
                <a href="/lms{{ $cardUser->getProfileUrl() }}">
                ...
                <div class="d-flex flex-column h-100">
                    @if(!empty($post) and !empty($post->parent))
                        <div class="post-quotation p-2 rounded-sm border mb-2">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="ml-2">
                                    <span class="d-block">{{ trans('lms/update.reply_to') }}</span>
                    ...
                    @if(!empty($post) and !empty($post->attach))
                        <div class="mt-auto d-inline-flex">
                            <a href="/lms{{ $post->getAttachmentUrl($forum->slug,$topic->slug) }}" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-2 py-1 rounded-pill">
                    ...
                    @elseif(empty($post) and !empty($topic->attachments) and count($topic->attachments))
                        <div class="mt-auto d-inline-flex align-items-center">
                            @foreach($topic->attachments as $attachment)
                                <a href="/lms{{ $attachment->getDownloadUrl($forum->slug,$topic->slug) }}" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-2 py-1 rounded-pill mr-2">
                                    <i class="fa fa-download"></i>
                    ...
                    <div class="d-flex align-items-center">
                        @if(!empty($post))
                            @include('lms.admin.includes.delete_button', [
                                        'url' => '/lms'.getAdminPanelUrl().'/forums/'.$forum->id.'/topics/'.$topic->id.'/posts/'.$post->id.'/delete',
                                        'btnText' => trans('lms/admin/main.delete'),
                                        'btnClass' => 'mr-3 font-14 font-weight-500 text-danger'
                                    ])
                        @else
                            @include('lms.admin.includes.delete_button', [
                                        'url' => '/lms'.getAdminPanelUrl().'/forums/'.$forum->id.'/topics/'.$topic->id.'/delete',
                                        'btnText' => trans('lms/admin/main.delete'),
                                        'btnClass' => 'mr-3 font-14 font-weight-500 text-danger'
                                    ])
                        @endif
                        ...
                        @if(!$topic->close)
                            @if(!empty($post))
                                <button type="button" data-action="/lms{{ getAdminPanelUrl() }}/forums/{{ $forum->id }}/topics/{{ $topic->id }}/posts/{{ $post->id }}/edit" class="js-post-edit btn-transparent mr-3 font-14 font-weight-500 text-gray">{{ trans('lms/public.edit') }}</button>
                            @else
                                <a href="/lms{{ getAdminPanelUrl() }}/forums/{{ $forum->id }}/topics/{{ $topic->id }}/edit" target="_blank" class="mr-3 font-14 font-weight-500 text-gray">{{ trans('lms/public.edit') }}</a>
                            @endif

                            @if(!empty($post))
                                @if($post->pin)
                                    <button type="button" data-action="/lms{{ getAdminPanelUrl() }}/forums/{{ $topic->forum_id }}/topics/{{ $topic->id }}/posts/{{ $post->id }}/un_pin" class="js-btn-post-un-pin btn-transparent font-14 font-weight-500 text-warning mr-3">{{ trans('lms/update.un_pin') }}</button>
                                @else
                                    <button type="button" data-action="/lms{{ getAdminPanelUrl() }}/forums/{{ $topic->forum_id }}/topics/{{ $topic->id }}/posts/{{ $post->id }}/pin" class="js-btn-post-pin btn-transparent font-14 font-weight-500 text-gray mr-3">{{ trans('lms/update.pin') }}</button>
                                @endif
                            @endif

                            @if(!empty($post))
                                <button type="button" data-id="{{ $post->id }}" class="js-reply-post-btn btn-transparent mr-3 font-14 font-weight-500 text-gray">{{ trans('lms/panel.reply') }}</button>
                            @endif
                        @endif
