    ...
    <section class="section">
        <div class="section-header">
            <div class="flex-grow-1">
                ...
                <span class="d-block font-14 font-weight-500 text-gray mt-1">{{ trans('lms/public.by') }} <span class="font-weight-bold">{{ $topic->creator->full_name }}</span> {{ trans('lms/public.in') }} {{ dateTimeFormat($topic->created_at, 'j M Y | H:i') }}</span>
            </div>
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a href="/lms{{ getAdminPanelUrl() }}/forums/{{ $topic->forum_id }}/topics">{{ trans('lms/update.topics') }}</a></div>
                <div class="breadcrumb-item">{{ trans('lms/site.posts') }}</div>
            </div>
        </div>
                ...
                <div class="col-12 ">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @include('lms.admin.forums.topics.post_card')

                            {{-- Topic Posts --}}
                            @if(!empty($topic->posts) and count($topic->posts))
                                @foreach($topic->posts as $postRow)
                                    @include('lms.admin.forums.topics.post_card',['post' => $postRow])
                                @endforeach
                            @endif

                            <div class="mt-4">
                                <h3 class="font-16 font-weight-bold text-dark">{{ trans('lms/update.reply_to_the_topic') }}</h3>

                                <div class="p-2 rounded-lg border bg-white mt-2">
                                    <form action="/lms{{ getAdminPanelUrl() }}/forums/{{ $topic->forum_id }}/topics/{{ $topic->id }}/posts" method="post">
                                        {{ csrf_field() }}
                            ...
                            <div class="mt-4">
                                <h3 class="font-16 font-weight-bold">{{ trans('lms/update.topic_actions') }}</h3>

                                <div class=" mt-2">
                                    <form action="/lms{{ getAdminPanelUrl() }}/forums/{{ $topic->forum_id }}/topics/{{ $topic->id }}/closeToggle" method="post">
                                        {{ csrf_field() }}
                                            ...
                                            <div class="col-12 col-md-3">
                                                <div class="form-group custom-switches-stacked">
                                                    <label class="custom-switch pl-0 d-flex align-items-center">
                                                        ...
                                                        <label class="custom-switch-description mb-0 cursor-pointer" for="forumCloseSwitch">{{ trans('lms/admin/main.close') }}</label>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
