<div class="gift-webinar-card bg-white">
    <figure>
        <div class="image-box">
            <a href="/lms{{ $bundle->getUrl() }}">
                <img src="{{get_path_lms()}}{{ $bundle->getImage() }}" class="img-cover" alt="{{ $bundle->title }}">
            </a>
        </div>
        ...
        <figcaption class="mt-10">
            <div class="user-inline-avatar d-flex align-items-center">
                ...
                <a href="/lms{{ $bundle->teacher->getProfileUrl() }}" target="_blank" class="user-name ml-5 font-14">{{ $bundle->teacher->full_name }}</a>
            </div>

            <a href="/lms{{ $bundle->getUrl() }}">
                <h3 class="mt-15 webinar-title font-weight-bold font-16 text-dark-blue">{{ ($bundle->title) }}</h3>
            </a>
