    ...
    <div class="mt-25 pt-25 border-top border-gray300">
        ...
        <div class="pt-10">
            @foreach(['supported_courses', 'quiz_included', 'certificate_included'] as $moreOption)
                <div class="d-flex align-items-center justify-content-between mt-20">
                    <label class="cursor-pointer" for="filterLanguage{{ $moreOption }}">{{ trans('lms/update.show_only_'.$moreOption) }}</label>
                    <div class="custom-control custom-checkbox">
            ...
            <div class="pt-10">
                @foreach($categoriesLists as $categoryItem)
                    @if(!empty($categoryItem->subCategories) and count($categoryItem->subCategories))
                        ...
                        <div class="pl-10">
                            @foreach($categoryItem->subCategories as $subCategory)
                                ...
                                <a href="/lms/upcoming_courses?{{ http_build_query($urlParams) }}" class="d-flex align-items-center font-14 font-weight-normal mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $subCategory->id) ? 'text-primary' : '' }}">
                    ...
                    @else
                        ...
                        <a href="/lms/upcoming_courses?{{ http_build_query($urlParams) }}" class="d-flex align-items-center font-14 font-weight-bold mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $categoryItem->id) ? 'text-primary' : '' }}">
