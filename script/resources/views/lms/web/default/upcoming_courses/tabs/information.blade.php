@if(!empty($upcomingCourse->webinar_id))
    <div class="d-flex align-items-center mt-20 p-15 success-transparent-alert">
        ...
        <div class="ml-10">
            <div class="font-14 font-weight-bold ">{{ trans('lms/update.course_published') }}</div>
            <div class="font-12 ">{{ trans('lms/update.this_course_was_published_already_and_you_can_check_the_main_course') }}</div>
        </div>
    </div>

    @include('lms.web.default.includes.webinar.list-card',['webinar' => $upcomingCourse->webinar])
@endif
...
@if($upcomingCourse->description)
    <div class="mt-20">
        <h2 class="section-title after-line">{{ trans('lms/update.course_description') }}</h2>
