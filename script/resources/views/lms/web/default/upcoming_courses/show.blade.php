...
@section('content')
    <section class="course-cover-container bg-gray200">
        <img src="{{get_path_lms()}}{{ $upcomingCourse->getImageCover() }}" class="img-cover course-cover-img" alt="{{ $upcomingCourse->title }}"/>
    </section>
        ...
        <div class="row">
                ...
                <div class="course-content-body user-select-none">
                    <div class="upcoming-course-body-on-cover d-flex flex-column text-white pb-15">
                        ...
                        @if(!empty($upcomingCourse->category))
                            <span class="d-block font-16 mt-10">{{ trans('lms/public.in') }} <a href="/lms{{ $upcomingCourse->category->getUrl() }}" target="_blank" class="font-weight-500 text-decoration-underline text-white">{{ $upcomingCourse->category->title }}</a></span>
                        @endif

                        <div class="mt-15">
                            <span class="font-14">{{ trans('lms/public.created_by') }}</span>
                            <a href="/lms{{ $upcomingCourse->teacher->getProfileUrl() }}" target="_blank" class="text-decoration-underline text-white font-14 font-weight-500">{{ $upcomingCourse->teacher->full_name }}</a>
                        </div>
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    ...
                    <div class="px-20 pb-30">
                        ...
                        @if(!empty($upcomingCourse->webinar_id))
                            <a href="/lms{{ $upcomingCourse->webinar->getUrl() }}" class="btn btn-primary btn-block mt-20">{{ trans('lms/update.view_course') }}</a>
                        @else
                            @if(!empty($authUser))
                                <button type="button" class="js-follow-upcoming-course btn btn-primary btn-block mt-20" data-path="/upcoming_courses/{{ $upcomingCourse->slug }}/toggleFollow">
                                    {{ $followed ? trans('lms/update.unfollow_course') : trans('lms/update.follow_course') }}
                                </button>
                            @else
                                <a href="/lms/login" class="btn btn-primary btn-block mt-20">{{ trans('lms/update.follow_course') }}</a>
                            @endif
                        @endif

                        <div class="mt-20 d-flex align-items-center justify-content-center text-gray">
                            <i data-feather="bell" width="20" height="20"></i>
                            <span class="ml-5 font-14">{{ trans('lms/update.youll_get_notified_about_course_publish') }}</span>
                        </div>

                        <div class="mt-35">
                            <strong class="d-block text-secondary font-weight-bold">{{ trans('lms/update.this_course_includes') }}:</strong>
                        ...
                        <div class="mt-40 p-10 rounded-sm border row align-items-center favorites-share-box">
                            @if(!empty($upcomingCourse->publish_date))
                                <div class="col">
                                    <a href="/lms{{ $upcomingCourse->addToCalendarLink() }}" target="_blank" class="d-flex flex-column align-items-center text-center text-gray">
                            ...
                            <div class="col">
                                <a href="/lms/upcoming_courses/{{ $upcomingCourse->slug }}/favorite" id="favoriteToggle" class="d-flex flex-column align-items-center text-gray">
                        ...
                        <div class="mt-30 text-center">
                            <button type="button" id="webinarReportBtn" class="font-14 text-gray btn-transparent">{{ trans('lms/update.report_this_course') }}</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-30">
                        ...
                        @if(!empty($upcomingCourse->duration))
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                ...
                                <span class="font-14">{{ convertMinutesToHourAndMinute($upcomingCourse->duration) }} {{ trans('lms/home.hours') }}</span>
                            </div>
                        @endif
                        ...
                        <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                            ...
                            <span class="font-14">{{ (!empty($upcomingCourse->price) and $upcomingCourse->price > 0) ? handlePrice($upcomingCourse->price) : trans('lms/public.free') }}</span>
                        </div>
                ...
                @if($upcomingCourse->creator_id != $upcomingCourse->teacher_id)
                    @include('lms.web.default.course.sidebar_instructor_profile', ['courseTeacher' => $upcomingCourse->creator])
                @endif
                {{-- teacher --}}
                @include('lms.web.default.course.sidebar_instructor_profile', ['courseTeacher' => $upcomingCourse->teacher])
                {{-- ./ teacher --}}
    ...
    <div id="webinarReportModal" class="d-none">
        ...
        <form action="/lms/upcoming_courses/{{ $upcomingCourse->id }}/report" method="post" class="mt-25">
    ...
    @include('lms.web.default.course.share_modal',['course' => $upcomingCourse])
@endsection
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/upcoming_course_show.min.js"></script>
@endpush
