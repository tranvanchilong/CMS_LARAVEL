...
@if((!empty($educations) and !$educations->isEmpty()) or (!empty($experiences) and !$experiences->isEmpty()) or (!empty($occupations) and !$occupations->isEmpty()) or !empty($user->about))
    @if(!empty($educations) and !$educations->isEmpty())
        <div class="mt-40">
            <h3 class="font-16 text-dark-blue font-weight-bold">{{ trans('lms/site.education') }}</h3>
    ...
    @if(!empty($experiences) and !$experiences->isEmpty())
        <div class="mt-40">
            <h3 class="font-16 text-dark-blue font-weight-bold">{{ trans('lms/site.experiences') }}</h3>
    ...
    @if(!empty($occupations) and !$occupations->isEmpty())
        <div class="mt-40">
            <h3 class="font-16 text-dark-blue font-weight-bold">{{ trans('lms/site.occupations') }}</h3>
...
@else
    ...
    @include('lms.' . getTemplate() . '.includes.no-result',[
        'file_name' => 'bio.png',
        'title' => trans('lms/site.not_create_bio'),
        'hint' => '',
    ])
