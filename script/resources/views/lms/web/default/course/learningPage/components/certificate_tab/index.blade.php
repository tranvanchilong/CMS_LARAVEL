        ...
        <div class="course-certificate-item cursor-pointer p-10 border border-gray200 rounded-sm mb-15" data-course-certificate="{{ !empty($courseCertificate) ? $courseCertificate->id : '' }}">
            <div class="d-flex align-items-center">
                ...
                <div class="flex-grow-1">
                    <span class="font-weight-500 font-14 text-dark-blue d-block">{{ trans('lms/update.course_certificate') }}</span>

                    <div class="d-flex align-items-center">
                        @if(!empty($courseCertificate))
                            <span class="font-12 text-gray">{{ trans("lms/public.date") }}: {{ dateTimeFormat($courseCertificate->created_at, 'j F Y') }}</span>
                        @else
                            <span class="font-12 text-gray">{{ trans("lms/update.not_achieve") }}</span>
                        @endif
                    </div>
                </div>
    ...
    @if(!$hasCertificateItem)
        <div class="learning-page-forum-empty d-flex align-items-center justify-content-center flex-column">
            <div class="learning-page-forum-empty-icon d-flex align-items-center justify-content-center">
                <img src="/assets/lms/assets/default/img/learning/certificate-empty.svg" class="img-fluid" alt="">
            </div>

            <div class="d-flex align-items-center flex-column mt-10 text-center">
                <h3 class="font-20 font-weight-bold text-dark-blue text-center">{{ trans('lms/update.learning_page_empty_certificate_title') }}</h3>
                <p class="font-14 font-weight-500 text-gray mt-5 text-center">{{ trans('lms/update.learning_page_empty_certificate_hint') }}</p>
            </div>
        </div>
    @endif
</div>
