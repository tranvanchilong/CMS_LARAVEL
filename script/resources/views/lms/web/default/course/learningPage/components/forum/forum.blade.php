    ...
    <div class="course-forum-top-stats d-flex flex-wrap flex-md-nowrap align-items-center justify-content-around">
        <div class="d-flex align-items-center justify-content-center pb-5 pb-md-0">
            <div class="d-flex flex-column align-items-center text-center">
                <img src="/assets/lms/assets/default/img/activity/47.svg" class="course-forum-top-stats__icon" alt="">
                <strong class="font-20 text-dark-blue font-weight-bold mt-5">{{ $questionsCount }}</strong>
                <span class="font-14 text-gray font-weight-500">{{ trans('lms/public.questions') }}</span>
            </div>
        </div>

        <div class="d-flex align-items-center justify-content-center pb-5 pb-md-0">
            <div class="d-flex flex-column align-items-center text-center">
                <img src="/assets/lms/assets/default/img/activity/120.svg" class="course-forum-top-stats__icon" alt="">
                <strong class="font-20 text-dark-blue font-weight-bold mt-5">{{ $resolvedCount }}</strong>
                <span class="font-14 text-gray font-weight-500">{{ trans('lms/update.resolved') }}</span>
            </div>
        </div>

        <div class="d-flex align-items-center justify-content-center pb-5 pb-md-0">
            <div class="d-flex flex-column align-items-center text-center">
                <img src="/assets/lms/assets/default/img/activity/119.svg" class="course-forum-top-stats__icon" alt="">
                <strong class="font-20 text-dark-blue font-weight-bold mt-5">{{ $openQuestionsCount }}</strong>
                <span class="font-14 text-gray font-weight-500">{{ trans('lms/update.open_questions') }}</span>
            </div>
        </div>

        <div class="d-flex align-items-center justify-content-center pb-5 pb-md-0">
            <div class="d-flex flex-column align-items-center text-center">
                <img src="/assets/lms/assets/default/img/activity/39.svg" class="course-forum-top-stats__icon" alt="">
                <strong class="font-20 text-dark-blue font-weight-bold mt-5">{{ $commentsCount }}</strong>
                <span class="font-14 text-gray font-weight-500">{{ trans('lms/update.answers') }}</span>
            </div>
        </div>

        <div class="d-flex align-items-center justify-content-center pb-5 pb-md-0">
            <div class="d-flex flex-column align-items-center text-center">
                <img src="/assets/lms/assets/default/img/activity/49.svg" class="course-forum-top-stats__icon" alt="">
                <strong class="font-20 text-dark-blue font-weight-bold mt-5">{{ $activeUsersCount }}</strong>
                <span class="font-14 text-gray font-weight-500">{{ trans('lms/update.active_users') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid p-15 rounded-lg bg-info-light font-14 text-gray mt-20">
        <div class="row align-items-center">
            <div class="col-12 col-lg-4">
                <div class="">
                    <h3 class="font-16 font-weight-bold text-dark-blue">{{ trans('lms/update.course_forum') }}</h3>
                    <span class="d-block font-14 font-weight-500 text-gray mt-1">{{ trans('lms/update.communicate_others_and_ask_your_questions') }}</span>
                </div>
            </div>
            <div class="col-12 col-lg-5 mt-15 mt-lg-0">
                <form action="/lms{{ request()->url() }}" method="get">
                    <div class="d-flex align-items-center">
                        <input type="text" name="search" class="form-control flex-grow-1" value="{{ request()->get('search') }}" placeholder="{{ trans('lms/update.search_in_this_forum') }}">
            ...
            <div class="col-12 col-lg-3 mt-15 mt-lg-0 text-right">
                <button type="button" id="askNewQuestion" class="btn btn-primary btn-sm course-forum-search-btn">
                    ...
                    <span class="ml-1">{{ trans('lms/update.ask_new_question') }}</span>
                </button>
            </div>
        </div>
    </div>
            ...
            <div class="row">
                <div class="col-12 col-lg-6">
                    <div class="d-flex align-items-start">
                        ...
                        <div class="ml-10">
                            <a href="/lms{{ $course->getForumPageUrl() }}/{{ $forum->id }}/answers" class="">
                                <h4 class="font-16 font-weight-bold text-dark-blue">{{ $forum->title }}</h4>
                            </a>

                            <span class="d-block font-12 text-gray mt-5">{{ trans('lms/public.by') }} {{ $forum->user->full_name }} {{ trans('lms/public.in') }} {{ dateTimeFormat($forum->created_at, 'j M Y | H:i') }}</span>
                ...
                <div class="col-12 col-lg-6 mt-15 mt-lg-0 border-left">
                    @if($course->isOwner($user->id))
                        <button type="button" data-action="/lms{{ $course->getForumPageUrl() }}/{{ $forum->id }}/pinToggle" class="question-forum-pin-btn d-flex align-items-center justify-content-center">
                            <img src="/assets/lms/assets/default/img/learning/{{ $forum->pin ? 'un_pin' : 'pin' }}.svg" alt="pin icon" class="">
                        </button>
                    @endif
                    ...
                    @if(!empty($forum->answers) and count($forum->answers))
                        <div class="py-15 row">
                            <div class="col-3">
                                <span class="d-block font-12 text-gray">{{ trans('lms/public.answers') }}</span>
                                <span class="d-block font-14 text-dark mt-10">{{ $forum->answer_count }}</span>
                            </div>

                            <div class="col-3">
                                <span class="d-block font-12 text-gray">{{ trans('lms/panel.users') }}</span>
                            ...
                            <div class="col-6 position-relative">
                                <span class="d-block font-12 text-gray">{{ trans('lms/update.last_activity') }}</span>
                        ...
                        <div class="py-15 border-top position-relative">
                            <span class="d-block font-12 text-gray">{{ trans('lms/update.last_answer') }}</span>
                            ...
                            <span class="badge-icon d-flex align-items-center justify-content-center">
                                <i data-feather="check" width="20" height="20"></i>
                                    ...
                                    {{ trans('lms/update.resolved') }}
                                </div>
                            @endif
                        </div>
                    @else
                        <div class="d-flex flex-column justify-content-center text-center py-15 h-100">
                            <p class="text-gray font-14 font-weight-bold">{{ trans('lms/update.be_the_first_to_answer_this_question') }}</p>

                            <div class="">
                                <a href="/lms{{ $course->getForumPageUrl() }}/{{ $forum->id }}/answers" class="btn btn-primary btn-sm mt-15">{{ trans('lms/public.answer') }}</a>
                            </div>
                        </div>
                    @endif
                </div>
...
@else
    <div class="learning-page-forum-empty d-flex align-items-center justify-content-center flex-column">
        ...
        <div class="d-flex align-items-center flex-column mt-10 text-center">
            ...
            <p class="font-14 font-weight-500 text-gray mt-5 text-center">{{ trans('lms/update.learning_page_empty_content_title_hint') }}</p>
        </div>
    </div>
@endif

@include('lms.web.default.course.learningPage.components.forum.ask_question_modal')
