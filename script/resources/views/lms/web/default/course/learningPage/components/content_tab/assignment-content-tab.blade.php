...
>
    ...
    <div>
        <div class="">
            ...
            @if(empty($itemHistory) or ($itemHistory->status == \App\Models\LMS\WebinarAssignmentHistory::$notSubmitted))
                <span class="text-danger font-12 d-block">{{ trans('lms/update.assignment_history_status_not_submitted') }}</span>
            @else
                @switch($itemHistory->status)
                    @case(\App\Models\LMS\WebinarAssignmentHistory::$passed)
                        <span class="text-primary font-12 d-block">{{ trans('lms/quiz.passed') }}</span>
                        @break
                    @case(\App\Models\LMS\WebinarAssignmentHistory::$pending)
                        <span class="text-warning font-12 d-block">{{ trans('lms/public.pending') }}</span>
                        @break
                    @case(\App\Models\LMS\WebinarAssignmentHistory::$notPassed)
                        <span class="font-12 d-block text-danger">{{ trans('lms/quiz.failed') }}</span>
                        @break
                @endswitch
            @endif
        </div>
        ...
        <div class="tab-item-info mt-15">
            ...
            <div class="d-block mt-10 font-12 text-gray">
                <span class="">{{ trans('lms/update.deadline') }}: </span>
                @if(is_bool($itemDeadline))
                    @if(!$itemDeadline)
                        <span class="text-danger">{{ trans('lms/panel.expired') }}</span>
                    @else
                        <span>{{ trans('lms/update.unlimited') }}</span>
                    @endif
