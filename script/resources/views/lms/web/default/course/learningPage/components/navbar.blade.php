...
<div class="learning-page-navbar d-flex align-items-lg-center justify-content-between flex-column flex-lg-row px-15 px-lg-35 py-10">
    <div class="d-flex align-items-lg-center flex-column flex-lg-row flex-grow-1">
        ...
        <div class="learning-page-logo-card d-flex align-items-center justify-content-between justify-content-lg-start">
            ...
            <div class="d-flex align-items-center d-lg-none ml-20">
                <a href="/lms{{ $course->getUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200 d-none d-md-block">{{ trans('lms/update.course_page') }}</a>

                <a href="/lms/panel/webinars/purchases" class="btn learning-page-navbar-btn btn-sm border-gray200 ml-0 ml-md-10">{{ trans('lms/update.my_courses') }}</a>
            </div>
        </div>

        <div class="learning-page-progress-card d-flex flex-column">
            <a href="/lms{{ $course->getUrl() }}" class="learning-page-navbar-title">
            ...
            <div class="d-flex align-items-center">
                ...
                <span class="ml-10 font-weight-500 font-14 text-gray">{{ $percent }}% {{ trans('lms/update.learnt') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex align-items-center mt-5 mt-md-0">
        ...
        @if(!empty($course->noticeboards_count) and $course->noticeboards_count > 0)
            <a href="/lms{{ $course->getNoticeboardsPageUrl() }}" target="_blank" class="btn learning-page-navbar-btn noticeboard-btn btn-sm border-gray200 mr-10">
        ...
        @if($course->forum)
            <a href="/lms{{ $course->getForumPageUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200 mr-10">{{ trans('lms/update.course_forum') }}</a>
        @endif

        <div class="d-none align-items-center d-lg-flex">
            <a href="/lms{{ $course->getUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200">{{ trans('lms/update.course_page') }}</a>

            <a href="/lms/panel/webinars/purchases" class="btn learning-page-navbar-btn btn-sm border-gray200 ml-10">{{ trans('lms/update.my_courses') }}</a>
        </div>
