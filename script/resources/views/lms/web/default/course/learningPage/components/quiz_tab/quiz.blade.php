        ...
        <div class="flex-grow-1">
            ...
            <div class="d-flex align-items-center justify-content-between">
                <span class="font-12 text-gray d-block">
                    @if(!empty($item->time))
                        {{ $item->time .' '. trans('lms/public.min') }}
                    @else
                        {{ trans('lms/update.unlimited_time') }}
                    @endif

                    {{ ($item->quizQuestions ? ' | ' . (($item->display_limited_questions and !empty($item->display_number_of_questions)) ? $item->display_number_of_questions : $item->quizQuestions->count()) .' '. trans('lms/public.questions') : '') }}
                </span>

                @if(!empty($quiz->result_status))
                    @if($quiz->result_status == 'passed')
                        <span class="font-12 text-primary">{{ trans('lms/quiz.passed') }}</span>
                    @elseif($quiz->result_status == 'failed')
                        <span class="font-12 text-danger">{{ trans('lms/quiz.failed') }}</span>
                    @elseif($quiz->result_status == 'waiting')
                        <span class="font-12 text-warning">{{ trans('lms/quiz.waiting') }}</span>
                    @endif
                @endif
            </div>
        </div>
