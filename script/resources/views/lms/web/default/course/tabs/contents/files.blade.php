        ...
        <div class="panel-collapse">
            ...
            @if(!empty($user) and $hasBought)
                <div class="d-flex align-items-center mt-20">
                    <label class="mb-0 mr-10 cursor-pointer font-weight-500" for="fileReadToggle{{ $file->id }}">{{ trans('lms/public.i_passed_this_lesson') }}</label>
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                ...
                <div class="">
                    @if(!empty($checkSequenceContent) and $sequenceContentHasError)
                        ...
                        >{{ trans('lms/public.play') }}</button>
                    @elseif($file->accessibility == 'paid')
                        @if(!empty($user) and $hasBought)
                            @if($file->downloadable)
                                <a href="/lms{{ $course->getUrl() }}/file/{{ $file->id }}/download" class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('lms/home.download') }}
                                </a>
                            @else
                                <a href="/lms{{ $course->getLearningPageUrl() }}?type=file&item={{ $file->id }}" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('lms/public.play') }}
                                </a>
                            @endif
                        @else
                    ...
                    @else
                        ...
                        @else
                            ...
                            @elseif($file->storage == 'upload_archive')
                                <a href="/lms/course/{{ $course->slug }}/file/{{ $file->id }}/showHtml" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('lms/public.play') }}
                                </a>
                            @elseif(in_array($file->storage, ['iframe', 'google_drive', 'dropbox']))
                                <a href="/lms/course/{{ $course->slug }}/file/{{ $file->id }}/play" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('lms/public.play') }}
                                </a>
                            @elseif($file->isVideo())
                            ...
                            @else
                                <a href="/lms{{ $file->file }}" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('lms/public.play') }}
                                </a>
                            @endif
                        @endif
