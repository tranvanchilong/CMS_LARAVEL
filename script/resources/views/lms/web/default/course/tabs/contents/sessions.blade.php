    ...
    <div class="d-flex align-items-center justify-content-between" role="tab" id="session_{{ $session->id }}">
        <div class="d-flex align-items-center" href="#collapseSession{{ $session->id }}" aria-controls="collapseSession{{ $session->id }}" data-parent="#{{ $accordionParent }}" role="button" data-toggle="collapse" aria-expanded="true">
            @if($session->date > time())
                <a href="/lms{{ $session->addToCalendarLink() }}" target="_blank" class="mr-15 d-flex" data-toggle="tooltip" data-placement="top" title="{{ trans('lms/public.add_to_calendar') }}">
        ...
        <div class="panel-collapse">
            ...
            @if(!empty($user) and $hasBought)
                <div class="d-flex align-items-center mt-20">
                    <label class="mb-0 mr-10 cursor-pointer font-weight-500" for="sessionReadToggle{{ $session->id }}">{{ trans('lms/public.i_passed_this_lesson') }}</label>
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                <div class="d-flex align-items-center">
                    <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                        ...
                        <span class="line-height-1">{{ convertMinutesToHourAndMinute($session->duration) }} {{ trans('lms/home.hours') }}</span>
                    </div>
                ...
                <div class="">
                    @if($session->isFinished())
                        <button type="button" class="course-content-btns btn btn-sm btn-gray disabled flex-grow-1 disabled session-finished-toast">{{ trans('lms/public.finished') }}</button>
                    @elseif(empty($user))
                        <button type="button" class="course-content-btns btn btn-sm btn-gray disabled flex-grow-1 disabled not-login-toast">{{ trans('lms/public.go_to_class') }}</button>
                    @elseif($hasBought)
                        @if(!empty($checkSequenceContent) and $sequenceContentHasError)
                            ...
                            >{{ trans('lms/public.go_to_class') }}</button>
                        @else
                            <a href="/lms{{ $course->getLearningPageUrl() }}?type=session&item={{ $session->id }}" target="_blank" class="course-content-btns btn btn-sm btn-primary flex-grow-1">{{ trans('lms/public.go_to_class') }}</a>
                        @endif
                    @else
                        <button type="button" class="course-content-btns btn btn-sm btn-gray flex-grow-1 disabled not-access-toast">{{ trans('lms/public.go_to_class') }}</button>
                    @endif
                </div>
            </div>
        </div>
