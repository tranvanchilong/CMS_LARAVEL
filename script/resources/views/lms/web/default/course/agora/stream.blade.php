    ...
    @if($notStarted)
        <div id="notStartedAlert" class="no-result default-no-result d-flex align-items-center justify-content-center flex-column w-100 h-100">
            <div class="no-result-logo">
                <img src="/assets/lms/assets/default/img/no-results/support.png" alt="">
            </div>
            <div class="d-flex align-items-center flex-column mt-30 text-center">
                <h2 class="text-dark-blue">{{ trans('lms/update.this_live_has_not_started_yet') }}</h2>
                <p class="mt-5 text-center text-gray font-weight-500">{{ trans('lms/update.this_live_has_not_started_yet_hint') }}</p>
            </div>
        </div>
    @else
        <div class="agora-stream-loading">
            <img src="/assets/lms/assets/default/img/loading.gif" alt="">
            <p class="mt-10">{{ trans('lms/update.wait_to_join_the_channel') }}</p>
        </div>
    @endif
...
<div class="stream-footer py-20 px-15 px-lg-30 mt-15 d-flex align-items-center justify-content-around bg-white">
    ...
    @if($sessionStreamType == 'multiple')
        <button type="button" id="microphoneEffect" class="stream-bottom-actions btn-transparent d-flex flex-column align-items-center active">
            ...
            <span class="mt-1 text-gray font-14">{{ trans('lms/update.microphone') }}</span>
        </button>
        ...
        <button type="button" id="cameraEffect" class="stream-bottom-actions btn-transparent d-flex flex-column align-items-center active">
            ...
            <span class="mt-1 text-gray font-14">{{ trans('lms/update.camera') }}</span>
        </button>
    @endif
    ...
    @if($isHost)
        <button type="button" id="shareScreen" class="stream-bottom-actions btn-transparent d-flex flex-column align-items-center ">
            ...
            <span class="mt-1 text-gray font-14">{{ trans('lms/update.share_screen') }}</span>
        </button>
        ...
        <button type="button" id="endShareScreen" class="stream-bottom-actions btn-transparent flex-column align-items-center dont-join-users d-none">
            ...
            <span class="mt-1 text-gray font-14">{{ trans('lms/update.end_share_screen') }}</span>
        </button>
        ...
        <button type="button" id="handleUsersJoin" class="stream-bottom-actions btn-transparent d-flex flex-column align-items-center {{ (!empty($session->agora_settings) and !empty($session->agora_settings->users_join) and $session->agora_settings->users_join) ? '' : 'dont-join-users' }}">
            ...
            <span class="mt-1 text-gray font-14">{{ (!empty($session->agora_settings) and !empty($session->agora_settings->users_join) and $session->agora_settings->users_join) ? trans('lms/update.join_is_active') : trans('lms/update.joining_is_disabled') }}</span>
        </button>
        ...
        <button type="button" class="stream-bottom-actions btn-transparent d-flex flex-column align-items-center text-danger" data-toggle="modal" data-target="#leaveModal">
            ...
            <span class="mt-1 font-14">{{ trans('lms/update.end_live') }}</span>
        </button>
            ...
            <div class="modal-dialog modal-sm">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="leaveModalLabel">{{ trans('lms/update.end_live') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body ">
                        <p class="">{{ trans('lms/update.end_live_confirm') }}</p>

                        <div class="mt-30 text-center">
                            <button type="button" class="btn btn-danger btn-sm" id="leave" data-id="{{ $session->id }}">{{ trans('lms/admin/main.yes') }}</button>
                            <button type="button" class="btn ml-10 btn-gray btn-sm" data-dismiss="modal">{{ trans('lms/admin/main.close') }}</button>
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts_bottom')
    <script>
        ...
        var joinIsActiveLang = '{{ trans('lms/update.join_is_active') }}';
        var joiningIsDisabledLang = '{{ trans('lms/update.joining_is_disabled') }}';
    ...
    <script src="/assets/lms/assets/vendors/agora/AgoraRTC_N.js"></script>
    <script src="/assets/lms/assets/default/agora/stream.min.js"></script>
@endpush
