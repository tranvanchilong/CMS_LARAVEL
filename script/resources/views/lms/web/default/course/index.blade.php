...
@section('content')
    <section class="course-cover-container {{ empty($activeSpecialOffer) ? 'not-active-special-offer' : '' }}">
        <img src="{{get_path_lms()}}{{ $course->getImageCover() }}" class="img-cover course-cover-img" alt="{{ $course->title }}"/>
                ...
                <div class="course-content-body user-select-none">
                    <div class="course-body-on-cover text-white">
                        ...
                        @if(!empty($course->category))
                            <span class="d-block font-16 mt-10">{{ trans('lms/public.in') }} <a href="/lms{{ $course->category->getUrl() }}" target="_blank" class="font-weight-500 text-decoration-underline text-white">{{ $course->category->title }}</a></span>
                        @endif

                        <div class="d-flex align-items-center">
                            @include('lms.web.default.includes.webinar.rate',['rate' => $course->getRate()])
                            <span class="ml-10 mt-15 font-14">({{ $course->reviews->pluck('creator_id')->count() }} {{ trans('lms/public.ratings') }})</span>
                        </div>
                            ...
                            <div class="mt-30 d-flex align-items-center">
                                ...
                                <span class="ml-15 font-14 font-weight-500">
                                    @if($hasBought and (!$course->isWebinar() or $course->isProgressing()))
                                        {{ trans('lms/public.course_learning_passed',['percent' => $percent]) }}
                                    @elseif(!is_null($course->capacity))
                                        {{ $course->sales_count }}/{{ $course->capacity }} {{ trans('lms/quiz.students') }}
                                    @else
                                        {{ trans('lms/public.course_learning_passed',['percent' => $percent]) }}
                                    @endif
                                </span>
                        ...
                        <ul class="nav nav-tabs bg-secondary rounded-sm p-15 d-flex align-items-center justify-content-between" id="tabs-tab" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="position-relative font-14 text-white {{ (request()->get('tab','') == 'content') ? 'active' : '' }}" id="content-tab" data-toggle="tab"
                                   ...
                                   aria-selected="false">{{ trans('lms/product.content') }} ({{ $webinarContentCount }})</a>
                            </li>
                            <li class="nav-item">
                                <a class="position-relative font-14 text-white {{ (request()->get('tab','') == 'reviews') ? 'active' : '' }}" id="reviews-tab" data-toggle="tab"
                                   href="#reviews" role="tab" aria-controls="reviews"
                                   aria-selected="false">{{ trans('lms/product.reviews') }} ({{ $course->reviews->count() > 0 ? $course->reviews->pluck('creator_id')->count() : 0 }})</a>
                            </li>
                        </ul>
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    <div class="course-img {{ $course->video_demo ? 'has-video' :'' }}">
                        ...
                        <img src="{{get_path_lms()}}{{ $course->getImage() }}" class="img-cover" alt="">
                    ...
                    <div class="px-20 pb-30">
                        <form action="/lms/cart/store" method="post">
                                ...
                                <div id="priceBox" class="d-flex align-items-center justify-content-center mt-20 {{ !empty($activeSpecialOffer) ? ' flex-column ' : '' }}">
                                    <div class="text-center">
                                        ...
                                        @if(!empty($realPrice['tax']) and empty($activeSpecialOffer))
                                            <span class="d-block font-14 text-gray">+ {{ $realPrice['tax'] }} {{ trans('lms/cart.tax') }}</span>
                                        @endif
                                    </div>
                                    ...
                                    @if(!empty($activeSpecialOffer))
                                        <div class="text-center">
                                            ...
                                            @if(!empty($priceWithDiscount['tax']))
                                                <span class="d-block font-14 text-gray">+ {{ $priceWithDiscount['tax'] }} {{ trans('lms/cart.tax') }}</span>
                                            @endif
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="mt-20 d-flex flex-column">
                                @if(!$canSale and $course->canJoinToWaitlist())
                                    <button type="button" data-slug="{{ $course->slug }}" class="btn btn-primary {{ (!empty($authUser)) ? 'js-join-waitlist-user' : 'js-join-waitlist-guest' }}">{{ trans('lms/update.join_waitlist') }}</button>
                                @elseif($hasBought or !empty($course->getInstallmentOrder()))
                                    <a href="/lms{{ $course->getLearningPageUrl() }}" class="btn btn-primary">{{ trans('lms/update.go_to_learning_page') }}</a>
                                @elseif($course->price > 0)
                                    ...
                                    @if($canSale and $course->subscribe)
                                        <a href="/lms/subscribes/apply/{{ $course->slug }}" class="btn btn-outline-primary btn-subscribe mt-20 @if(!$canSale) disabled @endif">{{ trans('lms/public.subscribe') }}</a>
                                    @endif
                                    ...
                                    @if($canSale and !empty($course->points))
                                        <a href="/lms{{ !(auth()->guard('lms_user')->check()) ? '/login' : '#' }}" class="{{ (auth()->guard('lms_user')->check()) ? 'js-buy-with-point' : '' }} btn btn-outline-warning mt-20 {{ (!$canSale) ? 'disabled' : '' }}" rel="nofollow">
                                            {!! trans('lms/update.buy_with_n_points',['points' => $course->points]) !!}
                                        </a>
                                    @endif
                                    ...
                                    @if(!empty($installments) and count($installments) and getInstallmentsSettings('display_installment_button'))
                                        <a href="/lms/course/{{ $course->slug }}/installments" class="btn btn-outline-primary mt-20">
                                            {{ trans('lms/update.pay_with_installments') }}
                                        </a>
                                    @endif
                                @else
                                    <a href="/lms{{ $canSale ? '/course/'. $course->slug .'/free' : '#' }}" class="btn btn-primary {{ (!$canSale) ? (' disabled ' . $course->cantSaleStatus($hasBought)) : '' }}">{{ trans('lms/public.enroll_on_webinar') }}</a>
                                @endif
                            </div>
                        ...
                        <div class="mt-35">
                            <strong class="d-block text-secondary font-weight-bold">{{ trans('lms/webinars.this_webinar_includes',['classes' => trans('lms/webinars.'.$course->type)]) }}</strong>
                            ...
                            @if($course->quizzes->where('status', \App\Models\LMS\Quiz::ACTIVE)->count() > 0)
                                <div class="mt-20 d-flex align-items-center text-gray">
                                    <i data-feather="file-text" width="20" height="20"></i>
                                    <span class="ml-5 font-14 font-weight-500">{{ trans('lms/webinars.online_quizzes_count',['quiz_count' => $course->quizzes->where('status', \App\Models\LMS\Quiz::ACTIVE)->count()]) }}</span>
                                </div>
                            @endif
                        ...
                        <div class="mt-40 p-10 rounded-sm border row align-items-center favorites-share-box">
                            ...
                            <div class="col">
                                <a href="/lms/favorites/{{ $course->slug }}/toggle" id="favoriteToggle" class="d-flex flex-column align-items-center text-gray">
                        ...
                        <div class="mt-30 text-center">
                            <button type="button" id="webinarReportBtn" class="font-14 text-gray btn-transparent">{{ trans('lms/webinars.report_this_webinar') }}</button>
                        </div>
                    </div>
                </div>

                {{-- Cashback Alert --}}
                @include('lms.web.default.includes.cashback_alert',['itemPrice' => $course->price])
                ...
                @if($course->canSale() and !empty(getGiftsGeneralSettings('status')) and !empty(getGiftsGeneralSettings('allow_sending_gift_for_courses')))
                    <a href="/lms/gift/course/{{ $course->slug }}" class="d-flex align-items-center mt-30 rounded-lg border p-15">
                        <div class="size-40 d-flex-center rounded-circle bg-gray200">
                            <i data-feather="gift" class="text-gray" width="20" height="20"></i>
                        </div>
                        <div class="ml-5">
                            <h4 class="font-14 font-weight-bold text-gray">{{ trans('lms/update.gift_this_course') }}</h4>
                            <p class="font-12 text-gray">{{ trans('lms/update.gift_this_course_hint') }}</p>
                        </div>
                    </a>
                @endif
                ...
                <div class="rounded-lg shadow-sm mt-35 px-25 py-20">
                    <h3 class="sidebar-title font-16 text-secondary font-weight-bold">{{ trans('lms/webinars.'.$course->type) .' '. trans('lms/webinars.specifications') }}</h3>
                    ...
                    <div class="mt-30">
                        @if($course->isWebinar())
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                <div class="d-flex align-items-center">
                                    <i data-feather="calendar" width="20" height="20"></i>
                                    <span class="ml-5 font-14 font-weight-500">{{ trans('lms/public.start_date') }}:</span>
                                </div>
                        ...
                        <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                            ...
                            @if(!is_null($course->capacity))
                                <span class="font-14">{{ $course->capacity }} {{ trans('lms/quiz.students') }}</span>
                            @else
                                <span class="font-14">{{ trans('lms/update.unlimited') }}</span>
                            @endif
                        </div>
                        ...
                        <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                            ...
                            <span class="font-14">{{ convertMinutesToHourAndMinute(!empty($course->duration) ? $course->duration : 0) }} {{ trans('lms/home.hours') }}</span>
                        </div>
                        ...
                        @if($course->isWebinar())
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="ml-5 font-14 font-weight-500">{{ trans('lms/public.sessions') }}:</span>
                                </div>
                        ...
                        @if($course->isTextCourse())
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                <div class="d-flex align-items-center">
                                    <img src="/assets/lms/assets/default/img/icons/sessions.svg" width="20" alt="">
                                    <span class="ml-5 font-14 font-weight-500">{{ trans('lms/webinars.text_lessons') }}:</span>
                                </div>
                        ...
                        @if($course->isCourse() or $course->isTextCourse())
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                <div class="d-flex align-items-center">
                                    <img src="/assets/lms/assets/default/img/icons/sessions.svg" width="20" alt="">
                                    <span class="ml-5 font-14 font-weight-500">{{ trans('lms/public.files') }}:</span>
                                </div>
                        ...
                        @if(!empty($course->access_days))
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                ...
                                <span class="font-14">{{ $course->access_days }} {{ trans('lms/public.days') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                @if($course->webinarPartnerTeacher->count() > 0)
                    @foreach($course->webinarPartnerTeacher as $webinarPartnerTeacher)
                        @include('lms.web.default.course.sidebar_instructor_profile', ['courseTeacher' => $webinarPartnerTeacher->teacher])
                    @endforeach
                @endif
                {{-- ./ teacher --}}
    ...
    <div id="webinarReportModal" class="d-none">
        <h3 class="section-title after-line font-20 text-dark-blue">{{ trans('lms/product.report_the_course') }}</h3>

        <form action="/lms/course/{{ $course->id }}/report" method="post" class="mt-25">
    ...
    @include('lms.web.default.course.share_modal')
    @include('lms.web.default.course.buy_with_point_modal')
@endsection
...
@push('scripts_bottom')
    ...
    <script>
        ...
        var courseHasBoughtStatusToastTitleLang = '{{ trans('lms/cart.fail_purchase') }}';
        var courseHasBoughtStatusToastMsgLang = '{{ trans('lms/site.you_bought_webinar') }}';
        var courseNotCapacityStatusToastTitleLang = '{{ trans('lms/public.request_failed') }}';
        var courseNotCapacityStatusToastMsgLang = '{{ trans('lms/cart.course_not_capacity') }}';
        var courseHasStartedStatusToastTitleLang = '{{ trans('lms/cart.fail_purchase') }}';
        var courseHasStartedStatusToastMsgLang = '{{ trans('lms/update.class_has_started') }}';
        var joinCourseWaitlistLang = '{{ trans('lms/update.join_course_waitlist') }}';
        var joinCourseWaitlistModalHintLang = "{{ trans('lms/update.join_course_waitlist_modal_hint') }}";
        var joinLang = '{{ trans('lms/footer.join') }}';
        var nameLang = '{{ trans('lms/auth.name') }}';
        var emailLang = '{{ trans('lms/auth.email') }}';
        var phoneLang = '{{ trans('lms/public.phone') }}';
        var captchaLang = '{{ trans('lms/site.captcha') }}';
    </script>
