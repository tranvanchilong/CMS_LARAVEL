<div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
    ...
    @if(!empty($webinarPartnerTeacher))
        <span class="user-select-none px-15 py-10 bg-gray200 off-label text-gray text-white font-12 rounded-sm ml-auto">{{ trans('lms/public.invited') }}</span>
    @endif
    ...
    @include('lms.web.default.includes.webinar.rate',['rate' => $courseTeacher->rates()])
    ...
    <div class="mt-25 d-flex flex-row align-items-center justify-content-center w-100">
        <a href="/lms{{ $courseTeacher->getProfileUrl() }}" target="_blank" class="btn btn-sm btn-primary {{ $hasMeeting ? 'teacher-btn-action' : 'btn-block' }}">{{ trans('lms/public.profile') }}</a>

        @if($hasMeeting)
            <a href="/lms{{ $courseTeacher->getProfileUrl() }}" class="btn btn-sm btn-primary teacher-btn-action ml-15">{{ trans('lms/public.book_a_meeting') }}</a>
        @endif
    </div>
</div>
