            ...
            <div class="row h-100 align-items-center justify-content-center text-center">
                <div class="col-12 col-md-9 col-lg-7">
                    ...
                    <div class="mt-20 font-16 font-weight-500 text-white">
                        <span>{{ trans('lms/public.lesson') }} {{ $textLesson->order }}/{{ count($course->textLessons) }} </span> | <span>{{ trans('lms/public.study_time') }}: {{ $textLesson->study_time }} {{ trans('lms/public.min') }}</span>
                    </div>

                    <div class="mt-20 font-16 font-weight-500 text-white">
                        <span>{{ trans('lms/product.course') }}: <a href="/lms{{ $course->getUrl() }}" class="font-16 font-weight-500 text-white text-decoration-underline">{{ $course->title }}</a></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12 col-lg-8">
                <div class="post-show mt-30">
                    ...
                    <div class="post-img pb-30">
                        <img src="{{get_path_lms()}}{{ url($textLesson->image) }}" alt="{{ $textLesson->title }}"/>
                    </div>
                ...
                <div class="mt-30 row align-items-center">
                    <div class="col-12 col-md-5">
                        @if(auth()->guard('lms_user')->check())
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer font-weight-500" for="readLessonSwitch">{{ trans('lms/public.i_passed_this_lesson') }}</label>
                    ...
                    <div class="col-12 col-md-7 text-right">
                        @if(!empty($course->textLessons) and count($course->textLessons))
                            <a href="/lms{{ (!empty($previousLesson)) ? $course->getUrl() .'/lessons/'. $previousLesson->id .'/read' : '#' }}" class="btn btn-sm {{ (!empty($previousLesson)) ? 'btn-primary' : 'btn-gray disabled' }}">{{ trans('lms/public.previous_lesson') }}</a>

                            @if(!empty($nextLesson))
                                <a href="/lms{{ (!$nextLesson->not_purchased) ? $course->getUrl() .'/lessons/'. $nextLesson->id .'/read' : '#' }}" class="btn btn-sm {{ (!$nextLesson->not_purchased) ? 'btn-primary' : 'btn-gray disabled' }} {{ ($nextLesson->not_purchased) ? 'js-not-purchased' : '' }}">{{ trans('lms/public.next_lesson') }}</a>
                            @endif
                        @endif
                    </div>
                </div>
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
                    ...
                    <span class="mt-5 font-weight-500 text-gray">{{ trans('lms/product.product_designer') }}</span>

                    @include('lms.web.default.includes.webinar.rate',['rate' => $course->teacher->rates()])
                    ...
                    <div class="mt-25 d-flex flex-row align-items-center justify-content-center w-100">
                        <a href="/lms{{ $course->teacher->getProfileUrl() }}" target="_blank" class="btn btn-sm btn-primary teacher-btn-action">{{ trans('lms/public.profile') }}</a>

                        @if(!empty($course->teacher->hasMeeting()))
                            <a href="/lms{{ $course->teacher->getProfileUrl() }}" class="btn btn-sm btn-primary teacher-btn-action ml-15">{{ trans('lms/public.book_a_meeting') }}</a>
                        @else
                            <button type="button" class="btn btn-sm btn-primary disabled teacher-btn-action ml-15">{{ trans('lms/public.book_a_meeting') }}</button>
                        @endif
                    </div>
                </div>
                ...
                @if(!empty($textLesson->attachments) and count($textLesson->attachments))
                    <div class="shadow-sm rounded-lg bg-white px-15 px-md-25 py-20 mt-30">
                        <h3 class="category-filter-title font-16 font-weight-bold text-dark-blue">{{ trans('lms/public.attachments') }}</h3>
                        ...
                        <ul class="p-0 m-0 pt-10">
                            @foreach($textLesson->attachments as $attachment)
                                <li class="mt-10 p-10 rounded bg-info-light font-14 font-weight-500 text-dark-blue d-flex align-items-center justify-content-between text-ellipsis">
                                    ...
                                    <a href="/lms{{ $course->getLearningPageUrl() }}?type=file&item={{ $attachment->file->id }}" target="_blank">
                ...
                @if(!empty($course->textLessons) and count($course->textLessons))
                    <div class="shadow-sm rounded-lg bg-white px-15 px-md-25 py-20 mt-30">
                        <h3 class="category-filter-title font-16 font-weight-bold text-dark-blue">{{ trans('lms/public.course_sessions') }}</h3>

                        <div class="p-0 m-0 pt-10">
                            @foreach($course->textLessons as $lesson)
                                <a href="/lms{{ $course->getUrl() }}/lessons/{{ $lesson->id }}/read"
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/text_lesson.min.js"></script>
@endpush
