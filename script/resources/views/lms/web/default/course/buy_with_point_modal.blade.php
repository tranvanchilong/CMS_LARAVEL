    ...
    @if(!empty($user))
        <div class="text-center">
            ...
            <p class="font-14 font-weight-500 text-gray mt-30">
                <span class="d-block">{{ trans('lms/update.this_course_requires_n_points',['points' => $course->points]) }}</span>
        ...
        <div class="d-flex align-items-center mt-25">
            <a href="/lms{{ ($user->getRewardPoints() >= $course->points) ? '/course/'. $course->slug .'/points/apply' : '#' }}" class="btn btn-sm flex-grow-1 {{ ($user->getRewardPoints() >= $course->points) ? 'btn-primary js-buy-course-with-point' : 'bg-gray300 text-gray disabled' }}">{{ trans('lms/update.buy') }}</a>
