...
<a href="/lms{{ $instructor->getProfileUrl() }}" class="">
    <div class="position-relative d-flex flex-wrap instructor-finder-card border border-gray300 rounded-sm py-25 mt-20">
        ...
        <div class="col-12 col-md-4 mt-10 mt-md-0 pt-10 pt-md-0 instructor-finder-card-right-side position-relative">
            @if(!empty($discount))
                <span class="off-badge badge badge-danger">{{ trans('lms/public.offer',['off' => $discount]) }}</span>
            @endif
            ...
            <div class="d-flex align-items-start">
                @if(!empty($instructor->meeting) and !empty($instructor->meeting->meetingTimes) and count($instructor->meeting->meetingTimes))
                    @if(!empty($price))
                        ...
                        <span class="font-14 font-weight-500 text-gray mt-5">/{{ trans('lms/update.hour') }}</span>
                    @else
                        <span class="font-weight-bold text-primary font-14">{{ trans('lms/public.free') }}</span>
                    @endif
                @else
                    <span class="font-weight-bold text-danger font-12">{{ trans('lms/update.not_available_for_meeting') }}</span>
                @endif
            </div>
            ...
            <div class="d-flex align-items-center mt-20">
                ...
                <span class="font-14 font-weight-500 text-dark-blue ml-10">{{ $instructor->getTotalHoursTutoring() }} {{ trans('lms/update.hours_tutoring') }}</span>
            </div>
            ...
            <div class="d-flex align-items-center flex-wrap mt-15">
                @foreach($instructor->getBadges() as $badge)
                    <div class="mr-15 mt-10 instructor-badge rounded-circle" data-toggle="tooltip" data-placement="bottom" data-html="true" title="{!! (!empty($badge->badge_id) ? nl2br($badge->badge->description) : nl2br($badge->description)) !!}">
                        <img src="{{get_path_lms()}}{{ !empty($badge->badge_id) ? $badge->badge->image : $badge->image }}" class="img-cover rounded-circle" alt="{{ !empty($badge->badge_id) ? $badge->badge->title : $badge->title }}">
                    </div>
                @endforeach
            </div>
        </div>
