...
<div class="wizard-step-1">
    <h3 class="font-20 text-dark font-weight-bold">{{ trans('lms/update.meeting_time') }}</h3>

    <span class="d-block mt-30 text-gray wizard-step-num">
        {{ trans('lms/update.step') }} 4/4
    </span>

    <span class="d-block font-16 font-weight-500 mt-30">{{ trans('lms/update.what_time_is_better_for_the_meeting') }}</span>
    ...
    <div class="mb-30 custom-control custom-checkbox mt-30 full-checkbox w-100">
        ...
        <label class="custom-control-label font-14 w-100" for="date">{{ trans('lms/update.im_flexible') }}</label>
    </div>
        ...
        <div class="mb-30 form-group d-flex align-items-center flex-wrap">
            @foreach($days as $day)
                <div class="wizard-custom-checkbox">
                    ...
                    <label for="{{ $day }}" class="cursor-pointer">{{ trans('lms/panel.'.$day) }}</label>
                </div>
            @endforeach
        </div>
