...
@if(!empty($installments) and count($installments) and getInstallmentsSettings('installment_plans_position') == 'top_of_page')
    @foreach($installments as $installmentRow)
        @include('lms.web.default.installment.card',['installment' => $installmentRow, 'itemPrice' => $bundle->getPrice(), 'itemId' => $bundle->id, 'itemType' => 'bundles'])
    @endforeach
@endif
...
@if($bundle->description)
    <div class="mt-20">
        <h2 class="section-title after-line">{{ trans('lms/update.bundle_description') }}</h2>
