                ...
                @if(!empty($userCarts) and count($userCarts) > 0)
                        ...
                        @foreach($userCarts as $cart)
                            ...
                            @if(!empty($cartItemInfo))
                                <div class="navbar-cart-box d-flex align-items-center">
                                    ...
                                    <a href="/lms{{ $cartItemInfo['itemUrl'] }}" target="_blank" class="navbar-cart-img">
                                    ...
                                    <div class="navbar-cart-info">
                                        ...
                                        <div class="price mt-10">
                                            ...
                                            @if(!empty($cartItemInfo['quantity']))
                                                <span class="font-12 text-warning font-weight-500 ml-10">({{ $cartItemInfo['quantity'] }} {{ trans('lms/update.product') }})</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="navbar-cart-actions">
                        <div class="navbar-cart-total mt-15 border-top d-flex align-items-center justify-content-between">
                            <strong class="total-text">{{ trans('lms/cart.total') }}</strong>
                        ...
                        <a href="/lms/cart/" class="btn btn-sm btn-primary btn-block mt-50 mt-md-15">{{ trans('lms/cart.go_to_cart') }}</a>
                    </div>
                @else
                    <div class="d-flex align-items-center text-center py-50">
                        <i data-feather="shopping-cart" width="20" height="20" class="mr-10"></i>
                        <span class="">{{ trans('lms/cart.your_cart_empty') }}</span>
                    </div>
                @endif
            </div>
        </div>
