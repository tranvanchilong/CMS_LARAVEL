...
@if(!empty($cookieSecuritySettings['cookie_settings_modal_message']) and !empty(strip_tags($cookieSecuritySettings['cookie_settings_modal_message'])))
    <div class="cookie-security-dialog p-20 bg-gray rounded-lg">
        <h3 class="font-14 font-weight-bold text-white">{{ trans('lms/update.your_privacy') }}</h3>
        <p class="mt-5 text-white font-12">{{ trans('lms/update.your_privacy_hint') }}</p>

        <div class="mt-10 d-flex flex-wrap align-items-center">
            <button type="button" class="js-accept-all-cookies btn btn-primary btn-sm flex-grow-1 mr-0 mr-md-5">{{ trans('lms/update.accept_all_cookies') }}</button>
            <button type="button" class="js-cookie-customize-settings btn btn-light btn-sm flex-grow-1 mt-10 mt-md-0">{{ trans('lms/update.customize_settings') }}</button>
        </div>
    </div>

    <div id="cookieSecurityModal" class="d-none">
        <h3 class="section-title after-line font-20 text-dark-blue mb-10">{{ trans('lms/update.cookie_settings') }}</h3>
        ...
        <div class="d-flex flex-wrap align-items-center mt-20 pt-15 border-top">
            <button type="button" class="js-store-customize-cookies d-inline-flex d-md-none btn btn-primary btn-sm">{{ trans('lms/update.confirm') }}</button>
            <button type="button" class="js-store-customize-cookies d-none d-md-inline-flex btn btn-primary btn-sm">{{ trans('lms/update.confirm_my_choices') }}</button>

            <button type="button" class="js-accept-all-cookies d-inline-flex d-md-none btn btn-outline-primary btn-sm mx-15">{{ trans('lms/update.accept_all') }}</button>
            <button type="button" class="js-accept-all-cookies d-none d-md-inline-flex btn btn-outline-primary btn-sm mx-15">{{ trans('lms/update.accept_all_cookies') }}</button>

            <button type="button" class="btn-transparent close-swl ml-auto font-14 text-danger">{{ trans('lms/public.cancel') }}</button>
        </div>
    </div>
    ...
    <script type="text/javascript" src="/assets/lms/assets/default/js/parts/cookie-security.min.js"></script>
@endif
