        ...
        <div class="d-flex flex-column h-100">
            <div class="mb-auto navbar-notification-card" data-simplebar>
                ...
                @if(!empty($unReadNotifications) and count($unReadNotifications))
                    ...
                    <div class="d-flex align-items-center p-15 border rounded-sm">
                        ...
                        <div class="ml-5">
                            <div class="text-secondary font-14"><span class="font-weight-bold">{{ count($unReadNotifications) }}</span> {{ trans('lms/panel.notifications') }}</div>

                            <a href="/lms/panel/notifications/mark-all-as-read" class="delete-action d-block mt-5 font-12 cursor-pointer text-hover-primary" data-title="{{ trans('lms/update.convert_unread_messages_to_read') }}" data-confirm="{{ trans('lms/update.yes_convert') }}">
                                {{ trans('lms/update.mark_all_notifications_as_read') }}
                            </a>
                        </div>
                    </div>

                    @foreach($unReadNotifications as $unReadNotification)
                        <a href="/lms/panel/notifications?notification={{ $unReadNotification->id }}">
                ...
                @else
                    <div class="d-flex align-items-center text-center py-50">
                        <i data-feather="bell" width="20" height="20" class="mr-10"></i>
                        <span class="">{{ trans('lms/notification.empty_notifications') }}</span>
                    </div>
                @endif
            ...
            @if(!empty($unReadNotifications) and count($unReadNotifications))
                <div class="mt-10 navbar-notification-action">
                    <a href="/lms/panel/notifications" class="btn btn-sm btn-danger btn-block">{{ trans('lms/notification.all_notifications') }}</a>
                </div>
            @endif
        </div>
    </div>
