<div class="mt-35">
    <h2 class="section-title after-line">{{ trans('lms/panel.comments') }} <span class="ml-5">({{ $comments->count() }})</span></h2>

    <div class="mt-20">
        <form action="/lms/comments/store" method="post">
            ...
            <button type="submit" class="btn btn-sm btn-primary">{{ trans('lms/product.post_comment') }}</button>
        </form>
    </div>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="btn-group dropdown table-actions">
                            ...
                            <div class="dropdown-menu">
                                <button type="button" class="btn-transparent webinar-actions d-block text-hover-primary reply-comment">{{ trans('lms/panel.reply') }}</button>
                                <button type="button" data-item-id="{{ $inputValue }}" data-comment-id="{{ $comment->id }}" class="btn-transparent webinar-actions d-block mt-10 text-hover-primary report-comment">{{ trans('lms/panel.report') }}</button>

                                @if(auth()->guard('lms_user')->check() and auth()->guard('lms_user')->user()->id == $comment->user_id)
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="btn-group dropdown table-actions">
                                        ...
                                        <div class="dropdown-menu">
                                            <button type="button" class="btn-transparent webinar-actions d-block text-hover-primary reply-comment">{{ trans('lms/panel.reply') }}</button>
                                            <button type="button" data-item-id="{{ $inputValue }}" data-comment-id="{{ $reply->id }}" class="btn-transparent webinar-actions d-block mt-10 text-hover-primary report-comment">{{ trans('lms/panel.report') }}</button>

                                            @if(auth()->guard('lms_user')->check() and auth()->guard('lms_user')->user()->id == $reply->user_id)
                                                <a href="/lms/comments/{{ $reply->id }}/delete" class="webinar-actions d-block mt-10 text-hover-primary">{{ trans('lms/public.delete') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
