...
@section('content')
    <div class="container">
        <section class="topics-title-section mt-30 mt-md-50 px-20 px-md-30 py-25 py-md-35 rounded-lg">
            <h1 class="font-30 font-weight-bold text-white">{{ !empty($topic) ? trans('lms/update.edit_topic') : trans('lms/update.new_topic') }}</h1>
            <p class="font-14 text-white">{{ trans('lms/update.new_topic_hint') }}</p>
            ...
            <div class="mt-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb p-0 m-0">
                        ...
                        <li class="breadcrumb-item font-12 text-white font-weight-bold" aria-current="page">{{ !empty($topic) ? trans('lms/update.edit_topic') : trans('lms/update.new_topic') }}</li>
                    </ol>
                </nav>
            </div>
        </section>

        <form action="/lms{{ !empty($topic) ? $topic->getEditUrl() : '/forums/create-topic' }}" method="post">
            {{ csrf_field() }}
                ...
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/update.topic_title') }}</label>
                            <input type="text" name="title" value="{{ !empty($topic) ? $topic->title : old('title') }}" class="form-control @error('title') is-invalid @enderror" placeholder="{{ trans('lms/update.topic_title_placeholder') }}">
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select name="forum_id" class="form-control @error('forum_id') is-invalid @enderror">
                                <option selected disabled>{{ trans('lms/admin/main.choose_category') }}</option>
                    ...
                    <div class="col-12 col-md-6">
                        <div id="topicImagesInputs" class="create-topic-attachments form-group mt-15">
                            <label class="input-label mb-0">{{ trans('lms/update.attachments') }}</label>
                            ...
                            @if(!empty($topic) and !empty($topic->attachments) and count($topic->attachments))
                                @foreach($topic->attachments as $topicAttachment)
                                    <div class="input-group product-images-input-group mt-10">
                                        ...
                                        <input type="text" name="attachments[]" id="attachments_{{ $topicAttachment->id }}" value="{{ $topicAttachment->path }}" class="form-control" placeholder="{{ trans('lms/update.attachments_size') }}"/>
            ...
            <div class="mt-15 p-10 bg-info-light rounded-lg d-flex align-items-center justify-content-between">
                <div class="py-5">
                    <div class="font-14 font-weight-bold text-gray">{{ trans('lms/update.terms_and_rules_confirmation') }}</div>
                    <p class="d-block font-14 text-gray mt-5">{{ trans('lms/update.terms_and_rules_confirmation_hint') }}</p>
                </div>

                <button type="submit" class="btn btn-primary">
                    <i data-feather="file" class="text-white" width="16" height="16"></i>
                    <span class="ml-1">{{ trans('lms/update.publish_topic') }}</span>
                </button>
            </div>
        </form>
    </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/create_topics.min.js"></script>
@endpush
