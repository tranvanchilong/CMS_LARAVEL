    ...
    <div class="d-flex flex-wrap">
        <div class="col-12 col-md-3">
            <div class="position-relative bg-info-light d-flex flex-column align-items-center justify-content-start rounded-lg w-100 h-100 p-20">
                ...
                <a href="/lms{{ $cardUser->getProfileUrl() }}" target="_blank">
                ...
                <span class="px-10 py-5 mt-5 rounded-lg border bg-info-light text-center font-12 text-gray">
                    ...
                    @elseif($cardUser->isTeacher())
                        {{ trans('lms/public.instructor') }}
                    @elseif($cardUser->isOrganization())
                        {{ trans('lms/home.organization') }}
                    @elseif($cardUser->isAdmin())
                        {{ trans('lms/panel.staff') }}
                    @endif
                        </span>
                    ...
                    <div class="d-flex flex-wrap align-items-center justify-content-center mt-20 w-100">
                        @foreach($cardUserBadges as $badge)
                            <div class="mr-10 mt-10" data-toggle="tooltip" data-placement="bottom" data-html="true" title="{!! (!empty($badge->badge_id) ? nl2br($badge->badge->description) : nl2br($badge->description)) !!}">
                                <img src="{{get_path_lms()}}{{ !empty($badge->badge_id) ? $badge->badge->image : $badge->image }}" width="32" height="32" alt="{{ !empty($badge->badge_id) ? $badge->badge->title : $badge->title }}">
                            </div>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="mt-20 w-100">
                    @if($cardUser->getTopicsPostsCount() > 0)
                        <div class="d-flex align-items-center justify-content-between font-12 text-gray">
                            <span class="">{{ trans('lms/site.posts') }}:</span>
                    ...
                    @if($cardUser->getTopicsPostsLikesCount() > 0)
                        <div class="d-flex align-items-center justify-content-between font-12 text-gray mt-10">
                            <span class="">{{ trans('lms/update.likes') }}:</span>
                ...
                @if(!empty($post) and $post->pin)
                    <span class="pinned-icon d-flex align-items-center justify-content-center">
                        <img src="/assets/lms/assets/default/img/learning/un_pin.svg" alt="pin icon" class="">
                    </span>
                @endif
            </div>
        </div>
                ...
                <div class="d-flex flex-column h-100">
                        ...
                        <div class="post-quotation p-15 rounded-sm border mb-15">
                            <div class="d-flex align-items-center">
                                <div class="post-quotation-icon rounded-circle">
                                    <img src="/assets/lms/assets/default/img/icons/quote-right.svg" class="img-cover" alt="quote-right">
                                </div>
                                <div class="ml-10">
                                    <span class="d-block font-12 text-gray">{{ trans('lms/update.reply_to') }}</span>
                    ...
                    @if(!empty($post) and !empty($post->attach))
                        <div class="mt-auto d-inline-flex">
                            <a href="/lms{{ $post->getAttachmentUrl($forum->slug,$topic->slug) }}" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill">
                    ...
                    @elseif(empty($post) and !empty($topic->attachments) and count($topic->attachments))
                        <div class="mt-auto d-inline-flex align-items-center">
                            @foreach($topic->attachments as $attachment)
                                <a href="/lms{{ $attachment->getDownloadUrl($forum->slug,$topic->slug) }}" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill mr-15">
                                    <i data-feather="paperclip" class="text-gray" width="16" height="16"></i>
                    ...
                    <div class="d-flex align-items-center">
                        @if(!empty($authUser) and !$topic->close)
                            @if($authUser->id == $cardUser->id)
                                @if(!empty($post))
                                    <button type="button" data-action="/lms{{ $post->getEditUrl($forum->slug,$topic->slug) }}" class="js-post-edit btn-transparent mr-25 font-14 font-weight-500 text-gray">{{ trans('lms/public.edit') }}</button>
                                @else
                                    <a href="/lms{{ $topic->getEditUrl($forum->slug) }}" class="mr-25 font-14 font-weight-500 text-gray">{{ trans('lms/public.edit') }}</a>
                                @endif
                            @endif

                            @if(!empty($post) and $authUser->id == $topic->creator_id)
                                @if($post->pin)
                                    <button type="button" data-action="/lms{{ $topic->getPostsUrl() }}/{{ $post->id }}/un_pin" class="js-btn-post-un-pin btn-transparent font-14 font-weight-500 text-warning mr-25">{{ trans('lms/update.un_pin') }}</button>
                                @else
                                    <button type="button" data-action="/lms{{ $topic->getPostsUrl() }}/{{ $post->id }}/pin" class="js-btn-post-pin btn-transparent font-14 font-weight-500 text-gray mr-25">{{ trans('lms/update.pin') }}</button>
                                @endif
                            @endif

                            @if(!empty($post))
                                <button type="button" data-id="{{ $post->id }}" class="js-reply-post-btn btn-transparent mr-25 font-14 font-weight-500 text-gray">{{ trans('lms/panel.reply') }}</button>
                            @endif

                            <button type="button" data-id="{{ !empty($post) ? $post->id : $topic->id }}" data-type="{{ !empty($post) ? 'topic_post' : 'topic' }}" class="js-topic-post-report btn-transparent mr-25 font-14 font-weight-500 text-gray">{{ trans('lms/panel.report') }}</button>
                        @endif

                        <div class="topic-post-like-btn d-flex align-items-center">
                            <button type="button" class="{{ !empty($authUser) ? 'js-topic-post-like' : 'login-to-access' }} badge-icon d-flex align-items-center justify-content-center {{ ((!empty($post) and in_array($post->id,$likedPostsIds)) or (empty($post) and $topic->liked)) ? 'liked' : '' }}" data-action="/lms{{ !empty($post) ? $post->getLikeUrl($forum->slug,$topic->slug) : $topic->getLikeUrl($forum->slug) }}">
