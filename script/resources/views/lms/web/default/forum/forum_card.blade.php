<div class="row align-items-center my-15">
    <div class="col-12 col-md-6">
        <div class="d-flex align-items-center">
            <div class="forums-categories-card__icon p-5">
                <img src="{{get_path_lms()}}{{ $forum->icon }}" alt="{{ $forum->title }}" class="img-cover">
            </div>
            <div class="ml-10">
                <a href="/lms{{ $forum->getUrl() }}" class="d-block">
    ...
    <div class="col-4 col-md-2 mt-10 mt-md-0 d-flex align-items-center justify-content-around">
        <div class="text-center">
            ...
            <div class="d-block font-12 text-gray">{{ trans('lms/update.topics') }}</div>
        </div>

        <div class="text-center">
            <span class="d-block font-14 text-gray font-weight-bold">{{ $forum->posts_count }}</span>
            <div class="d-block font-12 text-gray">{{ trans('lms/site.posts') }}</div>
        </div>
    </div>
    ...
    <div class="col-8 col-md-4 mt-10 mt-md-0 forums-categories-card__last-post d-flex align-items-center">
        @if(!empty($forum->lastTopic))
            ...
            <div class="ml-5">
                <a href="/lms{{ $forum->lastTopic->getPostsUrl() }}" class="d-block">
                    <span class="font-12 font-weight-500 text-gray text-ellipsis">{{ truncate($forum->lastTopic->title,30) }}</span>
                </a>
                <div class="text-gray font-12"><span class="font-weight-bold">{{ $forum->lastTopic->creator->full_name }}</span> {{ trans('lms/public.in') }} {{ dateTimeFormat($forum->lastTopic->created_at,'j M Y | H:i') }}</div>
            </div>
        @endif
    </div>
</div>
