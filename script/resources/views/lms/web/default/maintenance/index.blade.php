            ...
            <div class="d-flex-center flex-column">
                @if(!empty($maintenanceSettings['image']))
                    <div class="maintenance-image">
                        <img src="{{get_path_lms()}}{{ $maintenanceSettings['image'] }}" alt="{{ $maintenanceSettings['title'] }}" class="img-cover">
                    </div>
                @endif
                ...
                @if(!empty($maintenanceSettings['maintenance_button']) and !empty($maintenanceSettings['maintenance_button']['title']) and !empty($maintenanceSettings['maintenance_button']['link']))
                    <a href="/lms{{ $maintenanceSettings['maintenance_button']['link'] }}" class="btn btn-primary mt-20">{{ $maintenanceSettings['maintenance_button']['title'] }}</a>
                @endif
            </div>
        </div>
    </section>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/maintenance.min.js"></script>
@endpush
