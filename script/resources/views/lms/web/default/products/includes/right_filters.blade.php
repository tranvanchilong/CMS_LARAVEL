@if(!empty($authUser) and ($authUser->isOrganization() or $authUser->isTeacher()))
    <a href="/lms/panel/store/products/new" class="mt-20 btn btn-primary btn-flex align-items-center w-100">
        <i data-feather="shopping-bag" width="20" height="20" class="mr-5"></i>
        <span>{{ trans('lms/update.add_new_product') }}</span>
    </a>
@endif
        ...
        <div class="pt-10">
            @foreach(['virtual','physical'] as $typeOption)
                <div class="d-flex align-items-center justify-content-between mt-20">
                    <label class="cursor-pointer" for="filterTypes{{ $typeOption }}">{{ trans('lms/update.product_type_'.$typeOption) }}</label>
...
<div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container">
    <div class="">
        <h3 class="category-filter-title font-20 font-weight-bold text-dark-blue">{{ trans('lms/update.options') }}</h3>

        <div class="pt-10">

            <div class="d-flex align-items-center justify-content-between mt-20">
                <label class="cursor-pointer" for="filterOptionsOnlyAvailableProducts">{{ trans('lms/update.only_available_products') }}</label>
                <div class="custom-control custom-checkbox">
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                <label class="cursor-pointer" for="filterOptionsWithPoint">{{ trans('lms/update.products_with_points') }}</label>
                <div class="custom-control custom-checkbox">
...
@if(!empty($productCategories))
    ...
    <div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container">
        ...
        <div class="">
            <h3 class="category-filter-title font-20 font-weight-bold text-dark-blue">{{ trans('lms/categories.categories') }}</h3>
            ...
            <div class="pt-10">
                @foreach($productCategories as $productCategory)
                    @if(!empty($productCategory->subCategories) and count($productCategory->subCategories))
                        ...
                        <div class="pl-10">
                            @foreach($productCategory->subCategories as $subCategory)
                                <a href="/lms{{ $subCategory->getUrl() }}" class="d-flex align-items-center font-14 font-weight-normal mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $subCategory->id) ? 'text-primary' : '' }}">
                    ...
                    @else
                        <a href="/lms{{ $productCategory->getUrl() }}" class="d-flex align-items-center font-14 font-weight-bold mt-20 {{ (!empty($selectedCategory) and $selectedCategory->id == $productCategory->id) ? 'text-primary' : '' }}">
