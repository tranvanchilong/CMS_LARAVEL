...
@section('content')
    <section class="site-top-banner search-top-banner opacity-04 position-relative">
        <img src="{{get_path_lms()}}{{ getPageBackgroundSettings('categories') }}" class="img-cover" alt=""/>
            ...
            <div class="row h-100 align-items-center justify-content-center text-center">
                <div class="col-12 col-md-9 col-lg-7">
                    <div class="top-search-categories-form">
                        ...
                        <span class="course-count-badge py-5 px-10 text-white rounded">{{ $webinarsCount }} {{ trans('lms/product.courses') }}</span>

                        <div class="search-input bg-white p-10 flex-grow-1">
                            <form action="/lms/search" method="get">
                                <div class="form-group d-flex align-items-center m-0">
                                    <input type="text" name="search" class="form-control border-0" placeholder="{{ trans('lms/home.slider_search_placeholder') }}"/>
    ...
    <div class="container mt-30">
        ...
        @if(!empty($featureWebinars) and !$featureWebinars->isEmpty())
            <section class="mb-25 mb-lg-0">
                <h2 class="font-24 text-dark-blue">{{ trans('lms/home.featured_webinars') }}</h2>
                <span class="font-14 text-gray font-weight-400">{{ trans('lms/site.newest_courses_subtitle') }}</span>
                        ...
                        <div class="swiper-wrapper">
                            ...
                            @foreach($featureWebinars as $featureWebinar)
                                <div class="swiper-slide">
                                    @include('lms.web.default.includes.webinar.grid-card',['webinar' => $featureWebinar->webinar])
                                </div>
                            @endforeach
        ...
        <section class="mt-lg-50 pt-lg-20 mt-md-40 pt-md-40">
            <form action="/lms{{ $sortFormAction }}" method="get" id="filtersForm">

                @include('lms.web.default.pages.includes.top_filters')
                ...
                <div class="row mt-20">
                    <div class="col-12 col-lg-8">
                        ...
                        @elseif(!empty(request()->get('card')) and request()->get('card') == 'list')
                            ...
                            @foreach($webinars as $webinar)
                                @include('lms.web.default.includes.webinar.list-card',['webinar' => $webinar])
                            @endforeach
                        @endif
                    ...
                    <div class="col-12 col-lg-4">
                        <div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container">
                            ...
                            <div class="">
                                <h3 class="category-filter-title font-20 font-weight-bold text-dark-blue">{{ trans('lms/public.type') }}</h3>

                                <div class="pt-10">
                                    @foreach(['webinar','course','text_lesson'] as $typeOption)
                                        <div class="d-flex align-items-center justify-content-between mt-20">
                                            <label class="cursor-pointer" for="filterLanguage{{ $typeOption }}">{{ trans('lms/webinars.'.$typeOption) }}</label>
                            ...
                            <div class="mt-25 pt-25 border-top border-gray300">
                                <h3 class="category-filter-title font-20 font-weight-bold text-dark-blue">{{ trans('lms/site.more_options') }}</h3>

                                <div class="pt-10">
                                    @foreach(['bundles','subscribe','certificate_included','with_quiz','featured'] as $moreOption)
                                        <div class="d-flex align-items-center justify-content-between mt-20">
                                            <label class="cursor-pointer" for="filterLanguage{{ $moreOption }}">{{ trans('lms/webinars.show_only_'.$moreOption) }}</label>
                                            <div class="custom-control custom-checkbox">
                            ...
                            <button type="submit" class="btn btn-sm btn-primary btn-block mt-30">{{ trans('lms/site.filter_items') }}</button>
                        </div>
                    </div>
                </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/categories.min.js"></script>
@endpush
