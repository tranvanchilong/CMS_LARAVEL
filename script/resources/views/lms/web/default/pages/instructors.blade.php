...
@section('content')
    <section class="site-top-banner search-top-banner opacity-04 position-relative">
        <img src="{{get_path_lms()}}{{ getPageBackgroundSettings($page) }}" class="img-cover" alt=""/>
                ...
                <div class="col-12 col-md-9 col-lg-7">
                    <div class="top-search-categories-form">
                        ...
                        <div class="search-input bg-white p-10 flex-grow-1">
                            <form action="/lms/{{ $page }}" method="get">
                                <div class="form-group d-flex align-items-center m-0">
                                    <input type="text" name="search" class="form-control border-0" value="{{ request()->get('search') }}" placeholder="{{ trans('lms/public.search') }} {{ $title }}"/>
    ...
    <div class="container">
        ...
        <form id="filtersForm" action="/lms/{{ $page }}" method="get">
                ...
                <div class="row align-items-center">
                    <div class="col-lg-9 d-block d-md-flex align-items-center justify-content-start my-25 my-lg-0">
                        <div class="d-flex align-items-center justify-content-between justify-content-md-center">
                            <label class="mb-0 mr-10 cursor-pointer" for="available_for_meetings">{{ trans('lms/public.available_for_meetings') }}</label>
                        ...
                        <div class="d-flex align-items-center justify-content-between justify-content-md-center mx-0 mx-md-20 my-20 my-md-0">
                            <label class="mb-0 mr-10 cursor-pointer" for="free_meetings">{{ trans('lms/public.free_meetings') }}</label>
                            <div class="custom-control custom-switch">
                        ...
                        <div class="d-flex align-items-center justify-content-between justify-content-md-center">
                            <label class="mb-0 mr-10 cursor-pointer" for="discount">{{ trans('lms/public.discount') }}</label>
                            <div class="custom-control custom-switch">
                    ...
                    <div class="col-lg-3 d-flex align-items-center">
                        <select name="sort" class="form-control">
                            <option disabled selected>{{ trans('lms/public.sort_by') }}</option>
                            <option value="">{{ trans('lms/public.all') }}</option>
                            <option value="top_rate" @if(request()->get('sort',null) == 'top_rate') selected="selected" @endif>{{ trans('lms/site.top_rate') }}</option>
                            <option value="top_sale" @if(request()->get('sort',null) == 'top_sale') selected="selected" @endif>{{ trans('lms/site.top_sellers') }}</option>
                        </select>
                    </div>
            ...
            <div class="mt-30 px-20 py-15 rounded-sm shadow-lg border border-gray300">
                <h3 class="category-filter-title font-20 font-weight-bold">{{ trans('lms/categories.categories') }}</h3>
        ...
        <section>
            ...
            <div class="text-center">
                <button type="button" id="loadMoreInstructors" data-page="{{ ($page == 'instructors') ? \App\Models\LMS\Role::$teacher : \App\Models\LMS\Role::$organization }}" class="btn btn-border-white mt-50 {{ ($instructors->lastPage() <= $instructors->currentPage()) ? ' d-none' : '' }}">{{ trans('lms/site.load_more_instructors') }}</button>
            </div>
        </section>
        ...
        @if(!empty($bestRateInstructors) and !$bestRateInstructors->isEmpty() and (empty(request()->get('sort')) or !in_array(request()->get('sort'),['top_rate','top_sale'])))
            <section class="mt-30 pt-30">
                <div class="d-flex justify-content-between">
                    <div>
                        <h2 class="font-24 text-dark-blue">{{ trans('lms/site.best_rated_instructors') }}</h2>
                        <span class="font-14 text-gray">{{ trans('lms/site.best_rated_instructors_subtitle') }}</span>
                    </div>

                    <a href="/lms/{{ $page }}?sort=top_rate" class="btn btn-border-white">{{ trans('lms/home.view_all') }}</a>
                </div>
                        ...
                        <div class="swiper-wrapper pb-20">
                            ...
                            @foreach($bestRateInstructors as $bestRateInstructor)
                                <div class="swiper-slide">
                                    @include('lms.web.default.pages.instructor_card',['instructor' => $bestRateInstructor])
                                </div>
                            @endforeach
        ...
        @if(!empty($bestSalesInstructors) and !$bestSalesInstructors->isEmpty() and (empty(request()->get('sort')) or !in_array(request()->get('sort'),['top_rate','top_sale'])))
            <section class="mt-50 pt-50">
                <div class="d-flex justify-content-between">
                    <div>
                        <h2 class="font-24 text-dark-blue">{{ trans('lms/site.top_sellers') }}</h2>
                        <span class="font-14 text-gray">{{ trans('lms/site.top_sellers_subtitle') }}</span>
                    </div>

                    <a href="/lms/{{ $page }}?sort=top_sale" class="btn btn-border-white">{{ trans('lms/home.view_all') }}</a>
                </div>
                    ...
                    <div id="topSaleInstructorsSwiper" class="swiper-container px-12">
                        <div class="swiper-wrapper pb-20">
                            ...
                            @foreach($bestSalesInstructors as $bestSalesInstructor)
                                <div class="swiper-slide">
                                    @include('lms.web.default.pages.instructor_card',['instructor' => $bestSalesInstructor])
                                </div>
                            @endforeach
