...
<div class="rounded-lg shadow-sm mt-25 p-20 course-teacher-card instructors-list text-center d-flex align-items-center flex-column position-relative">
    @if(!empty($instructor->meeting) and $instructor->meeting->disabled)
        <span class="px-15 py-10 bg-gray off-label text-white font-12">{{ trans('lms/public.unavailable') }}</span>
    @elseif(!empty($instructor->meeting) and !empty($instructor->meeting->discount))
        <span class="px-15 py-10 bg-danger off-label text-white font-12">{{ $instructor->meeting->discount }}% {{ trans('lms/public.off') }}</span>
    @endif

    <a href="/lms{{ $instructor->getProfileUrl() }}{{ ($canReserve) ? '?tab=appointments' : '' }}" class="text-center d-flex flex-column align-items-center justify-content-center">
    ...
    <div class="stars-card d-flex align-items-center mt-10">
        @include('lms.web.default.includes.webinar.rate',['rate' => $instructor->rates()])
    </div>
    ...
    <div class="mt-20 d-flex flex-row align-items-center justify-content-center w-100">
        <a href="/lms{{ $instructor->getProfileUrl() }}{{ ($canReserve) ? '?tab=appointments' : '' }}" class="btn btn-primary btn-block">
            @if($canReserve)
                {{ trans('lms/public.reserve_a_meeting') }}
            @else
                {{ trans('lms/public.view_profile') }}
            @endif
        </a>
    </div>
</div>
