...
@section('content')
    @if((!empty($webinars) and count($webinars)) or (!empty($products) and count($products)) or (!empty($teachers) and count($teachers)) or (!empty($organizations) and count($organizations)))
        <section class="site-top-banner search-top-banner opacity-04 position-relative">
            <img src="{{get_path_lms()}}{{ getPageBackgroundSettings('search') }}" class="img-cover" alt=""/>

            <div class="container h-100">
                <div class="row h-100 align-items-center justify-content-center text-center">
                    <div class="col-12 col-md-9 col-lg-7">
                        <div class="top-search-form">
                            <h1 class="text-white font-30">{!! nl2br(trans('lms/site.result_find',['count' => $resultCount , 'search' => request()->get('search')])) !!}</h1>

                            <div class="search-input bg-white p-10 flex-grow-1">
                                <form action="/lms/search" method="get">
                                    <div class="form-group d-flex align-items-center m-0">
                                        <input type="text" name="search" class="form-control border-0" value="{{ request()->get('search','') }}" placeholder="{{ trans('lms/home.slider_search_placeholder') }}"/>
        ...
        <div class="container">
            @if(!empty($webinars) and count($webinars))
                <section class="mt-50">
                    <h2 class="font-24 font-weight-bold text-secondary">{{ trans('lms/webinars.webinars') }}</h2>

                    <div class="row">
                        @foreach($webinars as $webinar)
                            <div class="col-md-6 col-lg-4 mt-30">
                                @include('lms.' . getTemplate().'.includes.webinar.grid-card',['webinar' => $webinar])
                            </div>
                        @endforeach
                    </div>
                </section>
            ...
            @if(!empty($products) and count($products))
                <section class="mt-50">
                    <h2 class="font-24 font-weight-bold text-secondary">{{ trans('lms/update.products') }}</h2>

                    <div class="row">
                        @foreach($products as $product)
                            <div class="col-md-6 col-lg-4 mt-30">
                                @include('lms.web.default.products.includes.card',['product' => $product])
                            </div>
                        @endforeach
                    </div>
                </section>
            ...
            @if(!empty($teachers) and count($teachers))
                <section class="mt-50">
                    <h2 class="font-24 font-weight-bold text-secondary">{{ trans('lms/panel.users') }}</h2>
                        ...
                        @foreach($teachers as $teacher)
                            <div class="col-6 col-md-3 col-lg-2 mt-30">
                                <div class="user-search-card text-center d-flex flex-column align-items-center justify-content-center">
                                    ...
                                    <a href="/lms{{ $teacher->getProfileUrl() }}">
            ...
            @if(!empty($organizations) and count($organizations))
                <section class="mt-50">
                    <h2 class="font-24 font-weight-bold text-secondary">{{ trans('lms/home.organizations') }}</h2>

                    <div class="row">

                        @foreach($organizations as $organization)
                            <div class="col-md-6 col-lg-3 mt-30">
                                <a href="/lms{{ $organization->getProfileUrl() }}" class="home-organizations-card d-flex flex-column align-items-center justify-content-center">
                                    ...
                                    <div class="mt-25 d-flex flex-column align-items-center justify-content-center">
                                        ...
                                        <span class="home-organizations-badge badge mt-15">{{ $organization->getActiveWebinars(true) }} {{ trans('lms/product.courses') }}</span>
                                    </div>
                                </a>
                            </div>
                        @endforeach
                ...
                <div class="row h-100 align-items-center justify-content-center text-center">
                    <div class="col-12 col-md-9 col-lg-7">
                        <div class="d-flex align-items-center flex-column mt-30 text-center w-100">
                            <h2>{{ trans('lms/site.no_result_search') }}</h2>
                            <p class="mt-5 text-center">{!! trans('lms/site.no_result_search_hint',['search' => request()->get('search')]) !!}</p>
