...
@section('content')
                ...
                <div class="col-12 col-md-9 col-lg-7">
                    ...
                    <div class="d-flex flex-column flex-sm-row align-items-center align-sm-items-start justify-content-between">
                        @if(!empty($post->author))
                            <span class="mt-10 mt-md-20 font-16 font-weight-500 text-white">{{ trans('lms/public.created_by') }}
                                @if($post->author->isTeacher())
                                    <a href="/lms{{ $post->author->getProfileUrl() }}" target="_blank" class="text-white text-decoration-underline">{{ $post->author->full_name }}</a>
                        ...
                        <span class="mt-10 mt-md-20 font-16 font-weight-500 text-white">{{ trans('lms/public.in') }}
                            <a href="/lms{{ $post->category->getUrl() }}" class="text-white text-decoration-underline">{{ $post->category->title }}</a>
                        </span>
                        ...
                        <div class="js-share-blog d-flex align-items-center cursor-pointer mt-10 mt-md-20">
                            ...
                            <div class="ml-5 font-16 font-weight-500 text-white">{{ trans('lms/public.share') }}</div>
                        </div>
                    </div>
        ...
        <div class="row">
            <div class="col-12 col-lg-8">
                <div class="post-show mt-30">
                    ...
                    <div class="post-img pb-30">
                        <img src="{{get_path_lms()}}{{ $post->image }}" alt="">
                    </div>
            ...
            <div class="col-12 col-lg-4">
                @if(!empty($post->author) and !empty($post->author->full_name))
                    <div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
                        ...
                        <div class="mt-25 d-flex align-items-center  w-100">
                            <a href="/lms/blog?author={{ $post->author->id }}" class="btn btn-sm btn-primary btn-block px-15">{{ trans('lms/public.author_posts') }}</a>
                        </div>
                    </div>
                @endif
                ...
                <div class="p-20 mt-30 rounded-sm shadow-lg border border-gray300">
                    <h3 class="category-filter-title font-16 font-weight-bold text-dark-blue">{{ trans('lms/categories.categories') }}</h3>

                    <div class="pt-15">
                        @foreach($blogCategories as $blogCategory)
                            <a href="/lms{{ $blogCategory->getUrl() }}" class="font-14 text-dark-blue d-block mt-15">{{ $blogCategory->title }}</a>
                        @endforeach
                    </div>
                </div>
                ...
                {{-- recent_posts --}}
                <div class="p-20 mt-30 rounded-sm shadow-lg border border-gray300">
                    <h3 class="category-filter-title font-20 font-weight-bold text-dark-blue">{{ trans('lms/site.recent_posts') }}</h3>
                    ...
                    <div class="pt-15">
                        ...
                        @foreach($popularPosts as $popularPost)
                            <div class="popular-post d-flex align-items-start mt-20">
                                <div class="popular-post-image rounded">
                                    <img src="{{get_path_lms()}}{{ $popularPost->image }}" class="img-cover rounded" alt="{{ $popularPost->title }}">
                                </div>
                                <div class="popular-post-content d-flex flex-column ml-10">
                                    <a href="/lms{{ $popularPost->getUrl() }}">
                        ...
                        <a href="/lms/blog" class="btn btn-sm btn-primary btn-block mt-30">{{ trans('lms/home.view_all') }} {{ trans('lms/site.posts') }}</a>
                    </div>
                </div>
    ...
    @include('lms.web.default.blog.share_modal')
@endsection
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/blog.min.js"></script>
@endpush
