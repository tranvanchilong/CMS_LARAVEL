    ...
    <div class="container pt-50 mt-10">
        <div class="text-center">
            @if($installment->needToVerify())
                <h1 class="font-36">{{ trans('lms/update.verify_your_installments') }}</h1>
                <p class="mt-10 font-16 text-gray">{{ trans('lms/update.verify_your_installments_hint') }}</p>
            @else
                <h1 class="font-36">{{ trans('lms/update.verify_your_installments2') }}</h1>
                <p class="mt-10 font-16 text-gray">{{ trans('lms/update.verify_your_installments_hint2') }}</p>
            @endif
        </div>
        ...
        <div class="become-instructor-packages d-flex align-items-center flex-column flex-lg-row mt-50 border rounded-lg p-15 p-lg-25">
            <div class="default-package-icon">
                <img src="/assets/lms/assets/default/img/become-instructor/default.png" class="img-cover" alt="{{ trans('lms/update.installment_overview') }}" width="176" height="144">
            </div>

            <div class="ml-lg-25 w-100 mt-20 mt-lg-0">
                <h2 class="font-24 font-weight-bold text-dark-blue">{{ trans('lms/update.installment_overview') }}</h2>
                @if($itemType == 'course')
                    <a href="/lms{{ $item->getUrl() }}" target="_blank" class="font-14 font-weight-500 text-gray">{{ $item->title }}</a>
                @else
                ...
                <div class="d-flex flex-wrap align-items-center justify-content-between w-100">
                    ...
                    <div class="d-flex align-items-center mt-20">
                        ...
                        <span class="font-14 text-gray ml-5">{{ !empty($installment->upfront) ? handlePrice($installment->getUpfront($itemPrice)).' '. trans('lms/update.upfront') : trans('lms/update.no_upfront') }}</span>
                    </div>

                    <div class="d-flex align-items-center mt-20">
                        <i data-feather="menu" width="20" height="20" class="text-gray"></i>
                        <span class="font-14 text-gray ml-5">{{ $installment->steps_count }} {{ trans('lms/update.installments') }} ({{ handlePrice($installment->totalPayments($itemPrice, false)) }})</span>
                    </div>

                    <div class="d-flex align-items-center mt-20">
                        <i data-feather="dollar-sign" width="20" height="20" class="text-gray"></i>
                        <span class="font-14 text-gray ml-5">{{ handlePrice($installment->totalPayments($itemPrice)) }} {{ trans('lms/financial.total_amount') }}</span>
                    </div>

                    <div class="d-flex align-items-center mt-20">
                        <i data-feather="calendar" width="20" height="20" class="text-gray"></i>
                        <span class="font-14 text-gray ml-5">{{ $installment->steps->max('deadline') }} {{ trans('lms/update.days_duration') }}</span>
                    </div>
        ...
        <form action="/lms/installments/{{ $installment->id }}/store" method="post">
            {{ csrf_field() }}
            ...
            @if($installment->request_uploads or $installment->needToVerify())
                <div class="border rounded-lg p-15 mt-20">
                    @if($installment->needToVerify())
                        <h3 class="font-16 font-weight-bold text-dark-blue">{{ trans('lms/update.verify_installments') }}</h3>

                        <div class="font-16 text-gray mt-10">{!! nl2br($installment->verification_description) !!}</div>

                        {{-- Banner --}}
                        @if(!empty($installment->verification_banner))
                            <img src="{{get_path_lms()}}{{ $installment->verification_banner }}" alt="{{ $installment->main_title }}" class="img-fluid mt-30">
                        @endif
                    ...
                    @if($installment->request_uploads)
                        <div class="{{ ($installment->needToVerify()) ? 'mt-20' : '' }}">
                            <h4 class="font-16 font-weight-bold text-dark-blue">{{ trans('lms/update.attachments') }}</h4>
                            <p class="mt-5 font-12 text-gray">{{ trans('lms/update.attach_your_documents_and_send_them_to_admin') }}</p>
                                ...
                                <div class="js-main-row row">
                                    <div class="col-12 col-md-6 mt-15">
                                        <div class="form-group mb-0">
                                            <label class="font-14 text-dark-blue">{{ trans('lms/public.title') }}</label>
                                    ...
                                    <div class="col-12 col-md-6 mt-15">
                                        <div class="form-group">
                                            <label class="font-14 text-dark-blue">{{ trans('lms/update.attach_a_file_optional') }}</label>
            ...
            <div class="border rounded-lg p-15 mt-30">
                <h3 class="font-16 font-weight-bold text-dark-blue">{{ trans('lms/update.installment_terms_&_rules') }}</h3>
                ...
                <div class="mt-10 border bg-info-light p-15 rounded-sm">
                    <h4 class="font-14 text-gray font-weight-bold">{{ trans('lms/update.important') }}</h4>
                    <p class="mt-5 font-14 text-gray">{{ trans('lms/update.by_purchasing_installment_plans_you_will_accept_installment_terms_and_rules') }}</p>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center justify-content-between border-top pt-10 mt-20">
                <a href="/lms{{ url()->previous() }}" class="btn btn-sm btn-gray200">{{ trans('lms/update.back') }}</a>

                <button type="submit" class="btn btn-sm btn-primary">
                    @if($installment->needToVerify())
                        @if(!empty($installment->upfront))
                            {{ trans('lms/update.submit_and_checkout') }}
                        @else
                            {{ trans('lms/update.submit_request') }}
                        @endif
                    @else
                        @if(!empty($installment->upfront))
                            {{ trans('lms/update.proceed_to_checkout') }}
                        @else
                            {{ trans('lms/update.finalize_request') }}
                        @endif
                    @endif
                </button>
            </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/parts/installment_verify.min.js"></script>
@endpush
