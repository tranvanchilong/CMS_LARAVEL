    ...
    <div class="row">
        <div class="col-8">
            ...
            @if(!empty($installment->capacity))
                ...
                @if($reachedCapacityPercent > 0)
                    <div class="mt-20 d-flex align-items-center">
                        ...
                        <div class="ml-10 font-12 text-danger">{{ trans('lms/update.percent_capacity_reached',['percent' => $reachedCapacityPercent]) }}</div>
                    </div>
                @endif
            @endif
            ...
            @if(!empty($installment->banner))
                <div class="mt-20">
                    <img src="{{get_path_lms()}}{{ $installment->banner }}" alt="{{ $installment->main_title }}" class="img-fluid">
                </div>
            @endif
        ...
        <div class="col-4 p-0 pr-15">
            <div class="installment-card__payments d-flex flex-column w-100 h-100">
                ...
                <div class="d-flex align-items-center justify-content-center flex-column">
                    ...
                    <span class="mt-10 font-12 text-gray">{{ trans('lms/update.total_payment') }} @if($installmentTotalInterest > 0)
                            ({{ trans('lms/update.percent_interest',['percent' => $installmentTotalInterest]) }})
                        @endif</span>
                </div>

                <div class="mt-25 mb-15">
                    <div class="installment-step d-flex align-items-center font-12 text-gray">{{ !empty($installment->upfront) ? (trans('lms/update.amount_upfront',['amount' => handlePrice($installment->getUpfront($itemPrice))]) . ($installment->upfront_type == "percent" ? " ({$installment->upfront}%)" : '')) : trans('lms/update.no_upfront') }}</div>
                ...
                <a href="/lms/installments/{{ $installment->id }}?item={{ $itemId }}&item_type={{ $itemType }}&{{ http_build_query(request()->all()) }}" target="_blank" class="btn btn-primary btn-block mt-auto">{{ trans('lms/update.pay_with_installments') }}</a>
            </div>
        </div>
    </div>
</div>
