...
<div class="row">
    <div class="col-12 col-md-6 mt-15">
        ...
        <div class="form-group">
            <label class="input-label">{{ trans('lms/update.estimated_publish_date') }}</label>
        ...
        <div class="form-group mt-15">
            ...
            <input type="number" name="capacity" value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->capacity)) ? $upcomingCourse->capacity : old('capacity') }}" class="form-control @error('capacity')  is-invalid @enderror" placeholder="{{ trans('lms/forms.capacity_placeholder') }}"/>
            @error('capacity')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
            @enderror
            <p class="text-gray mt-5 font-12">{{  trans('lms/forms.empty_means_unlimited')  }}</p>
        </div>

        <div class="form-group">
            <label class="input-label">{{ trans('lms/public.price') }} ({{ $currency }})</label>
            <input type="text" name="price" value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->price)) ? convertPriceToUserCurrency($upcomingCourse->price) : old('price') }}" class="form-control @error('price')  is-invalid @enderror" placeholder="{{ trans('lms/public.0_for_free') }}"/>
            @error('price')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group">
            <label class="input-label">{{ trans('lms/update.sections') }}</label>
            ...
            @enderror
            <p class="text-gray mt-5 font-12">{{  trans('lms/update.upcoming_sections_hint')  }}</p>
        </div>

        <div class="form-group">
            <label class="input-label">{{ trans('lms/public.parts') }}</label>
            ...
            @enderror
            <p class="text-gray mt-5 font-12">{{  trans('lms/update.upcoming_parts_hint')  }}</p>
        </div>

        <div class="form-group">
            <label class="input-label">{{ trans('lms/update.course_progress') }}</label>
            <input type="number" min="0" max="100" name="course_progress" value="{{ (!empty($upcomingCourse) and !empty($upcomingCourse->course_progress)) ? $upcomingCourse->course_progress : old('course_progress') }}" class="form-control @error('course_progress')  is-invalid @enderror" placeholder="{{ trans('lms/update.progress_placeholder') }}"/>
            @error('course_progress')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
            @enderror
            <p class="text-gray mt-5 font-12">{{  trans('lms/update.upcoming_progress_hint')  }}</p>
        </div>

        <div class="form-group mt-30 d-flex align-items-center justify-content-between">
            <label class="cursor-pointer input-label" for="supportSwitch">{{ trans('lms/webinars.support') }}</label>
        ...
        <div class="form-group mt-30 d-flex align-items-center justify-content-between">
            <label class="cursor-pointer input-label" for="certificateSwitch">{{ trans('lms/update.certificate_included') }}</label>
            <div class="custom-control custom-switch">
        ...
        <div>
            <p class="font-12 text-gray">{{ trans('lms/update.upcoming_certificate_hint') }}</p>
        </div>

        <div class="form-group mt-30 d-flex align-items-center justify-content-between">
            <label class="cursor-pointer input-label" for="quizzesSwitch">{{ trans('lms/update.include_quizzes') }}</label>
            <div class="custom-control custom-switch">
        ...
        <div class="form-group mt-30 d-flex align-items-center justify-content-between">
            <label class="cursor-pointer input-label" for="downloadableSwitch">{{ trans('lms/home.downloadable') }}</label>
            <div class="custom-control custom-switch">
        ...
        <div class="form-group mt-30 d-flex align-items-center justify-content-between mb-5">
            <label class="cursor-pointer input-label" for="forumSwitch">{{ trans('lms/update.course_forum') }}</label>
            <div class="custom-control custom-switch">
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="tags" data-max-tag="5" value="{{ !empty($upcomingCourse) ? implode(',', $upcomingCourseTags) : '' }}" class="form-control inputtags" placeholder="{{ trans('lms/public.type_tag_name_and_press_enter') }} ({{ trans('lms/forms.max') }} : 5)"/>
        </div>
        ...
        <div class="form-group mt-15">
            ...
            <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror" name="category_id" required>
                <option {{ (!empty($upcomingCourse) and !empty($upcomingCourse->category_id)) ? '' : 'selected' }} disabled>{{ trans('lms/public.choose_category') }}</option>
