...
@section('content')
    <section>
        <h2 class="section-title">{{ trans('lms/update.assignment_statistics') }}</h2>
            ...
            <div class="row">
                <div class="col-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-dark-blue text-gray font-weight-500">{{ trans('lms/update.course_assignments') }}</span>
                    </div>
                </div>
                ...
                <div class="col-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-dark-blue text-gray font-weight-500">{{ trans('lms/update.pending_review') }}</span>
                    </div>
                </div>
                ...
                <div class="col-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/quiz.passed') }}</span>
                    </div>
                </div>

                <div class="col-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        <img src="/assets/lms/assets/default/img/activity/pin.svg" width="64" height="64" alt="">
                        <strong class="font-30 text-dark-blue font-weight-bold mt-5">{{ $failedCount }}</strong>
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/quiz.failed') }}</span>
                    </div>
                </div>
    ...
    <section class="mt-25">
        <h2 class="section-title">{{ trans('lms/update.filter_assignments') }}</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/assignments/my-assignments" method="get" class="row">
                        ...
                        <div class="col-12 col-lg-8">
                            <div class="form-group">
                                ...
                                <select name="webinar_id" class="form-control select2">
                                    <option value="">{{ trans('lms/webinars.all_courses') }}</option>
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="status" name="status">
                                    ...
                                    @foreach(\App\Models\LMS\WebinarAssignmentHistory::$assignmentHistoryStatus as $status)
                                        <option value="{{ $status }}" {{ (request()->get('status') == $status) ? 'selected' : '' }}>{{ trans('lms/update.assignment_history_status_'.$status) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
    ...
    <section class="mt-35">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h2 class="section-title">{{ trans('lms/update.my_assignments') }}</h2>
        </div>
        ...
        @if($assignments->count() > 0)
                        ...
                        <div class="table-responsive">
                            <table class="table text-center custom-table">
                                ...
                                <tr>
                                    ...
                                    <th class="text-center">{{ trans('lms/update.deadline') }}</th>
                                    <th class="text-center">{{ trans('lms/update.first_submission') }}</th>
                                    <th class="text-center">{{ trans('lms/update.last_submission') }}</th>
                                    <th class="text-center">{{ trans('lms/update.attempts') }}</th>
                                ...
                                @foreach($assignments as $assignment)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            @if(empty($assignment->assignmentHistory) or ($assignment->assignmentHistory->status == \App\Models\LMS\WebinarAssignmentHistory::$notSubmitted))
                                                <span class="text-danger font-weight-500">{{ trans('lms/update.assignment_history_status_not_submitted') }}</span>
                                            @else
                                                @switch($assignment->assignmentHistory->status)
                                                    @case(\App\Models\LMS\WebinarAssignmentHistory::$passed)
                                                    <span class="text-primary font-weight-500">{{ trans('lms/quiz.passed') }}</span>
                                                    @break
                                                    @case(\App\Models\LMS\WebinarAssignmentHistory::$pending)
                                                    <span class="text-warning font-weight-500">{{ trans('lms/public.pending') }}</span>
                                                    @break
                                                    @case(\App\Models\LMS\WebinarAssignmentHistory::$notPassed)
                                                    <span class="font-weight-500 text-danger">{{ trans('lms/quiz.failed') }}</span>
                                                    @break
                                                @endswitch
                                            @endif
                                        </td>
                                            ...
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu menu-lg">
                                                    @if($assignment->webinar->checkUserHasBought())
                                                        <a href="/lms{{ "{$assignment->webinar->getLearningPageUrl()}?type=assignment&item={$assignment->id}" }}" target="_blank"
                                                           class="webinar-actions d-block mt-10 font-weight-normal">{{ trans('lms/update.view_assignment') }}</a>
                                                    @else
                                                        <a href="/lms#!" class="not-access-toast webinar-actions d-block mt-10 font-weight-normal">{{ trans('lms/update.view_assignment') }}</a>
                                                    @endif
                                                </div>
                                            </div>
            ...
            <div class="my-30">
                {{ $assignments->appends(request()->input())->links('lms.vendor.pagination.panel') }}
            </div>
        @else
            @include('lms.' . getTemplate() . '.includes.no-result',[
                'file_name' => 'meeting.png',
                'title' => trans('lms/update.my_assignments_no_result'),
                'hint' => nl2br(trans('lms/update.my_assignments_no_result_hint_student')),
            ])
        @endif
    </section>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/my_assignments.min.js"></script>
@endpush
