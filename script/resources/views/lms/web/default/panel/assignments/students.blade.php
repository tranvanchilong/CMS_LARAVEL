...
@section('content')
    ...
    <section class="mt-25">
        ...
        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/assignments/{{ $assignment->id }}/students" method="get" class="row">
    ...
    <section class="mt-35">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h2 class="section-title">{{ trans('lms/update.your_course_assignments') }}</h2>
        </div>
        ...
        @if($histories->count() > 0)
                            ...
                            <table class="table text-center custom-table">
                                ...
                                @foreach($histories as $history)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            @if(empty($history) or ($history->status == \App\Models\LMS\WebinarAssignmentHistory::$notSubmitted))
                                        ...
                                        <td class="align-middle text-right">
                                            ...
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu menu-lg">
                                                    <a href="/lms{{ "{$assignment->webinar->getLearningPageUrl()}?type=assignment&item={$assignment->id}&student={$history->student_id}" }}" target="_blank"
            ...
            <div class="my-30">
                {{ $histories->appends(request()->input())->links('lms.vendor.pagination.panel') }}
            </div>
        @else
            @include('lms.' . getTemplate() . '.includes.no-result',[
                'file_name' => 'meeting.png',
                'title' => trans('lms/update.my_assignments_no_result'),
                'hint' => nl2br(trans('lms/update.my_assignments_no_result_hint')),
            ])
        @endif
    </section>
@endsection
