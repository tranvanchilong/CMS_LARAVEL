<section>
    <h3 class="section-title after-line mt-35">{{ trans('lms/auth.images') }}</h3>

    <div class="row mt-20">
        <div class="col-12 col-lg-4">

            <div class="form-group">
                <label class="input-label">{{ trans('lms/auth.profile_image') }}</label>
        ...
        <div class="col-12 col-lg-4">
            <div class="form-group">
                <label class="input-label">{{ trans('lms/auth.profile_cover') }}</label>

                <img src="{{get_path_lms()}}{{ (!empty($user)) ? $user->getCover() : '' }}" alt="" id="profileCoverPreview" height="150" class="rounded-sm my-15 d-block w-100">
                ...
                <div class="form-group">
                    <div class="input-group">
                        ...
                        <input type="text" name="cover_img" id="cover_img" value="{{ !empty($user) ? $user->cover_img : old('cover_img') }}" class="form-control " placeholder="{{ trans('lms/forms.course_cover_size') }}"/>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">{{ trans('lms/public.edit_selected_image') }}</h4>
            </div>
            <div class="modal-body">
                <div id="imageCropperContainer">
                    ...
                    <div>
                        <button class="btn btn-transparent" id="cancelAvatarCrop">{{ trans('lms/public.cancel') }}</button>
                        <button class="btn btn-green" id="storeAvatar">{{ trans('lms/public.select') }}</button>
                    </div>
