...
@section('content')
    <div class="">
        ...
        <form method="post" action="/lms/panel/store/products/{{ !empty($product) ? $product->id .'/update' : 'store' }}" id="productForm" class="webinar-form">
            @include('lms.web.default.panel.store.products.create_includes.progress')
            ...
            @if($currentStep == 1)
                @include('lms.web.default.panel.store.products.create_includes.step_1')
            @elseif(!empty($product))
                @include('lms.web.default.panel.store.products.create_includes.step_'.$currentStep)
            @endif
        ...
        <div class="create-webinar-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class="d-flex align-items-center">
                ...
                @if(!empty($product))
                    <a href="/lms/panel/store/products/{{ $product->id }}/step/{{ ($currentStep - 1) }}" class="btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('lms/webinars.previous') }}</a>
                @else
                    <a href="/lms" class="btn btn-sm btn-primary disabled">{{ trans('lms/webinars.previous') }}</a>
                @endif

                <button type="button" id="getNextStep" class="btn btn-sm btn-primary ml-15" @if($currentStep >= 5) disabled @endif>{{ trans('lms/webinars.next') }}</button>
            </div>
            ...
            <div class="mt-20 mt-md-0">
                ...
                @if(!empty($product) and $product->creator_id == $authUser->id)
                    <a href="/lms/panel/store/products/{{ $product->id }}/delete?redirect_to=/panel/store/products" class="delete-action webinar-actions btn btn-sm btn-danger mt-20 mt-md-0">{{ trans('lms/public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/new_product.min.js"></script>
@endpush
