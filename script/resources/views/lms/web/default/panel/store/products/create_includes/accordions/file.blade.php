<li data-id="{{ !empty($file) ? $file->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="file_{{ !empty($file) ? $file->id :'record' }}">
        <div class="d-flex align-items-center" href="#collapseFile{{ !empty($file) ? $file->id :'record' }}" aria-controls="collapseFile{{ !empty($file) ? $file->id :'record' }}" data-parent="#filesAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="font-weight-bold text-dark-blue d-block">{{ !empty($file) ? $file->title : trans('lms/public.add_new_files') }}</div>
        </div>

        <div class="d-flex align-items-center">
            @if(!empty($file) and $file->status != \App\Models\LMS\ProductFile::$Active)
            ...
            @if(!empty($file))
                <a href="/lms/panel/store/products/files/{{ $file->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
    ...
    <div id="collapseFile{{ !empty($file) ? $file->id :'record' }}" aria-labelledby="file_{{ !empty($file) ? $file->id :'record' }}" class=" collapse @if(empty($file)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form file-form" data-action="/lms/panel/store/products/files/{{ !empty($file) ? $file->id . '/update' : 'store' }}">
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group js-file-path-input">
                            <div class="local-input input-group">
                                ...
                                <input type="text" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][path]" id="file_path{{ !empty($file) ? $file->id : 'record' }}" value="{{ (!empty($file)) ? $file->path : '' }}" class="js-ajax-file_path form-control" placeholder="{{ trans('lms/webinars.file_upload_placeholder') }}"/>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="ajax[{{ !empty($file) ? $file->id : 'new' }}][description]" rows="4" class="js-ajax-description form-control" placeholder="{{ trans('lms/public.description') }}">{{ !empty($file) ? $file->description : '' }}</textarea>
                            <div class="invalid-feedback"></div>
                        </div>
