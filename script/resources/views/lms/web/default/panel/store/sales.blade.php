...
@section('content')
    <section>
        <h2 class="section-title">{{ trans('lms/update.orders_statistics') }}</h2>
    ...
    <section class="mt-25">
        <h2 class="section-title">{{ trans('lms/update.orders_report') }}</h2>
    ...
    @if(!empty($orders) and !$orders->isEmpty())
        <section class="mt-35">
            <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
                <h2 class="section-title">{{ trans('lms/update.orders_history') }}</h2>
            </div>
                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table text-center custom-table">
                                <thead>
                                <tr>
                                    <th>{{ trans('lms/update.customer') }}</th>
                                            ...
                                            @if(!empty($order) and $order->status != \App\Models\LMS\ProductOrder::$canceled)
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu font-weight-normal">
                                                        <a href="/lms/panel/store/sales/{{ $order->sale_id }}/productOrder/{{ $order->id }}/invoice" class="webinar-actions d-block mt-10" target="_blank">{{ trans('lms/public.invoice') }}</a>

                                                        @if($order->status == \App\Models\LMS\ProductOrder::$waitingDelivery)
                                                            <button type="button" data-sale-id="{{ $order->sale_id }}" data-product-order-id="{{ $order->id }}" class="js-enter-tracking-code webinar-actions btn-transparent d-block mt-10">{{ trans('lms/update.enter_tracking_code') }}</button>
                                                        @endif
                                                    </div>
                                                </div>
                                            @endif
    ...
    @else
        @include('lms.' . getTemplate() . '.includes.no-result',[
              'file_name' => 'sales.png',
              'title' => trans('lms/update.product_sales_no_result'),
              'hint' => nl2br(trans('lms/update.product_sales_no_result_hint')),
          ])
    @endif
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/store/sale.min.js"></script>
@endpush
