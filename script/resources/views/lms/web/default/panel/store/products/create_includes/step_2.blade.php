...
<div class="row">
    <div class="col-12 col-md-6 mt-15">
        ...
        <div class="form-group">
            ...
            <input type="number" name="price" value="{{ (!empty($product) and !empty($product->price)) ? convertPriceToUserCurrency($product->price) : old('price') }}" class="form-control @error('price')  is-invalid @enderror" placeholder="{{ trans('lms/public.0_for_free') }}"/>
            @error('price')
        ...
        @if($product->isPhysical())
            <div class="form-group">
                <label class="input-label">{{ trans('lms/update.delivery_fee') }} ({{ currencySign() }})</label>
                <input type="number" name="delivery_fee" value="{{ (!empty($product) and !empty($product->delivery_fee)) ? convertPriceToUserCurrency($product->delivery_fee) : old('delivery_fee') }}" class="form-control @error('delivery_fee')  is-invalid @enderror" placeholder="{{ trans('lms/public.0_for_free') }}"/>
                @error('delivery_fee')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group mt-30 mb-10 d-flex align-items-center">
            <label class="cursor-pointer mb-0 input-label" for="unlimitedInventorySwitch">{{ trans('lms/update.unlimited_inventory') }}</label>
