...
<div class="row">
    <div class="col-12 col-md-6 mt-15">
        ...
        <div class="form-group mt-15">
            ...
            <input type="text" name="tags" data-max-tag="5" value="{{ !empty($bundle) ? implode(',',$bundleTags) : '' }}" class="form-control inputtags" placeholder="{{ trans('lms/public.type_tag_name_and_press_enter') }} ({{ trans('lms/forms.max') }} : 5)"/>
        </div>
        ...
        <div class="form-group mt-15">
            ...
            <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror" name="category_id" required>
                <option {{ (!empty($bundle) and !empty($bundle->category_id)) ? '' : 'selected' }} disabled>{{ trans('lms/public.choose_category') }}</option>
