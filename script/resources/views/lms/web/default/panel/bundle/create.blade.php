...
@section('content')
    <div class="">
        ...
        <form method="post" action="/lms/panel/bundles/{{ !empty($bundle) ? $bundle->id .'/update' : 'store' }}" id="webinarForm" class="webinar-form">
            @include('lms.web.default.panel.bundle.create_includes.progress')
            ...
            @if($currentStep == 1)
                @include('lms.web.default.panel.bundle.create_includes.step_1')
            @elseif(!empty($bundle))
                @include('lms.web.default.panel.bundle.create_includes.step_'.$currentStep)
            @endif
        ...
        <div class="create-bundle-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class="d-flex align-items-center">
                ...
                @if(!empty($bundle))
                    <a href="/lms/panel/bundles/{{ $bundle->id }}/step/{{ ($currentStep - 1) }}" class="btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('lms/webinars.previous') }}</a>
                @else
            ...
            <div class="mt-20 mt-md-0">
                ...
                @if(!empty($bundle) and $bundle->creator_id == $authUser->id)
                    <a href="/lms/panel/bundles/{{ $bundle->id }}/delete?redirect_to=/panel/bundles" class="delete-action bundle-actions btn btn-sm btn-danger mt-20 mt-md-0">{{ trans('lms/public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/new_bundle.min.js"></script>
@endpush
