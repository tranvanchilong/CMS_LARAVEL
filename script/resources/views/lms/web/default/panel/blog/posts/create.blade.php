...
@section('content')
    ...
    <section>
        ...
        <form action="/lms/panel/blog/posts/{{ (!empty($post) ? $post->id.'/update' : 'store') }}" method="post">
            {{ csrf_field() }}
            ...
            <div class="form-group mt-15">
                ...
                <textarea id="summernote" name="description" class="main-summernote summernote form-control @error('description')  is-invalid @enderror" placeholder="{{ trans('lms/admin/main.create_blog_description_hint') }}">{!! (!empty($post) and !empty($post->translate($locale))) ? $post->translate($locale)->description : old('description') !!}</textarea>
                @error('description')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="form-group mt-15">
                ...
                <textarea id="contentSummernote" name="content" class="main-summernote summernote form-control @error('content')  is-invalid @enderror" placeholder="{{ trans('lms/admin/main.create_blog_content_hint') }}">{!! (!empty($post) and !empty($post->translate($locale))) ? $post->translate($locale)->content : old('content')  !!}</textarea>
                @error('content')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <button type="submit" class="btn btn-primary btn-sm mt-1">{{ trans('lms/admin/main.save_change') }}</button>
        </form>
