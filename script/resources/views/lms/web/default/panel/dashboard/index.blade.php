...
@push('styles_top')
    ...
    <link rel="stylesheet" href="/assets/lms/assets/default/vendors/apexcharts/apexcharts.css"/>
@endpush

@section('content')
    <section class="">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h1 class="section-title">{{ trans('lms/panel.dashboard') }}</h1>

            @if(!$authUser->isUser())
                <div class="d-flex align-items-center flex-row-reverse flex-md-row justify-content-start justify-content-md-center mt-20 mt-md-0">
                    <label class="mb-0 mr-10 cursor-pointer text-gray font-14 font-weight-500" for="iNotAvailable">{{ trans('lms/panel.i_not_available') }}</label>
        ...
        <div class="bg-white dashboard-banner-container position-relative px-15 px-ld-35 py-10 panel-shadow rounded-sm">
            <h2 class="font-30 text-primary line-height-1">
                <span class="d-block">{{ trans('lms/panel.hi') }} {{ $authUser->full_name }},</span>
                <span class="font-16 text-secondary font-weight-bold">{{ trans('lms/panel.have_event',['count' => !empty($unReadNotifications) ? count($unReadNotifications) : 0]) }}</span>
            </h2>
            ...
            <a href="/lms/panel/notifications" class="mt-15 font-weight-500 text-dark-blue d-inline-block">{{ trans('lms/panel.view_all_events') }}</a>

            <div class="dashboard-banner">
                <img src="{{get_path_lms()}}{{ getPageBackgroundSettings('dashboard') }}" alt="" class="img-cover">
            </div>
        </div>
    </section>
    ...
    <section class="dashboard">
        <div class="row">
            <div class="col-12 col-lg-3 mt-35">
                <div class="bg-white account-balance rounded-sm panel-shadow py-15 py-md-30 px-10 px-md-20">
                    <div class="text-center">
                        <img src="/assets/lms/assets/default/img/activity/36.svg" class="account-balance-icon" alt="">

                        <h3 class="font-16 font-weight-500 text-gray mt-25">{{ trans('lms/panel.account_balance') }}</h3>
                    ...
                    <div class="mt-20 pt-30 border-top border-gray300 d-flex align-items-center @if($can_drawable) justify-content-between @else justify-content-center @endif">
                        @if($can_drawable)
                            <span class="font-16 font-weight-500 text-gray">{{ trans('lms/panel.with_drawable') }}:</span>
                            <span class="font-16 font-weight-bold text-secondary">{{ handlePrice($drawable) }}</span>
                        @else
                            <a href="/lms/panel/financial/account" class="font-16 font-weight-bold text-dark-blue">{{ trans('lms/financial.charge_account') }}</a>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-lg-3 mt-35">
                <a href="@if($authUser->isUser()) /lms/panel/webinars/purchases @else /lms/panel/meetings/requests @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center">
                    <div class="stat-icon requests">
                        <img src="/assets/lms/assets/default/img/icons/request.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ $authUser->isUser() ? trans('lms/panel.purchased_courses') : trans('lms/panel.pending_appointments') }}</span>
                    </div>
                </a>

                <a href="@if($authUser->isUser()) /lms/panel/meetings/reservation @else /lms/panel/financial/sales @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                    ...
                    <div class="d-flex flex-column ml-15">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ $authUser->isUser() ? trans('lms/panel.meetings') : trans('lms/panel.monthly_sales') }}</span>
                    </div>
                </a>
            </div>

            <div class="col-12 col-lg-3 mt-35">
                <a href="/lms/panel/support" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center">
                    <div class="stat-icon support-messages">
                        <img src="/assets/lms/assets/default/img/icons/support.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
                        <span class="font-30 text-secondary">{{ !empty($supportsCount) ? $supportsCount : 0 }}</span>
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/panel.support_messages') }}</span>
                    </div>
                </a>

                <a href="@if($authUser->isUser()) /lms/panel/webinars/my-comments @else /lms/panel/webinars/comments @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                    <div class="stat-icon comments">
                        <img src="/assets/lms/assets/default/img/icons/comment.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
            ...
            <div class="col-12 col-lg-3 mt-35">
                <div class="bg-white account-balance rounded-sm panel-shadow py-15 py-md-15 px-10 px-md-20">
                    ...
                    <div class="mt-10 pt-10 border-top border-gray300 d-flex align-items-center justify-content-between">
                        <span class="font-16 font-weight-500 text-gray">{{ trans('lms/panel.next_badge') }}:</span>
                        <span class="font-16 font-weight-bold text-secondary">{{ (!empty($nextBadge) and !empty($nextBadge['badge'])) ? $nextBadge['badge']->title : trans('lms/public.not_defined') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12 col-lg-6 mt-35">
                <div class="bg-white noticeboard rounded-sm panel-shadow py-10 py-md-20 px-15 px-md-30">
                    <h3 class="font-16 text-dark-blue font-weight-bold">{{ trans('lms/panel.noticeboard') }}</h3>
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <div class="font-12 text-gray mt-5">
                                        <span class="mr-5">{{ trans('lms/public.created_by') }} {{ $getUnreadNoticeboard->sender }}</span>
                                        |
                                ...
                                <div>
                                    <button type="button" data-id="{{ $getUnreadNoticeboard->id }}" class="js-noticeboard-info btn btn-sm btn-border-white">{{ trans('lms/panel.more_info') }}</button>
            ...
            <div class="col-12 col-lg-6 mt-35">
                <div class="bg-white monthly-sales-card rounded-sm panel-shadow py-10 py-md-20 px-15 px-md-30">
                    <div class="d-flex align-items-center justify-content-between">
                        <h3 class="font-16 text-dark-blue font-weight-bold">{{ ($authUser->isUser()) ? trans('lms/panel.learning_statistics') : trans('lms/panel.monthly_sales') }}</h3>
    ...
    <div class="d-none" id="iNotAvailableModal">
        <div class="offline-modal">
            <h3 class="section-title after-line">{{ trans('lms/panel.offline_title') }}</h3>
            <p class="mt-20 font-16 text-gray">{{ trans('lms/panel.offline_hint') }}</p>

            <div class="form-group mt-15">
                <label>{{ trans('lms/panel.offline_message') }}</label>
                <textarea name="message" rows="4" class="form-control ">{{ $authUser->offline_message }}</textarea>
                <div class="invalid-feedback"></div>
            </div>

            <div class="mt-30 d-flex align-items-center justify-content-end">
                <button type="button" class="js-save-offline-toggle btn btn-primary btn-sm">{{ trans('lms/public.save') }}</button>
                <button type="button" class="btn btn-danger ml-10 close-swl btn-sm">{{ trans('lms/public.close') }}</button>
            </div>
        </div>
    </div>
...
@push('scripts_bottom')
    <script src="/assets/lms/assets/default/vendors/apexcharts/apexcharts.min.js"></script>
    <script src="/assets/lms/assets/default/vendors/chartjs/chart.min.js"></script>

    <script>
        var offlineSuccess = '{{ trans('lms/panel.offline_success') }}';
        var $chartDataMonths = @json($monthlyChart['months']);
        var $chartData = @json($monthlyChart['data']);
    </script>

    <script src="/assets/lms/assets/default/js/panel/dashboard.min.js"></script>
@endpush
