    ...
    <div class="col-12">
        <div class="accordion-content-wrapper mt-15" id="chapterAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="tablist" aria-multiselectable="true">
                        ...
                        <li data-id="{{ !empty($chapter) ? $chapter->id :'' }}" data-chapter-order="{{ $chapter->order }}" class="accordion-row bg-white rounded-sm panel-shadow mt-20 py-15 py-lg-30 px-10 px-lg-20">
                            <div class="d-flex align-items-center justify-content-between " role="tab" id="chapter_{{ !empty($chapter) ? $chapter->id :'record' }}">
                                <div class="d-flex align-items-center" href="#collapseChapter{{ !empty($chapter) ? $chapter->id :'record' }}" aria-controls="collapseChapter{{ !empty($chapter) ? $chapter->id :'record' }}" data-parent="#chapterAccordion" role="button" data-toggle="collapse" aria-expanded="true">
                                    ...
                                    <div class="">
                                        <span class="font-weight-bold text-dark-blue d-block">{{ !empty($chapter) ? $chapter->title : trans('lms/public.add_new_chapter') }}</span>
                                        <span class="font-12 text-gray d-block">
                                            {{ !empty($chapter->chapterItems) ? count($chapter->chapterItems) : 0 }} {{ trans('lms/public.topic') }}
                                            | {{ convertMinutesToHourAndMinute($chapter->getDuration()) }} {{ trans('lms/public.hr') }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    @if($chapter->status != \App\Models\LMS\WebinarChapter::$chapterActive)
                                        ...
                                        <div class="dropdown-menu ">
                                            ...
                                            @if(getFeaturesSettings('new_interactive_file'))
                                                <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $webinar->id }}" data-type="new_interactive_file" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">
                                                    {{ trans('lms/update.new_interactive_file') }}
                                                </button>
                                            @endif
                                            ...
                                            <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $webinar->id }}" data-type="text_lesson" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">
                                                {{ trans('lms/public.add_text_lesson') }}
                                            </button>
                                            ...
                                            @if(getFeaturesSettings('webinar_assignment_status'))
                                                <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $webinar->id }}" data-type="assignment" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">
                                                    {{ trans('lms/update.add_new_assignments') }}
                                                </button>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <a href="/lms/panel/chapters/{{ $chapter->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
                                        ...
                                        @if(!empty($chapter->chapterItems) and count($chapter->chapterItems))
                                            <ul class="draggable-content-lists draggable-lists-chapter-{{ $chapter->id }}" data-drag-class="draggable-lists-chapter-{{ $chapter->id }}" data-order-table="webinar_chapter_items">
                                                @foreach($chapter->chapterItems as $chapterItem)
                                                    @if($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterSession and !empty($chapterItem->session))
                                                        @include('lms.web.default.panel.webinar.create_includes.accordions.session' ,['session' => $chapterItem->session , 'chapter' => $chapter, 'chapterItem' => $chapterItem])
                                                    @elseif($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterFile and !empty($chapterItem->file))
                                                        @include('lms.web.default.panel.webinar.create_includes.accordions.file' ,['file' => $chapterItem->file , 'chapter' => $chapter, 'chapterItem' => $chapterItem])
                                                    @elseif($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterTextLesson and !empty($chapterItem->textLesson))
                                                        @include('lms.web.default.panel.webinar.create_includes.accordions.text-lesson' ,['textLesson' => $chapterItem->textLesson , 'chapter' => $chapter, 'chapterItem' => $chapterItem])
                                                    @elseif($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterAssignment and !empty($chapterItem->assignment))
                                                        @include('lms.web.default.panel.webinar.create_includes.accordions.assignment' ,['assignment' => $chapterItem->assignment , 'chapter' => $chapter, 'chapterItem' => $chapterItem])
                                                    @elseif($chapterItem->type == \App\Models\LMS\WebinarChapterItem::$chapterQuiz and !empty($chapterItem->quiz))
                                                        @include('lms.web.default.panel.webinar.create_includes.accordions.quiz' ,['quizInfo' => $chapterItem->quiz , 'chapter' => $chapter, 'chapterItem' => $chapterItem])
                                                    @endif
                                                @endforeach
                                            </ul>
                                        @else
                                            @include('lms.' . getTemplate() . '.includes.no-result',[
                                                'file_name' => 'meet.png',
                                                'title' => trans('lms/update.chapter_content_no_result'),
                                                'hint' => trans('lms/update.chapter_content_no_result_hint'),
                                            ])
                                        @endif
                                    </div>
            ...
            @else
                @include('lms.' . getTemplate() . '.includes.no-result',[
                    'file_name' => 'meet.png',
                    'title' => trans('lms/update.chapter_no_result'),
                    'hint' => trans('lms/update.chapter_no_result_hint'),
                ])
            @endif
        </div>
