...
@section('content')
    ...
    <section class="mt-25">
        <h2 class="section-title">{{ trans('lms/panel.filter_classes') }}</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/webinars/organization_classes" method="get" class="row">
                        ...
                        <div class="col-12 col-lg-5">
                            <div class="form-group">
                                ...
                                <select name="type" class="custom-select">
                                    ...
                                    <option value="webinar" @if(request()->get('type') == 'webinar') selected @endif>{{ trans('lms/webinars.webinar') }}</option>
                                    <option value="course" @if(request()->get('type') == 'course') selected @endif>{{ trans('lms/product.course') }}</option>
                                    <option value="text_lesson" @if(request()->get('type') == 'text_lesson') selected @endif>{{ trans('lms/webinars.text_lesson') }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-12 col-lg-7">
                            <div class="form-group">
                                <label class="input-label">{{ trans('lms/public.sort_by') }}</label>
                                <select name="sort" class="form-control">
                                    <option value="">{{ trans('lms/public.all') }}</option>
                                    <option value="newest" @if(request()->get('sort', null) == 'newest') selected="selected" @endif>{{ trans('lms/public.newest') }}</option>
                                    <option value="expensive" @if(request()->get('sort', null) == 'expensive') selected="selected" @endif>{{ trans('lms/public.expensive') }}</option>
                                    <option value="inexpensive" @if(request()->get('sort', null) == 'inexpensive') selected="selected" @endif>{{ trans('lms/public.inexpensive') }}</option>
                                    <option value="bestsellers" @if(request()->get('sort', null) == 'bestsellers') selected="selected" @endif>{{ trans('lms/public.bestsellers') }}</option>
                                    <option value="best_rates" @if(request()->get('sort', null) == 'best_rates') selected="selected" @endif>{{ trans('lms/public.best_rates') }}</option>
                                </select>
                            </div>
                        </div>
                    </div>
    ...
    <section class="mt-25">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h2 class="section-title">{{ trans('lms/panel.organization_classes') }}</h2>

            <form action="/lms" method="get">
                <div class="d-flex align-items-center flex-row-reverse flex-md-row justify-content-start justify-content-md-center mt-20 mt-md-0">
                    <label class="cursor-pointer mb-0 mr-10 text-gray font-14 font-weight-500" for="freeClassesSwitch">{{ trans('lms/panel.only_free_classes') }}</label>
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <a href="/lms{{ $webinar->getUrl() }}" target="_blank">
                                        <h3 class="font-16 text-dark-blue font-weight-bold">{{ $webinar->title }}
                                            <span class="badge badge-dark status-badge-dark ml-10">{{ trans('lms/webinars.'.$webinar->type) }}</span>

                                            @if($webinar->private)
                                                <span class="badge badge-danger status-badge-danger ml-10">{{ trans('lms/webinars.private') }}</span>
                                            @endif
                                        </h3>
                                    </a>
                                </div>
        ...
        @else
            @include('lms.' . getTemplate() . '.includes.no-result',[
                ...
                'btn' => ['url' => '/lms/panel/webinar/new','text' => trans('lms/panel.create_a_webinar') ]
            ])
        @endif
