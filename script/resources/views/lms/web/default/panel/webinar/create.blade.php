...
@section('content')
    <div class="">
        ...
        <form method="post" action="/lms/panel/webinars/{{ !empty($webinar) ? $webinar->id .'/update' : 'store' }}" id="webinarForm" class="webinar-form">
            @include('lms.web.default.panel.webinar.create_includes.progress')
            ...
            @if($currentStep == 1)
                @include('lms.web.default.panel.webinar.create_includes.step_1')
            @elseif(!empty($webinar))
                @include('lms.web.default.panel.webinar.create_includes.step_'.$currentStep)
            @endif
        ...
        <div class="create-webinar-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class="d-flex align-items-center">
                ...
                @if(!empty($webinar))
                    <a href="/lms/panel/webinars/{{ $webinar->id }}/step/{{ ($currentStep - 1) }}" class="btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('lms/webinars.previous') }}</a>
                @else
            ...
            <div class="mt-20 mt-md-0">
                ...
                @if(!empty($webinar) and $webinar->creator_id == $authUser->id)
                    <a href="/lms/panel/webinars/{{ $webinar->id }}/delete?redirect_to=/panel/webinars" class="delete-action webinar-actions btn btn-sm btn-danger mt-20 mt-md-0">{{ trans('lms/public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
...
@push('scripts_bottom')
    <script>
        ...
        var zoomJwtTokenInvalid = '{{ trans('lms/webinars.zoom_jwt_token_invalid') }}';
    ...
    <script src="/assets/lms/assets/default/js/panel/webinar.min.js"></script>
