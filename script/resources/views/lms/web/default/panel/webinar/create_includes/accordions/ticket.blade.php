<li data-id="{{ !empty($ticket) ? $ticket->id :'' }}" class="accordion-row bg-white rounded-sm panel-shadow mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="ticket_{{ !empty($ticket) ? $ticket->id :'record' }}">
        <div class="font-weight-bold text-dark-blue" href="#collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" aria-controls="collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" data-parent="#ticketsAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <span>{{ !empty($ticket) ? $ticket->title : trans('lms/public.add_new_ticket') }}</span>
        </div>
            ...
            @if(!empty($ticket))
                <div class="btn-group dropdown table-actions mr-15 font-weight-normal">
                    ...
                    <div class="dropdown-menu">
                        <a href="/lms/panel/tickets/{{ $ticket->id }}/delete" class="delete-action btn btn-sm btn-transparent">{{ trans('lms/public.delete') }}</a>
                    </div>
                </div>
            @endif
    ...
    <div id="collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" aria-labelledby="ticket_{{ !empty($ticket) ? $ticket->id :'record' }}" class=" collapse @if(empty($ticket)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form ticket-form" data-action="/lms/panel/tickets/{{ !empty($ticket) ? $ticket->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][title]" class="js-ajax-title form-control" value="{{ !empty($ticket) ? $ticket->title :'' }}" placeholder="{{ trans('lms/forms.maximum_64_characters') }}"/>
                ...
                <div class="row">
                    <div class="col-12 col-lg-4">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/public.discount') }} <span class="braces">(%)</span></label>
                        ...
                        <div class="form-group">
                            <label class="input-label d-block">{{ trans('lms/public.capacity') }}</label>
                            @if(empty($ticket) and !empty($webinar->capacity) and !empty($sumTicketsCapacities))
                                <span class="test-gray font-12 d-block">{{ trans('lms/panel.remaining') }}: <span class="js-ticket-remaining-capacity">{{ $webinar->capacity - $sumTicketsCapacities }}</span></span>
                            @endif

                            <input type="text" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][capacity]" class="js-ajax-capacity form-control mt-10" value="{{ !empty($ticket) ? $ticket->capacity :'' }}" placeholder="{{ $webinar->isWebinar() ? trans('lms/webinars.empty_means_webinar_capacity') : trans('lms/forms.empty_means_unlimited') }}"/>
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-12 col-lg-6">
                        <div class="row">
                            ...
                            <div class="col-12 col-lg-6 mt-15 mt-lg-0">
                                <div class="form-group">
                                    <label class="input-label">{{ trans('lms/webinars.end_date') }}</label>
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text" id="dateRangeLabel">
                                                <i data-feather="calendar" width="18" height="18" class="text-white"></i>
                ...
                <div class="mt-30 d-flex align-items-center">
                    <button type="button" class="js-save-ticket btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>
