<li data-id="{{ !empty($prerequisite) ? $prerequisite->id :'' }}" class="accordion-row bg-white rounded-sm panel-shadow mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="prerequisite_{{ !empty($prerequisite) ? $prerequisite->id :'record' }}">
        <div class="font-weight-bold text-dark-blue" href="#collapsePrerequisite{{ !empty($prerequisite) ? $prerequisite->id :'record' }}" aria-controls="collapsePrerequisite{{ !empty($prerequisite) ? $prerequisite->id :'record' }}" data-parent="#prerequisitesAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <span>{{ (!empty($prerequisite) and !empty($prerequisite->prerequisiteWebinar)) ? $prerequisite->prerequisiteWebinar->title .' - '. $prerequisite->prerequisiteWebinar->teacher->full_name : trans('lms/public.add_new_prerequisites') }}</span>
        </div>
            ...
            @if(!empty($prerequisite))
                <div class="btn-group dropdown table-actions mr-15">
                    ...
                    <div class="dropdown-menu">
                        <a href="/lms/panel/prerequisites/{{ $prerequisite->id }}/delete" class="delete-action btn btn-sm btn-transparent">{{ trans('lms/public.delete') }}</a>
                    </div>
                </div>
            @endif
    ...
    <div id="collapsePrerequisite{{ !empty($prerequisite) ? $prerequisite->id :'record' }}" aria-labelledby="prerequisite_{{ !empty($prerequisite) ? $prerequisite->id :'record' }}" class=" collapse @if(empty($prerequisite)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="prerequisite-form" data-action="/lms/panel/prerequisites/{{ !empty($prerequisite) ? $prerequisite->id . '/update' : 'store' }}">
                <input type="hidden" name="ajax[{{ !empty($prerequisite) ? $prerequisite->id : 'new' }}][webinar_id]" value="{{ !empty($webinar) ? $webinar->id :'' }}">

                <div class="row">
                    <div class="col-12 col-lg-6">
                        <div class="form-group mt-15">
                            <label class="input-label d-block">{{ trans('lms/public.select_prerequisites') }}</label>
                            <select name="ajax[{{ !empty($prerequisite) ? $prerequisite->id : 'new' }}][prerequisite_id]" class="js-ajax-prerequisite_id @if(empty($prerequisite)) form-control @endif prerequisites-select2" data-webinar-id="{{  !empty($webinar) ? $webinar->id : '' }}" data-placeholder="{{ trans('lms/public.search_prerequisites') }}">
                        ...
                        <div class="form-group mt-30 d-flex align-items-center justify-content-between mb-0">
                            <label class="cursor-pointer input-label" for="requiredPrerequisitesSwitch{{ !empty($prerequisite) ? $prerequisite->id : 'record' }}">{{ trans('lms/public.required') }}</label>
                        ...
                        <div class="mt-5">
                            <p class="font-12 text-gray">- {{ trans('lms/webinars.required_hint') }}</p>
                        </div>
                ...
                <div class="mt-30 d-flex align-items-center">
                    <button type="button" class="js-save-prerequisite btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>
