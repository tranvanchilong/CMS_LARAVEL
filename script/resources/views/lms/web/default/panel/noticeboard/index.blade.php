...
@section('content')
    @if(empty($isCourseNotice))
        <section>
            <h2 class="section-title">{{ trans('lms/update.noticeboard_statistics') }}</h2>
                ...
                <div class="row">
                    <div class="col-4 d-flex align-items-center justify-content-center">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <span class="font-16 text-dark-blue text-gray font-weight-500">{{ trans('lms/update.total_noticeboards') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="col-4 d-flex align-items-center justify-content-center">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <span class="font-16 text-dark-blue text-gray font-weight-500">{{ trans('lms/update.course_notices') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="col-4 d-flex align-items-center justify-content-center">
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <span class="font-16 text-gray font-weight-500">{{ trans('lms/update.general_notices') }}</span>
                        </div>
                    </div>
    ...
    <section class="mt-25">
        <h2 class="section-title">{{ trans('lms/update.filter_noticeboards') }}</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms{{ request()->url() }}" method="get" class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 {{ !empty($isCourseNotice) ? 'col-lg-5' : 'col-lg-8' }}">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" class="form-control" value="{{ request()->get('title') }}" placeholder="{{ trans('lms/public.search') }}">
                            </div>
                        </div>
                            ...
                            <div class="col-12 col-lg-3">
                                <div class="form-group">
                                    ...
                                    <select name="color" class="form-control select2">
                                        <option value="">{{ trans('lms/update.all_colors') }}</option>

                                        @foreach(\App\Models\LMS\CourseNoticeboard::$colors as $noticeColor)
                                            <option value="{{ $noticeColor }}" @if(request()->get('color') == $noticeColor) selected @endif>{{ trans('lms/update.course_noticeboard_color_'.$noticeColor) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <section class="mt-20">
        <h2 class="section-title">{{ trans('lms/panel.noticeboards') }}</h2>
                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table custom-table text-center ">
                                <thead>
                                <tr>
                                    <th class="text-left text-gray">{{ trans('lms/webinars.title') }}</th>
                                    <th class="text-center text-gray">{{ trans('lms/site.message') }}</th>

                                    @if(!empty($isCourseNotice) and $isCourseNotice)
                                        <th class="text-center text-gray">{{ trans('lms/update.color') }}</th>
                                    @else
                                        <th class="text-center text-gray">{{ trans('lms/public.type') }}</th>
                                    @endif
                                ...
                                @foreach($noticeboards as $noticeboard)
                                    <tr class="noticeboard-item">
                                        ...
                                        <td class="align-middle">
                                            <button type="button" class="js-view-message btn btn-sm btn-gray200">{{ trans('lms/public.view') }}</button>
                                        ...
                                        <td class="text-dark-blue font-weight-500 align-middle">
                                            ...
                                            @else
                                                @if(!empty($noticeboard->instructor_id) and !empty($noticeboard->webinar_id))
                                                    {{ trans('lms/product.course') }}
                                                @else
                                                    {{ trans('lms/public.'.$noticeboard->type) }}
                                                @endif
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-right align-middle">
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a href="/lms/panel/{{ (!empty($isCourseNotice) and $isCourseNotice) ? 'course-noticeboard' : 'noticeboard' }}/{{ $noticeboard->id }}/edit" class="webinar-actions d-block mt-10 text-hover-primary">{{ trans('lms/public.edit') }}</a>
                                                    <a href="/lms/panel/{{ (!empty($isCourseNotice) and $isCourseNotice) ? 'course-noticeboard' : 'noticeboard' }}/{{ $noticeboard->id }}/delete" class="delete-action webinar-actions d-block mt-10 text-hover-primary">{{ trans('lms/public.delete') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <div class="my-30">
        {{ $noticeboards->appends(request()->input())->links('lms.vendor.pagination.panel') }}
    </div>
