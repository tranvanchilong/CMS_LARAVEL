...
@section('content')
    <section>
        <div class="d-flex align-items-center justify-content-between">
            <h2 class="section-title">{{ trans('lms/panel.notifications') }}</h2>

            <a href="/lms/panel/notifications/mark-all-as-read" class="delete-action d-flex align-items-center cursor-pointer text-hover-primary" data-title="{{ trans('lms/update.convert_unread_messages_to_read') }}" data-confirm="{{ trans('lms/update.yes_convert') }}">
                <i data-feather="check" width="20" height="20"></i>
                <span class="ml-5 font-16">{{ trans('lms/update.mark_all_notifications_as_read') }}</span>
            </a>
        </div>
        ...
        @if(!empty($notifications) and !$notifications->isEmpty())
                ...
                <div class="notification-card rounded-sm panel-shadow bg-white py-15 py-lg-20 px-15 px-lg-40 mt-20">
                    <div class="row align-items-center">
                        ...
                        <div class="col-12 col-lg-4 mt-10 mt-lg-0 text-right">
                            <button type="button" data-id="{{ $notification->id }}" id="showNotificationMessage{{ $notification->id }}" class="js-show-message btn btn-border-white @if(!empty($notification->notificationStatus)) seen-at @endif">{{ trans('lms/public.view') }}</button>
            ...
            <div class="my-30">
                {{ $notifications->appends(request()->input())->links('lms.vendor.pagination.panel') }}
            </div>
        @else
            @include('lms.' . getTemplate() . '.includes.no-result',[
               'file_name' => 'webinar.png',
               'title' => trans('lms/panel.notification_no_result'),
               'hint' => nl2br(trans('lms/panel.notification_no_result_hint')),
           ])
        @endif
    </section>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/notifications.min.js"></script>
@endpush
