...
@section('content')
    ...
    <section>
        <h2 class="section-title">{{ trans('lms/panel.create_a_new_discount') }}</h2>
        @if (\Session::has('msg'))
        ...
        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/marketing/special_offers/store" method="post" class="row">
                {{ csrf_field() }}
                ...
                <div class="col-12 col-lg-6">
                    <div class="row">
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="form-group">
                                <label class="input-label d-block">{{ trans('lms/panel.webinar') }}</label>
                                <select name="webinar_id"
                                        class="form-control custom-select @error('webinar_id')  is-invalid @enderror">
                                    <option selected disabled>{{ trans('lms/panel.select_course') }}</option>
                        ...
                        <div class="col-12 col-lg-3">
                            <div class="form-group">
                                <label class="input-label">{{ trans('lms/panel.amount') }}(%)</label>
                ...
                <div class="col-12 col-lg-1 d-flex align-items-center justify-content-end">
                    <button type="button" id="formSubmit" class="btn btn-sm btn-primary">{{ trans('lms/public.create') }}</button>
                </div>
            </form>
        </div>
    </section>
    ...
    @if(!empty($specialOffers) and $specialOffers->count() > 0)
        ...
        <section class="mt-35">
            <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
                <h2 class="section-title">{{ trans('lms/panel.discounts') }}</h2>

                <form action="/lms" method="get" class="d-flex align-items-center flex-row-reverse flex-md-row justify-content-start justify-content-md-center mt-20 mt-md-0">
                    <label class="cursor-pointer mb-0 mr-10 text-gray font-14 font-weight-500" for="activeDiscountsSwitch">{{ trans('lms/panel.show_only_active_discounts') }}</label>
                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table custom-table text-center">
                                <thead>
                                <tr>
                                    <th class="text-left text-gray">{{ trans('lms/panel.name') }}</th>
                                    <th class="text-left text-gray">{{ trans('lms/panel.webinar') }}</th>
                                    <th class="text-center text-gray">{{ trans('lms/panel.amount') }}</th>
                                    <th class="text-center text-gray">{{ trans('lms/public.status') }}</th>
                                    <th class="text-center text-gray">{{ trans('lms/public.from') }}</th>
                                    <th class="text-center text-gray">{{ trans('lms/public.to') }}</th>
                                    <th></th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($specialOffers as $specialOffer)
                                    <tr>
                                        ...
                                        <td class="text-left align-middle">
                                            <a href="/lms{{ $specialOffer->webinar->getUrl() }}" class="text-dark-blue font-weight-500" target="_blank">{{ $specialOffer->webinar->title }}</a>
                                        </td>
                                        ...
                                        <td class="align-middle">
                                            @switch($specialOffer->status)
                                                @case(\App\Models\LMS\SpecialOffer::$active)
                                                <span class="text-primary font-weight-500">{{ trans('lms/public.active') }}</span>

                                                @break
                                                @case(\App\Models\LMS\SpecialOffer::$inactive)
                                                <span class="text-warning font-weight-500">{{ trans('lms/public.inactive') }}</span>
                                                @break
                                            @endswitch
                                        </td>
                                        ...
                                        <td class="text-right align-middle">
                                            @if($specialOffer->status != \App\Models\LMS\SpecialOffer::$inactive)
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a href="/lms/panel/marketing/special_offers/{{ $specialOffer->id }}/disable" type="button"
                                                           class="delete-action btn-transparent">{{ trans('lms/public.disable') }}</a>
                                                    </div>
                                                </div>
                                            @endif
                                        </td>
        ...
        <div class="my-30">
            {{ $specialOffers->appends(request()->input())->links('lms.vendor.pagination.panel') }}
        </div>
    ...
    @else
        ...
        @include('lms.' . getTemplate() . '.includes.no-result',[
            'file_name' => 'offer.png',
            'title' => trans('lms/panel.discount_no_result'),
            'hint' =>  nl2br(trans('lms/panel.discount_no_result_hint')) ,
        ])
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/marketing/special_offers.min.js"></script>
@endpush
