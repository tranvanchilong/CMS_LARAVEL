...
@section('content')
    <section class="">
        <h2 class="section-title">{{ trans('lms/panel.select_promotion_plan') }}</h2>
            ...
            @foreach($promotions as $promotion)
                <div class="col-12 col-sm-6 col-lg-3 mt-15">
                    <div class="subscribe-plan position-relative bg-white d-flex flex-column align-items-center rounded-sm shadow pt-50 pb-20 px-20">
                        ...
                        <div class="plan-icon">
                            <img src="{{get_path_lms()}}{{ $promotion->icon }}" class="img-cover" alt="">
                        </div>

                        <h3 class="mt-20 font-30 text-secondary subscribe-plan-title">{{ $promotion->title }}</h3>
                        <p class="font-weight-500 text-gray mt-10">{{ trans('lms/panel.promotion_days',['day' => $promotion->days]) }}</p>

                        <div class="d-flex align-items-start text-primary mt-30">
                            <span class="font-36 line-height-1 subscribe-plan-price">{{ (!empty($promotion->price) and $promotion->price > 0) ? handlePrice($promotion->price) : trans('lms/public.free') }}</span>
                        </div>

                        <p class="text-dark-blue font-14 mt-25">{{ nl2br($promotion->description) }}</p>

                        <button type="button" data-promotion-id="{{ $promotion->id }}"
                                class="js-pay-promotion btn btn-primary btn-block mt-50">{{ trans('lms/update.purchase') }}</button>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    @if($promotionSales->count() > 0)
        <section class="mt-35">
            <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
                <h2 class="section-title">{{ trans('lms/panel.promotions_history') }}</h2>
                ...
                <div
                    ...
                    <label class="mb-0 mr-10 text-gray font-14 font-weight-500"
                           for="activePromotionSwitch">{{ trans('lms/panel.show_only_active_promotions') }}</label>
                        ...
                        <div class="table-responsive">
                            <table class="table custom-table text-center ">
                                ...
                                <tr>
                                    ...
                                    <th class="text-center text-gray">{{ trans('lms/panel.plan') }}</th>
                                    <th class="text-center text-gray">{{ trans('lms/public.price') }}</th>
                                ...
                                @foreach($promotionSales as $promotionSale)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <span class="text-dark-blue font-weight-500">{{ (!empty($promotionSale->promotion->price) and $promotionSale->promotion->price > 0) ? handlePrice($promotionSale->promotion->price) : trans('lms/public.free') }}</span>
                                        </td>
    ...
    @else
        @include('lms.' . getTemplate() . '.includes.no-result',[
            ...
            'title' => trans('lms/panel.promotion_no_result'),
            'hint' =>  nl2br(trans('lms/panel.promotion_no_result_hint')) ,
        ])

    @endif

    <div class="my-30">
        {{ $promotionSales->appends(request()->input())->links('lms.vendor.pagination.panel') }}
    </div>

    <div id="promotionModal" class="d-none">
        <form action="/lms/panel/marketing/pay-promotion" method="post">
            {{ csrf_field() }}
            <input type="hidden" name="promotion_id" value="">


            <h3 class="section-title after-line">{{ trans('lms/panel.promote_the_webinar') }}</h3>
            <div class="mt-25 d-flex flex-column align-items-center">
                ...
                <p class="mt-10">{{ trans('lms/panel.select_webinar_for_promotion') }}</p>
                <div class="w-75">

                    <div class="mt-15 d-flex justify-content-between">
                        <span class="text-gray font-weight-bold">{{ trans('lms/panel.plan') }}:</span>
                        <span class="text-gray modal-title"></span>
                    </div>

                    <div class="mt-10 d-flex justify-content-between">
                        <span class="text-gray font-weight-bold">{{ trans('lms/public.price') }}:</span>
            ...
            <div class="mt-30 d-flex align-items-center justify-content-end">
                <button type="button"
                        class="btn btn-sm btn-primary js-submit-promotion">{{ trans('lms/panel.pay') }}</button>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/marketing/promotions.min.js"></script>
@endpush
