...
@section('content')
    ...
    @if(!empty($accounting))
        <div class="d-flex align-items-center mb-20 p-15 success-transparent-alert">
            ...
            <div class="ml-10">
                <div class="font-14 font-weight-bold ">{{ trans('lms/update.you_got_the_bonus') }}</div>
                <div class="font-12 ">{{ trans('lms/update.your_registration_bonus_was_unlocked_on_date',['date' => dateTimeFormat($accounting->created_at, 'j M Y')]) }}</div>
            </div>
        </div>
    @endif

    <section class="">
        <h2 class="section-title">{{ trans('lms/update.registration_bonus') }}</h2>
            ...
            <div class="row">
                ...
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/update.registration_bonus') }}</span>
                    </div>
                </div>
                ...
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <strong class="font-36 font-weight-bold mt-5 {{ !empty($accounting) ? 'text-primary' : 'text-danger' }}">{{ !empty($accounting) ? trans('lms/update.unlocked') : trans('lms/update.locked') }}</strong>
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/update.bonus_status') }}</span>
                    </div>
                </div>

                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        <img src="/assets/lms/assets/default/img/activity/computer.png" width="64" height="64" alt="">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/update.bonus_date') }}</span>
                    </div>
                </div>
                ...
                <div class="p-15 rounded-sm panel-shadow bg-white h-100">
                    <div class="row">
                        <div class="col-5 d-flex align-items-center justify-content-center">
                            <img src="/assets/lms/assets/default/img/rewards/registration_bonus.png" class="img-fluid" alt="{{ trans('lms/update.registration_bonus') }}">
                        </div>

                        <div class="col-7">
                            <h4 class="font-16 font-weight-bold">{{ trans('lms/update.bonus_status') }}</h4>

                            <p class="mt-10 font-14 font-weight-500 text-gray">{{ trans('lms/update.your_bonus_is_locked_To_unlock_the_bonus_please_check_the_following_statuses') }}:</p>
                            ...
                            @if(!empty($registrationBonusSettings['number_of_referred_users']))
                                <div class="d-flex align-items-center position-relative mt-15 p-15 border border-gray200 rounded-lg">
                                    ...
                                    <div class="ml-5">
                                        <span class="d-block font-14 font-weight-bold text-gray">{{ trans('lms/update.referred_users') }}</span>
                                        <span class="d-block font-12 text-gray">{{ $bonusStatusReferredUsersChart['complete'] == 0 ? trans('lms/update.you_havent_referred_any_users') : trans('lms/update.you_referred_count_users_to_the_platform',['count' => "{$bonusStatusReferredUsersChart['referred_users']}/{$registrationBonusSettings['number_of_referred_users']}"]) }}</span>
                                    </div>
                            ...
                            @if($purchaseAmountCount)
                                <div class="d-flex align-items-center position-relative mt-15 p-15 border border-gray200 rounded-lg">
                                    ...
                                    <div class="ml-5">
                                        <span class="d-block font-14 font-weight-bold text-gray">{{ trans('lms/update.users_purchases') }}</span>
                                        <span class="d-block font-12 text-gray">{{ $bonusStatusUsersPurchasesChart['complete'] == 0 ? trans('lms/update.you_havent_referred_any_users_to_purchase') : trans('lms/update.count_users_achieved_purchase_target',['count' => "{$bonusStatusUsersPurchasesChart['reached_user_purchased']}/{$bonusStatusUsersPurchasesChart['total_user_purchased']}"]) }}</span>
                                    </div>
        ...
        @if(!empty($registrationBonusTermsSettings) and !empty($registrationBonusTermsSettings['items']))
            <div class="mt-25 {{ (!empty($registrationBonusSettings['number_of_referred_users']) or !empty($registrationBonusSettings['purchase_amount_for_unlocking_bonus'])) ? 'col-12 col-md-6' : 'col-12' }}">
                <div class="p-15 rounded-sm panel-shadow bg-white h-100">
                    <div class="row">
                        <div class="col-7">
                            <h4 class="font-16 font-weight-bold mb-20">{{ trans('lms/update.how_to_get_bonus') }}</h4>
                                ...
                                @if(!empty($termItem['icon']) and !empty($termItem['title']) and !empty($termItem['description']))
                                    <div class="how-to-get-bonus-items d-flex align-items-start">
                                        <div class="icon-box d-flex align-items-center justify-content-center">
                                            <img src="{{get_path_lms()}}{{ $termItem['icon'] }}" alt="{{ $termItem['title'] }}" width="16" height="16">
                                        </div>
                                        <div class="ml-10">
                        ...
                        @if(!empty($registrationBonusTermsSettings['term_image']))
                            <div class="col-5 d-flex align-items-center justify-content-center">
                                <img src="{{get_path_lms()}}{{ $registrationBonusTermsSettings['term_image'] }}" class="img-fluid" alt="{{ trans('lms/update.how_to_get_bonus') }}">
                            </div>
                        @endif
                    </div>
                </div>
    ...
    @if($checkReferralUserCount)
        <section class="mt-25">
            <h2 class="section-title">{{ trans('lms/update.referral_history') }}</h2>
                        ...
                        <div class="col-12 ">
                            <div class="table-responsive">
                                <table class="table text-center custom-table">
                                    <thead>
                                    <tr>
                                        <th>{{ trans('lms/panel.user') }}</th>
                                        @if($purchaseAmountCount)
                                            <th class="text-center">{{ trans('lms/update.purchase_status') }}</th>
                                        @endif
                                        <th class="text-right">{{ trans('lms/panel.registration_date') }}</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            @if($purchaseAmountCount)
                                                <td>
                                                    @if((!empty($registrationBonusSettings['purchase_amount_for_unlocking_bonus']) and $user->totalPurchase >= $registrationBonusSettings['purchase_amount_for_unlocking_bonus']) or (empty($registrationBonusSettings['purchase_amount_for_unlocking_bonus']) and $user->totalPurchase > 0))
                                                        <span class="font-16 font-weight-500 text-primary">{{ trans('lms/update.reached') }}</span>
                                                    @else
                                                        <span class="font-16 font-weight-500 text-dark">{{ trans('lms/update.not_reached') }}</span>
                                                    @endif
                                                </td>
                                            @endif
            ...
            @else
                <div class="no-result my-50 d-flex align-items-center justify-content-center flex-column">
                    <div class="no-result-logo">
                        <img src="/assets/lms/assets/default/img/no-results/no_followers.png" alt="{{ trans('lms/update.no_referred_users') }}">
                    </div>
                    <div class="d-flex align-items-center flex-column mt-30 text-center">
                        <h2>{{ trans('lms/update.no_referred_users') }}</h2>
                        <p class="mt-5 text-center">{{ trans('lms/update.you_havent_referred_any_users_yet') }}</p>
                    </div>
                </div>
            @endif
        </section>
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/registration_bonus.min.js"></script>
