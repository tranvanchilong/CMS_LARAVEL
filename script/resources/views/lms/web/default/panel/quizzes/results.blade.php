...
@section('content')
    <section>
        <h2 class="section-title">{{ trans('lms/quiz.results_statistics') }}</h2>

        <div class="activities-container mt-25 p-20 p-lg-35">
            <div class="row">
                <div class="col-6 col-md-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        <img src="/assets/lms/assets/default/img/activity/43.svg" width="64" height="64" alt="">
                        <strong class="font-30 text-dark-blue font-weight-bold mt-5">{{ $waitingCount }}</strong>
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/quiz.need_to_review') }}</span>
                    </div>
                </div>

                <div class="col-6 col-md-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        <img src="/assets/lms/assets/default/img/activity/42.svg" width="64" height="64" alt="">
                        <strong class="font-30 text-dark-blue font-weight-bold mt-5">{{ $quizResultsCount }}</strong>
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/public.results') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center mt-5 mt-md-0">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/quiz.average_grade') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center mt-5 mt-md-0">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/quiz.success_rate') }}</span>
                    </div>
                </div>
    ...
    <section class="mt-25">
        <h2 class="section-title">{{ trans('lms/quiz.filter_results') }}</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/quizzes/results" method="get" class="row">
                            ...
                            <div class="row">
                                <div class="col-12 col-lg-7">
                                    <div class="form-group">
                                        ...
                                        <select name="user_id" class="form-control select2" data-placeholder="{{ trans('lms/public.all') }}">
                                            <option value="all">{{ trans('lms/public.all') }}</option>
                                ...
                                <div class="col-12 col-lg-5">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="status" name="status">
                                            <option value="all">{{ trans('lms/public.all') }}</option>
                                            <option value="passed" {{ request()->get('status','') === "passed" ? 'selected' : '' }}>{{ trans('lms/quiz.passed') }}</option>
                                            <option value="failed" {{ request()->get('status','') === "failed" ? 'selected' : '' }}>{{ trans('lms/quiz.failed') }}</option>
                                            <option value="waiting" {{ request()->get('status','') === "waiting" ? 'selected' : '' }}>{{ trans('lms/quiz.waiting') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
    ...
    <section class="mt-35">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h2 class="section-title">{{ trans('lms/quiz.student_results') }}</h2>

            <form action="/lms/panel/quizzes/results" method="get" class="">
                <div class="d-flex align-items-center flex-row-reverse flex-md-row justify-content-start justify-content-md-center mt-20 mt-md-0">
                    <label class="mb-0 mr-10 cursor-pointer font-14 text-gray font-weight-500" for="onlyOpenQuizzesSwitch">{{ trans('lms/quiz.show_only_open_results') }}</label>
                        ...
                        <div class="table-responsive">
                            <table class="table custom-table">
                                ...
                                <tr>
                                    ...
                                    <th>{{ trans('lms/quiz.quiz') }}</th>
                                    <th class="text-center">{{ trans('lms/quiz.quiz_grade') }}</th>
                                    <th class="text-center">{{ trans('lms/quiz.student_grade') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            @switch($result->status)
                                                ...
                                                <span class="text-primary">{{ trans('lms/quiz.passed') }}</span>
                                                @break
                                            ...
                                            <div class="btn-group dropdown table-actions table-actions-lg table-actions-lg">
                                                ...
                                                <div class="dropdown-menu font-weight-normal">
                                                    @if($result->status != 'waiting')
                                                        <a href="/lms/panel/quizzes/{{ $result->id }}/result" class="webinar-actions d-block mt-10">{{ trans('lms/public.view') }}</a>
                                                    @endif

                                                    @if($result->status == 'waiting')
                                                        <a href="/lms/panel/quizzes/{{ $result->id }}/edit-result" class="webinar-actions d-block mt-10">{{ trans('lms/public.review') }}</a>
                                                    @endif

                                                    <a href="/lms/panel/quizzes/results/{{ $result->id }}/delete" class="webinar-actions d-block mt-10 delete-action">{{ trans('lms/public.delete') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
        ...
        @else
            ...
            @include('lms.' . getTemplate() . '.includes.no-result',[
                    ...
                    'title' => trans('lms/quiz.quiz_result_no_result'),
                    'hint' => trans('lms/quiz.quiz_result_no_result_hint'),
                ])
        @endif
    </section>

    <div class="my-30">
        {{ $quizzesResults->appends(request()->input())->links('lms.vendor.pagination.panel') }}
    </div>
@endsection
