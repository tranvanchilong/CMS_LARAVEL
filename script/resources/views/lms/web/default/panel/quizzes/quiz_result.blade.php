...
@section('content')
    <div class="container">
        <section class="mt-40">
            ...
            <p class="text-gray font-14 mt-5">
                ...
                <span class="font-weight-bold">
                    <a href="/lms{{ $quiz->creator->getProfileUrl() }}" target="_blank" class=""> {{ $quiz->creator->full_name }}</a>
                </span>
            </p>
        ...
        <section class="mt-30 quiz-form">
            <form action="/lms{{ !empty($newQuizStart) ? '/panel/quizzes/'. $newQuizStart->quiz->id .'/update-result' : '' }} " method="post">
                {{ csrf_field() }}
                            ...
                            <div class="quiz-card">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        ...
                                        <p class="text-gray font-14 mt-5">
                                            <span>{{ trans('lms/quiz.question_grade') }} : {{ $question->grade }}</span> | <span>{{ trans('lms/quiz.your_grade') }} : {{ (!empty($userAnswers[$question->id]) and !empty($userAnswers[$question->id]["grade"])) ? $userAnswers[$question->id]["grade"] : 0 }}</span>
                                        </p>
                                    </div>
                                ...
                                @if($question->type === \App\Models\LMS\QuizzesQuestion::$descriptive)
                                    ...
                                    <div class="form-group mt-35">
                                        <label class="input-label text-secondary">{{ trans('lms/quiz.student_answer') }}</label>
                                    ...
                                    <div class="form-group mt-35">
                                        <label class="input-label text-secondary">{{ trans('lms/quiz.correct_answer') }}</label>
                                    ...
                                    @if(!empty($newQuizStart) and $newQuizStart->quiz->creator_id == $authUser->id)
                                        <div class="form-group mt-35">
                                            <label class="font-16 text-secondary">{{ trans('lms/quiz.grade') }}</label>
                                    ...
                                    <div class="question-multi-answers mt-35">
                                        @foreach($question->quizzesQuestionsAnswers as $key => $answer)
                                            <div class="answer-item">
                                                @if($answer->correct)
                                                    <span class="badge badge-primary correct">{{ trans('lms/quiz.correct') }}</span>
                                                @endif
                                                    ...
                                                    <label for="asw-{{ $answer->id }}" class="answer-label font-16 d-flex text-dark-blue align-items-center justify-content-center ">
                                                        <span class="answer-title">
                                                            ...
                                                            @if(!empty($userAnswers[$question->id]) and (int)$userAnswers[$question->id]["answer"] ===  $answer->id)
                                                                <span class="d-block">({{ trans('lms/quiz.student_answer') }})</span>
                                                            @endif
                                                        </span>
                                                    </label>
                                                @else
                                                    <label for="asw-{{ $answer->id }}" class="answer-label font-16 d-flex align-items-center text-dark-blue justify-content-center ">
                                                        <div class="image-container">
                                                            @if(!empty($userAnswers[$question->id]) and (int)$userAnswers[$question->id]["answer"] ===  $answer->id)
                                                                <span class="selected font-14">{{ trans('lms/quiz.student_answer') }}</span>
                                                            @endif
                ...
                <div class="d-flex align-items-center mt-30">
                    <button type="button" disabled class="previous btn btn-sm btn-primary mr-20">{{ trans('lms/quiz.previous_question') }}</button>
                    <button type="button" class="next btn btn-primary btn-sm mr-auto">{{ trans('lms/quiz.next_question') }}</button>
