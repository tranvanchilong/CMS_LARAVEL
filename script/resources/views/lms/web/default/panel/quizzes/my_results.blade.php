...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center mt-5 mt-md-0">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/quiz.open_results') }}</span>
                    </div>
                </div>
    ...
    <section class="mt-25">
        ...
        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/quizzes/my-results" method="get" class="row">
    ...
    <section class="mt-35">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h2 class="section-title">{{ trans('lms/quiz.my_quizzes') }}</h2>
                        ...
                        <div class="table-responsive">
                            <table class="table custom-table">
                                ...
                                <tr>
                                    ...
                                    <th class="text-center">{{ trans('lms/quiz.my_grade') }}</th>
                                ...
                                @foreach($quizzesResults as $result)
                                    <tr>
                                        ...
                                        <span class="d-block text-{{ ($result->status == 'passed') ? 'primary' : ($result->status == 'waiting' ? 'warning' : 'danger') }}">
                                            {{ trans('lms/quiz.'.$result->status) }}
                                        </span>

                                            @if($result->status =='failed' and $result->can_try)
                                                <span class="d-block font-12 text-gray">{{ trans('lms/quiz.quiz_chance_remained',['count' => $result->count_can_try]) }}</span>
                                            @endif
                                        </td>
                                            ...
                                            <div class="btn-group dropdown table-actions table-actions-lg table-actions-lg">
                                                ...
                                                <div class="dropdown-menu">
                                                    @if(!$result->can_try and $result->status != 'waiting')
                                                        <a href="/lms/panel/quizzes/{{ $result->id }}/result" class="webinar-actions d-block mt-10">{{ trans('lms/public.view_answers') }}</a>
                                                    @endif

                                                    @if($result->status != 'passed')
                                                        @if($result->can_try)
                                                            <a href="/lms/panel/quizzes/{{ $result->quiz->id }}/start" class="webinar-actions d-block mt-10">{{ trans('lms/public.try_again') }}</a>
                                                        @endif
                                                    @endif

                                                    <a href="/lms{{ $result->quiz->webinar->getUrl() }}" class="webinar-actions d-block mt-10">{{ trans('lms/webinars.webinar_page') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
