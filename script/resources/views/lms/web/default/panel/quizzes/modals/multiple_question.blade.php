<div class="@if(!empty($quiz)) multipleQuestionModal{{ $quiz->id }} @endif {{ empty($question_edit) ? 'd-none' : ''}}">
    <div class="custom-modal-body">
        <h2 class="section-title after-line">{{ trans('lms/quiz.multiple_choice_question') }}</h2>

        <div class="quiz-questions-form" data-action="/lms/panel/quizzes-questions/{{ empty($question_edit) ? 'store' : $question_edit->id.'/update' }}">

            <input type="hidden" name="ajax[quiz_id]" value="{{ !empty($quiz) ? $quiz->id :'' }}">
            <input type="hidden" name="ajax[type]" value="{{ \App\Models\LMS\QuizzesQuestion::$multiple }}">
            ...
            <div class="row mt-25">
                ...
                <div class="col-12 col-md-8">
                    <div class="form-group">
                        <label class="input-label">{{ trans('lms/quiz.question_title') }}</label>
                ...
                <div class="col-12 col-md-6">
                    <div class="form-group">
                        <label class="input-label">{{ trans('lms/public.image') }} ({{ trans('lms/public.optional') }})</label>
                ...
                <div class="col-12 col-md-6">
                    <div class="form-group">
                        <label class="input-label">{{ trans('lms/update.video') }} ({{ trans('lms/public.optional') }})</label>
            ...
            <p class="font-12 text-gray col-12 pb-10">{{ trans('lms/update.quiz_question_image_validation_by_video') }}</p>
            </div>

            <div class="mt-25">
                <h2 class="section-title after-line">{{ trans('lms/public.answers') }}</h2>

                <div class="d-flex justify-content-between align-items-center">
                    <button type="button" class="btn btn-sm btn-primary mt-15 add-answer-btn">{{ trans('lms/quiz.add_an_answer') }}</button>
            ...
            <div class="add-answer-container">
                ...
                @if (!empty($question_edit->quizzesQuestionsAnswers) and !$question_edit->quizzesQuestionsAnswers->isEmpty())
                    @foreach ($question_edit->quizzesQuestionsAnswers as $answer)
                        @include('lms.' . getTemplate() .'.panel.quizzes.modals.multiple_answer_form',['answer' => $answer])
                    @endforeach
                @else
                    @include('lms.' . getTemplate() .'.panel.quizzes.modals.multiple_answer_form')
                @endif
            </div>

            <div class="d-flex align-items-center justify-content-end mt-25">
                <button type="button" class="save-question btn btn-sm btn-primary">{{ trans('lms/public.save') }}</button>
                <button type="button" class="close-swl btn btn-sm btn-danger ml-10">{{ trans('lms/public.close') }}</button>
            </div>
