<div class="">
    <div data-action="/lms{{ !empty($quiz) ? ('/panel/quizzes/'. $quiz->id .'/update') : ('/panel/quizzes/store') }}" class="js-content-form quiz-form webinar-form">

        <section>
            <h2 class="section-title after-line">{{ !empty($quiz) ? (trans('lms/public.edit').' ('. $quiz->title .')') : trans('lms/quiz.new_quiz') }}</h2>
            ...
            <div class="row">
                <div class="col-12 col-md-4">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][time]" value="{{ !empty($quiz) ? $quiz->time : old('time') }}"  class="js-ajax-time form-control" placeholder="{{ trans('lms/forms.empty_means_unlimited') }}" min="0"/>
                        <div class="invalid-feedback"></div>
                    </div>

                    <div class="form-group">
                        <label class="input-label">{{ trans('lms/quiz.number_of_attemps') }}</label>
                        <input type="number" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][attempt]" value="{{ !empty($quiz) ? $quiz->attempt : old('attempt') }}" class="js-ajax-attempt form-control " placeholder="{{ trans('lms/forms.empty_means_unlimited') }}" min="0"/>
                        <div class="invalid-feedback"></div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <p class="font-12 text-gray mt-5">{{ trans('lms/update.quiz_expiry_days_hint') }}</p>
                    </div>
                    ...
                    <div class="form-group mt-20 d-flex align-items-center justify-content-between">
                        <label class="cursor-pointer input-label" for="certificateSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}">{{ trans('lms/quiz.certificate_included') }}</label>
                        <div class="custom-control custom-switch">
                    ...
                    <div class="form-group mt-20 d-flex align-items-center justify-content-between">
                        <label class="cursor-pointer input-label" for="statusSwitch{{ !empty($quiz) ? $quiz->id : 'record' }}">{{ trans('lms/quiz.active_quiz') }}</label>
                        <div class="custom-control custom-switch">
            ...
            <section class="mt-30">
                <div class="d-block d-md-flex justify-content-between align-items-center pb-20">
                    ...
                    <div class="d-flex align-items-center mt-20 mt-md-0">
                        <button id="add_multiple_question" data-quiz-id="{{ $quiz->id }}" type="button" class="quiz-form-btn btn btn-primary btn-sm ml-10">{{ trans('lms/quiz.add_multiple_choice') }}</button>
                        <button id="add_descriptive_question" data-quiz-id="{{ $quiz->id }}" type="button" class="quiz-form-btn btn btn-primary btn-sm ml-10">{{ trans('lms/quiz.add_descriptive') }}</button>
                    </div>
                </div>
                            ...
                            <li data-id="{{ $question->id }}" class="quiz-question-card d-flex align-items-center mt-20">
                                <div class="flex-grow-1">
                                    ...
                                    <div class="font-12 mt-5 question-infos">
                                        <span>{{ $question->type === App\Models\LMS\QuizzesQuestion::$multiple ? trans('lms/quiz.multiple_choice') : trans('lms/quiz.descriptive') }} | {{ trans('lms/quiz.grade') }}: {{ $question->grade }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="btn-group dropdown table-actions">
                                    ...
                                    <div class="dropdown-menu">
                                        <button type="button" data-question-id="{{ $question->id }}" class="edit_question btn btn-sm btn-transparent d-block">{{ trans('lms/public.edit') }}</button>
                                        <a href="/lms/panel/quizzes-questions/{{ $question->id }}/delete" class="delete-action btn btn-sm btn-transparent d-block">{{ trans('lms/public.delete') }}</a>
                                    </div>
                                </div>
                            </li>
                        @endforeach
    ...
    <!-- Modal -->
@if(!empty($quiz))
    @include('lms.' . getTemplate() .'.panel.quizzes.modals.multiple_question',['quiz' => $quiz])
    @include('lms.' . getTemplate() .'.panel.quizzes.modals.descriptive_question',['quiz' => $quiz])
@endif
