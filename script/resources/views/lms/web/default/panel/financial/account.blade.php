...
@section('content')
    ...
    @if(!empty($registrationBonusAmount))
        <div class="mb-25 d-flex align-items-center justify-content-between p-15 bg-white panel-shadow">
            <div class="d-flex align-items-center">
                <img src="/assets/lms/assets/default/img/icons/money.png" alt="money" width="51" height="51">

                <div class="ml-15">
                    <span class="d-block font-16 text-dark font-weight-bold">{{ trans('lms/update.unlock_registration_bonus') }}</span>
                    <span class="d-block font-14 text-gray font-weight-500 mt-5">{{ trans('lms/update.your_wallet_includes_amount_registration_bonus_This_amount_is_locked',['amount' => handlePrice($registrationBonusAmount)]) }}</span>
                </div>
            </div>

            <a href="/lms/panel/marketing/registration_bonus" class="btn btn-border-gray300">{{ trans('lms/update.view_more') }}</a>
        </div>
    @endif
    ...
    <section class="mt-30">
        <h2 class="section-title">{{ trans('lms/financial.select_the_payment_gateway') }}</h2>

        <form action="/lms/panel/financial/{{ !empty($editOfflinePayment) ? 'offline-payments/'. $editOfflinePayment->id .'/update' : 'charge' }}" method="post" enctype="multipart/form-data" class="mt-25">
            {{csrf_field()}}
                    ...
                    @if(!empty($paymentChannel->currencies) and in_array($userCurrency, $paymentChannel->currencies))
                        <div class="col-6 col-lg-3 mb-40 charge-account-radio">
                            ...
                            <label for="{{ $paymentChannel->class_name }}" class="rounded-sm p-20 p-lg-45 d-flex flex-column align-items-center justify-content-center">
                                <img src="{{get_path_lms()}}{{ $paymentChannel->image }}" width="120" height="60" alt="">
                                <p class="mt-30 font-14 font-weight-500 text-dark-blue">{{ trans('lms/financial.pay_via') }}
                ...
                @if(!empty(getOfflineBankSettings('offline_banks_status')))
                    <div class="col-6 col-lg-3 mb-40 charge-account-radio">
                        ...
                        <label for="offline" class="rounded-sm p-20 p-lg-45 d-flex flex-column align-items-center justify-content-center">
                            <img src="/assets/lms/assets/default/img/activity/pay.svg" width="120" height="60" alt="">
                            <p class="mt-30 font-14 font-weight-500 text-dark-blue">{{ trans('lms/financial.pay_via') }}
                                <span class="font-weight-bold">{{ trans('lms/financial.offline') }}</span>
                            </p>
                        </label>
                    </div>
                @endif
            ...
            @if(!empty($invalidChannels))
                <div class="d-flex align-items-center rounded-lg border p-15">
                    ...
                    <div class="ml-5">
                        <h4 class="font-14 font-weight-bold text-gray">{{ trans('lms/update.disabled_payment_gateways') }}</h4>
                        <p class="font-12 text-gray">{{ trans('lms/update.disabled_payment_gateways_hint') }}</p>
                    </div>
                </div>
                    ...
                    @foreach($invalidChannels as $invalidChannel)
                        <div class="col-6 col-lg-3 mb-40 charge-account-radio">
                            <div class="disabled-payment-channel bg-white border rounded-sm p-20 p-lg-45 d-flex flex-column align-items-center justify-content-center">
                                <img src="{{get_path_lms()}}{{ $invalidChannel->image }}" width="120" height="60" alt="">

                                <p class="mt-30 mt-lg-50 font-weight-500 text-dark-blue">
                                    {{ trans('lms/financial.pay_via') }}
            ...
            <div class="">
                <h3 class="section-title mb-20">{{ trans('lms/financial.finalize_payment') }}</h3>
                ...
                <div class="row">
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="amount" min="0" class="form-control @error('amount') is-invalid @enderror"
                                   ...
                                   placeholder="{{ trans('lms/panel.number_only') }}"/>
                    ...
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0 js-offline-payment-input " style="{{ (!$showOfflineFields) ? 'display:none' : '' }}">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/financial.account') }}</label>
                            <select name="account" class="form-control @error('account') is-invalid @enderror">
                                <option selected disabled>{{ trans('lms/financial.select_the_account') }}</option>
                    ...
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0 js-offline-payment-input " style="{{ (!$showOfflineFields) ? 'display:none' : '' }}">
                        <div class="form-group">
                            <label for="referralCode" class="input-label">{{ trans('lms/financial.referral_code') }}</label>
                    ...
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0 js-offline-payment-input " style="{{ (!$showOfflineFields) ? 'display:none' : '' }}">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/public.date_time') }}</label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                    ...
                    <div class="col-12 col-lg-3 mb-25 mb-lg-0 js-offline-payment-input " style="{{ (!$showOfflineFields) ? 'display:none' : '' }}">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/update.attach_the_payment_photo') }}</label>
                    ...
                    <div class="col-12 col-lg-3">
                        <div class="mt-30">
                            <button type="button" id="submitChargeAccountForm" class="btn btn-primary btn-sm">{{ trans('lms/public.pay') }}</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="mt-40">
        <h2 class="section-title">{{ trans('lms/financial.bank_accounts_information') }}</h2>
            ...
            @foreach($offlineBanks as $offlineBank)
                <div class="col-12 col-lg-3 mb-30 mb-lg-0">
                    <div class="py-25 px-20 rounded-sm panel-shadow d-flex flex-column align-items-center justify-content-center">
                        <img src="{{get_path_lms()}}{{ $offlineBank->logo }}" width="120" height="60" alt="">

                        <div class="mt-15 mt-30 w-100">

                            <div class="d-flex align-items-center justify-content-between">
                                <span class="font-14 font-weight-500 text-secondary">{{ trans('lms/public.name') }}:</span>
    ...
    @if($offlinePayments->count() > 0)
        <section class="mt-40">
            <h2 class="section-title">{{ trans('lms/financial.offline_transactions_history') }}</h2>
                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table text-center custom-table">
                                <thead>
                                <tr>
                                    <th>{{ trans('lms/financial.bank') }}</th>
                                    <th>{{ trans('lms/financial.referral_code') }}</th>
                                    <th class="text-center">{{ trans('lms/panel.amount') }} ({{ $currency }})</th>
                                    <th class="text-center">{{ trans('lms/update.attachment') }}</th>
                                    <th class="text-center">{{ trans('lms/public.status') }}</th>
                                    <th class="text-right">{{ trans('lms/public.controls') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center align-middle">
                                            @if(!empty($offlinePayment->attachment))
                                                <a href="/lms{{ $offlinePayment->getAttachmentPath() }}" target="_blank" class="text-primary">{{ trans('lms/public.view') }}</a>
                                            @else
                                                ---
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center align-middle">
                                            @switch($offlinePayment->status)
                                                @case(\App\Models\LMS\OfflinePayment::$waiting)
                                                    <span class="text-warning">{{ trans('lms/public.waiting') }}</span>
                                                    @break
                                                @case(\App\Models\LMS\OfflinePayment::$approved)
                                                    <span class="text-primary">{{ trans('lms/financial.approved') }}</span>
                                                    @break
                                                @case(\App\Models\LMS\OfflinePayment::$reject)
                                            ...
                                            @if($offlinePayment->status != 'approved')
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a href="/lms/panel/financial/offline-payments/{{ $offlinePayment->id }}/edit"
                                                           class="webinar-actions d-block mt-10">{{ trans('lms/public.edit') }}</a>
                                                        <a href="/lms/panel/financial/offline-payments/{{ $offlinePayment->id }}/delete" data-item-id="1"
                                                           class="webinar-actions d-block mt-10 delete-action">{{ trans('lms/public.delete') }}</a>
                                                    </div>
                                                </div>
                                            @endif
                                        </td>
    ...
    @else
        ...
        @include('lms.' . getTemplate() . '.includes.no-result',[
            ...
            'title' => trans('lms/financial.offline_no_result'),
            'hint' => nl2br(trans('lms/financial.offline_no_result_hint')),
        ])
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/financial/account.min.js"></script>
