...
@section('content')
    <form method="post" action="/lms/panel/support/store">
        {{ csrf_field() }}

        <section>
            <h2 class="section-title">{{ trans('lms/panel.create_support_message') }}</h2>

            <div class="mt-25 rounded-sm shadow py-20 px-10 px-lg-25 bg-white">

                <div class="form-group">
                    <label class="input-label">{{ trans('lms/site.subject') }}</label>
                ...
                <div class="form-group">
                    ...
                    <select name="type" id="supportType" class="form-control  @error('type')  is-invalid @enderror" data-allow-clear="false" data-search="false">
                        ...
                        <option value="course_support" @if($errors->has('webinar_id')) selected @endif>{{ trans('lms/panel.course_support') }}</option>
                        <option value="platform_support" @if($errors->has('department_id')) selected @endif>{{ trans('lms/panel.platform_support') }}</option>
                    </select>
                ...
                <div id="courseInput" class="form-group @if(!$errors->has('webinar_id')) d-none @endif">
                    <label class="input-label d-block">{{ trans('lms/product.course') }}</label>
