...
@section('content')
    <section>
        <h2 class="section-title">{{ trans('lms/panel.support_summary') }}</h2>
            ...
            <div class="row">
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/panel.open_conversations') }}</span>
                    </div>
                </div>
                ...
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/panel.closed_conversations') }}</span>
                    </div>
                </div>
                ...
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ trans('lms/panel.total_conversations') }}</span>
                    </div>
                </div>
    ...
    <section class="mt-25">
        <h2 class="section-title">{{ trans('lms/panel.message_filters') }}</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="/lms/panel/support" method="get">
                <div class="row">
                    ...
                    @if(!$authUser->isUser())
                        <div class="col-12 col-md-4 col-lg-2">
                            <div class="form-group">
                                <label class="input-label">{{ trans('lms/public.user_role') }}</label>
                                <select class="form-control" id="userRole" name="role">
                                    <option value="all">{{ trans('lms/public.all_roles') }}</option>
                                    <option value="student" @if(request()->get('role') == 'student') selected @endif >{{ trans('lms/quiz.student') }}</option>
                                    <option value="teacher" @if(request()->get('role') == 'teacher') selected @endif >{{ trans('lms/panel.teacher') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div id="studentSelectInput" class="col-12 col-md-4 col-lg-2 @if(request()->get('role') != 'student' and (empty(request()->get('student'))  or request()->get('student') == 'all')) d-none @endif">
                            <div class="form-group">
                                <label class="input-label">{{ trans('lms/public.students') }}</label>
                                <select name="student" class="form-control select2" data-placeholder="{{ trans('lms/public.all') }}">
                    ...
                    <div id="teacherSelectInput" class="col-12 col-md-4 col-lg-2 @if(!$authUser->isUser() and request()->get('role') != 'teacher' and (empty(request()->get('teacher')) or request()->get('teacher') == 'all')) d-none @endif">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/home.teachers') }}</label>
                            <select name="teacher" class="form-control select2" data-placeholder="{{ trans('lms/public.all') }}">
                                <option value="all">{{ trans('lms/public.all') }}</option>
                    ...
                    <div class="col-12 col-md-4 col-lg-2">
                        <div class="form-group">
                            <label class="input-label">{{ trans('lms/product.courses') }}</label>
                            <select name="webinar" class="form-control select2" data-placeholder="{{ trans('lms/public.all') }}">
                                <option value="all">{{ trans('lms/public.all') }}</option>

                                @foreach($webinars as $webinar)
                                    <option value="{{ $webinar->id }}" @if(request()->get('webinar') == $webinar->id) selected @endif>{{ $webinar->title }} @if(in_array($webinar->id,$purchasedWebinarsIds)) ({{ trans('lms/panel.purchased') }}) @endif</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-4 col-lg-2">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="status" name="status">
                                <option value="all">{{ trans('lms/public.all') }}</option>
                                <option value="open" @if(request()->get('status') == 'open') selected @endif >{{ trans('lms/public.open') }}</option>
                                <option value="close" @if(request()->get('status') == 'close') selected @endif >{{ trans('lms/public.close') }}</option>
                                <option value="replied" @if(request()->get('status') == 'replied') selected @endif >{{ trans('lms/panel.replied') }}</option>
                            </select>
                        </div>
                    </div>
    ...
    <section class="mt-40">
        <h2 class="section-title">{{ trans('lms/panel.messages_history') }}</h2>
                    ...
                    <div id="conversationsList" class="col-12 col-lg-6 conversations-list">
                        <div class="table-responsive">
                            <table class="table table-md">
                                <tr>
                                    <th class="text-left text-gray font-14 font-weight-500">{{ trans('lms/navbar.contact') }}</th>
                                    <th class="text-left text-gray font-14 font-weight-500">{{ trans('lms/public.title') }}</th>
                                    <th class="text-center text-gray font-14 font-weight-500">{{ trans('lms/public.status') }}</th>
                                </tr>
                                <tbody>
                                ...
                                @foreach($supports as $support)
                                    <tr class="@if(!empty($selectSupport) and $selectSupport->id == $support->id) selected-row @endif">
                                        <td class="text-left">
                                            <a href="/lms/panel/support/{{ $support->id }}/conversations" class="">
                                                <div class="user-inline-avatar d-flex align-items-center">
                                                    ...
                                                    <div class="ml-10">
                                                        ...
                                                        <span class="mt-1 font-12 text-gray d-block">
                                                            {{ (!empty($support->webinar) and $support->webinar->teacher_id != $authUser->id) ? trans('lms/panel.teacher') : ( ($support->user->isUser()) ? trans('lms/quiz.student') : trans('lms/panel.staff')) }}
                                                        </span>
                                                    </div>
                                                </div>
                                            </a>
                                        ...
                                        <td class="text-center align-middle">
                                            @if($support->status == 'close')
                                                <span class="text-danger font-weight-500 font-14">{{  trans('lms/panel.closed') }}</span>
                                            @elseif($support->status == 'supporter_replied')
                                                <span class="text-primary font-weight-500 font-14">{{  trans('lms/panel.replied') }}</span>
                                            @else
                                                <span class="text-warning font-weight-500 font-14">{{  trans('lms/public.waiting') }}</span>
                                            @endif
                                        </td>
                        ...
                        <div class="col-12 col-lg-6 border-left border-gray300">
                            <div class="conversation-box p-15 d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <span class="font-12 mt-1 text-gray d-block">{{ trans('lms/public.created') }}: {{ dateTimeFormat($support->created_at,'j M Y | H:i') }}</span>

                                    @if(!empty($selectSupport->webinar))
                                        <span class="font-12 text-gray d-block mt-5">{{ trans('lms/webinars.webinar') }}: {{ $selectSupport->webinar->title }}</span>
                                    @endif
                                </div>

                                @if($selectSupport->status != 'close')
                                    <a href="/lms/panel/support/{{ $selectSupport->id }}/close" class="btn btn-primary btn-sm">{{ trans('lms/panel.close_request') }}</a>
                                @endif
                            </div>
                                            ...
                                            <div class="d-flex align-items-center justify-content-between pb-20 border-bottom border-gray300">
                                                <div class="user-inline-avatar d-flex align-items-center">
                                                    ...
                                                    <div class="ml-10">
                                                        ...
                                                        <span class="mt-1 font-12 text-gray d-block">{{ (!empty($conversations->supporter)) ? trans('lms/panel.staff') : $conversations->sender->role_name }}</span>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="d-flex flex-column align-items-end">
                                                    ...
                                                    @if(!empty($conversations->attach))
                                                        <a href="/lms{{ url($conversations->attach) }}" target="_blank" class="font-12 mt-10 text-danger"><i data-feather="paperclip" height="14"></i> {{ trans('lms/panel.attach') }}</a>
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            <div class="conversation-box mt-30 py-10 px-15">
                                <h3 class="font-14 text-dark-blue font-weight-bold">{{ trans('lms/panel.reply_to_the_conversation') }}</h3>
                                <form action="/lms/panel/support/{{ $selectSupport->id }}/conversations" method="post" class="mt-5">
                                    {{ csrf_field() }}

                                    <div class="form-group mt-10">
                                        <label class="input-label d-block">{{ trans('lms/site.message') }}</label>
                                    ...
                                    <div class="d-flex d-flex align-items-center">
                                        <div class="form-group">
                                            <label class="input-label">{{ trans('lms/panel.attach_file') }}</label>
                                            <div class="input-group">
                                                <div class="input-group-prepend">
                                        ...
                                        <button type="submit" class="btn btn-primary btn-sm ml-40 mt-10">{{ trans('lms/site.send_message') }}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    @else
                        <div class="col-12 col-lg-6 border-left border-gray300">
                            @include('lms.' . getTemplate() . '.includes.no-result',[
                                ...
                                'title' => trans('lms/panel.select_support'),
                                'hint' => nl2br(trans('lms/panel.select_support_hint')),
                            ])
                        </div>
                    @endif
                </div>
        ...
        @else
            ...
            @include('lms.' . getTemplate() . '.includes.no-result',[
                'file_name' => 'support.png',
                'title' => trans('lms/panel.support_no_result'),
                'hint' => nl2br(trans('lms/panel.support_no_result_hint')),
            ])
...
@push('scripts_bottom')
    ...
    <script src="/assets/lms/assets/default/js/panel/conversations.min.js"></script>
@endpush
