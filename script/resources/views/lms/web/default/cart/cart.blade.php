...
@section('content')
    <section class="cart-banner position-relative text-center">
        <h1 class="font-30 text-white font-weight-bold">{{ trans('lms/cart.shopping_cart') }}</h1>
        <span class="payment-hint font-20 text-white d-block"> {{ handlePrice($subTotal) . ' ' . trans('lms/cart.for_items',['count' => $carts->count()]) }}</span>
    </section>
    ...
    <div class="container">
        ...
        @if(!empty($totalCashbackAmount))
            <div class="d-flex align-items-center mt-45 p-15 success-transparent-alert">
                ...
                <div class="ml-10">
                    <div class="font-14 font-weight-bold ">{{ trans('lms/update.get_cashback') }}</div>
                    <div class="font-12 ">{{ trans('lms/update.by_purchasing_this_cart_you_will_get_amount_as_cashback',['amount' => handlePrice($totalCashbackAmount)]) }}</div>
                </div>
            </div>
        @endif

        <section class="mt-45">
            <h2 class="section-title">{{ trans('lms/cart.cart_items') }}</h2>

            <div class="rounded-sm shadow mt-20 py-25 px-10 px-md-30">
                @if($carts->count() > 0)
                    <div class="row d-none d-md-flex">
                        <div class="col-12 col-lg-8"><span
                                class="text-gray font-weight-500">{{ trans('lms/cart.item') }}</span></div>
                        <div class="col-6 col-lg-2 text-center"><span
                                class="text-gray font-weight-500">{{ trans('lms/public.price') }}</span></div>
                        <div class="col-6 col-lg-2 text-center"><span
                                class="text-gray font-weight-500">{{ trans('lms/public.remove') }}</span></div>
                    </div>
                @endif
                @foreach($carts as $cart)
                    <div class="row mt-5 cart-row">
                                ...
                                <div class="col-8">
                                    <div class="webinar-card-body p-0 w-100 h-100 d-flex flex-column">
                                        <div class="d-flex flex-column">
                                            <a href="/lms{{ $cartItemInfo['itemUrl'] ?? '#!' }}" target="_blank">
                                                <h3 class="font-16 font-weight-bold text-dark-blue">{{ $cartItemInfo['title'] }}</h3>
                                            </a>

                                            @if(!empty($cart->gift_id) and !empty($cart->gift))
                                                <span class="d-block mt-5 text-gray font-12">{!! trans('lms/update.a_gift_for_name_on_date',['name' => $cart->gift->name, 'date' => (!empty($cart->gift->date) ? dateTimeFormat($cart->gift->date, 'j M Y H:i') : trans('lms/update.instantly'))]) !!}</span>
                                            @endif
                                        </div>
                                        ...
                                        @if(!empty($cartItemInfo['profileUrl']) and !empty($cartItemInfo['teacherName']))
                                            <span class="text-gray font-14 mt-auto">
                                                {{ trans('lms/public.by') }}
                                                <a href="/lms{{ $cartItemInfo['profileUrl'] }}" target="_blank" class="text-gray text-decoration-underline">{{ $cartItemInfo['teacherName'] }}</a>
                                            </span>
                                        @endif
                                        ...
                                        @if(!is_null($cartItemInfo['rate']))
                                            @include('lms.web.default.includes.webinar.rate',['rate' => $cartItemInfo['rate']])
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-6 col-lg-2 d-flex flex-md-column align-items-center justify-content-center">
                            <span class="text-gray d-inline-block d-md-none">{{ trans('lms/public.price') }} :</span>
                            ...
                            @if(!empty($cartItemInfo['quantity']))
                                <span class="font-12 text-warning font-weight-500 mt-0 mt-md-5">({{ $cartItemInfo['quantity'] }} {{ trans('lms/update.product') }})</span>
                            @endif
                        ...
                        <div class="col-6 col-lg-2 d-flex flex-md-column align-items-center justify-content-center">
                            <span class="text-gray d-inline-block d-md-none mr-10 mr-md-0">{{ trans('lms/public.remove') }} :</span>

                            <a href="/lms/cart/{{ $cart->id }}/delete" class="delete-action btn-cart-list-delete d-flex align-items-center justify-content-center">
                ...
                <button type="button" onclick="window.history.back()" class="btn btn-sm btn-primary mt-25">{{ trans('lms/cart.continue_shopping') }}</button>
            </div>
        </section>

        <form action="/lms/cart/checkout" method="post" id="cartForm">
            {{ csrf_field() }}
            <input type="hidden" name="discount_id" value="">

            @if($hasPhysicalProduct)
                @include('lms.web.default.cart.includes.shipping_and_delivery')
            @endif

            <div class="row mt-30">
                <div class="col-12 col-lg-6">
                    <section class="mt-45">
                        <h3 class="section-title">{{ trans('lms/cart.coupon_code') }}</h3>
                        <div class="rounded-sm shadow mt-20 py-25 px-20">
                            <p class="text-gray font-14">{{ trans('lms/cart.coupon_code_hint') }}</p>

                            @if(!empty($userGroup) and !empty($userGroup->discount))
                                <p class="text-gray mt-25">{{ trans('lms/cart.in_user_group',['group_name' => $userGroup->name , 'percent' => $userGroup->discount]) }}</p>
                            @endif

                            <form action="/lms/carts/coupon/validate" method="Post">
                                {{ csrf_field() }}
                                <div class="form-group">
                                    <input type="text" name="coupon" id="coupon_input" class="form-control mt-25"
                                           placeholder="{{ trans('lms/cart.enter_your_code_here') }}">
                                    <span class="invalid-feedback">{{ trans('lms/cart.coupon_invalid') }}</span>
                                    <span class="valid-feedback">{{ trans('lms/cart.coupon_valid') }}</span>
                                </div>

                                <button type="submit" id="checkCoupon"
                                        class="btn btn-sm btn-primary mt-50">{{ trans('lms/cart.validate') }}</button>
                            </form>
                        </div>
                    </section>
                </div>
                ...
                <div class="col-12 col-lg-6">
                    <section class="mt-45">
                        <h3 class="section-title">{{ trans('lms/cart.cart_totals') }}</h3>
                        <div class="rounded-sm shadow mt-20 pb-20 px-20">

                            <div class="cart-checkout-item">
                                <h4 class="text-secondary font-14 font-weight-500">{{ trans('lms/cart.sub_total') }}</h4>
                                <span class="font-14 text-gray font-weight-bold">{{ handlePrice($subTotal) }}</span>
                            </div>

                            <div class="cart-checkout-item">
                                <h4 class="text-secondary font-14 font-weight-500">{{ trans('lms/public.discount') }}</h4>
                            ...
                            <div class="cart-checkout-item">
                                <h4 class="text-secondary font-14 font-weight-500">{{ trans('lms/cart.tax') }}
                                    @if(!$taxIsDifferent)
                            ...
                            <div class="cart-checkout-item border-0">
                                <h4 class="text-secondary font-14 font-weight-500">{{ trans('lms/cart.total') }}</h4>
                                <span class="font-14 text-gray font-weight-bold"><span id="totalAmount">{{ handlePrice($total) }}</span></span>
                            </div>

                            <button type="submit" class="btn btn-sm btn-primary mt-15">{{ trans('lms/cart.checkout') }}</button>
                        </div>
                    </section>
                </div>
            </div>
...
@push('scripts_bottom')
    <script>
        var couponInvalidLng = '{{ trans('lms/cart.coupon_invalid') }}';
    ...
    <script src="/assets/lms/assets/default/js/parts/cart.min.js"></script>
@endpush
