@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Gallery Category')])
@endsection
@section('content')
<div class="card"  >
    <div class="card-body">
        ...
        <div class="card-action-filter">
            <form method="post" class="basicform_with_reload" action="{{ route('seller.gallery_category.destroys') }}">
                @csrf
                <div class="row">
                    <div class="col-lg-4">
                        <div class="d-flex">
                    ...
                    <div class="col-lg-4">
                        <div class="add-new-btn">
                            <a href="#" class="btn btn-primary float-right" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{__('Add Gallery Category')}}</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="table-responsive custom-table">
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($gallery_categorys as $key => $gallery_category)
                        <tr>
                            <td><input type="checkbox" name="ids[]" value="{{ base64_encode($gallery_category->id) }}"></td>
                            <td>{{$gallery_category->name}}</td>
                            <td>{{$gallery_category->slug}}</td>
                            <td>
                                @if ($gallery_category->featured == 1)
                            ...
                            <td>
                                @foreach(json_decode($gallery_category->lang_id) ?? [] as $lang)
                                    <span class="badge badge-sm badge-info mb-1">{{language_name($lang)}}</span>
                                @endforeach
                            </td>
                            <td>{{$gallery_category->serial_number ?? ''}}</td>
                            <td>
                                <a class="btn btn-primary btn-sm editbtn" href="{{route('seller.gallery_category.edit', $gallery_category->id)}}">
            ...
            {{ $gallery_categorys->links('vendor.pagination.bootstrap-4') }}
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form id="ajaxFormLoad" class="modal-form" action="{{route('seller.gallery_category.store')}}" method="post" enctype="multipart/form-data">
                @csrf
