@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Faqs')])
@endsection
@section('content')
<div class="card"  >
    <div class="card-body">
        ...
        <a href="{{ url('/').'/'.permalink_type('faq').'' }}" target="_blank" class="text-success">{{ url('/').'/'.permalink_type('faq').'' }}</a>
		<br>
		<br>
        <div class="card-action-filter">
            <form method="post" class="basicform_with_reload" action="{{ route('seller.faq.destroys') }}">
                @csrf
                <div class="row">
                    <div class="col-lg-4">
                        <div class="d-flex">
                    ...
                    <div class="col-lg-4">
                        <div class="add-new-btn">
                            <a href="#" class="btn btn-primary float-right" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{__('Add Faq')}}</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="table-responsive custom-table">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="ids[]" value="{{ base64_encode($faq->id) }}"></td>
                            <td>{{$faq->question}}</td>
                            ...
                            <td>
                                @foreach(json_decode($faq->lang_id) ?? [] as $lang)
                                <span class="badge badge-sm badge-info mb-1">{{language_name($lang)}}</span>
                                @endforeach
                            </td>
                            <td>
                                <a class="btn btn-primary btn-sm editbtn" href="{{route('seller.faq.edit', $faq->id)}}">
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form id="ajaxFormLoad" class="modal-form" action="{{route('seller.faq.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">               
                    ...
                    <div class="form-group">
                        ...
                        <textarea rows="5" class="form-control" name="answer"placeholder="Enter Answer"></textarea>
