				...
				<form method="post" class="basicform" action="{{ route('seller.payment.update',$info->active_getway->id) }}">
					...
					<div class="card-body">
						...
						<div class="form-group">
							...
							<input class="form-control" required="" value="{{ $data->vnpTmnCode }}" name="vnpTmnCode" required="" type="text">
						</div>

						<div class="form-group">
							<label>VNPay HashSecret</label>
							<input class="form-control" required="" value="{{ $data->vnpHashSecret }}" name="vnpHashSecret" required="" type="text">
						</div>
						<div class="form-group">
