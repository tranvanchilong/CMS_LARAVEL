@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Seo And Sitemap')])
@endsection
@section('content')
@php
...
<div class="card"  >
	<div class="card-header">
		<h4 class="mb-20">{{ __('Site Map') }}</h4>
	</div>
	<div class="card-body">
		
		<form method="post"  class="basicform_with_reset" action="{{ route('seller.seo.store') }}">
			@csrf
			
			<div class="custom-form">
				<div class="row">
					<div class="form-group col-lg-12">
						<label for="title">{{ __('Site Link') }}</label>
						<input type="text"  class="form-control" value="{{ url('/sitemap.xml') }}" disabled="">
					</div>
