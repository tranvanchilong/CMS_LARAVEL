@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Knowledges')])
@endsection
@section('content')
<div class="card"  >
    <div class="card-body">
        ...
        <a href="{{ url('/').'/'.permalink_type('knowledge').'' }}" target="_blank" class="text-success">{{ url('/').'/'.permalink_type('knowledge').'' }}</a>
		<br>
		<br>
        <div class="card-action-filter">
            <form method="post" class="basicform_with_reload" action="{{ route('seller.guide.destroys') }}">
                @csrf
                <div class="row">
                    <div class="col-lg-4">
                        <div class="d-flex">
                    ...
                    <div class="col-lg-4">
                        <div class="add-new-btn">
                            <a href="#" class="btn btn-primary float-right" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{__('Add Knowledge') }}</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="table-responsive custom-table">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="ids[]" value="{{ base64_encode($guide->id) }}"></td>
                            <td><img src="{{asset($guide->image)}}" alt="" width="100"></td>
                            <td>{{$guide->title}}</td>
                            ...
                            <td>
                                @foreach(json_decode($guide->lang_id) ?? [] as $lang)
                                <span class="badge badge-sm badge-info mb-1">{{language_name($lang)}}</span>
                                @endforeach
                            </td>
                            <td>
                                <a class="btn btn-primary btn-sm editbtn" href="{{route('seller.guide.edit', $guide->id)}}">
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form id="ajaxFormLoad" class="modal-form" action="{{route('seller.guide.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{__('Add Knowledge')}}</h5>
