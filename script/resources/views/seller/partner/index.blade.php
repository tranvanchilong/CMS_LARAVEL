@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Partners')])
@endsection
@section('content')
<div class="card"  >
    <div class="card-body">
        ...
        <a href="{{ url('/').'/'.permalink_type('partner').'' }}" target="_blank" class="text-success">{{ url('/').'/'.permalink_type('partner').'' }}</a>
		<br>
		<br>
        <div class="card-action-filter">
            <form method="post" class="basicform_with_reload" action="{{ route('seller.partner.destroys') }}">
                @csrf
                <div class="row">
                    <div class="col-lg-4">
                        <div class="d-flex">
                    ...
                    <div class="col-lg-4">
                        <div class="add-new-btn">
                            <a href="#" class="btn btn-primary float-right" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{__('Add Partner')}}</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="table-responsive custom-table">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="ids[]" value="{{ base64_encode($partner->id) }}"></td>
                            <td><img src="{{asset($partner->image)}}" alt="" width="100"></td>
                            <td>{{$partner->name}}</td>
                            <td>{{$partner->url}}</td>
                            <td>
                            ...
                            <td>
                                @foreach(json_decode($partner->lang_id) ?? [] as $lang)
                                <span class="badge badge-sm badge-info mb-1">{{language_name($lang)}}</span>
                                @endforeach
                            </td>
                            <td>
                                <a class="btn btn-primary btn-sm editbtn" href="{{route('seller.partner.edit', $partner->id)}}">
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form id="ajaxFormLoad" class="modal-form" action="{{route('seller.partner.store')}}" method="post" enctype="multipart/form-data">
                @csrf
