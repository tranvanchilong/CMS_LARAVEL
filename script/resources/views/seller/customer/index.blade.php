@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Customers')])
@endsection
@section('content')
<div class="card">
	<div class="card-body">
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th class="am-title">{{ __('Active Account') }}</th>
							<th class="am-title">{{ __('Wallet Balance') }}</th>
					...
					<tbody>
						...
						<tr id="row{{  $row->id }}">
							...
							<td>
								<span class="badge badge-success">{{ amount_format($row->wallet_balance) ?? '0' }}</span>	
							</td>
                      ...
                      <div class="dropdown-menu">
                        ...
                        <a class="dropdown-item has-icon" @if($plan == true) href="{{ route('seller.customer.login',$row->id) }}" target="_blank" @endif>@if($plan !== true) <i class="fa fa-lock text-danger"></i> @else <i class="fas fa-key"></i>  @endif {{ __('Login as Customer') }}</a>
