@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>__('Edit Customer')])
@endsection
@section('content')
<div class="row">
  <div class="col-12">
      ...
      <div class="card-body">
        <form class="basicform" action="{{ route('seller.customer.update',$info->id) }}" method="post">
          ...
          <div class="form-group row mb-4">
            ...
            <div class="col-sm-12 col-md-7">
              <input type="number" class="form-control" required="" name="phone" value="{{ $info->phone }}">
            </div>
          </div>
          ...
          <div class="form-group row mb-4">
            <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3" >{{ __('Active Account') }}</label>
            <div class="col-sm-12 col-md-7">
            <select class="form-control" name="email_verified">
                <option value="" {{$info->email_verified_at != null ? 'selected' : ''}}>{{__('Active')}}</option>
                <option value="null" {{$info->email_verified_at == null ? 'selected' : ''}}>{{__('Deactive')}}</option>
            </select>
            </div>
          </div>
