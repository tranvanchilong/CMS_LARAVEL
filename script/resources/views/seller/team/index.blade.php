...
@endpush
@section('head')
@include('layouts.partials.headersection',['title'=>__('Teams')])
@endsection
@section('content')
<div class="card"  >
    <div class="card-body">
        ...
        <a href="{{ url('/').'/'.permalink_type('team').'' }}" target="_blank" class="text-success">{{ url('/').'/'.permalink_type('team').'' }}</a>
		<br>
		<br>
        <div class="card-action-filter">
            <form method="post" class="basicform_with_reload" action="{{ route('seller.team.destroys') }}">
                @csrf
                <div class="row">
                    <div class="col-lg-4">
                        <div class="d-flex">
                    ...
                    <div class="col-lg-4">
                        <div class="add-new-btn">
                            <a href="#" class="btn btn-primary float-right" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{__('Add Team')}}</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="table-responsive custom-table">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="ids[]" value="{{ base64_encode($team->id) }}"></td>
                            <td><img src="{{asset($team->image)}}" alt="" width="100"></td>
                            <td>{{$team->name}}</td>
                            <td>{{$team->rank}}</td>
                            <td>
                                @if ($team->featured == 1)
                            ...
                            <td>
                                @foreach(json_decode($team->lang_id) ?? [] as $lang)
                                <span class="badge badge-sm badge-info mb-1">{{language_name($lang)}}</span>
                                @endforeach
                            </td> 
                            <td>
                                <a class="btn btn-primary btn-sm editbtn" href="{{route('seller.team.edit', $team->id). '?language=' . request()->input('language')}}">
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form id="ajaxFormLoad" class="modal-form" action="{{route('seller.team.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{__('Add Team')}}</h5>
