				...
				<div class="card-footer">
					...
					<div class="text-right">
						...
						<form method="POST" action="{{ route('seller.order.update',$info->id) }}" accept-charset="UTF-8" class="d-inline basicform">
							...
							@else
                            	<input type="text" class="form-control" @if($info->payment_status=='1') value="{{ __('Payment Complete') }}" @endif disabled>
                        	@endif
							&nbsp&nbsp
						@if ($info->status != 'completed' && $info->status != 'canceled')
						...
						@else
                            <input type="text" class="form-control" @if($info->status=='completed') value="{{ __('Completed') }}" @elseif($info->status=='canceled')  value="{{ __('Canceled') }}" @endif disabled>
                        @endif
							</div>
					...
					<button type="submit" class="btn btn-primary float-right mt-2 ml-2 basicbtn">{{ __('Save change') }}</button>
