		...
		<div class="col-sm-12">
			<div class="table-responsive">
				<table class="table table-striped ">
					...
					$price=amount_format($row->price_single->price ?? 0);
					...
					<form method="post" class="basicform" action="{{ route('seller.order.store') }}">
						...
						<tr>
							...
							<td>
								<div class="d-flex">
								@foreach ($row->attributes->groupBy('category_id') as $attributes)
	                                <label>{{ $attributes->first()->attribute->name }} :</label>
	                                <select class="form-control" name="variation[]" required="">
	                                    <option value="" disabled="" selected="" hidden="">Select</option>
	                                    @foreach ($attributes as $attribute)
	                                    	<option value="{{$attribute->variation->id}}">{{$attribute->variation->name}}</option>
	                                    @endforeach
	                                </select>
	                            @endforeach
                                </div>   
							...
							<td>
								<div class="input-group">
									<input type="number" class="form-control"  @if($row->stock->stock_manage == 1) max="{{ $row->stock->stock_qty }}" min="1" @endif required="" value="1" name="qty">
							...
							<tr class="cart-row cart{{ $row->rowId }}">
								...
								<td>
									@foreach ($row->options->attribute as $attribute)
                                        <p class="m-1"><b>{{ $attribute->attribute->name }}</b> : {{ $attribute->variation->name }}</p>
                                    @endforeach
								</td>
								<td>{{ $row->qty }}</td>
