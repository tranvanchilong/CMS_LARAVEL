...
@endphp
          <div class="col-lg-4 mb-5 mb-lg-0  @if($row->featured == 0) priceing @endif">
           ...
           <h2 class="h1 font-weight-bold">{{ amount_format($row->price) }}</h2>
              <span class="font-weight-bold">@if($row->days == 365) {{ __('Yearly') }} @elseif($row->days == 30) {{ __('Monthly') }} @elseif($row->days == 9999) {{ __('Lifetime') }} @else {{ $row->days }}  {{ __('Days') }} @endif</span>
          <ul class="list-unstyled my-5 text-small text-left font-weight-normal">
              <p>{{ $row->description }}</p>
         <li class="mb-3">
            @if($row->is_trial == '1')
              {{ __('Trial Expiry Date') }} <b>{{ $row->trial_expiry_date }} Day</b>
            @endif
         </li>
         <li class="mb-3">
      ...
      <a href="{{ route('merchant.form') }}" class="btn site_color text-white btn-block p-2 shadow rounded-pill">{{ __('Register') }}</a>
