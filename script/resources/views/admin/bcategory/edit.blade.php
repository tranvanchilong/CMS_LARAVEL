...
<div class="row">
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <form class="basicform" action="{{ route('admin.bcategory.update',$info->id) }}" method="post" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          ...
          <div class="form-group row mb-4">
            <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Parent Blog Category') }}</label>
            <div class="col-sm-12 col-md-7">
              <select class="form-control selectric" name="p_id" id="p_id">
                <option value="">{{ __('None') }}</option>
                @foreach($bcategory as $bcate)
            	    <option {{ $bcate->id==$info->p_id ? 'selected' : ''}} value="{{ $bcate->id }}">{{ $bcate->name }}</option>
            	@endforeach
              </select>
            </div>
          </div>
