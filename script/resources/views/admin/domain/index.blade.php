      ...
      <div class="card-body">
        ...
        <div class="row mb-2">
          <div class="col-sm-8">
            ...
            <a href="{{ route('admin.domain.show',2) }}" class="mr-2 btn btn-outline-danger @if($type==2) active @endif">{{ __('Drafts') }} ({{ $drafts }})</a>
              ...
              <tbody>
                ...
                <tr id="row{{ $row->id }}">
                  ...
                  <td>
                    ...
                    @elseif($row->status==2) <span class="badge badge-danger">{{ __('Drafts') }}</span>
                     @elseif($row->status==3) <span class="badge badge-warning">{{ __('Requested') }}</span>
                    @endif
                  </td>
                  <td>{{ $row->created_at->diffforHumans()  }}</td>
                  <td>{{ !is_null($row->will_expire) ? date('d-F-Y', strtotime($row->will_expire)) : '' }}</td>
                  <td>
                    @can('domain.edit')
