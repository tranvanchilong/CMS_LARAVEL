<section class="pt-25 pb-25 package" style="background: {{$item->background_color}}">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                <div class="filter-nav text-center mb-15">
                    <ul class="filter-btn">
                    <a href="{{url('/'.permalink_type('fp').''.'/' .request()->slug)}}" class="@if(isset($_GET['category'])) @else active @endif">{{__('All')}}</a>
                    @foreach ($packages_category as $category)
                        ...
                        <a href="{{url('/'.permalink_type('fp').'').'/' .request()->slug .'?category='.$filterValue.''}}" class="@if($filterValue == $get_category) active @endif">{{ ($category->name) }}</a>
                    @endforeach
                    </ul>
                </div>
            </div>
                    ...
                    <div class="pricing card">
                        <div class="card-body">
                            <div class="text-center">
                                ...
                                <h2 class="price-value">{{ $row->price }}</h2>
                            </div>
                            ...
                            @if (!empty($row->btn_url_2) && !empty($row->btn_text_2))
                                <a class="p-3 px-4 f-left"
                                    href="{{ $row->btn_url_2 }}">{{ $row->btn_text_2 }}</a>
                            @endif
                            @if (!empty($row->btn_url) && !empty($row->btn_text))
                                <div
                                    class="btn-style-1 mt-30 @if (empty($row->btn_url_2) && empty($row->btn_text_2)) text-center @else text-right mrr-30 @endif">
