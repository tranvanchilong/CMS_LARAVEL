...
function render_cart_item(data) {
	...
	$.each(data, function(index, row){
		$.each(row.options.attribute, function(index, val){
			variation += '<p><b>'+val.attribute.name+':</b> '+val.variation.name+'</p>';
		});
		var r_id=row.rowId;
		var delete_url=url+'/'+r_id;
		var html='<tr class="cart-row cart'+row.rowId+'">';
		    html +='<td><img src="'+row.options.preview+'" height="50"></td>';
			html +='<td>'+row.name+'</td><td>'+row.price+'</td><td>'+variation+'</td><td>'+row.qty+'</td>';
