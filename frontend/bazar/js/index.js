(function ($) {
  ...
  function get_data() {
      $.ajax({
            ...
            data:{latest_product:1,get_offerable_products:20,random_product:1,best_selling_product:1,trending_products:1,sliders:1,bump_adds:1,banner_adds:1,banner_adds_2:1,brand_adds:1,featured_category:1,menu_category:1},
            dataType: 'json',            
            beforeSend: function() {
            ...
            success: function(response){ 
                ...
                if(response.brand_adds.length > 0){
                  $.each(response.brand_adds, function(index, value){
                    var html='<div class="brand-slide"><a href="'+value.url+'"><img src="'+preloader+'" data-src="'+value.image+'" class="lazy"  alt=""></a></div>';
